...
import { ADD_ADDON_TO_COLLECTION, CREATE_COLLECTION, DELETE_COLLECTION, DELETE_COLLECTION_ADDON_NOTES, FETCH_CURRENT_COLLECTION, FETCH_CURRENT_COLLECTION_PAGE, FETCH_USER_COLLECTIONS, REMOVE_ADDON_FROM_COLLECTION, UPDATE_COLLECTION, UPDATE_COLLECTION_ADDON, abortAddAddonToCollection, abortFetchCurrentCollection, abortFetchUserCollections, addonAddedToCollection, addonRemovedFromCollection, beginCollectionModification, convertFiltersToQueryParams, finishCollectionModification, finishEditingCollectionDetails, fetchCurrentCollectionPage as fetchCurrentCollectionPageAction, loadCurrentCollection, loadCurrentCollectionPage, loadUserCollections, unloadCollectionBySlug } from 'amo/reducers/collections';
import * as api from 'amo/api/collections';
import log from 'amo/logger';
import { createErrorHandler, getState } from 'amo/sagas/utils';
import type { CreateCollectionAddonParams, CreateCollectionParams, DeleteCollectionParams, GetAllUserCollectionsParams, GetCollectionAddonsParams, GetCollectionParams, RemoveAddonFromCollectionParams, UpdateCollectionParams, UpdateCollectionAddonParams } from 'amo/api/collections';
import type { AddAddonToCollectionAction, CreateCollectionAction, DeleteCollectionAction, FetchCurrentCollectionAction, FetchCurrentCollectionPageAction, FetchUserCollectionsAction, RemoveAddonFromCollectionAction, UpdateCollectionAction, UpdateCollectionAddonAction } from 'amo/reducers/collections';
...
export function* fetchCurrentCollection({
  ...
  try {
    ...
    const detailParams: $Shape<GetCollectionParams> = { ...baseParams,
    };
    const addonsParams: $Shape<GetCollectionAddonsParams> = { ...baseParams,
      filters,
    };
    const {
      detail,
    ...
    yield put(loadCurrentCollection({
      addonsResponse,
      detail,
    }));
  } catch (error) {
...
export function* fetchUserCollections({
  ...
  try {
    ...
    yield put(errorHandler.createErrorAction(error));
    yield put(abortFetchUserCollections({
      userId,
    }));
  }
}
export function* addAddonToCollection({
  ...
  try {
    ...
    yield put(errorHandler.createErrorAction(error));
    yield put(abortAddAddonToCollection({
      addonId,
      userId,
    }));
  }
}
export function* modifyCollection(action: CreateCollectionAction | UpdateCollectionAction): Saga {
  const {
    type,
    payload,
  } = action;
...
export default function* collectionsSaga(): Saga {
  ...
  yield takeLatest([DELETE_COLLECTION_ADDON_NOTES, UPDATE_COLLECTION_ADDON], updateCollectionAddon);
}