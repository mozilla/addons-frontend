...
import { DELETE_USER_ACCOUNT, DELETE_USER_PICTURE, FETCH_USER_ACCOUNT, FETCH_USER_NOTIFICATIONS, UNSUBSCRIBE_NOTIFICATION, UPDATE_USER_ACCOUNT, abortUnsubscribeNotification, finishUnsubscribeNotification, finishUpdateUserAccount, loadCurrentUserAccount, loadUserAccount, loadUserNotifications, logOutUser, unloadUserAccount } from 'amo/reducers/users';
...
import type { CurrentUserAccountParams, CurrentUserAccountResponse, UnsubscribeNotificationParams, UpdateUserAccountParams, UpdateUserNotificationsParams, UserApiParams } from 'amo/api/users';
import type { DeleteUserAccountAction, DeleteUserPictureAction, FetchUserAccountAction, FetchUserNotificationsAction, UnsubscribeNotificationAction, UpdateUserAccountAction } from 'amo/reducers/users';
...
export function* fetchCurrentUserAccount({
  ...
  try {
    const response: CurrentUserAccountResponse = yield call(api.currentUserAccount, params);
    yield put(loadCurrentUserAccount({
      user: response,
    }));
    const {
      site_status: {
  ...
  try {
    ...
    if (Object.keys(notifications).length) {
      ...
      if (typeof notifications.announcements !== 'undefined') {
        ...
        const index = allNotifications.findIndex((notification) => notification.name === 'announcements');

        if (index !== -1) {
          allNotifications[index].enabled = notifications.announcements;
          log.debug('Optimistically set user value for "announcements" notification');
        }
      }
...
export function* unsubscribeNotification({
  ...
  try {
    ...
    yield put(finishUnsubscribeNotification({
      hash,
      notification,
      token,
    }));
  } catch (error) {
    log.warn(`Could not unsubscribe from ${notification} notification: ${error}`);
    yield put(errorHandler.createErrorAction(error));
    yield put(abortUnsubscribeNotification({
      hash,
      notification,
      token,
    }));
