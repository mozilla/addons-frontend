...
import { AUTOCOMPLETE_CANCELLED, AUTOCOMPLETE_STARTED, autocompleteLoad } from 'amo/reducers/autocomplete';
...
export function* fetchAutocompleteResults({
  ...
  try {
    ...
    const {
      results,
    } = yield call(autocompleteApi, params);
    yield put(autocompleteLoad({
      results,
    }));
  } catch (error) {
