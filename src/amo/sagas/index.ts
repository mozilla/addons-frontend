...
import type { Saga } from 'amo/types/sagas'; // Export all sagas for this app so runSaga can consume them.

export default function* rootSaga(): Saga {
  yield all([fork(abuse), fork(addons), fork(addonsByAuthors), fork(autocomplete), fork(blocks), fork(categories), fork(collections), fork(home), fork(landing), fork(languageTools), fork(recommendations), fork(reviews), fork(search), fork(site), fork(userAbuseReports), fork(users), fork(versions)]);
}