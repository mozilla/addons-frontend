...
import { deleteReview, flagReview, getLatestUserReview, getReview, getReviews, replyToReview, submitReview } from 'amo/api/reviews';
import { ABORTED, CREATE_ADDON_REVIEW, DELETE_ADDON_REVIEW, FETCH_LATEST_USER_REVIEW, FETCH_REVIEW, FETCH_REVIEW_PERMISSIONS, FETCH_REVIEWS, FETCH_USER_REVIEWS, SAVED_RATING, SAVED_REVIEW, SEND_REPLY_TO_REVIEW, SEND_REVIEW_FLAG, STARTED_SAVE_RATING, STARTED_SAVE_REVIEW, UPDATE_ADDON_REVIEW, createInternalReview, unloadAddonReviews, flashReviewMessage, hideEditReviewForm, hideFlashedReviewMessage, hideReplyToReviewForm, setAddonReviews, setLatestReview, setReview, setReviewPermissions, setReviewReply, setReviewWasFlagged, setUserReviews, updateRatingCounts } from 'amo/actions/reviews';
import log from 'amo/logger';
import { createErrorHandler, getState } from 'amo/sagas/utils';
import type { AppState } from 'amo/store';
import type { ExternalReviewReplyType, ExternalReviewType, GetLatestUserReviewParams, GetLatestUserReviewResponse, GetReviewsApiResponse, GetReviewParams, GetReviewsParams, SubmitReviewParams, SubmitReviewResponse } from 'amo/api/reviews';
import type { CreateAddonReviewAction, DeleteAddonReviewAction, FetchLatestUserReviewAction, FetchReviewAction, FetchReviewPermissionsAction, FetchReviewsAction, FetchUserReviewsAction, FlagReviewAction, SendReplyToReviewAction, UpdateAddonReviewAction } from 'amo/actions/reviews';
...
function* fetchReviewPermissions({
  ...
  try {
    ...
    const response: GetReviewsApiResponse = yield call(getReviews, params);
    invariant(response.can_reply !== undefined, 'response.can_reply was unexpectedly undefined');
    yield put(setReviewPermissions({
      addonId,
      canReplyToReviews: response.can_reply,
      userId,
    }));
  } catch (error) {
    log.warn(`Failed to load review permissions for add-on ID ${addonId}, user ID ${userId}: ${error}`);
    yield put(errorHandler.createErrorAction(error));
  }
}
...
function* handleReplyToReview({
  ...
  try {
    ...
    yield put(setReviewReply({
      originalReviewId,
      reply: reviewResponse,
    }));
    yield put(hideReplyToReviewForm({
...
function* manageAddonReview(action: CreateAddonReviewAction | UpdateAddonReviewAction, {
  _delay = delay,
}: Options = {}) {
  const {
    body,
  ...
  try {
    ...
    if (action.type === CREATE_ADDON_REVIEW) {
      ...
      invariant(oldReview, `review with ID=${action.payload.reviewId} does not exist in state`);
    }

    invariant(params, `params was unexpectedly empty; action.type: ${action.type}`);
    const submitParams: SubmitReviewParams = params;
    // $FlowIgnore: Flow can't tell what redux-saga's `call` returns.
    const reviewFromResponse: SubmitReviewResponse = yield call(submitReview, submitParams);
    ...
    log.warn(`Failed to create/update review with action ${action.type}: ${error}`);
    yield put(errorHandler.createErrorAction(error));
...
function* fetchLatestUserReview({
  ...
  try {
    ...
    const review: GetLatestUserReviewResponse = yield call(getLatestUserReview, params);
    ...
    if (review) {
      ...
      log.debug(`No saved review found for userId ${userId}, addonId ${addonId}`);
      yield put(_setLatestReview(null));
    }
  } catch (error) {
    log.warn(`Failed to fetchLatestUserReview for addonId "${addonId}", userId "${userId}": ${error}`);
    yield put(errorHandler.createErrorAction(error));
  }
}
...
export default function* reviewsSaga(options?: Options): Saga {
  ...
  yield takeLatest(CREATE_ADDON_REVIEW, (action) => manageAddonReview(action, options));
  yield takeLatest(UPDATE_ADDON_REVIEW, (action) => manageAddonReview(action, options));
