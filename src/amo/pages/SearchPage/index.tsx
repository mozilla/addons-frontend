...
import { ADDON_TYPE_DICT, ADDON_TYPE_EXTENSION, ADDON_TYPE_LANG, DEFAULT_CATEGORY_SORT, DEFAULT_TAG_SORT } from 'amo/constants';
import { makeQueryString } from 'amo/api';
import { sendServerRedirect } from 'amo/reducers/redirectTo';
import { convertFiltersToQueryParams, convertQueryParamsToFilters, fixFiltersFromLocation } from 'amo/searchUtils';
...
export class SearchPageBase extends React.Component<InternalProps> {
  constructor(props: InternalProps) {
    ...
    if (newFilters.category && newFilters.addonType && visibleAddonTypeIsValid(newFilters.addonType) && // We have to add this below the call to visibleAddonTypeIsValid or
    // Flow gets angry.
    newFilters.addonType) {
      ...
      shouldRedirect = true;
    } else if ( // If this is a tag search we want to redirect to new tag pages.
    newFilters.tag) {
      pathname = getTagResultsPathname({
        tag: newFilters.tag,
      });
    ...
    if (shouldRedirect) {
      const queryString = makeQueryString(convertFiltersToQueryParams(newFilters));
      props.dispatch(sendServerRedirect({
        status: 301,
  ...
  render(): React.ReactNode {
    ...
    return <Page>
        <Search filters={filters} paginationQueryParams={convertFiltersToQueryParams(filters)} />
      </Page>;
  }
...
const SearchPage: React.ComponentType<Props> = compose(connect(mapStateToProps))(SearchPageBase);
