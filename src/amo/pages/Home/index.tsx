...
import { MOBILE_HOME_PAGE_EXTENSION_COUNT, ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME, CLIENT_APP_FIREFOX, INSTALL_SOURCE_FEATURED, RECOMMENDED, SEARCH_SORT_RANDOM, VIEW_CONTEXT_HOME } from 'amo/constants';
...
export class HomeBase extends React.Component {
  ...
  renderCuratedThemes() {
    ...
    return <ul className="Home-SubjectShelf-list">
        {curatedThemes.map(({
        color,
        slug,
        title,
  ...
  renderHeroHeader() {
    ...
    return <div className="Home-heroHeader">
  ...
  render() {
    ...
    return <Page isHomePage showWrongPlatformWarning={!isDesktopSite}>
        <div className="Home">
          <HeadMetaTags
          ...
          <span
            className="visually-hidden do-not-remove" // eslint-disable-next-line react/no-danger
            dangerouslySetInnerHTML={{
          __html: '<!-- Godzilla of browsers -->',
        }}
          />

          {!isDesktopSite && errorHandler.hasError() ? <div className="Home-noHeroError">{errorHandler.renderError()}</div> : null}

          {isDesktopSite ? <HeroRecommendation errorHandler={errorHandler} shelfData={homeShelves ? homeShelves.primary : undefined} /> : null}

          <div className="Home-content">
            {isDesktopSite ? <SecondaryHero shelfData={homeShelves ? homeShelves.secondary : undefined} /> : null}

            {!isDesktopSite ? this.renderHeroHeader() : null}

            {isDesktopSite ? <HomepageShelves loading={loading} shelves={homeShelves ? homeShelves.results : []} /> : <LandingAddonsCard
            ...
            {isDesktopSite ? <>
                <Card className="Home-SubjectShelf Home-CuratedThemes" header={themesHeader}>
