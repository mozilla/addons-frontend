...
import { getUnsubscribeKey, isUnsubscribedFor, unsubscribeNotification } from 'amo/reducers/users';
...
export class UsersUnsubscribeBase extends React.Component<InternalProps> {
  ...
  render(): React.ReactNode {
    ...
    const editProfileLink = replaceStringsWithJSX({
      text: i18n.gettext('You can edit your notification settings by %(linkStart)sediting your profile%(linkEnd)s.'),
      replacements: [['linkStart', 'linkEnd', (text) => <Link key="edit-profile" to="/users/edit">
              {text}
            </Link>]],
    });
    return <Page>
        <div className="UsersUnsubscribe">
          ...
          {errorHandler.hasError() ? errorHandler.renderError() : <Card header={isUnsubscribed ? i18n.gettext('You are successfully unsubscribed!') : <LoadingText />}>
              {isUnsubscribed ? <p
                className="UsersUnsubscribe-content-explanation" // eslint-disable-next-line react/no-danger
                dangerouslySetInnerHTML={sanitizeHTML(i18n.sprintf( // translators: a list of notifications will be displayed under this prompt.
          i18n.gettext(`The email address %(strongStart)s%(email)s%(strongEnd)s
            ...
            email: decode(token),
              ...
              <blockquote className="UsersUnsubscribe-content-notification">
                {isUnsubscribed ? getNotificationDescription(i18n, notificationName) : <LoadingText minWidth={40} />}
              </blockquote>
...
const mapStateToProps = (state: AppState, ownProps: Props): PropsFromState => {
  ...
  return {
    isUnsubscribed: isUnsubscribedFor(state.users, hash, notificationName, token),
  };
};

const UsersUnsubscribe: React.ComponentType<Props> = compose(connect(mapStateToProps), translate(), withFixedErrorHandler({
  fileName: __filename,
  extractId,
