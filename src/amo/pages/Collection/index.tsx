...
import { collectionEditUrl, collectionName, collectionUrl, convertFiltersToQueryParams, deleteCollectionAddonNotes, deleteCollection, fetchCurrentCollection, fetchCurrentCollectionPage, getCurrentCollection, removeAddonFromCollection, updateCollectionAddon } from 'amo/reducers/collections';
import { getCurrentUser } from 'amo/reducers/users';
import AuthenticateButton from 'amo/components/AuthenticateButton';
import Paginate from 'amo/components/Paginate';
import { COLLECTION_SORT_DATE_ADDED_DESCENDING, INSTALL_SOURCE_COLLECTION } from 'amo/constants';
...
type InternalProps = Props & PropsFromState & DefaultProps & {
...
export type DeleteAddonNoteFunc = (addonId: number, errorHandler: ErrorHandlerType) => void;
export type SaveAddonNoteFunc = (addonId: number, errorHandler: ErrorHandlerType, notes: string) => void;
...
export class CollectionBase extends React.Component<InternalProps> {
  ...
  loadDataIfNeeded(prevProps?: InternalProps) {
    ...
    if (collection) {
      ...
      if (/^\d+$/.test(params.userId)) {
        ...
        isSameCollectionUser = collection.authorUsername.toLowerCase() === params.userId.toLowerCase();
      }

      if (collection.slug.toLowerCase() !== params.slug.toLowerCase() || isSameCollectionUser === false) {
        collectionChanged = true;
      }
    }
    ...
    if (!collectionChanged && collection) {
      if (params.slug !== collection.slug || !/^\d+$/.test(params.userId)) {
        ...
        const path = editing ? collectionEditUrl({
          collection,
        }) : collectionUrl({
          collection,
        });
  ...
  deleteNote: DeleteAddonNoteFunc = (addonId: number, errorHandler: ErrorHandlerType) => {
    const {
      collection,
      dispatch,
      filters,
  ...
  saveNote: SaveAddonNoteFunc = (addonId: number, errorHandler: ErrorHandlerType, notes: string) => {
    const {
      collection,
      dispatch,
      filters,
  ...
  renderDeleteButton(): null | React.ReactNode {
    const {
      i18n,
      isOwner,
    } = this.props;
    ...
    return <ConfirmButton buttonType="cancel" className={confirmButtonClassName} id={confirmButtonClassName} message={i18n.gettext('Do you really want to delete this collection?')} onConfirm={this.onDelete}>
  ...
  renderCollection(): React.ReactNode {
    ...
    if ((creating || editing) && !isLoggedIn) {
      const logInText = creating ? i18n.gettext('Log in to create a collection') : i18n.gettext('Log in to edit this collection');
      return <Card className="Collection-login">
          <AuthenticateButton noIcon logInText={logInText} />
        </Card>;
    }

    const addons: Array<CollectionAddonType> = collection && collection.addons || [];
    const paginator = collection && collection.pageSize && collection.numberOfAddons && collection.numberOfAddons > Number(collection.pageSize) ? <Paginate
      LinkComponent={Link}
      count={collection.numberOfAddons}
      currentPage={filters.page}
      pathname={editing ? collectionEditUrl({
      collection,
    }) : collectionUrl({
      collection,
    })}
    ...
    if (isLoggedIn && (creating || !loading && addons.length === 0)) {
      placeholderText = creating ? i18n.gettext('First, create your collection. Then you can add extensions and themes.') : i18n.gettext('Search for extensions and themes to add to your collection.');
    }

    return <div className="Collection-wrapper">
        <div className="Collection-detail-wrapper">
          <Card className="Collection-detail">
            <CollectionDetailsCard collection={collection} creating={creating} editing={editing} filters={filters} />
            {this.renderDeleteButton()}
          </Card>
          {!creating && <CollectionControls collection={collection} editing={editing} filters={filters} />}
        </div>
        <div className="Collection-items">
          {editing && <CollectionAddAddon collection={collection} filters={filters} />}
          {!creating && <AddonsCard addonInstallSource={INSTALL_SOURCE_COLLECTION} addons={addons} deleteNote={this.deleteNote} editing={editing} footer={paginator} loading={!collection || loading} placeholderCount={this.addonPlaceholderCount} removeAddon={this.removeAddon} saveNote={this.saveNote} showFullSizePreview />}
          {placeholderText && <p className="Collection-placeholder">{placeholderText}</p>}
        </div>
      </div>;
  }
  ...
  getPageDescription(): string {
    ...
    invariant(collection, 'collection is required');
    return i18n.sprintf(collection.description ? i18n.gettext(`Download and create Firefox collections to keep track
          of favorite extensions and themes. Explore the
          %(collectionName)s—%(collectionDescription)s.`) : i18n.gettext(`Download and create Firefox collections to keep track
...
export const mapStateToProps = (state: AppState, ownProps: InternalProps): PropsFromState => {
  const {
    api,
    collections,
  } = state;
  ...
  const filters = {
    ...
    collectionSort: location.query.collection_sort || COLLECTION_SORT_DATE_ADDED_DESCENDING,
  };
  const currentUser = getCurrentUser(state.users);
  const collection = creating ? null : getCurrentCollection(state.collections);
  const isOwner = Boolean(collection && currentUser && collection.authorId === currentUser.id);
  return {
...
export const extractId = (ownProps: InternalProps): string => {
  return [ownProps.match.params.userId, ownProps.match.params.slug, ownProps.location.query.page].join('/');
};
const Collection: React.ComponentType<Props> = compose(translate(), withFixedErrorHandler({
  fileName: __filename,
  extractId,
}), connect(mapStateToProps))(CollectionBase);
