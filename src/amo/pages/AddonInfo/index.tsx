...
import { fetchAddon, fetchAddonInfo, getAddonByIdInURL, getAddonInfoBySlug, isAddonInfoLoading, isAddonLoading } from 'amo/reducers/addons';
import { fetchVersion, getLoadingBySlug, getVersionById } from 'amo/reducers/versions';
...
export type AddonInfoTypeType = typeof ADDON_INFO_TYPE_CUSTOM_LICENSE | typeof ADDON_INFO_TYPE_EULA | typeof ADDON_INFO_TYPE_PRIVACY_POLICY;
type Props = {
...
export class AddonInfoBase extends React.Component<InternalProps> {
  ...
  loadDataIfNeeded(prevProps?: InternalProps) {
    ...
    if (infoType === ADDON_INFO_TYPE_CUSTOM_LICENSE) {
      const needsLicenceText = addonVersion && addonVersion.license && addonVersion.license.text === undefined;

      if (addon && addon.currentVersionId && !addonVersionIsLoading && (!addonVersion || needsLicenceText || addonHasChanged)) {
  ...
  render(): React.ReactNode {
    ...
    if (addon) {
      header = i18n.sprintf(title, {
        addonName: addon.name,
      });
    }

    if (infoContent || infoType === ADDON_INFO_TYPE_CUSTOM_LICENSE && infoContent) {
    ...
    return <Page errorHandler={errorHandler}>
        <div className={makeClassName('AddonInfo', `AddonInfo--${infoType}`)}>
          ...
          <Card className="AddonInfo-info" header={header}>
            {infoHtml ? <p
              className="AddonInfo-info-html" // eslint-disable-next-line react/no-danger
...
function mapStateToProps(state: AppState, ownProps: InternalProps): PropsFromState {
  ...
  return {
    ...
    addonInfo: getAddonInfoBySlug({
      slug,
      state: state.addons,
    }),
    addonInfoIsLoading: isAddonInfoLoading({
      slug,
      state: state.addons,
    }),
    addonVersion,
    addonVersionIsLoading: getLoadingBySlug({
      slug,
      state: state.versions,
    }),
  };
...
const AddonInfo: React.ComponentType<Props> = compose(connect(mapStateToProps), translate(), withFixedErrorHandler({
  fileName: __filename,
  extractId,
}))(AddonInfoBase);
