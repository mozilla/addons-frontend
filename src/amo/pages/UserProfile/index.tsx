...
import { EXTENSIONS_BY_AUTHORS_PAGE_SIZE, THEMES_BY_AUTHORS_PAGE_SIZE } from 'amo/reducers/addonsByAuthors';
import { getReviewsByUserId } from 'amo/reducers/reviews';
import { fetchUserAccount, getCurrentUser, getUserById, getUserByUsername, hasPermission, isDeveloper } from 'amo/reducers/users';
import Paginate from 'amo/components/Paginate';
import { ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME, USERS_EDIT, VIEW_CONTEXT_HOME } from 'amo/constants';
...
import type { ReactRouterLocationType, ReactRouterMatchType } from 'amo/types/router';
...
export class UserProfileBase extends React.Component<InternalProps> {
  ...
  componentDidUpdate(prevProps: InternalProps) {
    ...
    } else if (user && isOwner && (oldLocation.query.page !== newLocation.query.page || !reviews)) {
      dispatch(fetchUserReviews({
        errorHandlerId: errorHandler.id,
  ...
  renderReviews(): null | React.ReactNode {
    const {
      location,
      i18n,
      isOwner,
    ...
    const paginator = reviewCount && pageSize && reviewCount > Number(pageSize) ? <Paginate LinkComponent={Link} count={reviewCount} currentPage={this.getReviewsPage(location)} pathname={this.getURL()} perPage={Number(pageSize)} queryParams={location.query} /> : null;
    return <CardList className="UserProfile-reviews" footer={paginator} header={i18n.gettext('My reviews')}>
        <ul>
          {reviews.map((review) => {
          return <li key={String(review.id)}>
                <AddonReviewCard review={review} shortByLine siteUserCanReply={false} />
              </li>;
        })}
        </ul>
      </CardList>;
  ...
  getMetaDescription(): null | string {
    ...
    return i18n.sprintf(description, {
      user: user.name,
    });
  }
  ...
  render(): React.ReactNode {
    ...
    const userProfileHeader = <div className="UserProfile-header">
        <UserAvatar className="UserProfile-avatar" user={user} altText={i18n.gettext('User Avatar')} />

        {user && isDeveloper(user) && <div className="UserProfile-tags">
            {user.is_addon_developer && <p className="UserProfile-developer">
                {i18n.gettext('Add-ons developer')}
                <Icon name="developer" />
              </p>}
            {user.is_artist && <p className="UserProfile-artist">
    ...
    const userProfileTitle = user ? i18n.sprintf(i18n.gettext('User Profile for %(user)s'), {
      user: user.name,
    }) : i18n.gettext('User Profile');
    return <Page errorHandler={errorHandler}>
        <div className="UserProfile">
          <UserProfileHead title={userProfileTitle} description={this.getMetaDescription()} />
          ...
          <div className="UserProfile-wrapper">
            <Card className="UserProfile-user-info" header={userProfileHeader}>
              <DefinitionList className="UserProfile-dl">
                {user && user.homepage ? <Definition className="UserProfile-homepage" term={i18n.gettext('Homepage')}>
                    <a href={user.homepage}>{i18n.gettext('Homepage')}</a>
                  </Definition> : null}
                {user && user.location ? <Definition className="UserProfile-location" term={i18n.gettext('Location')}>
                    {user.location}
                  </Definition> : null}
                {user && user.occupation ? <Definition className="UserProfile-occupation" term={i18n.gettext('Occupation')}>
                    {user.occupation}
                  </Definition> : null}
                <Definition className="UserProfile-user-since" term={i18n.gettext('User since')}>
                  {user ? i18n.moment(user.created).format('ll') : <LoadingText />}
                </Definition>
                <Definition className="UserProfile-number-of-addons" term={i18n.gettext('Number of add-ons')}>
                  {user ? user.num_addons_listed : <LoadingText />}
                </Definition>
                <Definition className="UserProfile-rating-average" term={i18n.gettext('Average rating of developer’s add-ons')}>
                  {user ? <Rating rating={user.average_addon_rating} readOnly styleSize="small" /> : <LoadingText />}
                </Definition>
                {user && user.biography && user.biography.length ? <Definition className="UserProfile-biography" term={i18n.gettext('Biography')}>
              ...
              {!isOwner && <ReportUserAbuse className="UserProfile-abuse-button" user={user} />}

              {canEditProfile ? <Button className="UserProfile-edit-link" buttonType="neutral" to={this.getEditURL()} puffy>
                  {i18n.gettext('Edit profile')}
                </Button> : null}

              {canAdminUser && user ? <Button className="UserProfile-admin-link" buttonType="neutral" href={`/admin/models/users/userprofile/${user.id}/`} puffy>
            ...
            <div className="UserProfile-addons-and-reviews">
              <AddonsByAuthorsCard addonType={ADDON_TYPE_EXTENSION} authorDisplayName={user ? user.name : null} authorIds={user ? [user.id] : null} errorHandler={errorHandler} numberOfAddons={EXTENSIONS_BY_AUTHORS_PAGE_SIZE} pageParam="page_e" paginate pathname={this.getURL()} showMore={false} showSummary type="vertical" />

              <AddonsByAuthorsCard addonType={ADDON_TYPE_STATIC_THEME} authorDisplayName={user ? user.name : null} authorIds={user ? [user.id] : null} errorHandler={errorHandler} numberOfAddons={THEMES_BY_AUTHORS_PAGE_SIZE} pageParam="page_t" paginate pathname={this.getURL()} showMore={false} />
...
function mapStateToProps(state: AppState, ownProps: Props): PropsFromState {
  ...
  if (/^\d+$/.test(params.userId)) {
    ...
    canEditProfile = currentUser && (currentUser.id === userId || hasPermission(state, USERS_EDIT));
...
const UserProfile: React.ComponentType<Props> = compose(connect(mapStateToProps), translate(), withFixedErrorHandler({
  fileName: __filename,
  extractId,
}))(UserProfileBase);
