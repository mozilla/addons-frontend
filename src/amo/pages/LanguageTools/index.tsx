...
import { ADDON_TYPE_DICT, ADDON_TYPE_LANG, VIEW_CONTEXT_LANGUAGE_TOOLS } from 'amo/constants';
import { unfilteredLanguages } from 'amo/languages';
import translate from 'amo/i18n/translate';
import { fetchLanguageTools, getAllLanguageTools } from 'amo/reducers/languageTools';
...
export const LanguageToolList = ({
  languageTools,
}: LanguageToolListProps): null | React.ReactNode => {
  invariant(languageTools.length, 'languageTools should not be empty');
  return <ul className="LanguageTools-addon-list">
      {languageTools.map((addon: LanguageToolType) => {
      return <li key={addon.slug}>
...
export class LanguageToolsBase extends React.Component<InternalProps> {
  constructor(props: InternalProps) {
    ...
    if (languageTools.length === 0) {
      dispatch(fetchLanguageTools({
        errorHandlerId: errorHandler.id,
      }));
    }
  }

  languageToolsInYourLocale(): null | React.ReactNode {
    const {
      i18n,
      lang,
      languageTools,
    ...
    return <div className="LanguageTools-in-your-locale">
        ...
        <ul className="LanguageTools-in-your-locale-list">
          ...
          return <li className="LanguageTools-in-your-locale-list-item" key={languageTool.slug}>
                <Link className={`LanguageTools-in-your-locale-list-item--${languageTool.type}`} to={getAddonURL(languageTool.slug)}>
                  {languageTool.name}
                </Link>
              </li>;
        })}
        ...
        <Card className="LanguageTools" header={header}>
          ...
          <Table className="LanguageTools-table">
            ...
            <Tbody>
              ...
              return (// Required to preserve space between strong and span.

                /* eslint-disable react/jsx-closing-tag-location */
                <Tr className={makeClassName('LanguageTools-table-row', `LanguageTools-lang-${language.locale}`)} key={language.locale}>
                        <Td>
                        ...
                        <Td className={`LanguageTools-lang-${language.locale}-languagePacks`}>
                          {languagePacks.length ? <LanguageToolList languageTools={languagePacks} /> : null}
                        </Td>
                        <Td className={`LanguageTools-lang-${language.locale}-dictionaries`}>
                          {dictionaries.length ? <LanguageToolList languageTools={dictionaries} /> : null}
                        </Td>
                      </Tr>
              );
            }) : Array.from(Array(50)).map((_, i) => // eslint-disable-next-line react/jsx-indent, react/no-array-index-key
...
const LanguageTools: React.ComponentType<Props> = compose(withErrorHandler({
  name: 'LanguageTools',
}), connect(mapStateToProps), translate())(LanguageToolsBase);
