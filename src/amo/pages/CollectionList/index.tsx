...
import { expandCollections, fetchUserCollections } from 'amo/reducers/collections';
...
export type InternalProps = Props & PropsFromState & {
  ...
  renderCollections(): React.ReactNode {
    ...
    if (collections) {
      collections.forEach((collection) => {
        ...
        collectionElements.push(<UserCollection authorId={authorId} id={id} key={id} name={name} numberOfAddons={numberOfAddons} slug={slug} />);
      });
    } else {
      // Create 4 "loading" components.
      for (let count = 0; count < 4; count++) {
        collectionElements.push( // numberOfAddons is null when the collection is in a loading state.
        <UserCollection id={count} key={count} numberOfAddons={null} />);
      }
    }

    const footer = collectionElements.length ? null : noCollectionsText;
    return <CardList className="CollectionList-list" footer={footer} header={i18n.gettext('My collections')}>
        {collectionElements.length && <ul className="CollectionList-listing">{collectionElements}</ul>}
      </CardList>;
  }
    ...
    return <Page>
        <div className="CollectionList">
          <div className="CollectionList-wrapper">
            <Card className="CollectionList-info" header={i18n.gettext('Collections')}>
              {!isLoggedIn ? <AuthenticateButton noIcon logInText={i18n.gettext('Log in to view your collections')} /> : <>
                  ...
                  <Button buttonType="action" className="CollectionList-create" puffy to="/collections/add/">
...
const CollectionList: React.ComponentType<Props> = compose(connect(mapStateToProps), translate(), withFixedErrorHandler({
  fileName: __filename,
  extractId,
}))(CollectionListBase);
