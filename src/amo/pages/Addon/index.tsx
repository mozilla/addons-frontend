...
import { fetchAddon, getAddonByIdInURL, isAddonLoading } from 'amo/reducers/addons';
...
export class AddonBase extends React.Component {
  ...
  headerImage() {
    ...
    const label = addon ? i18n.sprintf(i18n.gettext('Preview of %(title)s'), {
      title: addon.name,
    }) : null;
    return <div className="Addon-icon" key="Addon-icon-header">
        <div className="Addon-icon-wrapper">
          <img alt={label} className="Addon-icon-image" src={getAddonIconUrl(addon)} />
        </div>
      </div>;
  }
  ...
  renderRatingsCard() {
    ...
    if (addon && currentVersion) {
      ...
      ratingManager = <p className="Addon-no-rating-manager">
    ...
    if (!addon) {
      ...
      const linkText = i18n.sprintf(i18n.ngettext('Read %(count)s review', 'Read all %(count)s reviews', count), {
    ...
    const props = {
      [footerPropName]: <div className="Addon-read-reviews-footer">{content}</div>,
    };
    return <Card header={i18n.gettext('Rate your experience')} className="Addon-overall-rating" {...props}>
        {ratingManager}
      </Card>;
  }
  ...
  renderShowMoreCard() {
    ...
    return showAbout ? <ShowMoreCard contentId={addon && addon.id} className={showMoreCardName} header={title} id={showMoreCardName}>
  ...
  renderDevCommentsCard = () => {
    ...
    return <ShowMoreCard contentId={addon.id} className={showMoreCardName} header={i18n.gettext('Developer comments')} id={showMoreCardName}>
        <div className="Addon-developer-comments-contents" dangerouslySetInnerHTML={devComments} />
      </ShowMoreCard>;
  ...
  renderVersionReleaseNotes() {
    ...
    const header = i18n.sprintf(i18n.gettext('Release notes for %(addonVersion)s'), {
    ...
    /* eslint-disable react/no-danger */
    return <ShowMoreCard contentId={addon.id} className={showMoreCardNotesName} id={showMoreCardNotesName} header={header}>
  ...
  renderAddonsByAuthorsCard({
    ...
    if (!addon || !addon.authors.length || isForTheme && !isThemeType || !isForTheme && isThemeType) {
      return null;
    }
    ...
    */
    return <div>
        <AddonsByAuthorsCard addonType={addon.type} authorDisplayName={addon.authors[0].name} authorIds={addon.authors.map((author) => author.id)} className="Addon-MoreAddonsCard" forAddonSlug={addon.slug} numberOfAddons={ADDONS_BY_AUTHORS_COUNT} />
      </div>;
  }
  ...
  render() {
    ...
    if (addon) {
      ...
      if (summary && summary.length) {
        summaryProps.dangerouslySetInnerHTML = sanitizeHTML(nl2br(summary), ['a', 'br']);
        showSummary = true;
      }
    } else {
    ...
    return <Page showVPNPromo={Boolean(addon && addon.type === ADDON_TYPE_EXTENSION)} errorHandler={errorHandler} isAddonInstallPage showWrongPlatformWarning={false}>
        <div
        ...
        >
          ...
          <div className="Addon-header-wrapper">
            <Card className="Addon-header-info-card" photonStyle>
              ...
              {addon && (addon.status !== STATUS_PUBLIC || addon.is_disabled) ? <Notice type="error" className="Addon-non-public-notice">
              ...
              <header className="Addon-header">
                ...
                <div className="Addon-summary-and-install-button-wrapper">
                  {showSummary ? <p className="Addon-summary" {...summaryProps} /> : null}
              ...
              {addon ? <WrongPlatformWarning addon={addon} className="Addon-WrongPlatformWarning" /> : null}
            </Card>
          ...
          <div className="Addon-details">
            <div className="Addon-main-content">
              {this.renderAddonsByAuthorsCard({
              isForTheme: true,
            })}

              {addonPreviews.length > 0 && !isThemeType ? <Card className="Addon-screenshots" header={i18n.gettext('Screenshots')}>
              ...
              {addonType === ADDON_TYPE_EXTENSION && <AddonRecommendations addon={addon} />}
            </div>
      ...
      </Page>; // eslint-enable react/no-danger
  }
...
function mapStateToProps(state, ownProps) {
  ...
  if (addon) {
    ...
    currentVersion = addon.currentVersionId ? getVersionById({
...
export default compose(translate(), connect(mapStateToProps), withFixedErrorHandler({
  fileName: __filename,
  extractId,
}))(AddonBase);