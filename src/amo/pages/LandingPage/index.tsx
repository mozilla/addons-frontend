...
import { ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME, INSTALL_SOURCE_FEATURED, INSTALL_SOURCE_TOP_RATED, INSTALL_SOURCE_TRENDING, RECOMMENDED, SEARCH_SORT_RANDOM, SEARCH_SORT_TOP_RATED, SEARCH_SORT_TRENDING } from 'amo/constants';
import { withErrorHandler } from 'amo/errorHandler';
import { apiAddonType, apiAddonTypeIsValid, visibleAddonType as getVisibleAddonType } from 'amo/utils';
...
export class LandingPageBase extends React.Component {
  ...
  componentDidUpdate() {
    ...
    invariant(apiAddonTypeIsValid(params.visibleAddonType), 'LandingPage should not be accessible with an invalid addonType');
    this.getLandingDataIfNeeded();
    this.setViewContextType();
  }
  ...
  getLandingDataIfNeeded() {
    ...
    if (!loading && !errorHandler.hasError() && (!resultsLoaded || addonTypeOfResults !== requestedAddonType)) {
      dispatch(getLanding({
  ...
  contentForType = (visibleAddonType) => {
    ...
    return {
      addonType,
      html: contentForTypes[addonType],
    };
  };
  ...
  render() {
    ...
    const {
      addonType,
      html,
    } = this.contentForType(visibleAddonType);
    const headingText = {
    ...
    return <Page>
        ...
        >
          ...
          <Button buttonType="light" className="LandingPage-button" to={`/${getVisibleAddonType(addonType)}/categories/`}>
            {i18n.gettext('Explore all categories')}
          </Button>

          {this.renderIfNotEmpty(recommendedAddons, <LandingAddonsCard addonInstallSource={INSTALL_SOURCE_FEATURED} addons={recommendedAddons} className="RecommendedAddons" footerText={html.recommendedFooterText} footerLink={html.recommendedFooterLink} header={html.recommendedHeader} isTheme={isAddonTheme} loading={loading} />)}
          {this.renderIfNotEmpty(highlyRatedAddons, <LandingAddonsCard addonInstallSource={INSTALL_SOURCE_TOP_RATED} addons={highlyRatedAddons} className="HighlyRatedAddons" footerLink={html.highlyRatedFooterLink} footerText={html.highlyRatedFooterText} header={html.highlyRatedHeader} isTheme={isAddonTheme} loading={loading} />)}
          {this.renderIfNotEmpty(trendingAddons, <LandingAddonsCard addonInstallSource={INSTALL_SOURCE_TRENDING} addons={trendingAddons} className="TrendingAddons" footerLink={html.trendingFooterLink} footerText={html.trendingFooterText} header={html.trendingHeader} isTheme={isAddonTheme} loading={loading} />)}
        </div>
      </Page>;
  }
...
export default compose(connect(mapStateToProps), translate(), withErrorHandler({
  id: 'LandingPage',
}))(LandingPageBase);