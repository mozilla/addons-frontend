...
const CRITERIA_URL = 'https://extensionworkshop.com/documentation/publish/add-ons-blocking-process/?utm_source=addons.mozilla.org&utm_medium=referral&utm_content=blocked-addon';
const POLICIES_URL = 'https://extensionworkshop.com/documentation/publish/add-on-policies/?utm_source=addons.mozilla.org&utm_medium=referral&utm_content=blocked-addon';
const SUPPORT_URL = 'https://support.mozilla.org/kb/add-ons-cause-issues-are-on-blocklist';
export class BlockBase extends React.Component<InternalProps> {
  ...
  renderReason(): null | React.ReactNode {
    const {
      block,
    } = this.props;
    ...
    return <p className="Block-reason">
        {block ? sanitizeHTML(block.reason).__html : <LoadingText />}
      </p>;
  }

  renderDateAndURL(): React.ReactNode | Array<React.ReactNode | string> {
    const {
      block,
      i18n,
    } = this.props;
    ...
    const content: Array<React.ReactNode | string> = [i18n.sprintf(i18n.gettext('Blocked on %(date)s.'), {
      date: i18n.moment(block.created).format('ll'),
    })];

    if (block.url) {
      content.push(' ', <a key={block.url.url} href={block.url.outgoing} title={block.url.url}>
  ...
  renderVersions(): React.ReactNode | string {
    const {
      block,
      i18n,
    } = this.props;
  ...
  render(): React.ReactNode {
    ...
    const title = block && block.name ? i18n.sprintf(i18n.gettext(`%(addonName)s has been blocked for your protection.`), {
        ...
        <div className="Block-page">
          ...
          <Card className="Block-content" header={title}>
            ...
            <p // eslint-disable-next-line react/no-danger
              dangerouslySetInnerHTML={sanitizeHTML(i18n.sprintf(i18n.gettext(`This add-on violates %(startLink)sMozilla's
            ...
            <p // eslint-disable-next-line react/no-danger
              dangerouslySetInnerHTML={sanitizeHTML(i18n.sprintf(i18n.gettext(`When Mozilla becomes aware of add-ons, plugins,
...
const Block: React.ComponentType<Props> = compose(translate(), connect(mapStateToProps), withFixedErrorHandler({
  fileName: __filename,
  extractId,
}))(BlockBase);
