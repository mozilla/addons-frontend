...
import { getCategoryResultsPathname, getCategoryName } from 'amo/utils/categories';
...
export class CategoryPageBase extends React.Component<InternalProps> {
  getPageTitle(categoryName: string | null): string {
    ...
    if (addonType === ADDON_TYPE_STATIC_THEME) {
      return categoryName ? i18n.sprintf(i18n.gettext('Themes in %(categoryName)s'), {
        categoryName,
      }) : i18n.gettext('Themes');
    }

    return categoryName ? i18n.sprintf(i18n.gettext('Extensions in %(categoryName)s'), {
      categoryName,
  ...
  render(): React.ReactNode {
    ...
    const filtersForSearch = { ...filters,
      addonType,
...
const CategoryPage: React.ComponentType<Props> = compose(connect(mapStateToProps), translate(), withFixedErrorHandler({
  fileName: __filename,
  extractId,
}))(CategoryPageBase);
