...
import { ADDON_TYPE_STATIC_THEME, DISABLED, ENABLED, GLOBAL_EVENTS, GLOBAL_EVENT_STATUS_MAP, INACTIVE, INSTALL_EVENT_LIST, ON_INSTALLING_EVENT, ON_OPERATION_CANCELLED_EVENT, ON_UNINSTALLED_EVENT, SET_ENABLE_NOT_AVAILABLE } from 'amo/constants';
...
export type MozAddonManagerType = {
  ...
  addEventListener: (eventName: string, handler: (...args: Array<any>) => any) => void;
  createInstall: (arg0: {
    url: string;
    hash?: string | null;
  }) => Promise<any>;
...
export function getAddon(guid: string, {
  _mozAddonManager = window.navigator.mozAddonManager,
}: OptionalParams = {}): Promise<FirefoxAddon> {
...
export function hasAbuseReportPanelEnabled(_mozAddonManager: MozAddonManagerType = window.navigator.mozAddonManager): boolean {
...
export function reportAbuse(addonId: string, {
  _mozAddonManager = window.navigator.mozAddonManager,
}: OptionalParams = {}): Promise<boolean> {
  if (hasAbuseReportPanelEnabled(_mozAddonManager) && _mozAddonManager.reportAbuse) {
    return _mozAddonManager.reportAbuse && _mozAddonManager.reportAbuse(addonId);
  }

  return Promise.reject(new Error('Cannot report abuse via Firefox'));
}
type OptionalInstallParams = OptionalParams & {
  _log?: typeof log;
  hash?: string | null;
  onIgnoredRejection?: () => void;
};
export function install(_url: string, eventCallback: (...args: Array<any>) => any, {
  _log = log,
  _mozAddonManager = window.navigator.mozAddonManager,
  hash,
  onIgnoredRejection = () => {},
}: OptionalInstallParams = {}): Promise<void> {
  return _mozAddonManager.createInstall({
    url: _url,
    hash,
  }).then((installObj) => {
...
export function uninstall(guid: string, {
  _mozAddonManager = window.navigator.mozAddonManager,
}: OptionalParams = {}): Promise<void> {
  return getAddon(guid, {
    _mozAddonManager,
  }).then((addon) => {
...
export function addChangeListeners(callback: (arg0: {
  guid: string;
  status: $Values<typeof GLOBAL_EVENT_STATUS_MAP>;
  needsRestart: boolean;
  canUninstall: boolean;
}) => Promise<void>, mozAddonManager: MozAddonManagerType, {
  _log = log,
}: {
  _log: typeof log;
  ...
  function handleChangeEvent(e: AddonChangeEvent) {
    ...
    if (type === ON_OPERATION_CANCELLED_EVENT) {
      ...
      return getAddon(guid, {
        ...
        // eslint-disable-next-line amo/only-log-strings
        _log.error('Unexpected error after having received onOperationCancelled event: %o', error);
      });
    }
  ...
  if (mozAddonManager && mozAddonManager.addEventListener) {
    ...
    mozAddonManager.addEventListener(ON_OPERATION_CANCELLED_EVENT, handleChangeEvent);
...
export function enable(guid: string, {
  _mozAddonManager = window.navigator.mozAddonManager,
}: OptionalParams = {}): Promise<void> {
  return getAddon(guid, {
    _mozAddonManager,
