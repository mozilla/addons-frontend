 ...
 * )(MyComponent);
 */
export function render404IfConfigKeyIsFalse(configKey: string, {
  _config = config,
}: {
  _config: ConfigType;
} = {}): (Component: React.ComponentType<any>) => (props: unknown) => React.ReactNode {
  if (!configKey) {
    throw new TypeError('configKey cannot be empty');
  }

  return (Component: React.ComponentType<any>) => (props: unknown) => {
    if (!_config.get(configKey)) {
      log.debug(`config.${configKey} was false; not rendering ${getDisplayName(Component)}`);
      return <NotFound />;
    }
