...
import { USER_AGENT_BROWSER_FIREFOX, USER_AGENT_OS_ANDROID, USER_AGENT_OS_IOS } from 'amo/reducers/api';
import { ADDON_TYPE_EXTENSION, CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX, INCOMPATIBLE_ANDROID_UNSUPPORTED, INCOMPATIBLE_FIREFOX_FOR_IOS, INCOMPATIBLE_NOT_FIREFOX, INCOMPATIBLE_OVER_MAX_VERSION, INCOMPATIBLE_UNDER_MIN_VERSION, INCOMPATIBLE_UNSUPPORTED_PLATFORM, RECOMMENDED } from 'amo/constants';
...
export const isFirefoxForAndroid = (userAgentInfo: UserAgentInfoType): boolean => {
...
export const isAndroidInstallable = ({
  ...
  return !!addon && addon.type === ADDON_TYPE_EXTENSION && getPromotedCategory({
    addon,
...
export const getMobileHomepageLink = (lang: string): string => `/${lang}/${CLIENT_APP_ANDROID}/`;
export const correctedLocationForPlatform = ({
  ...
  if (isFirefoxForIOS(userAgentInfo) || !isFirefox({
    userAgentInfo,
  })) {
    return null;
  }

  if ( // Android browser on desktop site.
  isFirefoxForAndroid(userAgentInfo) && clientApp === CLIENT_APP_FIREFOX || // Android browser on page other than Home or Search.
  isFirefoxForAndroid(userAgentInfo) && !isHomePage && !location.pathname.includes('/search/')) {
    // Redirect to `android` home page.
    return getMobileHomepageLink(lang);
  }

  if (os.name !== USER_AGENT_OS_ANDROID && browser.name === USER_AGENT_BROWSER_FIREFOX && clientApp === CLIENT_APP_ANDROID) {
    // Desktop browser on android site: Redirect to same page on `firefox`.
    return `${location.pathname.replace(CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX)}${location.search}`;
  }
