...
type ExpressHandler = (req: typeof $Request, res: typeof $Response) => void | Promise<void>;
...
export const viewFrontendVersionHandler = ({
  ...
  const featureFlags = Object.keys(_config).filter((key) => {
  ...
  return (req: typeof $Request, res: typeof $Response) => {
    fs.stat(version, async (error) => {
      if (error) {
        _log.error(`Could not stat version file ${version}: ${error.toString()}`);
...
export const viewHeartbeatHandler = ({
  ...
  const apiURL = `${_config.get('apiHost')}${_config.get('apiPath')}${_config.get('apiVersion')}/site/?disable_caching`;
