...
import { API_ADDON_TYPES_MAPPING, DEFAULT_UTM_MEDIUM, DEFAULT_UTM_SOURCE, DOWNLOAD_FIREFOX_UTM_CAMPAIGN, PROMOTED_ADDONS_SUMO_URL, VISIBLE_ADDON_TYPES_MAPPING } from 'amo/constants';
...
export const makeQueryStringWithUTM = ({
  ...
  return makeQueryString({ ...otherQueryParams,
    utm_campaign,
    utm_content,
    utm_medium,
    utm_source,
...
export const checkInternalURL = ({
  ...
  const isRelative = // The double slash is for protocol-free URLs.
  urlString.startsWith('/') && !urlString.startsWith('//');
  const currentHost = url.parse(baseURL).host || '';
  const isForCurrentHost = currentHost === urlParts.host || urlString.startsWith(`//${currentHost}`);
  ...
  let relativeURL = urlString.startsWith(`//${currentHost}`) ? urlString.replace(`//${currentHost}`, '') : urlString.replace(baseURL, '');
...
export function getClientConfig(_config: typeof config): Record<string, unknown> {
  const clientConfig = {};
...
export function isValidClientApp(value: string, {
  _config = config,
}: {
  _config: typeof config;
} = {}): boolean {
...
export function sanitizeHTML(text: string | null | undefined, allowTags: Array<string> = [], _purify: typeof purify = purify): {
  __html: string;
} {
 ...
 * the textarea field.
 */
export function sanitizeUserHTML(text: string | null | undefined): {
  __html: string;
} {
  return sanitizeHTML(nl2br(text), ['a', 'abbr', 'acronym', 'b', 'blockquote', 'br', 'code', 'em', 'i', 'li', 'ol', 'strong', 'ul']);
}
...
export function apiAddonTypeIsValid(addonType: string): boolean {
  ...
  return Object.prototype.hasOwnProperty.call(API_ADDON_TYPES_MAPPING, addonType.toLowerCase());
}
export function apiAddonType(addonType: string): string {
  ...
  if (!apiAddonTypeIsValid(lcasedAddonType)) {
    throw new Error(`"${lcasedAddonType}" not found in API_ADDON_TYPES_MAPPING`);
  }
...
export function visibleAddonTypeIsValid(addonType: string): boolean {
  // $FlowFixMe: Deal with method-unbinding error.
  return Object.prototype.hasOwnProperty.call(VISIBLE_ADDON_TYPES_MAPPING, addonType.toLowerCase());
}
export function visibleAddonType(addonType: string): string {
  const lcasedAddonType = addonType.toLowerCase();

  if (!visibleAddonTypeIsValid(lcasedAddonType)) {
    throw new Error(`"${lcasedAddonType}" not found in VISIBLE_ADDON_TYPES_MAPPING`);
  }
...
export function isValidLocaleUrlException(value: string, {
  _config = config,
}: {
  _config?: typeof config;
} = {}): boolean {
...
export function isValidClientAppUrlException(value: string, {
  _config = config,
}: {
  _config?: typeof config;
} = {}): boolean {
...
export function isValidTrailingSlashUrlException(value: string, {
  _config = config,
}: {
  _config?: typeof config;
} = {}): boolean {
 ...
 */
export const safePromise = (callback: (...args: Array<any>) => any): (...args: any) => any | Promise<any> => (...args: any) => {
  try {
...
export const getClientAppAndLangFromPath = (urlString: string): {
  lang: string;
  clientApp: string;
} => {
