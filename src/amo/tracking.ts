...
export type SendTrackingEventParams = MakeTrackingEventDataParams & {
  _config?: typeof config;
  sendSecondEventWithOverrides?: Record<string, any>;
};
type IsDoNoTrackEnabledParams = {
  _log: typeof log;
  _navigator: typeof navigator | null | undefined;
  _window: typeof window | null | undefined;
};
...
export class Tracking {
  ...
  constructor({
    ...
    if (this.trackingEnabled) {
      ...
      declare let ga: {
        (arg0: string, arg1: string, arg2: string | null | undefined): void;
        q: Array<string>;
        l: number;
      };
  ...
  log(message: string, obj?: Record<string, any>) {
    if (this._log) {
  ...
  sendEvent({
    ...
    if (_config.get('server')) {
      ...
      if (typeof sendSecondEventWithOverrides === 'object') {
        const secondEventData = makeTrackingEventData({ ...trackingData,
          ...sendSecondEventWithOverrides,
        });
  ...
  pageView(data: Record<string, any> = {}) {
...
export const getAddonEventCategory = (type: string, installAction: string): string => {
  ...
  switch (installAction) {
    ...
    case INSTALL_CANCELLED_ACTION:
      return isThemeType ? INSTALL_CANCELLED_THEME_CATEGORY : INSTALL_CANCELLED_EXTENSION_CATEGORY;

    case INSTALL_DOWNLOAD_FAILED_ACTION:
      return isThemeType ? INSTALL_DOWNLOAD_FAILED_THEME_CATEGORY : INSTALL_DOWNLOAD_FAILED_EXTENSION_CATEGORY;

    case INSTALL_STARTED_ACTION:
      return isThemeType ? INSTALL_STARTED_THEME_CATEGORY : INSTALL_STARTED_EXTENSION_CATEGORY;

    case UNINSTALL_ACTION:
      return isThemeType ? UNINSTALL_THEME_CATEGORY : UNINSTALL_EXTENSION_CATEGORY;
...
export default (new Tracking() as Tracking);