import type { VersionIdType, ExternalAddonVersionType, PartialExternalAddonVersionType } from 'amo/reducers/versions';
import type { AddonTypeType, PromotedCategoryType } from 'amo/constants';
import type { LocalizedUrlWithOutgoing, LocalizedString, UrlWithOutgoing } from 'amo/types/api';

export type AddonStatusType = 'lite' | 'public' | 'deleted' | 'pending' | 'disabled' | 'rejected' | 'nominated' | 'incomplete' | 'unreviewed' | 'lite-nominated' | 'review-pending';
...
export type AddonAuthorType = PartialAddonAuthorType & {
  picture_url: string;
};
...
export type LanguageToolType = ExternalLanguageToolType & {
  name: string;
};
...
export type ExternalAddonType = {
  ...
  latest_unlisted_version?: ExternalAddonVersionType | null | undefined;
  locale_disambiguation?: string;
  name: LocalizedString;
...
export type PartialExternalAddonType = ExternalAddonType & {
 ...
 */
export type AddonType = ExternalAddonType & {
...
export type CollectionAddonType = AddonType & {
  notes: string | null;
};