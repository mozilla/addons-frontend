...
import { ADDON_TYPE_EXTENSION, CLIENT_APP_ANDROID, RECOMMENDED } from 'amo/constants';
...
export function addVersionCompatibilityToFilters({
  ...
  if (userAgentInfo.browser.name === 'Firefox' && userAgentInfo.os.name !== USER_AGENT_OS_IOS) {
...
export function convertFiltersToQueryParams(filters) {
  return Object.keys(paramsToFilter).reduce((object, key) => {
    if (filters && typeof filters[paramsToFilter[key]] !== 'undefined' && filters[paramsToFilter[key]] !== '') {
      return { ...object,
        [key]: filters[paramsToFilter[key]],
      };
    }
...
export const fixFiltersForClientApp = ({
  api,
  filters,
}) => {
  const newFilters = { ...filters,
  ...
  if (!newFilters.clientApp && api.clientApp) {
    log.debug(`No clientApp found in filters; using api.clientApp (${api.clientApp})`);
