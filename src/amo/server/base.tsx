...
import { viewFrontendVersionHandler, viewHeartbeatHandler } from 'amo/utils/server';
import { setAuthToken, setClientApp, setLang, setRegionCode, setRequestId, setUserAgent } from 'amo/reducers/api';
import { getDirection, isValidLang, langToLocale, makeI18n } from 'amo/i18n/utils';
...
export function getPageProps({
  ...
  const sriData = isDeployed ? JSON.parse(fs.readFileSync(path.join(config.get('basePath'), 'dist/sri.json'))) : {};
  ...
  return {
    ...
    trackingEnabled: convertBoolean(config.get('trackingEnabled')) && req.header('dnt') !== '1',
  };
}
...
function renderHTML({
  ...
  return ReactDOM.renderToString(<ServerHtml {...pageProps} {...props} appState={appState} />);
}
...
function sendHTML({
  ...
  const componentDeclaredStatus = NestedStatus.rewind();
  return res.status(componentDeclaredStatus).send(`<!DOCTYPE html>\n${html}`).end();
}
...
function baseServer(App, createStore, {
  _HotShots,
  ...
  app.use(async (req, res, next) => {
    try {
      ...
      const isAnonymousPage = config.get('anonymousPagePatterns').filter((pattern) => new RegExp(pattern).test(req.originalUrl)).length !== 0;
      ...
      try {
        ...
        if (error instanceof URIError) {
          ...
          return res.status(404).send(`<!DOCTYPE html>\nWe're sorry, we were unable to parse the request URL: ${error}`).end();
        }
      ...
      try {
        ...
        _log.info(`Falling back to default lang: "${config.get('defaultLang')}"`);
      }

      const i18n = makeI18n(i18nData, htmlLang);
      const props = {
        component: <Root cookies={req.universalCookies} history={connectedHistory} i18n={i18n} store={store}>
            <App />
          </Root>,
      };
      ...
      try {
        ...
        const responseStatusCode = isRedirecting ? redirectTo.status : componentDeclaredStatus || res.statusCode;

        if (['GET', 'HEAD'].includes(req.method) && responseStatusCode >= 200 && responseStatusCode < 400 && !token) {
...
export function runServer({
  ...
  return isoMorphicServer.server(config.get('basePath')).then(() => {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (listen === true) {
        if (useHttpsForDev) {
          if (host === 'example.com') {
            const options = {
              ...
              cert: fs.readFileSync('bin/local-dev-server-certs/example.com.pem'),
              passphrase: '',
            };
            server = https.createServer(options, server);
          } else {
            log.debug(`To use the HTTPS server you must serve the site at example.com (host was "${host}")`);
          }
        }
        ...
        server.listen(port, host, (err) => {
          ...
          // eslint-disable-next-line amo/only-log-strings
          log.info([`🔥 Addons-frontend server is running`, `[ENV:${config.util.getEnv('NODE_ENV')}]`, `[isDevelopment:${config.get('isDevelopment')}]`, `[isDeployed:${config.get('isDeployed')}]`, `[apiHost:${config.get('apiHost')}]`, `[apiPath:${config.get('apiPath')}]`, `[apiVersion:${config.get('apiVersion')}]`].join(' '));

          if (proxyEnabled) {
            const proxyPort = config.get('proxyPort');
            log.debug(`🚦 Proxy detected, frontend running at http://${host}:${port}.`);
            log.debug(`👁  Open your browser at http${useHttpsForDev ? 's' : ''}://${host}:${proxyPort} to view it.`);
          } else {
            log.debug(`👁  Open your browser at http${useHttpsForDev ? 's' : ''}://${host}:${port} to view it.`);
          }
