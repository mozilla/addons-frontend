...
export function trailingSlashesMiddleware(req, res, next, {
  ...
  if (isValidLang(UrlSlashSeparated[0], {
    _config,
  })) {
    UrlSlashSeparated[0] = '$lang';
  }

  if (isValidClientApp(UrlSlashSeparated[1], {
    _config,
  })) {
    UrlSlashSeparated[1] = '$clientApp';
  } else if (isValidClientApp(UrlSlashSeparated[0], {
    _config,
  })) {
  ...
  if (!isValidTrailingSlashUrlException(urlToCheck, {
    _config,
  }) && UrlParts[0].substr(-1) !== '/') {
  ...
  if (isValidTrailingSlashUrlException(urlToCheck, {
    ...
    log.info(`Not adding a trailing slash; validTrailingSlashUrlException found: ${urlToCheck}`);
  }
