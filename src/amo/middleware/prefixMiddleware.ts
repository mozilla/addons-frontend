...
import { getClientApp, isValidClientApp, isValidClientAppUrlException, isValidLocaleUrlException } from 'amo/utils';
import { getLanguage, isValidLang } from 'amo/i18n/utils';
import { ONE_YEAR_IN_SECONDS } from 'amo/constants';
import log from 'amo/logger';

export function prefixMiddleware(req, res, next, {
  _config = config,
} = {}) {
  ...
  const hasValidClientAppUrlExceptionInPartTwo = isValidClientAppUrlException(URLPathParts[1], {
    _config,
  });
  ...
  } else if (hasValidClientAppInPartTwo || hasValidClientAppUrlExceptionInPartTwo) {
  ...
  const hasValidClientAppUrlException = isValidClientAppUrlException(URLPathParts[2], {
    _config,
  });
