...
export class LocalState {
  ...
  load(): Promise<Record<string, any> | null> {
    return this.localForage.getItem(this.id).then((data) => {
      if (!data) {
        return null;
      }
  ...
  save(data: Record<string, any>): Promise<void> {
    if (typeof data !== 'object' || data === null) {
      return Promise.reject(new Error('The argument to save() must be an object'));
    }
...
export default function createLocalState(id: string, options?: LocalStateOptions): LocalState {
