...
export default async function createClient(createStore, {
  _config = config,
  ...
  if (_config.get('isDevelopment')) {
    ...
    const {
      fetchBufferedLogs,
    } = require('pino-devtools/src/client');
  ...
  const history = addQueryParamsToHistory({
    history: _createBrowserHistory({
      ...
      forceRefresh: initialState && initialState.site ? initialState.site.loadedPageIsAnonymous : false,
    }),
  });
  ...
  const renderApp = (App) => {
    render(<Root history={connectedHistory} i18n={i18n} store={store}>
        <App />
      </Root>, document.getElementById('react-view'));
  };
