...
export function localeToLang(locale?: string, log_: typeof log = log): void | string {
  let lang;
...
export function langToLocale(language?: string, log_: typeof log = log): void | string {
  let locale;
...
}: IsSupportedLangOptions = {}): boolean {
 ...
 * { lang: 'pl', quality: 0.7 }
 */
export function parseAcceptLanguage(header: string): Array<{
  lang: string;
  quality: number;
}> {
...
type GetLangFromHeaderOptions = {
  _supportedLangs?: Record<string, any>;
};
 ...
 *
 */
export function getLangFromHeader(acceptLanguage: string, {
  _supportedLangs,
}: GetLangFromHeaderOptions = {}): void | string {
  let userLang;
  ...
  if (acceptLanguage) {
    ...
    for (const langPref of langList) {
      if (isSupportedLang(normalizeLang(langPref.lang), {
        _supportedLangs,
      })) {
        userLang = langPref.lang;
        break; // Match locale, even if region isn't supported
      } else if (isSupportedLang(normalizeLang(langPref.lang.split('-')[0]), {
        _supportedLangs,
      })) {
...
export function getLanguage({
  ...
  return {
    lang: sanitizeLanguage(userLang),
  };
}
...
export const formatFilesize = ({
  ...
  if (!sizeNumber || !sizeName) {
    _log.error(`Filesize returned sizeNumber: "${sizeNumber}", sizeName: "${sizeName}" size "${size}"`);
  ...
  return i18n.sprintf(sizeString, {
    localizedSize,
  });
};
type I18nConfig = {
  ...
  _momentDefineLocale?: (...args: Array<any>) => any;
};
...
export function makeI18n(i18nData: I18nConfig, lang: string, _Jed: typeof Jed = Jed, {
  ...
  if (typeof _Intl === 'object' && // $FlowFixMe: Deal with method-unbinding error.
  ...
  i18n.dcnpgettext = function dcnpgettext(domain, context, singularKey, pluralKey, val) {
    return i18n._dcnpgettext(domain, context, oneLineTranslationString(singularKey), oneLineTranslationString(pluralKey), val);
  };
...
type ReplaceStringsWithJSXParams = {
  text: string;
  replacements: Array<[string, string, (text: string) => React.ReactNode]>;
};
export const replaceStringsWithJSX = ({
  text,
  replacements,
}: ReplaceStringsWithJSXParams): Array<React.ReactNode | string> => {
  ...
  if (numberOfPairs !== replacements.length) {
    throw new Error(`Expected ${numberOfPairs} replacements but only got ${replacements.length}`);
  }

  const expression = new RegExp([// Before any replacement (pair of keys).
  '^(.*?)', // Add a generic regexp to match each replacement entry, possibly
  // separated by some more text.
  replacements.map(() => '%\\((\\w+)\\)s(.+?)%\\((\\w+)\\)s').join('(.*?)'), // After all replacements (pairs of keys).
  '(.*?)$'].join(''));
  ...
  if (!matches) {
    ...
    throw new Error('`text` does not appear to be compatible with the provided replacements');
  }
  ...
  if (Object.keys(internalReplacements).length > 0) {
    throw new Error(`Not all replacements have been used; unused keys: ${Object.keys(internalReplacements).join('; ')}`);
  }
