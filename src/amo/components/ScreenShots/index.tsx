...
export default class ScreenShots extends React.Component<Props> {
  ...
  onOpenPhotoswipe: (photoswipe: Record<string, any>) => void = (photoswipe: Record<string, any>) => {
    ...
    return <div className="ScreenShots">
        ...
        this.viewport = el;
          ...
          <div className="ScreenShots-list">
            <Gallery options={PHOTO_SWIPE_OPTIONS} onOpen={this.onOpenPhotoswipe}>
              {previews.map((preview) => <Item key={preview.src} original={preview.src} thumbnail={preview.thumbnail_src} width={preview.w} height={preview.h} title={preview.title}>
                  {({
                ref,
                open,
              }) => // eslint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-noninteractive-element-interactions
              <img alt={preview.title} className="ScreenShots-image" ref={ref} onClick={open} src={preview.thumbnail_src} width={preview.thumbnail_w} height={preview.thumbnail_h} />}
                </Item>)}
            </Gallery>
          </div>
        </div>
