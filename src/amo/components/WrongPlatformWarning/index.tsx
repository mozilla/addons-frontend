...
import { correctedLocationForPlatform, getMobileHomepageLink, isAndroidInstallable, isFirefoxForAndroid, isFirefoxForIOS } from 'amo/utils/compatibility';
...
export class WrongPlatformWarningBase extends React.Component<InternalProps> {
  ...
  render(): null | React.ReactNode {
    ...
    if (_isFirefoxForIOS(userAgentInfo)) {
      // Firefox for iOS.
      message = i18n.gettext(`Add-ons are not compatible with Firefox for iOS. Try installing them on Firefox for desktop.`);
    } else if (addon && _isFirefoxForAndroid(userAgentInfo) && _isAndroidInstallable({
      addon,
    })) {
      ...
      message = i18n.sprintf(i18n.gettext(`To find add-ons compatible with Firefox for Android,
      ...
      message = i18n.sprintf(`This listing is not intended for this platform.
        <a href="%(newLocation)s">Browse add-ons for Firefox on desktop</a>.`, {
        newLocation,
      });
    } else if (newLocation) {
      // Redirecting to new page on the desktop site.
      message = i18n.sprintf(`To find add-ons compatible with Firefox on desktop,
               <a href="%(newLocation)s">visit our desktop site</a>.`, {
        newLocation,
      });
    }

    return message ? <div className={makeClassName('WrongPlatformWarning', className)}>
        <Notice id="WrongPlatformWarning-Notice" type={warningInfoType}>
          <span
            className="WrongPlatformWarning-message" // eslint-disable-next-line react/no-danger
...
const WrongPlatformWarning: React.ComponentType<Props> = compose(withRouter, connect(mapStateToProps), translate())(WrongPlatformWarningBase);
