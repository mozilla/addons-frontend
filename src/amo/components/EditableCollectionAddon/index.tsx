...
import type { DeleteAddonNoteFunc, RemoveCollectionAddonFunc, SaveAddonNoteFunc } from 'amo/pages/Collection';
...
type InternalProps = Props & {
  ...
  i18n: I18nType;
  setUIState: (arg0: $Shape<UIStateType>) => void;
  uiState: UIStateType;
};
...
export class EditableCollectionAddonBase extends React.Component<InternalProps> {
  ...
  render(): React.ReactNode {
    ...
    return <li className={makeClassName('EditableCollectionAddon', `EditableCollectionAddon--${addon.type}`)}>
        <div className="EditableCollectionAddon-details">
          <img className="EditableCollectionAddon-icon" src={iconURL} alt={addon.name} />
        ...
        <div className="EditableCollectionAddon-buttons">
          ...
          >
            <Button buttonType="action" className="EditableCollectionAddon-leaveNote-button" micro onClick={this.onEditNote}>
              {i18n.gettext('Leave a note')}
            </Button>
          </div>
          <Button buttonType="alert" className="EditableCollectionAddon-remove-button" micro name={addon.id} onClick={this.onRemoveAddon}>
            {i18n.gettext('Remove')}
          </Button>
        </div>
        {showNotes && <div className="EditableCollectionAddon-notes">
            <h4 className="EditableCollectionAddon-notes-header">
              <Icon name="comments-blue" />
              {this.props.uiState.editingNote ? i18n.gettext('Leave a note') : i18n.gettext("Collector's note")}
            </h4>

            {this.props.uiState.editingNote ? <>
                {errorHandler.renderErrorIfPresent()}
                <DismissibleTextForm className="EditableCollectionAddon-notes-form" id={`${normalizeFileNameId(__filename)}-${extractId(this.props)}`} microButtons onDelete={addon.notes ? this.onDeleteNote : null} onDismiss={this.onDismissNoteForm} onSubmit={this.onSaveNote} placeholder={i18n.gettext('Add a comment about this add-on.')} submitButtonText={i18n.gettext('Save')} text={sanitizeHTML(addon.notes || '').__html} />
              </> : <div className="EditableCollectionAddon-notes-read-only">
                <span
                  className="EditableCollectionAddon-notes-content" // eslint-disable-next-line react/no-danger
                  dangerouslySetInnerHTML={sanitizeHTML(nl2br(addon.notes || ''), ['br', 'a'])}
                />
                <div className="EditableCollectionAddon-notes-buttons">
                  <Button buttonType="action" className="EditableCollectionAddon-notes-edit-button" micro onClick={this.onEditNote}>
                    {i18n.gettext('Edit')}
                  </Button>
                </div>
              </div>}
...
const EditableCollectionAddon: React.ComponentType<Props> = compose(translate(), withFixedErrorHandler({
  fileName: __filename,
  extractId,
}), withUIState({
  fileName: __filename,
