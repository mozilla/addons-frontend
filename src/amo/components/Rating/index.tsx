...
import 'amo/types/i18n';
import './styles.scss';

export const RATING_STYLE_SIZE_TYPES = {
  small: '',
  large: '',
};
...
type InternalProps = Props & {
  _setState?: (arg0: $Shape<StateType>) => void;
  i18n: I18nType;
};
export class RatingBase extends React.Component<InternalProps, StateType> {
  ...
  onSelectRating: (event: React.SyntheticEvent<HTMLButtonElement>) => void = (event: React.SyntheticEvent<HTMLButtonElement>) => {
    const {
      onSelectRating,
    } = this.props;
    invariant(onSelectRating, 'onSelectRating was empty. Did you mean to set readOnly=true?');
  ...
  renderTitle: (rating: number | null | undefined, readOnly: boolean | void, starRating: number | null) => string = (rating: number | null | undefined, readOnly: boolean | void, starRating: number | null) => {
    const {
      i18n,
    } = this.props;
    ...
    if (rating) {
      ...
      return i18n.sprintf(i18n.gettext(`Update your rating to %(starRating)s out of 5`), {
        starRating,
      });
    }

    return i18n.sprintf(i18n.gettext(`Rate this add-on %(starRating)s out of 5`), {
      starRating,
    });
  };
  ...
  renderRatings(): Array<React.ReactNode> {
    const {
      readOnly,
    } = this.props;
    const {
    ...
    return [1, 2, 3, 4, 5].map((thisRating) => {
      ...
      return (// eslint-disable-next-line react/jsx-key
        <>
          <button aria-describedby={id} type="button" value={thisRating} {...props}>
  ...
  render(): React.ReactNode {
    ...
    if (!styleSize || !RATING_STYLE_SIZES.includes(styleSize)) {
      throw new Error(oneLine`styleSize=${styleSize || '[empty string]'} is not a valid
    ...
    const allClassNames = makeClassName('Rating', `Rating--${styleSize}`, className, {
    ...
    return <div className={allClassNames} title={description} onMouseLeave={this.stopHovering} {...extraProps}>
        {this.renderRatings()}
