...
const validTypes = [errorType, genericType, genericWarningType, firefoxRequiredType, successType, warningInfoType, warningType];
type UIState = {
  wasDismissed: boolean;
};
export type NoticeType = typeof errorType | typeof firefoxRequiredType | typeof genericType | typeof genericWarningType | typeof successType | typeof warningInfoType | typeof warningType;
type Props = {
  actionHref?: string;
  actionOnClick?: (...args: Array<any>) => any;
...
export class NoticeBase extends React.Component<InternalProps> {
  ...
  render(): null | React.ReactNode {
    ...
    if (Object.values(buttonProps).some((val) => val !== undefined)) {
      invariant(actionText, 'When specifying an action button, actionText is required');
      actionButton = <Button className="Notice-button" micro target={actionTarget} {...buttonProps}>
          {actionText}
        </Button>;
    }
    ...
    return <div className={finalClass}>
        ...
        {dismissible && <div className="Notice-dismisser">
            <Button className="Notice-dismisser-button" onClick={this.onDismissNotice}>
              <IconXMark className="Notice-dismisser-icon" alt={i18n.gettext('Dismiss this notice')} />
            </Button>
          </div>}
      </div>;
  }
...
const extractId = (props: Props) => {
  if (props.dismissible) {
    invariant(props.id, 'When dismissible=true, the id property must be defined');
  }
...
const Notice: React.ComponentType<Props> = compose(withUIState({
  fileName: __filename,
  extractId,
  initialState,
}), translate())(NoticeBase);
