...
export class PermissionUtils {
  ...
  permissionStrings: Record<string, any>;
  ...
  formatPermissions(addonPermissions: Array<string>): Array<React.ReactNode> {
    ...
    for (const permission of addonPermissions) {
      const {
        type,
        value,
      } = this.classifyPermission(permission);
    ...
    if (permissions.permissions.includes(nativeMessagingPermission)) {
      permissionsToDisplay.push(<Permission type={nativeMessagingPermission} description={this.permissionStrings[nativeMessagingPermission]} key={nativeMessagingPermission} />);
    }
    ...
    for (const permission of permissionsCopy.sort()) {
      ...
      if (this.permissionStrings[permission]) {
        permissionsToDisplay.push(<Permission type={permission} description={this.permissionStrings[permission]} key={permission} />);
      }
    }

    // Finally, Add the host permissions.
    if (permissions.hosts.length) {
      permissionsToDisplay.push(<HostPermissions permissions={permissions.hosts} />);
    }
