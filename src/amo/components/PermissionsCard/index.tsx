...
export class PermissionsCardBase extends React.Component<InternalProps> {
  render(): null | React.ReactNode {
    ...
    if (!addonPermissions.optional.length && !addonPermissions.required.length) {
      return null;
    }

    const optionalContent = permissionUtils.formatPermissions(addonPermissions.optional);
    const requiredContent = permissionUtils.formatPermissions(addonPermissions.required);

    if (!optionalContent.length && !requiredContent.length) {
      return null;
    }

    const header = <div className="PermissionsCard-header">
        {i18n.gettext('Permissions')}
        <Link className="PermissionsCard-learn-more" href="https://support.mozilla.org/kb/permission-request-messages-firefox-extensions" target="_blank" externalDark prependClientApp={false} prependLang={false}>
          {i18n.gettext('Learn more')}
        </Link>
      </div>;
    return <ShowMoreCard header={header} contentId={version.id} className="PermissionsCard" id="AddonDescription-permissions-card" maxHeight={300}>
...
const PermissionsCard: React.ComponentType<Props> = compose(translate())(PermissionsCardBase);
