...
export function changeLocaleURL({
  currentLocale,
  location,
  newLocale,
}) {
  const newPath = location.pathname.replace(new RegExp(`^/${currentLocale}/`), `/${newLocale}/`);
...
export class LanguagePickerBase extends React.Component {
  ...
  render() {
    ...
    return <div className="LanguagePicker">
        ...
        <select className="LanguagePicker-selector" defaultValue={currentLocale} id="lang-picker" onChange={this.onChange}>
          {Object.keys(languages).map((locale) => <option key={locale} value={locale}>
...
function mapStateToProps(state) {
  return {
    currentLocale: state.api.lang,
  };
}

export default compose(withRouter, connect(mapStateToProps), translate())(LanguagePickerBase);