...
import type { ElementEvent, HTMLElementEventHandler, InputEvent } from 'amo/types/dom';
import './styles.scss';
...
export class DropdownMenuBase extends React.Component<InternalProps, State> {
  ...
  render(): React.ReactNode {
    ...
    >
        <button className="DropdownMenu-button" onClick={this.handleOnClick} title={text} type="button" aria-haspopup="true">
          <span className="DropdownMenu-button-text">{text}</span>
          <Icon name="inverted-caret" />
        </button>

        {children && <ul className="DropdownMenu-items" aria-hidden={!this.state.buttonIsActive} aria-label="submenu">
            {children}
          </ul>}
...
export default (onClickOutside(DropdownMenuBase) as React.ComponentType<Props>);