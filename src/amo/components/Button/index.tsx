...
export type ButtonType = 'neutral' | 'light' | 'action' | 'cancel' | 'confirm' | 'alert' | 'none';
...
const BUTTON_TYPES = ['neutral', 'light', 'action', 'cancel', 'confirm', 'alert', 'none'];
export default class Button extends React.Component<Props> {
  ...
  render(): React.ReactNode {
    ...
    const getClassName = (...classConfig) => {
      return makeClassName('Button', `Button--${buttonType}`, className, ...classConfig, {
    ...
    if (noLink) {
      return <span className={getClassName()} title={rest.title}>
          {children}
        </span>;
    }
