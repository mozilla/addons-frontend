...
type Props = {
  ...
  as?: string;
  linkToAddon?: boolean;
  queryParamsForAttribution?: Record<string, string>;
};
type PropsFromState = {
  isRTL: boolean;
};
...
export const AddonTitleBase = ({
  ...
  if (addon && addon.authors) {
    ...
    addonAuthors.forEach((author, index) => {
      authors.push(author.url ? <Link key={author.id} to={`/user/${author.id}/`}>
            {author.name}
          </Link> : author.name);
  ...
  return <Component className="AddonTitle">
      {addon ? <>
          {linkToAddon ? <Link to={addQueryParams(getAddonURL(addon.slug), queryParamsForAttribution)}>
              {addon.name}
            </Link> : addon.name}
          {authors.length > 0 && <span className="AddonTitle-author">
              {' '}
              {isRTL ? <>
                  {authors}{' '}
...
const AddonTitle: React.ComponentType<Props> = compose(translate(), connect(mapStateToProps))(AddonTitleBase);
