...
import { setClientApp as setClientAppAction, setUserAgent as setUserAgentAction } from 'amo/reducers/api';
...
interface MozNavigator extends Navigator {
  mozAddonManager?: Record<string, any>;
}
type PropsFromState = {
  clientApp: string;
  lang: string;
...
type DefaultProps = {
  _addChangeListeners: (callback: (...args: Array<any>) => any, mozAddonManager: Record<string, any>) => any;
  _navigator: typeof navigator | null;
  mozAddonManager: $PropertyType<MozNavigator, 'mozAddonManager'>;
  userAgent: string | null;
};
type Props = PropsFromState & DefaultProps & {
...
export class AppBase extends React.Component<Props> {
  ...
  static defaultProps: DefaultProps = {
    ...
    mozAddonManager: config.get('server') ? {} : (navigator as MozNavigator).mozAddonManager,
    userAgent: null,
  };
  ...
  componentDidMount() {
    ...
    if (!userAgent && _navigator && _navigator.userAgent) {
      log.info('userAgent not in state on App load; using navigator.userAgent.');
  ...
  componentDidUpdate() {
    ...
    const {
      clientApp: clientAppFromURL,
    } = getClientAppAndLangFromPath(location.pathname);
  ...
  render(): React.ReactNode {
    ...
    let defaultTitle = i18n.sprintf(i18n.gettext('Add-ons for Firefox (%(locale)s)'), i18nValues);
    let titleTemplate = i18n.sprintf(i18n.gettext('%(title)s – Add-ons for Firefox (%(locale)s)'), // We inject `%s` as a named argument to avoid localizer mistakes. Helmet
    ...
    if (clientApp === CLIENT_APP_ANDROID) {
      defaultTitle = i18n.sprintf(i18n.gettext('Add-ons for Firefox Android (%(locale)s)'), i18nValues);
      titleTemplate = i18n.sprintf(i18n.gettext('%(title)s – Add-ons for Firefox Android (%(locale)s)'), // We inject `%s` as a named argument to avoid localizer mistakes.
...
const App: React.ComponentType<Props> = compose(withRouter, connect(mapStateToProps, mapDispatchToProps), translate())(AppBase);
export default App;