...
import { autocompleteCancel, autocompleteStart } from 'amo/reducers/autocomplete';
...
type Props = {
  ...
  onSearch?: (arg0: SearchFilters) => void;
  onSuggestionSelected: (arg0: SuggestionType) => void;
...
type InternalProps = Props & DefaultProps & PropsFromState & {
...
export class AutoSearchInputBase extends React.Component<InternalProps, State> {
  dispatchAutocompleteStart: (filters: Record<string, any>) => void;
  ...
  createFiltersFromQuery(query: string): Record<string, any> {
    const {
      location,
    } = this.props;
    ...
    // we remove it here.
    if (filtersFromLocation.sort && filtersFromLocation.sort === SEARCH_SORT_RANDOM) {
  ...
  handleSuggestionsFetchRequested: (arg0: OnSuggestionsFetchRequestedParams) => void = ({
    value,
  }: OnSuggestionsFetchRequestedParams) => {
    invariant(value, 'It should not be possible to have a falsey value');
    invariant(value.length <= SEARCH_TERM_MAX_LENGTH, `It should not be possible to have a value > ${SEARCH_TERM_MAX_LENGTH}`);
    ...
    this.dispatchAutocompleteStart({
      filters,
    });
  };
  ...
  handleSearch: HTMLElementEventHandler = (event: ElementEvent) => {
    ...
    if (onSearch) {
      const filters = this.createFiltersFromQuery(this.state.searchValue.trim());
      onSearch(filters);
    }
  };

  handleSearchChange: (event: ElementEvent, arg1: OnSearchChangeParams) => void = (event: ElementEvent, {
    newValue,
  ...
  handleSuggestionSelected: (event: ElementEvent, arg1: {
    suggestion: SuggestionType;
  }) => void = (event: ElementEvent, {
    suggestion,
  }: {
    suggestion: SuggestionType;
  }) => {
  ...
  renderSuggestion: (suggestion: SuggestionType) => React.ReactNode = (suggestion: SuggestionType) => {
    const {
      loadingSuggestions,
      selectSuggestionText,
    } = this.props;
    return <SearchSuggestion arrowAlt={selectSuggestionText} loading={loadingSuggestions} suggestion={suggestion} />;
  };
  ...
  render(): React.ReactNode {
    ...
    const autocompleteIsOpen = this.state.autocompleteIsOpen && // This prevents the input to look like Autosuggest is open when
    ...
    >
        ...
        <div className="AutoSearchInput-search-box">
          <Icon className="AutoSearchInput-icon-magnifying-glass" name="magnifying-glass" />
          <Autosuggest
          ...
          if (autosuggest) {
            ...
            theme={theme}
          />
          <button className="AutoSearchInput-submit-button" onClick={this.handleSearch} type="submit">
...
const AutoSearchInput: React.ComponentType<Props> = compose(withRouter, withFixedErrorHandler({
  fileName: __filename,
  extractId,
}), connect(mapStateToProps), translate())(AutoSearchInputBase);
