...
import { ADDON_TYPE_STATIC_THEME, DISABLED, DISABLING, DOWNLOADING, ENABLED, ENABLING, INACTIVE, INSTALLED, INSTALLING, UNINSTALLING, UNKNOWN } from 'amo/constants';
...
type Props = WithInstallHelpersInjectedProps & {
  addon: AddonType;
...
type ButtonProps = {
  ...
  disabled: boolean;
  href: string | void;
  onClick: ((...args: Array<any>) => any) | null;
  puffy: boolean;
};
...
export class AMInstallButtonBase extends React.Component<InternalProps> {
  ...
  showLoadingAnimation(): boolean {
    ...
    return [DISABLING, DOWNLOADING, ENABLING, INSTALLING, UNINSTALLING].includes(status);
  }
  ...
  getButtonText(): string {
    ...
    switch (status) {
      ...
      default:
        ...
        return ADDON_TYPE_STATIC_THEME === addon.type ? i18n.gettext('Install Theme') : i18n.gettext('Add to Firefox');
    }
  }
  ...
  render(): null | React.ReactNode {
    ...
    const buttonIsDisabled = disabled === true || !installURL ? true : hasAddonManager && status === UNKNOWN;
    ...
    if (!buttonIsDisabled) {
      if ([ENABLED, INSTALLED].includes(status)) {
        ...
        buttonProps.className = makeClassName(buttonProps.className, 'AMInstallButton-button--uninstall');
        ...
        buttonProps.className = makeClassName(buttonProps.className, 'AMInstallButton-button--enable');
    ...
    return <TransitionGroup className={makeClassName('AMInstallButton', className)}>
        {this.showLoadingAnimation() ? <CSSTransition key="loading" {...transitionProps}>
            <div
          ...
          </CSSTransition> : <CSSTransition key="button" {...transitionProps}>
...
const AMInstallButton: React.ComponentType<Props> = compose(translate())(AMInstallButtonBase);
