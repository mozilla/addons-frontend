...
import { INCOMPATIBLE_ANDROID_UNSUPPORTED, INCOMPATIBLE_FIREFOX_FOR_IOS, INCOMPATIBLE_NOT_FIREFOX, INCOMPATIBLE_OVER_MAX_VERSION, INCOMPATIBLE_UNSUPPORTED_PLATFORM, UNKNOWN } from 'amo/constants';
...
type InternalProps = Props & WithInstallHelpersInjectedProps & PropsFromState & {
  i18n: I18nType;
};
export const InstallButtonWrapperBase = (props: InternalProps): React.ReactNode => {
  const {
  ...
  if (addon && browserIsFirefox) {
    ...
    if (!isCompatible && // We want to show the download button and hide the install button for
    ...
    ![INCOMPATIBLE_ANDROID_UNSUPPORTED, INCOMPATIBLE_FIREFOX_FOR_IOS, INCOMPATIBLE_NOT_FIREFOX, INCOMPATIBLE_OVER_MAX_VERSION, INCOMPATIBLE_UNSUPPORTED_PLATFORM].includes(compatibility.reason)) {
  ...
  const installURL = currentVersion && currentVersion.file ? currentVersion.file.url : undefined;
  const showDownloadLink = !isCompatible && installURL;

  const showFileLink = () => {
    return <div className="InstallButtonWrapper-download">
  ...
  return addon && <div className="InstallButtonWrapper">
        {!showLinkInsteadOfButton && <>
            {showInstallButton ? <AMInstallButton
              addon={addon}
              canUninstall={canUninstall}
              className={makeClassName(className ? `AMInstallButton--${className}` : '', {
        'AMInstallButton--noDownloadLink': !showDownloadLink,
              ...
              uninstall={uninstall}
            /> : null}
            {showDownloadButton ? <GetFirefoxButton addon={addon} className={className ? `GetFirefoxButton--${className}` : ''} forIncompatibleAddon={forIncompatibleAddon} /> : null}
          </>}
...
const InstallButtonWrapper: React.ComponentType<Props> = compose(withRouter, withInstallHelpers, connect(mapStateToProps), translate())(InstallButtonWrapperBase);
