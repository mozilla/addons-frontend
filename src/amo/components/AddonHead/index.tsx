...
import { ADDON_TYPE_DICT, ADDON_TYPE_EXTENSION, ADDON_TYPE_LANG, ADDON_TYPE_STATIC_THEME, CLIENT_APP_ANDROID } from 'amo/constants';
...
export class AddonHeadBase extends React.Component<InternalProps> {
    ...
    if (clientApp === CLIENT_APP_ANDROID) {
      switch (addon.type) {
        ...
        case ADDON_TYPE_STATIC_THEME:
          // translators: please keep the fox emoji next to "Firefox Android".
          localizedTitle = i18n.gettext(`%(addonName)s – Get this Theme for 🦊 Firefox Android (%(locale)s)`);
          break;
  ...
  getPageDescription(): string {
    ...
    invariant(addon, 'addon is required');
    return i18n.sprintf(i18n.gettext('Download %(addonName)s for Firefox. %(summary)s'), {
      addonName: addon.name,
    ...
    return <>
        <Helmet titleTemplate={null}>
          ...
          <script type="application/ld+json">
            {serialize(_getAddonJsonLinkedData({
            addon,
            currentVersion,
          }), {
            isJSON: true,
        ...
        <HeadMetaTags appendDefaultTitle={false} date={addon.created} description={this.getPageDescription()} image={getPreviewImage(addon)} lastModified={lastUpdated} title={this.getPageTitle()} withTwitterMeta={addon.type === ADDON_TYPE_EXTENSION} />
...
const AddonHead: React.ComponentType<Props> = compose(translate(), connect(mapStateToProps))(AddonHeadBase);
