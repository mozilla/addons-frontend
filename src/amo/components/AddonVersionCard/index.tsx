...
export const AddonVersionCardBase = (props: InternalProps): React.ReactNode => {
  const {
    addon,
    headerText,
    i18n,
  ...
  if (version === null) {
    return <li className="AddonVersionCard">
        <div>
          {headerText && <h1 className="AddonVersionCard-header">{headerText}</h1>}
  ...
  const versionNumber = version ? i18n.sprintf(i18n.gettext('Version %(versionNumber)s'), {
  ...
  const getFileInfoText = () => {
    ...
    return <div className="AddonVersionCard-fileInfo">
        {i18n.sprintf(i18n.gettext('Released %(dateReleased)s - %(fileSize)s'), i18nParams)}
      </div>;
  };
  ...
  if (version) {
    ...
    if (addon && license) {
      if (license.url) {
        ...
        const licenseText = license.name ? i18n.sprintf(i18n.gettext('Source code released under %(linkStart)s%(licenseName)s%(linkEnd)s'), {
          licenseName: license.name,
          // Keep the link placeholders so that we can use them to inject a
          // `<Link />` using `replaceStringsWithJSX`.
          linkStart: '%(linkStart)s',
          linkEnd: '%(linkEnd)s',
        }) : i18n.gettext('Source code released under %(linkStart)sCustom License%(linkEnd)s');
        const licenseLink = replaceStringsWithJSX({
          text: licenseText,
          replacements: [['linkStart', 'linkEnd', (text) => // eslint-disable-next-line react/prop-types
          <Link key={addon.slug} {...licenseLinkParams}>
                  {text}
                </Link>]],
        });
        licenseSection = <p className="AddonVersionCard-license">{licenseLink}</p>;
      } else {
        licenseSection = <p className="AddonVersionCard-license">
            {i18n.sprintf(i18n.gettext('Source code released under %(licenseName)s'), {
            licenseName: license.name,
          })}
          </p>;
      }
  ...
  return <li className="AddonVersionCard">
      <div className="AddonVersionCard-content">
        <>
          ...
          {versionInfo && <div className="AddonVersionCard-compatibility">
              {versionInfo.compatibilityString}
            </div>}

          {version ? <div
            className="AddonVersionCard-releaseNotes" // eslint-disable-next-line react/no-danger
      ...
      <InstallButtonWrapper addon={addon} version={version} showLinkInsteadOfButton={!isCurrentVersion} />
    </li>;
};
...
function mapStateToProps(state: AppState, ownProps: InternalProps): PropsFromState {
  ...
  return {
    ...
    installError: installedAddon && installedAddon.error ? installedAddon.error : null,
  };
}

const AddonVersionCard: React.ComponentType<Props> = compose(translate(), connect(mapStateToProps))(AddonVersionCardBase);
