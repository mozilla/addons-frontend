...
export const AddonSummaryCardBase = ({
  ...
  const addonUrl = addon ? addQueryParams(getAddonURL(addon.slug), queryParamsForAttribution) : '';
  const iconUrl = getAddonIconUrl(addon);
  const iconImage = <img className="AddonSummaryCard-header-icon-image" src={iconUrl} alt={i18n.gettext('Add-on icon')} />;
  const metaHeader = <div className="AddonSummaryCard-header">
      ...
      <div className="AddonSummaryCard-header-text">
        ...
        <AddonTitle addon={addon} linkToAddon queryParamsForAttribution={queryParamsForAttribution} />
      </div>
    </div>;
  let addonAverage;
  ...
  if (addon && addon.ratings) {
    ...
    addonAverage = i18n.sprintf( // eslint-disable-next-line max-len
    // translators: roundedAverage is a number rounded to one digit, such as 4.5 in English or ٤٫٧ in Arabic.
    i18n.ngettext('%(rating)s Star out of 5', '%(rating)s Stars out of 5', roundedAverage), {
      rating: i18n.formatNumber(roundedAverage),
    });
  }

  return <Card header={metaHeader} className="AddonSummaryCard">
      <div className="AddonSummaryCard-overallRatingStars">
        <Rating rating={addon && addon.ratings && addon.ratings.average} readOnly yellowStars />
...
const AddonSummaryCard: React.ComponentType<Props> = compose(withRouter, translate())(AddonSummaryCardBase);
