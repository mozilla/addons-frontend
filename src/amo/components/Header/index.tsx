...
import AuthenticateButton, { createHandleLogOutFunction } from 'amo/components/AuthenticateButton';
...
export class HeaderBase extends React.Component {
  ...
  renderMenuOrAuthButton() {
    ...
    return siteUser ? <DropdownMenu text={siteUser.name} className="Header-authenticate-button Header-button">
        <DropdownMenuItem>{i18n.gettext('My Account')}</DropdownMenuItem>
        <DropdownMenuItem>
          <Link className="Header-user-menu-collections-link" to="/collections/">
            {i18n.gettext('View My Collections')}
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem>
          <Link className="Header-user-menu-view-profile-link" to={siteUser ? `/user/${siteUser.id}/` : null}>
            {i18n.gettext('View My Profile')}
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem>
          <Link className="Header-user-menu-edit-profile-link" to={siteUser ? '/users/edit' : null}>
        ...
        <DropdownMenuItem>
          <Link href="/developers/addon/submit/distribution" prependClientApp={false}>
        ...
        <DropdownMenuItem>
          <Link className="Header-user-menu-developers-submissions-link" href="/developers/addons/" prependClientApp={false}>
            {i18n.gettext('Manage My Submissions')}
          </Link>
        </DropdownMenuItem>
        {isReviewer && <DropdownMenuItem>
            <Link className="Header-user-menu-reviewer-tools-link" href="/reviewers/" prependClientApp={false}>
        ...
        <DropdownMenuItem
          ...
          title={siteIsReadOnly ? i18n.gettext(`This action is currently unavailable.
                          Please reload the page in a moment.`) : null}
        >
          {i18n.gettext('Log out')}
        </DropdownMenuItem>
      </DropdownMenu> : <AuthenticateButton buttonType="none" className="Header-authenticate-button Header-button" noIcon />;
  }
  ...
  render() {
    ...
    const headerLink = <Link className="Header-title" to="/" prependClientApp={!forBlog} prependLang={!forBlog}>
        <span className="visually-hidden">
          {// translators: "Firefox" should not be translated. :-)
        i18n.gettext('Firefox Browser Add-ons')}
        </span>
      </Link>;
    const otherSiteLinks = isFirefox({
      userAgentInfo,
    }) ? <>
        <Link className="Header-blog-link Header-button" href="/blog/" prependClientApp={false} prependLang={false}>
          {i18n.gettext('Firefox Add-ons Blog')}
        </Link>
        <Link
          className="Header-extension-workshop-link Header-button"
          href={`${_config.get('extensionWorkshopUrl')}/${makeQueryStringWithUTM({
        ...
        <Link className="Header-developer-hub-link Header-button" href="/developers/" external prependClientApp={false} target="_blank" title={i18n.gettext('Submit and manage extensions and themes')}>
    ...
    >
        ...
        <div className="Header-wrapper">
          <div className="Header-content">
            {isHomePage ? <h1 className="Header-title-wrapper">{headerLink}</h1> : headerLink}
          </div>

          {clientApp === CLIENT_APP_FIREFOX ? <SectionLinks className="Header-SectionLinks" location={location} forBlog={forBlog} /> : null}
...
export default compose(connect(mapStateToProps, mapDispatchToProps), translate())(HeaderBase);