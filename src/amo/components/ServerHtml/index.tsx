...
export function getStatic({
  ...
  if (!JS_CHUNK_EXCLUDES.test(leafName)) {
    switch (type) {
      case 'css':
        return <link href={filePath} {...getSriProps(leafName, includeSri, sriData)} key={type + index} rel="stylesheet" type="text/css" />;

      case 'js':
        return <script key={type + index} src={filePath} {...getSriProps(leafName, includeSri, sriData)} />;

      case 'font':
        return <link
          href={filePath} // We don't generate integrity data for fonts, so avoid calling
...
export default class ServerHtml extends Component {
  ...
  getStyle() {
    ...
    return Object.keys(assets.styles).map((style, index) => getStatic({
  ...
  getScript() {
    ...
    return Object.keys(assets.javascript).map((js, index) => getStatic({
  ...
  getFontPreload() {
    ...
    return Object.keys(assets.assets).filter((asset) => subsetFontPattern.test(asset)).map((asset, index) => getStatic({
  ...
  render() {
    ...
    return <html lang={htmlLang} dir={htmlDir}>
        <head>
