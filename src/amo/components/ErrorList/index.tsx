  ...
  render() {
    ...
    if (API_ERRORS_SESSION_EXPIRY.includes(code)) {
      ...
      if (items.length > 1) {
        ...
        log.warn('The API unexpectedly returned multiple signature expired errors');
      }
    }

    return <ul className={makeClassName('ErrorList', className)}>
        {items.map((item, index) => {
        return <li
          className="ErrorList-item" // We don't have message IDs but it's safe to rely on
        // array indices since they are returned from the API
        // in a predictable order.
        // eslint-disable-next-line react/no-array-index-key
          key={`erroritem-${index}`}
        >
              <Notice type="error" actionOnClick={action} actionText={actionText}>
                {item}
              </Notice>
            </li>;
      })}
