...
import { ADDONS_CONTENT_REVIEW, ADDONS_EDIT, ADDONS_REVIEW, ADDON_TYPE_STATIC_THEME, STATIC_THEMES_REVIEW } from 'amo/constants';
...
export class AddonAdminLinksBase extends React.Component<InternalProps> {
  render(): null | React.ReactNode {
    ...
    const hasALink = hasEditPermission || showContentReviewLink || showCodeReviewLink || showStaticThemeReviewLink;

    if (!hasALink) {
      return null;
    }

    const editLink = hasEditPermission ? <li>
        <a className="AddonAdminLinks-edit-link" href={`/developers/addon/${addon.slug}/edit`}>
    ...
    const adminLink = hasEditPermission ? <li>
        <a className="AddonAdminLinks-admin-link" href={`/admin/models/addons/addon/${addon.id}`}>
          {// translators: This action allows an admin to maintain an add-on.
        i18n.gettext('Admin add-on')}
        </a>
      </li> : null;
    const contentReviewLink = showContentReviewLink ? <li>
        <a className="AddonAdminLinks-contentReview-link" href={`/reviewers/review-content/${addon.id}`}>
          {// translators: This action allows a reviewer to perform a content review of an add-on.
        i18n.gettext('Content review add-on')}
        </a>
      </li> : null;
    const codeReviewLinkText = isTheme ? // translators: This action allows a reviewer to perform a review of a theme.
    i18n.gettext('Review theme') : // translators: This action allows a reviewer to perform a review of an add-on's code.
    i18n.gettext('Review add-on code');
    const codeReviewLink = showCodeReviewLink || showStaticThemeReviewLink ? <li>
          <a className={`AddonAdminLinks-${isTheme ? 'themeReview' : 'codeReview'}-link`} href={`/reviewers/review/${addon.id}`}>
            {codeReviewLinkText}
          </a>
        </li> : null;
    return <DefinitionList className="AddonAdminLinks">
        <Definition
          term={// translators: This is a list of links to administrative functions.
...
const AddonAdminLinks: React.ComponentType<Props> = compose(connect(mapStateToProps), translate())(AddonAdminLinksBase);
