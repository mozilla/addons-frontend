...
import { ADDON_TYPE_EXTENSION, CLIENT_APP_FIREFOX, EXCLUDE_WARNING_CATEGORIES } from 'amo/constants';
import translate from 'amo/i18n/translate';
import { getPromotedCategory } from 'amo/utils/addons';
import { correctedLocationForPlatform, isFirefox } from 'amo/utils/compatibility';
...
export class InstallWarningBase extends React.Component<InternalProps> {
  ...
  couldShowWarning: () => boolean = () => {
    ...
    return !correctedLocation && isFirefox({
      userAgentInfo,
    }) && clientApp === CLIENT_APP_FIREFOX && addon.type === ADDON_TYPE_EXTENSION && (!promotedCategory || !EXCLUDE_WARNING_CATEGORIES.includes(promotedCategory));
  };
  ...
  render(): null | React.ReactNode {
    ...
    if (this.couldShowWarning()) {
      return <Notice actionHref={WARNING_LINK_DESTINATION} actionTarget="_blank" actionText={i18n.gettext('Learn more')} className="InstallWarning" type={genericWarningType}>
          {i18n.gettext('This add-on is not actively monitored for security by Mozilla. Make sure you trust it before installing.')}
        </Notice>;
    }
...
const InstallWarning: React.ComponentType<Props> = compose(withRouter, connect(mapStateToProps), translate())(InstallWarningBase);
