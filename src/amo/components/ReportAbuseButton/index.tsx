...
import { hideAddonAbuseReportUI, initiateAddonAbuseReportViaFirefox, sendAddonAbuseReport, showAddonAbuseReportUI } from 'amo/reducers/abuse';
...
export class ReportAbuseButtonBase extends React.Component<InternalProps> {
  ...
  dismissReportUI: () => void = () => {
    ...
    dispatch(hideAddonAbuseReportUI({
      addon,
    }));
  };

  sendReport: (arg0: OnSubmitParams) => void = ({
    text,
  }: OnSubmitParams) => {
  ...
  onReportButtonClick: (event: ElementEvent) => Promise<void> = async (event: ElementEvent) => {
    const {
    ...
    if (_hasAbuseReportPanelEnabled() && // The integrated abuse report panel currently supports only extensions
    ...
    [ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME].includes(addon.type)) {
      dispatch(initiateAddonAbuseReportViaFirefox({
        addon,
      }));
      return;
    }

    dispatch(showAddonAbuseReportUI({
      addon,
    }));
  };
  ...
  render(): null | React.ReactNode {
    ...
    if (abuseReport && abuseReport.message !== undefined) {
      return <div className="ReportAbuseButton ReportAbuseButton--report-sent">
          ...
          <p className="ReportAbuseButton-first-paragraph">
            {i18n.gettext(`We have received your report. Thanks for letting us know about
              your concerns with this add-on.`)}
          </p>

          <p>
            {i18n.gettext(`We can't respond to every abuse report but we'll look into
              this issue.`)}
          </p>
        </div>;
    }

    const prefaceText = i18n.sprintf(i18n.gettext(`If you think this add-on violates
    ...
    return <div
      ...
      'ReportAbuseButton--is-expanded': abuseReport && abuseReport.uiVisible,
    })}
    >
        <div className="ReportAbuseButton--preview">
          <Button buttonType="neutral" className="ReportAbuseButton-show-more" disabled={loading} onClick={this.onReportButtonClick} puffy>
            {prompt}
          </Button>
        </div>
        ...
        <div className="ReportAbuseButton--expanded">
          ...
          <p className="ReportAbuseButton-first-paragraph" dangerouslySetInnerHTML={sanitizeHTML(prefaceText, ['a'])} />

          <p>
            {i18n.gettext(`Please don't use this form to report bugs or request add-on
          ...
          <DismissibleTextForm id={normalizeFileNameId(__filename)} isSubmitting={loading} onSubmit={this.sendReport} submitButtonText={i18n.gettext('Send abuse report')} submitButtonInProgressText={i18n.gettext('Sending abuse report')} onDismiss={this.dismissReportUI} dismissButtonText={i18n.gettext('Dismiss')} placeholder={i18n.gettext('Explain how this add-on is violating our policies.')} />
        </div>
      </div>;
...
const mapStateToProps = (state: AppState, ownProps: Props): PropsFromState => {
  ...
  return {
    abuseReport: addon && state.abuse.bySlug[addon.slug] ? state.abuse.bySlug[addon.slug] : null,
    loading: state.abuse.loading,
  };
};

const ReportAbuseButton: React.ComponentType<Props> = compose(connect(mapStateToProps), translate(), withErrorHandler({
