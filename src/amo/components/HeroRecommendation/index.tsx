...
import { checkInternalURL, getAddonURL, getPromotedBadgesLinkUrl, sanitizeUserHTML } from 'amo/utils';
import { DEFAULT_UTM_SOURCE, DEFAULT_UTM_MEDIUM, LINE, RECOMMENDED } from 'amo/constants';
...
export type InternalProps = Props & PropsFromState & DefaultProps & {
  i18n: I18nType;
};
export class HeroRecommendationBase extends React.Component<InternalProps> {
  ...
  makeCallToActionURL: () => null | string = () => {
    ...
    return external.homepage ? addQueryParams(external.homepage.url, {
      utm_source: DEFAULT_UTM_SOURCE,
      utm_medium: DEFAULT_UTM_MEDIUM,
      utm_content: PRIMARY_HERO_SRC,
    }) : null;
  ...
  render(): null | React.ReactNode {
    ...
    let link;
    let loading = false;
    const heightClassName = siteIsReadOnly || siteNotice ? 'HeroRecommendation--height-with-notice' : 'HeroRecommendation--height-without-notice';

    if (shelfData) {
      gradientsClassName = `HeroRecommendation-${gradient.start}-${gradient.end}`;
      log.debug(`className ${gradientsClassName} generated from the API response. This should match a selector in styles.scss`);
      ...
      if (callToActionURL) {
        ...
        if (addon) {
          heading = addon.name;
          link = <Link className="HeroRecommendation-link" onClick={this.onHeroClick} to={callToActionURL}>
              {linkInsides}
            </Link>;
        } else if (external) {
          heading = external.name;
          link = <a className="HeroRecommendation-link" href={callToActionURL} onClick={this.onHeroClick} {...linkProps}>
              {linkInsides}
            </a>;
        }
      }
    ...
    const renderHeroTitle = () => {
      ...
      return <div className="HeroRecommendation-title">
          <div className="HeroRecommendation-title-text">
            {titleText || <LoadingText width={20} />}
          </div>
          {![LINE, RECOMMENDED].includes(promotedCategory) && !loading ? <a
          ...
          utm_content: PRIMARY_HERO_SRC,
            ...
            rel="noopener noreferrer"
            target="_blank"
            title={i18n.gettext('Firefox only recommends extensions that meet our standards for security and performance.')}
          >
              What is this?
            </a> : null}
        </div>;
    ...
    return <section
      className={makeClassName('HeroRecommendation', gradientsClassName, heightClassName, {
    ...
    >
        <div className="HeroRecommendation-wrapper">
          ...
          <div className="HeroRecommendation-content">
            {loading ? <div className="HeroRecommendation-image-loading">
                <LoadingText width={100} />
              </div> : featuredImage && <div className="HeroRecommendation-image-wrapper">
                  <img className="HeroRecommendation-image" alt="" src={featuredImage} />
                </div>}
            <div className="HeroRecommendation-info">
              ...
              {loading ? <div className="HeroRecommendation-body">
                  <>
                    <LoadingText width={100} />
                    <br />
                  ...
                  className="HeroRecommendation-body" // eslint-disable-next-line react/no-danger
...
const HeroRecommendation: React.ComponentType<Props> = compose(connect(mapStateToProps), translate())(HeroRecommendationBase);
