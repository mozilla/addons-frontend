...
type InternalProps = Props & PropsFromState & WithExperimentInjectedProps & {
...
export const PageBase = ({
  ...
  if (errorHandler && errorHandler.hasError()) {
    ...
    if (errorHandler.capturedError.responseStatusCode === 401 && API_ERRORS_SESSION_EXPIRY.includes(errorHandler.capturedError.code)) {
      errorContent = <AuthExpired />;
    } else if (errorHandler.capturedError.responseStatusCode === 401 || errorHandler.capturedError.responseStatusCode === 403 || errorHandler.capturedError.responseStatusCode === 404) {
  ...
  return <div className="Page-amo">
      {showVPNPromo && _config.get('enableFeatureVPNPromo') && <VPNPromoBanner variant={variant} />}

      <Header isAddonInstallPage={isAddonInstallPage} isHomePage={isHomePage} location={location} />
      ...
      <div className="Page-content">
        ...
        (!isHomePage || clientApp === CLIENT_APP_ANDROID) && <AppBanner />}
          {showWrongPlatformWarning && <WrongPlatformWarning className="Page-WrongPlatformWarning" isHomePage={isHomePage} />}
...
const Page: React.ComponentType<Props> = compose(withRouter, connect(mapStateToProps), withExperiment({
