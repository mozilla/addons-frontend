...
import { beginDeleteAddonReview, cancelDeleteAddonReview, deleteAddonReview, hideEditReviewForm, hideReplyToReviewForm, sendReplyToReview, showEditReviewForm, showReplyToReviewForm } from 'amo/actions/reviews';
...
type Props = DefaultProps & {
  ...
  siteUserCanReply: boolean | null | undefined;
};
type PropsFromState = {
...
export class AddonReviewCardBase extends React.Component<InternalProps> {
  ...
  onBeginDeleteReview: HTMLElementEventHandler = (event: ElementEvent) => {
    ...
    dispatch(beginDeleteAddonReview({
      reviewId: review.id,
    }));
  };
  ...
  onCancelDeleteReview: HTMLElementEventHandler = (event: ElementEvent) => {
    ...
    event.preventDefault();
    invariant(review, 'review is required');
    dispatch(cancelDeleteAddonReview({
      reviewId: review.id,
    }));
  };
  ...
  onCancelEditReview: () => void = () => {
    ...
    invariant(review, 'review is required');
    dispatch(hideEditReviewForm({
      reviewId: review.id,
    }));
  };

  onClickToBeginReviewReply: HTMLElementEventHandler = (event: ElementEvent) => {
    event.preventDefault();
    const {
      dispatch,
      review,
    } = this.props;
    invariant(review, 'Cannot show review form because no review has been loaded.');
    dispatch(showReplyToReviewForm({
      reviewId: review.id,
    }));
  };
  ...
  onDismissReviewReply: () => void = () => {
    ...
    invariant(review, 'Cannot hide review form because no review has been loaded.');
    dispatch(hideReplyToReviewForm({
      reviewId: review.id,
    }));
  };
  ...
  onSubmitReviewReply: (reviewData: OnSubmitParams) => void = (reviewData: OnSubmitParams) => {
    ...
    invariant(review, 'The review property cannot be empty when replying to a review.');
  ...
  isReply(): boolean {
    ...
    return isReplyToReviewId !== undefined || Boolean(review && review.isDeveloperReply);
  }
  ...
  renderReply(): null | React.ReactNode {
    const {
      addon,
      errorHandler,
      i18n,
    ...
    const formId = [normalizeFileNameId(__filename), 'addon', addon ? addon.id.toString() : 'no-addon', 'review', review ? review.id.toString() : 'unsaved-review'].join('-');
    return <div className="AddonReviewCard-reply">
        {replyingToReview ? <DismissibleTextForm className="AddonReviewCard-reply-form" id={formId} isSubmitting={submittingReply && !errorHandler.hasError()} onDismiss={this.onDismissReviewReply} onSubmit={this.onSubmitReviewReply} placeholder={i18n.gettext('Write a reply to this review.')} submitButtonText={review.reply ? i18n.gettext('Update reply') : i18n.gettext('Publish reply')} submitButtonInProgressText={review.reply ? i18n.gettext('Updating reply') : i18n.gettext('Publishing reply')} text={review.reply && review.reply.body} /> : <AddonReviewCard addon={addon} isReplyToReviewId={review.id} review={review.reply} slim={slim} siteUserCanReply={siteUserCanReply} />}
      </div>;
  }
  ...
  render(): React.ReactNode {
    ...
    if (review) {
      ...
      const byLineString = noAuthor ? // translators: Example in English: "posted last week"
      i18n.gettext('posted %(linkStart)s%(timestamp)s%(linkEnd)s') : // translators: Example in English: "by UserName123, last week"
      showUserProfileLink ? i18n.gettext('by %(linkUserProfileStart)s%(authorName)s%(linkUserProfileEnd)s, %(linkStart)s%(timestamp)s%(linkEnd)s') : i18n.gettext('by %(authorName)s, %(linkStart)s%(timestamp)s%(linkEnd)s');
      ...
      if (!review.reviewAddon.slug) {
        log.error(`The add-on for reviewId: ${review.id} has an falsey slug: ${review.reviewAddon.slug}`);
      }

      if (!review.reviewAddon.id) {
        log.error(`The add-on for reviewId: ${review.id} has an falsey id: ${review.reviewAddon.id}`);
      }

      const replacements = [['linkStart', 'linkEnd', (text) => {
        return slugForReviewLink ? <Link
      ...
      if (showUserProfileLink) {
        replacements.push(['linkUserProfileStart', 'linkUserProfileEnd', (text) => <Link key={`${review.id}-${review.userId}`} to={`/user/${review.userId}/`}>
              {text}
            </Link>]);
      }
      ...
      const byLineLink = replaceStringsWithJSX({
        text: i18n.sprintf(byLineString, {
          ...
          linkUserProfileStart: showUserProfileLink ? '%(linkUserProfileStart)s' : undefined,
          linkUserProfileEnd: showUserProfileLink ? '%(linkUserProfileEnd)s' : undefined,
        }),
        replacements,
      });
      byLine = <span
    ...
    const showEditControls = review && siteUser && (review.userId === siteUser.id || this.isReply() && siteUserCanManageReplies);
    const controls = controlsAreVisible ? <div className="AddonReviewCard-allControls">
        {showEditControls ? <>
            {!this.isRatingOnly() && <a href="#edit" onClick={this.onClickToEditReview} className="AddonReviewCard-edit AddonReviewCard-control">
                {this.editPrompt()}
              </a>}
            {deletingReview && !errorHandler.hasError() ? <span className="AddonReviewCard-control AddonReviewCard-deleting">
                {i18n.gettext('Deleting…')}
              </span> : <Button buttonType="neutral" className={makeClassName('AddonReviewCard-control', 'AddonReviewCard-delete')} onClick={this.onBeginDeleteReview}>
                {this.deletePrompt()}
              </Button>}
          </> : null}

        {review && !replyingToReview && !review.reply && !this.isReply() && !this.isRatingOnly() && siteUserCanManageReplies && siteUser && review.userId !== siteUser.id ? <a href="#reply" onClick={this.onClickToBeginReviewReply} className="AddonReviewCard-begin-reply AddonReviewCard-control">
            <Icon name="reply-arrow" />
            {i18n.gettext('Reply to this review')}
          </a> : null}

        {flaggable && !this.isRatingOnly() && review && (!siteUser || siteUser.id !== review.userId) ? <FlagReviewMenu isDeveloperReply={this.isReply()} openerClass="AddonReviewCard-control" review={review} /> : null}
      </div> : null;
    return <div
    ...
    >
        <div className="AddonReviewCard-container">
          ...
          {review && review.isDeleted && <Notice type="error" className="AddonReviewCard-non-public-notice">
              {i18n.gettext('This rating or review has been deleted. You are only seeing it because of elevated permissions.')}
            </Notice>}
          {review && editingReview ? <AddonReviewManager onCancel={this.onCancelEditReview} puffyButtons={slim} review={review} /> : <UserReview controls={controls} review={review} byLine={byLine} showRating={Boolean(!this.isReply() && showRating)} isReply={this.isReply()} />}
        </div>
        {beginningToDeleteReview && <ConfirmationDialog className="AddonReviewCard-confirmDeleteDialog" cancelButtonText={this.cancelDeleteButtonText()} cancelButtonType="neutral" confirmButtonText={this.confirmDeleteButtonText()} onCancel={this.onCancelDeleteReview} onConfirm={this.onClickToDeleteReview} message={slim ? undefined : this.confirmDeletePrompt()} puffyButtons={slim} />}
        {this.renderReply()}
        {siteUser && review && review.userId === siteUser.id && this.isRatingOnly() && !beginningToDeleteReview && !editingReview && <Button className="AddonReviewCard-writeReviewButton" onClick={this.onClickToEditReview} buttonType="action" puffy={slim}>
...
const AddonReviewCard: React.ComponentType<Props> = compose(withRouter, connect(mapStateToProps), withErrorHandler({
  name: 'AddonReviewCard',
  extractId,
}), translate())(AddonReviewCardBase);
