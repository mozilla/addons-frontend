...
import { ADDON_TYPE_STATIC_THEME, DEFAULT_UTM_SOURCE, DEFAULT_UTM_MEDIUM } from 'amo/constants';
...
export class SearchResultBase extends React.Component<InternalProps> {
  ...
  getAddonLink(addon: AddonType | CollectionAddonType, addonInstallSource?: string): string {
  ...
  renderResult(): React.ReactNode {
    ...
    if (addon) {
      addonTitle = <Link className="SearchResult-link" to={this.getAddonLink(addon, addonInstallSource)} onClick={this.onClickAddon}>
          {addon.name}
        </Link>;
    }
    ...
    const iconWrapperClassnames = makeClassName('SearchResult-icon-wrapper', {
      'SearchResult-icon-wrapper--no-theme-image': addon ? imageURL === null : useThemePlaceholder,
    });
    let addonAuthors = null;
    const addonAuthorsData = addon && addon.authors && addon.authors.length ? addon.authors : null;
    ...
    if (!addon || addonAuthorsData) {
      ...
      addonAuthors = <h3 className="SearchResult-author SearchResult--meta-section">
    ...
    return <div className="SearchResult-wrapper">
        <div className="SearchResult-result">
          <div className={iconWrapperClassnames}>
            {addon && imageURL || !addon && !useThemePlaceholder ? <img
          ...
          <div className="SearchResult-contents">
            <h2 className="SearchResult-name">
              {addonTitle}
              {showPromotedBadge && addon && promotedCategory ? <PromotedBadge category={promotedCategory} onClick={(e) => e.stopPropagation()} size="small" /> : null}
            </h2>
            {summary}

            {showMetadata ? <div className="SearchResult-metadata">
                <div className="SearchResult-rating">
                  <Rating rating={addon && addon.ratings ? addon.ratings.average : 0} readOnly styleSize="small" />
                </div>
                {addonAuthors}
              </div> : null}

            {addon && addon.notes && <div className="SearchResult-note">
                <h4 className="SearchResult-note-header">
                  <Icon name="comments-blue" />
                  {i18n.gettext('Add-on note')}
                </h4>
                <p
                  className="SearchResult-note-content" // eslint-disable-next-line react/no-danger
                  dangerouslySetInnerHTML={sanitizeHTML(nl2br(addon.notes), ['br'])}
                />
              </div>}
          </div>
          ...
          <h3 className="SearchResult-users SearchResult--meta-section">
            ...
            <span className="SearchResult-users-text">
              {averageDailyUsers !== null && averageDailyUsers !== undefined ? i18n.sprintf(i18n.ngettext('%(total)s user', '%(total)s users', averageDailyUsers), {
  ...
  onClickResult: () => void = () => {
    ...
    if (addon) {
      history.push(`/${lang}/${clientApp}${this.getAddonLink(addon, addonInstallSource)}`);
  ...
  render(): React.ReactNode {
    ...
    return (// Note: The link in question is still keyboard accessible because we've
...
const SearchResult: React.ComponentType<Props> = compose(withRouter, connect(mapStateToProps), translate())(SearchResultBase);
