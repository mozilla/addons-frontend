...
import { SAVED_RATING, STARTED_SAVE_RATING, createAddonReview, fetchLatestUserReview, updateAddonReview } from 'amo/actions/reviews';
...
import { ADDON_TYPE_DICT, ADDON_TYPE_EXTENSION, ADDON_TYPE_LANG, ADDON_TYPE_STATIC_THEME, validAddonTypes as defaultValidAddonTypes } from 'amo/constants';
...
export class RatingManagerBase extends React.Component<InternalProps> {
  componentDidMount() {
    ...
    if (userId && userReview === undefined) {
      ...
      dispatch(fetchLatestUserReview({
        userId,
        addonId: addon.id,
  ...
  onSelectRating: (score: number) => void = (score: number) => {
    ...
    if (userReview) {
      dispatch(updateAddonReview({
        errorHandlerId: errorHandler.id,
        score,
        reviewId: userReview.id,
      }));
    } else {
      dispatch(createAddonReview({
        addonId: addon.id,
        errorHandlerId: errorHandler.id,
        score,
      ...
      default:
        {
          ...
          log.warn(`Using generic prompt for ${validAddonType ? 'add-on' : 'unknown add-on'} type: ${addonType}`);
  ...
  renderLogInToRate(): React.ReactNode {
    const {
      addon,
    } = this.props;
  ...
  renderUserRatingForm(): React.ReactNode {
    const {
      addon,
    ...
    if (userReview && onDeleteScreen) {
      if (userReview.body) {
        prompt = i18n.gettext('Are you sure you want to delete your review of %(addonName)s?');
      } else {
        // A review without a body is a rating.
        prompt = i18n.gettext('Are you sure you want to delete your rating of %(addonName)s?');
      }
    } else {
      prompt = i18n.gettext('How are you enjoying %(addonName)s?');
    }

    const promptHTML = sanitizeHTML(i18n.sprintf(prompt, {
    ...
    return <form action="">
        <fieldset>
          ...
          <legend className="RatingManager-legend" dangerouslySetInnerHTML={promptHTML} />
          {
          /* eslint-enable react/no-danger */
        }
          <div className="RatingManager-ratingControl">
            {!this.isSignedIn() ? this.renderLogInToRate() : null}
            {userReview && onDeleteScreen ? <AddonReviewManagerRating className="RatingManager-AddonReviewManagerRating" onSelectRating={undefined} rating={userReview.score} /> : <UserRating className="RatingManager-UserRating" readOnly={!this.isSignedIn()} onSelectRating={this.onSelectRating} review={!this.isSignedIn() ? null : userReview} />}
          </div>
          <RatingManagerNotice className={userReview && userReview.body ? 'RatingManager-savedRating-withReview' : null} hideMessage={!this.isMessageVisible()} message={flashMessage === STARTED_SAVE_RATING ? i18n.gettext('Saving star rating') : i18n.gettext('Star rating saved')} type={flashMessage === STARTED_SAVE_RATING ? genericType : successType} />
        </fieldset>
      </form>;
  }
  ...
  render(): React.ReactNode {
    ...
    return <div className="RatingManager">
        {!editingReview && this.renderUserRatingForm()}
        {userReview && <AddonReviewCard addon={addon} className="RatingManager-AddonReviewCard" flaggable={false} review={userReview} shortByLine showControls={!this.isMessageVisible()} showRating={false} siteUserCanReply={false} slim />}
        <ReportAbuseButton addon={addon} />
      </div>;
  }

}
export const mapStateToProps = (state: AppState, ownProps: Props): PropsFromState => {
...
export const RatingManagerWithI18n: React.ComponentType<Props> = translate()(RatingManagerBase);
const RatingManager: React.ComponentType<Props> = compose(withRenderedErrorHandler({
  id: 'RatingManager',
}), connect(mapStateToProps))(RatingManagerWithI18n);
