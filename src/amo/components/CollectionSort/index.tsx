...
import { collectionEditUrl, collectionUrl, convertFiltersToQueryParams } from 'amo/reducers/collections';
import { COLLECTION_SORT_DATE_ADDED_ASCENDING, COLLECTION_SORT_DATE_ADDED_DESCENDING, COLLECTION_SORT_NAME, COLLECTION_SORT_POPULARITY_DESCENDING } from 'amo/constants';
...
export class CollectionSortBase extends React.Component<InternalProps> {
  onSortSelect: (event: React.SyntheticEvent<HTMLSelectElement>) => void = (event: React.SyntheticEvent<HTMLSelectElement>) => {
    const {
      collection,
      clientApp,
      editing,
    ...
    const pathname = `/${lang}/${clientApp}${editing ? collectionEditUrl({
      collection,
    }) : collectionUrl({
      collection,
    })}`;
  ...
  render(): React.ReactNode {
    ...
    return <form className="CollectionSort">
        <label className="CollectionSort-label" htmlFor="CollectionSort-select">
          {i18n.gettext('Sort add-ons by')}
        </label>
        <Select className="CollectionSort-select" defaultValue={filters.collectionSort} id="CollectionSort-select" name="sort" onChange={this.onSortSelect}>
...
const CollectionSort: React.ComponentType<Props> = compose(withRouter, connect(mapStateToProps), translate())(CollectionSortBase);
