...
export class CategoriesBase extends React.Component<InternalProps> {
  ...
  render(): React.ReactNode {
    ...
    invariant(addonType, 'addonType is undefined');
    let categories: Array<unknown> = [];

    if (categoriesState && categoriesState[clientApp] && categoriesState[clientApp][addonType]) {
    ...
    if (!errorHandler.hasError() && !loading && !categories.length) {
      return <Card className={classNameProp}>
    ...
    return <Card className={classNameProp} header={i18n.gettext('Categories')}>
        {errorHandler.renderErrorIfPresent()}
        {loading ? <div className="Categories-loading">
            <span className="Categories-loading-info visually-hidden">
              {i18n.gettext('Loading categories.')}
            </span>
            {Array(8).fill(0).map((value, index) => {
          return <LoadingText className="Categories-loading-text" key={`Categories-loading-text-${index}`} />;
        })}
          </div> : <ul className="Categories-list">
          ...
          return <li className="Categories-item" key={name}>
                  <Button // `12` is the number of colors declared in
            // "$category-colors".
...
const Categories: React.ComponentType<Props> = compose(withErrorHandler({
  extractId,
  name: 'Categories',
}), connect(mapStateToProps), translate())(CategoriesBase);
