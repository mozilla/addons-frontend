...
import type { ElementEvent, HTMLElementEventHandler, TypedElementEvent } from 'amo/types/dom';
...
export class DismissibleTextFormBase extends React.Component<InternalProps, State> {
  ...
  async checkForStoredState() {
    ...
    if (storedState) {
      log.debug(oneLine`Initializing DismissibleTextForm state from LocalState
  ...
  persistState: typeof debounce = this.props._debounce((state) => {
  ...
  onTextChange: (event: TypedElementEvent<HTMLInputElement>) => void = (event: TypedElementEvent<HTMLInputElement>) => {
    event.preventDefault();
    const newState = {
  ...
  render(): React.ReactNode {
    ...
    const submitButtonIsDisabled = isSubmitting || !this.state.text.trim() || this.state.initialText === this.state.text.trim();
    ...
    const text = {
      ...
      submitButtonInProgressText: submitButtonInProgressText || i18n.gettext('Submitting'),
    };
    invariant(!(microButtons && puffyButtons), 'microButtons and puffyButtons cannot both be true; choose one');
    const cancelButton = onDismiss ? <Button buttonType="neutral" className="DismissibleTextForm-dismiss" disabled={isSubmitting} key="cancel" micro={microButtons} onClick={this.onDismiss} puffy={puffyButtons} type="cancel">
        {dismissButtonText || i18n.gettext('Cancel')}
      </Button> : null;
    const deleteButton = onDelete ? <Button buttonType="alert" className="DismissibleTextForm-delete" disabled={deleteButtonIsDisabled} key="delete" onClick={this.onDelete} micro={microButtons} puffy={puffyButtons} type="button">
        {i18n.gettext('Delete')}
      </Button> : null;
    const submitButton = <Button buttonType="action" className={makeClassName('DismissibleTextForm-submit', submitButtonClassName)} disabled={submitButtonIsDisabled} key="submit" onClick={this.onSubmit} micro={microButtons} puffy={puffyButtons} type="submit">
    ...
    const actionButtonsContainer = <span className="DismissibleTextForm-delete-submit-buttons" key="actionButtons">
        {actionButtons}
      </span>;
    ...
    return <form className={makeClassName('DismissibleTextForm-form', className)}>
        <Textarea
        ...
        />
        {formFooter && <div className="DismissibleTextForm-formFooter">{formFooter}</div>}
        <div className="DismissibleTextForm-buttons">{allButtons}</div>
      </form>;
  }

}
const DismissibleTextForm: React.ComponentType<Props> = compose(translate())(DismissibleTextFormBase);
