...
export class AddonAuthorLinksBase extends React.Component<InternalProps> {
  render(): null | React.ReactNode {
    ...
    const editLink = <li>
        <a className="AddonAuthorLinks-edit-link" href={`/developers/addon/${addon.slug}/edit`}>
          {// eslint-disable-next-line max-len
        // translators: This action allows the add-on developer to edit an add-on's properties.
        i18n.gettext('Edit add-on')}
        </a>
      </li>;
    return <DefinitionList className="AddonAuthorLinks">
        <Definition
          term={// translators: This is a list of links to Developer functions.
...
const AddonAuthorLinks: React.ComponentType<Props> = compose(connect(mapStateToProps), translate())(AddonAuthorLinksBase);
