...
import type { LinkWithTextType, SecondaryHeroShelfType } from 'amo/reducers/home';
...
type Props = {
  shelfData?: SecondaryHeroShelfType;
};
...
export const SecondaryHeroBase = ({
  ...
  const onHeroClick = (event: AnchorEvent) => {
    _tracking.sendEvent({
      ...
      label: _stripLangFromAmoUrl({
        urlString: event.currentTarget.href,
      }),
    });
  };
  ...
  const getLinkProps = (link: LinkWithTextType | null) => {
    ...
    if (link) {
      const urlInfo = _checkInternalURL({
        urlString: link.url,
      });

      if (urlInfo.isInternal) {
        return { ...props,
          to: makeCallToActionURL(urlInfo.relativeURL),
        };
      }
  ...
  modules.forEach((module) => {
    renderedModules.push(<div className="SecondaryHero-module" key={module.description}>
        {module.icon ? <img alt={module.description} className="SecondaryHero-module-icon" src={module.icon} /> : <div className="SecondaryHero-module-icon" />}
        <div className="SecondaryHero-module-description">
          {module.description || <LoadingText width={60} />}
        </div>
        {module.cta && <Link className="SecondaryHero-module-link" {...getLinkProps(module.cta)}>
            <span className="SecondaryHero-module-linkText">
              {module.cta && module.cta.text}
            </span>
          </Link>}
        {!module.description && <div className="SecondaryHero-module-link">
            <LoadingText width={60} />
          </div>}
      </div>);
  });
  return <section className="SecondaryHero">
      <div className="SecondaryHero-message">
        ...
        {cta && <Link className="SecondaryHero-message-link" {...getLinkProps(cta)}>
            <span className="SecondaryHero-message-linkText">{cta.text}</span>
          </Link>}
        {!headline && <div className="SecondaryHero-message-link">
            <LoadingText width={60} />
          </div>}
      </div>
      {renderedModules}
