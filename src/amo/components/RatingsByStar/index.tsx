...
export class RatingsByStarBase extends React.Component<InternalProps> {
  renderBarValue(starCount: number): React.ReactNode {
    const {
      addon,
    } = this.props;
    ...
    return <div
      className={makeClassName('RatingsByStar-bar', 'RatingsByStar-barValue', `RatingsByStar-barValue--${width}pct`, {
  ...
  render(): React.ReactNode {
    ...
    const getLinkTitles = (rating, total) => {
      switch (rating) {
        ...
        case '5':
          if (total && total > 0) {
            return i18n.sprintf(i18n.ngettext('Read the 1 five-star review', 'Read all %(total)s five-star reviews', total), {
        ...
        case '4':
          if (total && total > 0) {
            return i18n.sprintf(i18n.ngettext('Read the 1 four-star review', 'Read all %(total)s four-star reviews', total), {
              total: i18n.formatNumber(total || 0),
            });
          }
        ...
        case '3':
          if (total && total > 0) {
            return i18n.sprintf(i18n.ngettext('Read the 1 three-star review', 'Read all %(total)s three-star reviews', total), {
              total: i18n.formatNumber(total || 0),
            });
          }
        ...
        case '2':
          if (total && total > 0) {
            return i18n.sprintf(i18n.ngettext('Read the 1 two-star review', 'Read all %(total)s two-star reviews', total), {
              total: i18n.formatNumber(total || 0),
            });
          }
        ...
        case '1':
          if (total && total > 0) {
            return i18n.sprintf(i18n.ngettext('Read the 1 one-star review', 'Read all %(total)s one-star reviews', total), {
              total: i18n.formatNumber(total || 0),
            });
          }
    ...
    return <div className="RatingsByStar">
        <div className="RatingsByStar-graph">
          ...
          const loadingRow = <div key={star} className="RatingsByStar-row">
          ...
          const ratingsByStarRow = <>
                ...
                <div className="RatingsByStar-barContainer">
                  <div className="RatingsByStar-bar RatingsByStar-barFrame">
                    {starCount !== undefined ? this.renderBarValue(starCount) : null}
                  </div>
                </div>
...
const RatingsByStar: React.ComponentType<Props> = compose(withRouter, translate())(RatingsByStarBase);
