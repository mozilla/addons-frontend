...
type HandleLogInFunc = (location: ReactRouterLocationType, options?: {
  _window: typeof window;
}) => void;
type HandleLogOutFunction = (arg0: {
  api: ApiState;
}) => Promise<void>;
type DefaultProps = {
...
export class AuthenticateButtonBase extends React.Component<InternalProps> {
  ...
  render(): React.ReactNode {
    ...
    const buttonText = siteUser ? logOutText || i18n.gettext('Log out') : logInText || i18n.gettext('Log in');
    const title = siteIsReadOnly ? i18n.gettext(`This action is currently unavailable. Please reload the
        page in a moment.`) : null;
    ...
    return <Button href={`#${siteUser ? 'logout' : 'login'}`} buttonType={buttonType} className={className} disabled={siteIsReadOnly} onClick={this.onClick} title={title} micro>
...
export const mapStateToProps = (state: AppState, ownProps: Props): PropsFromState => {
  const defaultHandleLogIn = (location, {
    _window = window,
  } = {}) => {
    // eslint-disable-next-line no-param-reassign
    _window.location.assign(startLoginUrl({
      location,
    }));
  };
...
export const createHandleLogOutFunction = (dispatch: DispatchFunc): HandleLogOutFunction => {
  return ({
    api,
  }) => {
...
export const mapDispatchToProps = (dispatch: DispatchFunc, ownProps: Props): DispatchMappedProps => ({
  handleLogOut: ownProps.handleLogOut || createHandleLogOutFunction(dispatch),
});
const AuthenticateButton: React.ComponentType<Props> = compose(withRouter, connect(mapStateToProps, mapDispatchToProps), translate())(AuthenticateButtonBase);
