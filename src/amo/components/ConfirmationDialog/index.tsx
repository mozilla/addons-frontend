...
export const ConfirmationDialogBase = ({
  ...
  return <div className={makeClassName('ConfirmationDialog', className)}>
      {message && <span className="ConfirmationDialog-message">{message}</span>}
      <div className="ConfirmationDialog-buttons">
        <Button buttonType={confirmButtonType} className="ConfirmationDialog-confirm-button" onClick={onConfirm} puffy={puffyButtons}>
          {confirmButtonText || i18n.gettext('Confirm')}
        </Button>
        <Button buttonType={cancelButtonType} className="ConfirmationDialog-cancel-button" onClick={onCancel} puffy={puffyButtons}>
          {cancelButtonText || i18n.gettext('Cancel')}
        </Button>
      </div>
    </div>;
};
const ConfirmationDialog: React.ComponentType<Props> = compose(translate())(ConfirmationDialogBase);
