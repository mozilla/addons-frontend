...
export class LinkBase extends React.Component {
  ...
  render() {
    ...
    const createLinkDest = (urlString) => {
      return urlPrefix && !urlString.startsWith(urlPrefix) ? joinUrl.pathname(urlPrefix, urlString) : urlString;
    };
    ...
    if (typeof href === 'string' && typeof to !== 'undefined') {
      throw new Error('Cannot use "href" prop and "to" prop in the same Link component');
    }

    if (typeof to !== 'undefined' && (typeof to === 'string' && !to.startsWith('/') || to && to.pathname && !to.pathname.startsWith('/'))) {
      throw new Error('"to" prop cannot contain a relative path; it must start with a "/".');
    }

    const linkProps = { ...customProps,
      target,
      rel: target === '_blank' ? 'noopener noreferrer' : customProps.rel,
    };

    if (typeof href === 'string') {
      return <a {...linkProps} href={createLinkDest(href)}>
          {children}
