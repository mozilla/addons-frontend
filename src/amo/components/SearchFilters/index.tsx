...
import { ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME, CLIENT_APP_ANDROID, LINE, RECOMMENDED, REVIEWED_FILTER, SEARCH_SORT_POPULAR, SEARCH_SORT_RANDOM, SEARCH_SORT_RECOMMENDED, SEARCH_SORT_RELEVANCE, SEARCH_SORT_TOP_RATED, SEARCH_SORT_TRENDING, SEARCH_SORT_UPDATED, VERIFIED_FILTER } from 'amo/constants';
...
export class SearchFiltersBase extends React.Component<InternalProps> {
  onSelectElementChange: (event: SelectEvent) => boolean = (event: SelectEvent) => {
    ...
    if (filterName) {
      ...
      // similar.
      if ((newFilters.category || newFilters.tag) && filterName === sortSelectName && filterValue !== SEARCH_SORT_RECOMMENDED) {
    ...
    if (!newFilters.promoted && newFilters.sort && newFilters.sort === SEARCH_SORT_RANDOM) {
  ...
  addonTypeOptions(): Array<SelectOption> {
    ...
    const options = [{
      children: i18n.gettext('All'),
      value: NO_FILTER,
    }, {
      children: i18n.gettext('Extension'),
  ...
  sortOptions(): Array<SelectOption> {
    ...
    return [{
      children: i18n.gettext('Relevance'),
      value: SEARCH_SORT_RELEVANCE,
    }, {
      children: i18n.gettext('Recently Updated'),
      value: SEARCH_SORT_UPDATED,
    }, {
      children: i18n.gettext('Most Users'),
      value: SEARCH_SORT_POPULAR,
    }, {
      children: i18n.gettext('Top Rated'),
      value: SEARCH_SORT_TOP_RATED,
    }, {
      children: i18n.gettext('Trending'),
  ...
  promotedOptions(): Array<SelectOption> {
    ...
    return [{
      children: i18n.gettext('Any'),
      value: NO_FILTER,
    }, {
      children: i18n.gettext('Recommended'),
      value: RECOMMENDED,
    }, {
  ...
  render(): React.ReactNode {
    ...
    const selectedSortFields = filters.sort ? filters.sort.split(',').filter((field) => field !== SEARCH_SORT_RECOMMENDED) : [''];
    const selectedSort = selectedSortFields[0];
    return <ExpandableCard className={expandableCardName} header={i18n.gettext('Filter results')} id={expandableCardName}>
        <form autoComplete="off">
          <label className="SearchFilters-label" htmlFor="SearchFilters-Sort">
            {i18n.gettext('Sort by')}
          </label>
          <Select className="SearchFilters-Sort SearchFilters-select" id="SearchFilters-Sort" name={sortSelectName} onChange={this.onSelectElementChange} value={selectedSort || SEARCH_SORT_RELEVANCE}>
          ...
          {!filters.category && clientApp !== CLIENT_APP_ANDROID && <div>
              <label className="SearchFilters-AddonType-label SearchFilters-label" htmlFor="SearchFilters-AddonType">
                {i18n.gettext('Add-on Type')}
              </label>
              <Select className="SearchFilters-AddonType SearchFilters-select" id="SearchFilters-AddonType" name="addonType" onChange={this.onSelectElementChange} value={filters.addonType || NO_FILTER}>
          ...
          {clientApp !== CLIENT_APP_ANDROID && <div>
              <label className="SearchFilters-Badging-label SearchFilters-label" htmlFor="SearchFilters-Badging">
                {i18n.gettext('Badging')}
              </label>
              <Select className="SearchFilters-Badging SearchFilters-select" id="SearchFilters-Badging" name="promoted" onChange={this.onSelectElementChange} value={filters.promoted || NO_FILTER}>
...
const SearchFilters: React.ComponentType<Props> = compose(withRouter, connect(mapStateToProps), translate(), withErrorHandler({
  name: 'SearchFilters',
}))(SearchFiltersBase);
