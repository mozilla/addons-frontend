...
type InternalProps = Props & {
  _setUIState?: (arg0: $Shape<UIStateType>) => void;
  _truncateToMaxHeight: typeof truncateToMaxHeight;
  i18n: I18nType;
  maxHeight: number;
  setUIState: (arg0: $Shape<UIStateType>) => void;
...
export class ShowMoreCardBase extends React.Component<InternalProps> {
  ...
  render(): React.ReactNode {
    ...
    const readMoreLink = <a
      ...
      dangerouslySetInnerHTML={sanitizeHTML(i18n.gettext( // l10n: The "Expand to" text is for screenreaders so the link
    // makes sense out of context. The HTML makes it hidden from
    // non-screenreaders and must stay.
    '<span class="visually-hidden">Expand to</span> Read more'), ['span'])}
    />;
    return <Card
...
export default (compose(translate(), withUIState({
  fileName: __filename,
  extractId,
  initialState: initialUIState,
  resetOnUnmount: true,
}))(ShowMoreCardBase) as React.ComponentType<Props>);