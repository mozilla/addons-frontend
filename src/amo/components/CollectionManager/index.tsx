...
import { createCollection, finishEditingCollectionDetails, updateCollection } from 'amo/reducers/collections';
...
import type { ElementEvent, HTMLElementEventHandler, InputOrTextAreaEvent } from 'amo/types/dom';
import type { ErrorHandlerType } from 'amo/types/errorHandler';
import type { DispatchFunc } from 'amo/types/redux';
import type { ReactRouterHistoryType, ReactRouterLocationType } from 'amo/types/router';
import './styles.scss';
...
const propsToState = (props: InternalProps): State => {
  ...
  return {
    ...
    customSlug: false,
    description: props.collection && props.collection.description ? props.collection.description : '',
...
export class CollectionManagerBase extends React.Component<InternalProps, State> {
  ...
  onSubmit: HTMLElementEventHandler = (event: ElementEvent) => {
    ...
    const payload = {
      description: {
        [siteLang]: this.state.description,
      },
    ...
    if (creating && currentUserId) {
      ...
      invariant(collection, 'The form cannot be submitted without a collection');
  ...
  onTextInput: (event: InputOrTextAreaEvent) => void = (event: InputOrTextAreaEvent) => {
    event.preventDefault();
    const {
      name,
      value,
    ...
    if (creating && name === 'name' && !this.state.customSlug) {
      this.setState({
        slug: trimmedValue.split(/[^A-Za-z0-9]/).filter((s) => s !== '').join('-').substring(0, SLUG_MAX_LENGTH),
        name: value,
      });
  ...
  render(): React.ReactNode {
    ...
    const collectionUrlPrefix = oneLineTrim`${config.get('apiHost')}/${siteLang}/firefox/collections/
       ${collection && collection.authorId || currentUserId}/`;
    const formIsUnchanged = collection && collection.name === name && collection.slug === slug && (collection.description === description || collection.description === null && !description);
    const formIsDisabled = !collection && !creating || isCollectionBeingModified;
    const isNameBlank = !(name && name.trim().length);
    const isSlugBlank = !(slug && slug.trim().length);
    const isSubmitDisabled = formIsDisabled || formIsUnchanged || isNameBlank || isSlugBlank;
    const buttonText = creating ? i18n.gettext('Create collection') : i18n.gettext('Save changes');
    return <form className="CollectionManager" onSubmit={this.onSubmit}>
        {errorHandler.renderErrorIfPresent()}
        <label className="CollectionManager-collectionName" htmlFor="collectionName">
          {i18n.gettext('Collection name')}
        </label>
        {collection || creating ? <input onChange={this.onTextInput} id="collectionName" name="name" type="text" value={this.state.name} /> : <LoadingText minWidth={60} />}
        <label htmlFor="collectionDescription">
          {i18n.gettext('Description')}
        </label>
        {collection || creating ? <textarea value={this.state.description} id="collectionDescription" name="description" onChange={this.onTextInput} /> : <LoadingText minWidth={60} />}
        <label htmlFor="collectionSlug">{i18n.gettext('Custom URL')}</label>
        <div className="CollectionManager-slug">
          <div id="collectionUrlPrefix" title={collectionUrlPrefix} className="CollectionManager-slug-url-hint">
          ...
          <input onChange={this.onTextInput} id="collectionSlug" maxLength={SLUG_MAX_LENGTH} name="slug" type="text" value={this.state.slug} />
        </div>
        <footer className="CollectionManager-footer">
          ...
          */
          ...
          <Button buttonType="neutral" disabled={formIsDisabled} onClick={this.onCancel} className="CollectionManager-cancel" puffy type="button">
            {i18n.gettext('Cancel')}
          </Button>
          <Button buttonType="action" disabled={isSubmitDisabled} className="CollectionManager-submit" type="submit" puffy>
            {buttonText}
          </Button>
        </footer>
      </form>;
...
const CollectionManager: React.ComponentType<Props> = compose(withRouter, translate(), withFixedErrorHandler({
  fileName: __filename,
  extractId,
}), connect(mapStateToProps))(CollectionManagerBase);
