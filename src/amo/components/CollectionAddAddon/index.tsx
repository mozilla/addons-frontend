...
export type DefaultProps = {
  clearTimeout: (arg0: TimeoutID) => void;
  setTimeout: (arg0: (...args: Array<any>) => any, delay?: number) => TimeoutID | void;
};
...
export class CollectionAddAddonBase extends React.Component<InternalProps> {
  ...
  componentDidUpdate(prevProps: InternalProps) {
    ...
    if ((addStatusChanged || removeStatusChanged) && (hasAddonBeenAddedNew || hasAddonBeenRemovedNew)) {
      errorHandler.clear();
  ...
  render(): React.ReactNode {
    ...
    return <Card className="CollectionAddAddon">
        {errorHandler.hasError() ? errorHandler.renderError() : <TransitionGroup className="CollectionAddAddon-noticePlaceholder">
            {addonAction && <CSSTransition classNames="CollectionAddAddon-noticePlaceholder-transition" timeout={MESSAGE_FADEOUT_TIME}>
                <Notice type={addonAdded ? 'success' : 'generic'}>
                  {addonAdded ? i18n.gettext('Added to collection') : i18n.gettext('Removed from collection')}
                </Notice>
              </CSSTransition>}
          </TransitionGroup>}

        <AutoSearchInput inputName="collection-addon-query" inputPlaceholder={i18n.gettext('Find an add-on to include in this collection')} onSuggestionSelected={this.onAddonSelected} selectSuggestionText={i18n.gettext('Add to collection')} key={collection ? collection.id : ''} />
      </Card>;
  }
...
const CollectionAddAddon: React.ComponentType<Props> = compose(connect(mapStateToProps), translate(), withFixedErrorHandler({
  fileName: __filename,
  extractId,
}), withUIState({
  fileName: __filename,
