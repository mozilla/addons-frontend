...
import { ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME, LINE, RECOMMENDED, REVIEWED_FILTER, SEARCH_SORT_POPULAR, SEARCH_SORT_TOP_RATED, SEARCH_SORT_TRENDING, VIEW_CONTEXT_HOME, VERIFIED_FILTER } from 'amo/constants';
...
type DefaultExternalProps = {
  paginationQueryParams?: Record<string, any>;
  pathname?: string;
  pageTitle?: string;
};
...
type DefaultProps = DefaultExternalProps & DefaultInternalProps;
type Props = DefaultExternalProps & {
...
export class SearchBase extends React.Component<InternalProps> {
  ...
  render(): React.ReactNode {
    ...
    const queryParams = paginationQueryParams || convertFiltersToQueryParams(filters);
    const currentPage = filters ? filters.page : undefined;
    const paginator = count > Number(pageSize) ? <Paginate LinkComponent={LinkComponent} count={count} currentPage={currentPage} pathname={pathname} perPage={Number(pageSize)} queryParams={queryParams} /> : null;
    return <div className="Search">
        ...
        <SearchResults count={count} filters={filters} loading={loading} paginator={paginator} results={results} />
      </div>;
  }
...
const Search: React.ComponentType<Props> = compose(connect(mapStateToProps), translate(), withFixedErrorHandler({
  fileName: __filename,
  extractId,
}))(SearchBase);
