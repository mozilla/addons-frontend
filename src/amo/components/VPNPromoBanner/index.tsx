...
import { EXPERIMENT_CONFIG, VARIANT_SHOW, shouldExcludeUser } from 'amo/experiments/20210714_amo_vpn_promo';
...
export type DefaultProps = {
  ...
  _localStorage: typeof window.localStorage | Record<string, any>;
};
type PropsFromState = {
...
export const getImpressionCount = (_localStorage: typeof window.localStorage): number => {
  const impressionCount = _localStorage.getItem(IMPRESSION_COUNT_KEY);

  const parsedCount = parseInt(impressionCount || 0, 10);
  invariant(!Number.isNaN(parsedCount), `A non-number was stored in ${IMPRESSION_COUNT_KEY}: ${impressionCount}`);
  return parsedCount;
};
type State = {
  dismissed: boolean;
...
export class VPNPromoBannerBase extends React.Component<InternalProps, State> {
  ...
  constructor(props: InternalProps) {
    super(props);
    this.state = {
      dismissed: props.variant !== VARIANT_SHOW,
    };
  }
  ...
  shouldShowBanner(): boolean {
    ...
    return variant === VARIANT_SHOW && !this.state.dismissed && !shouldExcludeUser({
      clientApp,
      regionCode,
    });
  }
  ...
  render(): null | React.ReactNode {
    ...
    const copy = i18n.gettext(`Protect your online data—and choose a VPN subscription plan that works
       for you.`);
    const ctaURL = `${VPN_URL}${makeQueryStringWithUTM({
      utm_campaign: VPN_PROMO_CAMPAIGN,
      utm_content: addonIdentifier,
    })}`;
    return <div className="VPNPromoBanner">
        <div className="VPNPromoBanner-wrapper">
          <img alt="Mozilla VPN" className="VPNPromoBanner-logo" src={vpnLogo} />
          <div className="VPNPromoBanner-copy">
            <strong>{headline}</strong>
            <span>{copy}</span>
          </div>
          <a className="VPNPromoBanner-cta" href={ctaURL} onClick={this.onButtonClick}>
            {i18n.gettext('Get Mozilla VPN')}
          </a>
          <Button className="VPNPromoBanner-dismisser-button" onClick={this.onDismiss}>
...
const VPNPromoBanner: React.ComponentType<Props> = compose(withRouter, translate(), withCookies, connect(mapStateToProps))(VPNPromoBannerBase);
