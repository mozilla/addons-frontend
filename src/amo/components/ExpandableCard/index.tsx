...
export class ExpandableCardBase extends React.Component<InternalProps> {
  ...
  render(): React.ReactNode {
    ...
    const headerWithExpandLink = <a aria-checked={expanded} className="ExpandableCard-ToggleLink" href="#toggle-content" onClick={this.onClick} role="switch" title={i18n.gettext('Toggle contents')}>
        {header}
        <Icon className="ExpandableCard-ToggleArrow" name="triangle-down-black" />
      </a>;
    return <Card
...
const ExpandableCard: React.ComponentType<Props> = compose(translate(), withUIState({
  fileName: __filename,
  extractId,
