...
import { addAddonToCollection, expandCollections, fetchUserCollections, collectionName } from 'amo/reducers/collections';
...
import type { TypedElementEvent, HTMLElementEventHandlerWithTarget } from 'amo/types/dom';
...
type SelectData = {
  actionOptions: Array<Record<string, any>>;
  collectionOptions: Array<Record<string, any>>;
  disabled: boolean;
};
export class AddAddonToCollectionBase extends React.Component<InternalProps> {
  optionSelectHandlers: Record<string, OnSelectOptionType>;
  ...
  onSelectOption: HTMLElementEventHandlerWithTarget<HTMLSelectElement> = (event: TypedElementEvent<HTMLSelectElement>) => {
  ...
  addToCollection(collection: CollectionType) {
    ...
    invariant(addon, 'Cannot add to collection because no add-on has been loaded');
    invariant(currentUserId, 'Cannot add to collection because you are not signed in');
  ...
  createOption({
    ...
    return <option className="AddAddonToCollection-option" key={key} value={key}>
        {text}
      </option>;
  }
  ...
  getSelectData(): SelectData {
    ...
    actionOptions.push(this.createOption({
      ...
      onSelect: () => {
        ...
        history.push(`/${lang}/${clientApp}/collections/add/?include_addon_id=${addon.id}`);
      },
    }));

    if (userCollections && userCollections.length) {
      userCollections.sort((a, b) => a.name.localeCompare(b.name)).forEach((collection) => {
  ...
  render(): React.ReactNode {
    ...
    let addedNotices: Array<React.ReactNode> = [];

    if (addonInCollections) {
      addedNotices = addonInCollections.map((collection) => {
        const notice = i18n.sprintf(i18n.gettext('Added to %(collectionName)s'), {
          collectionName: collectionName({
            name: collection.name,
            i18n,
          }),
        });
        return <Notice type="success" key={collection.id} className="AddAddonToCollection-added">
            {notice}
          </Notice>;
      });
    }
    ...
    return <Card className="AddAddonToCollection" header={i18n.gettext('Add to collection')}>
        {errorHandler.renderErrorIfPresent()}
        {addedNotices}
        <Select disabled={disabled} onChange={this.onSelectOption} className="AddAddonToCollection-select">
          {actionOptions}
          {collectionOptions.length ? <optgroup label={collectionOptLabel}>{collectionOptions}</optgroup> : null}
        </Select>
      </Card>;
  }
...
const mapStateToProps = (state: AppState, ownProps: Props): PropsFromState => {
  ...
  if (currentUserId) {
    ...
    if (addon) {
      addonInCollections = collections.addonInCollections[currentUserId] && collections.addonInCollections[currentUserId][addon.id];
    }
  }
  ...
  return {
    ...
    currentUserId,
    lang: state.api.lang,
    loadingAddonsInCollections: addonInCollections ? addonInCollections.loading : false,
...
const AddAddonToCollection: React.ComponentType<Props> = compose(withRouter, connect(mapStateToProps), translate(), withFixedErrorHandler({
  fileName: __filename,
  extractId,
