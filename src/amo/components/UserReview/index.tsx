...
type Props = {
  ...
  isReply: boolean;
  review: UserReviewType | null | undefined;
  showRating: boolean;
};
...
function reviewBody({
  ...
  invariant(content !== undefined || html !== undefined || id !== undefined, 'content or html or id is required');
  const bodyAttr = {};
...
export const UserReviewBase = (props: InternalProps): React.ReactNode => {
  const {
    byLine,
    children,
    controls,
  ...
  return <div className="UserReview">
      <div className="UserReview-byLine">
        {review && showRating ? <UserRating styleSize="small" review={review} readOnly /> : null}
        {review && isReply && <h4 className="UserReview-reply-header">
...
const UserReview: React.ComponentType<Props> = compose(translate())(UserReviewBase);
