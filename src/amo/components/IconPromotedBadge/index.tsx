...
export const IconPromotedBadgeBase = ({
  ...
  >
      {category !== 'line' && <svg className="IconPromotedBadge-svg" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
          <circle className={`IconPromotedBadge-circle-bgColor--${category}`} cx="50%" cy="50%" r="50%" />

          <g fillRule="nonzero">
            <path className={makeClassName('IconPromotedBadge-iconPath', `IconPromotedBadge-iconPath--${category}`)} d={paths[category]} />
          </g>
        </svg>}
    </Icon>;
};
const IconPromotedBadge: React.ComponentType<Props> = compose(translate())(IconPromotedBadgeBase);
