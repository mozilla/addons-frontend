...
type ExtractIdFunc = (props: Record<string, any>) => string;
...
export const createUIStateMapper = ({
  ...
  fileName?: string;
}): (state: AppState, props: Record<string, any>) => {
  uiState: Record<string, any>;
  uiStateID: string;
} => {
  invariant(initialState, 'initialState is required');

  const mapStateToProps = (state: AppState, props: Record<string, any>) => {
    invariant(extractId, 'extractId is required.');
    invariant(fileName, 'fileName is required.');
    const uiStateID = generateId({
      fileName,
      id: extractId(props),
    });
    const uiState = selectUIState({
...
export const mergeUIStateProps = (stateProps: Record<string, any>, dispatchProps: Record<string, any>, ownProps: Record<string, any>): Record<string, any> => {
  const {
    dispatch,
  } = dispatchProps;
  return { ...ownProps,
    ...stateProps,
    ...dispatchProps,
    setUIState: (change: Record<string, any>) => {
      dispatch(setUIState({
        id: stateProps.uiStateID,
        change,
      }));
    },
  };
...
const withUIState = ({
...
}): (arg0: React.ComponentType<any>) => React.ComponentType<any> => {
  invariant(fileName, 'fileName is required');
  invariant(extractId, 'extractId is required');
  invariant(initialState, 'initialState is required');
  ...
  return (WrappedComponent) => {
    ...
    WithUIState.displayName = `WithUIState(${getDisplayName(WrappedComponent)})`;
