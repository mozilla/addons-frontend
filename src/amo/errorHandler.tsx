...
export class ErrorHandler {
  ...
  addMessage(message) {
    this.dispatchAction(setErrorMessage({
      id: this.id,
      message,
    }));
  }
...
export function withErrorHandler({
  ...
  if (extractId && typeof extractId !== 'function') {
    throw new Error('`extractId` must be a function taking `ownProps` as unique argument.');
  }
  ...
  return (WrappedComponent) => {
    const mapStateToProps = () => {
      ...
      if (!extractId) {
        ...
        if (!defaultErrorId) {
          defaultErrorId = generateHandlerId({
            name,
          });
      ...
      return (state, ownProps) => {
        ...
        const errorId = ownProps.errorHandler ? ownProps.errorHandler.id : defaultErrorId;
        return {
    ...
    return compose(connect(mapStateToProps, undefined, mergeProps))(WrappedComponent);
  };
}
 ...
 * `uniqueId`, `page`, etc.).
 */
export const withFixedErrorHandler = ({
  fileName,
  extractId,
}) => {
  if (!fileName) {
  ...
  return withErrorHandler({
    name: normalizeFileNameId(fileName),
    extractId,
  });
};
 ...
 * )(SomeComponent);
 */
export function withRenderedErrorHandler({
  name,
  id,
} = {}) {
  ...
  return (WrappedComponent) => {
    ...
    return compose(withErrorHandler({
      name,
      id,
    }))(ErrorBanner);
  };
