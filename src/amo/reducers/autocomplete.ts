...
export const AUTOCOMPLETE_STARTED: 'AUTOCOMPLETE_STARTED' = 'AUTOCOMPLETE_STARTED';
export const AUTOCOMPLETE_CANCELLED: 'AUTOCOMPLETE_CANCELLED' = 'AUTOCOMPLETE_CANCELLED';
...
export const createInternalSuggestion = (externalSuggestion: ExternalSuggestion, lang: string): SuggestionType => {
  return {
...
type Action = AutocompleteCancelAction | AutocompleteLoadAction | AutocompleteStartAction;
    ...
    case AUTOCOMPLETE_LOADED:
      {
        ...
        const suggestions = payload.results // TODO: Remove this when `null` names are not returned. See:
        // https://github.com/mozilla/addons-server/issues/6189
        .filter((result) => result.name !== null).map((result) => createInternalSuggestion(result, state.lang));
        return { ...state,
          loading: false,
          suggestions,
        };
