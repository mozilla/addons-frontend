...
import { ADDONS_CONTENT_REVIEW, ADDONS_EDIT, ADDONS_RECOMMENDED_REVIEW, ADDONS_REVIEW, ADDONS_REVIEW_UNLISTED, ALL_SUPER_POWERS, RATINGS_MODERATE, REVIEWER_TOOLS_VIEW, STATIC_THEMES_REVIEW } from 'amo/constants';
import type { AppState } from 'amo/store';
import type { ExternalSiteStatus } from 'amo/reducers/site';

export const FINISH_UPDATE_USER_ACCOUNT: 'FINISH_UPDATE_USER_ACCOUNT' = 'FINISH_UPDATE_USER_ACCOUNT';
...
export const UNSUBSCRIBE_NOTIFICATION: 'UNSUBSCRIBE_NOTIFICATION' = 'UNSUBSCRIBE_NOTIFICATION';
export const ABORT_UNSUBSCRIBE_NOTIFICATION: 'ABORT_UNSUBSCRIBE_NOTIFICATION' = 'ABORT_UNSUBSCRIBE_NOTIFICATION';
export const FINISH_UNSUBSCRIBE_NOTIFICATION: 'FINISH_UNSUBSCRIBE_NOTIFICATION' = 'FINISH_UNSUBSCRIBE_NOTIFICATION';
...
export type NotificationsUpdateType = Record<string, boolean>;
...
export type ExternalUserType = BaseExternalUserType & {
  site_status: ExternalSiteStatus;
};
export type UserType = BaseExternalUserType & {
  notifications: NotificationsType | null;
};
export type UsersState = {
  currentUserID: UserId | null;
  byID: Record<UserId, UserType>;
  byUsername: Record<string, UserId>;
  isUpdating: boolean;
  userPageBeingViewed: {
    loading: boolean;
    userId: UserId | null;
  };
  isUnsubscribedFor: Record<string, boolean | null>;
...
export const getCurrentUser = (users: UsersState): UserType | null => {
  ...
  invariant(currentUser, 'currentUserID is defined but no matching user found in users state.');
  return currentUser;
};
...
export const hasPermission = (state: AppState, permission: string): boolean => {
  ...
  const appsWithAllPermissions = permissions // Only consider permissions with wildcards.
  .filter((perm) => perm.endsWith(':*')) // Return the permission "app".
...
export const hasAnyReviewerRelatedPermission = (state: AppState): boolean => {
  ...
  return permissions.includes(ADDONS_CONTENT_REVIEW) || permissions.includes(ADDONS_EDIT) || permissions.includes(ADDONS_RECOMMENDED_REVIEW) || permissions.includes(ADDONS_REVIEW) || permissions.includes(ADDONS_REVIEW_UNLISTED) || permissions.includes(RATINGS_MODERATE) || permissions.includes(REVIEWER_TOOLS_VIEW) || permissions.includes(STATIC_THEMES_REVIEW);
};
...
export const isUnsubscribedFor = (usersState: UsersState, hash: string, notification: string, token: string): null | boolean | void => {
  return usersState.isUnsubscribedFor[getUnsubscribeKey({
    hash,
    notification,
    token,
  })];
};
type Action = AbortUnsubscribeNotificationAction | FetchUserAccountAction | FetchUserNotificationsAction | FinishUnsubscribeNotificationAction | FinishUpdateUserAccountAction | LoadCurrentUserAccountAction | LoadUserAccountAction | LoadUserNotificationsAction | LogOutUserAction | UnsubscribeNotificationAction | UpdateUserAccountAction;

const reducer = (state: UsersState = initialState, action: Action, _config: typeof config = config): UsersState => {
  switch (action.type) {
    ...
    case UNSUBSCRIBE_NOTIFICATION:
      {
        ...
        return { ...state,
          isUnsubscribedFor: { ...state.isUnsubscribedFor,
            [getUnsubscribeKey({
              hash,
              notification,
              token,
