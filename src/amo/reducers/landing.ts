...
export default function reducer(state: LandingState = initialState, action: Action): LandingState {
  switch (action.type) {
    case SET_LANG:
      return { ...state,
        ...
        ['recommended', 'highlyRated', 'trending'].forEach((key) => {
          if (payload[key]) {
            newState[key] = {
              ...
              results: payload[key].results.map((addon) => createInternalAddon(addon, state.lang)),
            };
          }
        });
        return newState;
