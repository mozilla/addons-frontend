...
export type AddonsByAuthorsState = {
  ...
  // expensive.
  byAddonId: Record<AddonId, AddonType>;
  byAddonSlug: Record<string, Array<AddonId>>;
  byAuthorId: Record<number, Array<AddonId>>;
  countFor: Record<string, number>;
  lang: string;
...
export const FETCH_ADDONS_BY_AUTHORS: 'FETCH_ADDONS_BY_AUTHORS' = 'FETCH_ADDONS_BY_AUTHORS';
export const LOAD_ADDONS_BY_AUTHORS: 'LOAD_ADDONS_BY_AUTHORS' = 'LOAD_ADDONS_BY_AUTHORS';
...
export const fetchAddonsByAuthors = ({
  ...
  invariant(Array.isArray(authorIds), 'The authorIds parameter must be an array.');
...
export const joinAuthorIdsAndAddonType = (authorIds: Array<number>, addonType?: string): string => {
  return authorIds.sort().join('-') + (addonType ? `-${addonType}` : '');
};
export const getLoadingForAuthorIds = (addonsByAuthorsState: AddonsByAuthorsState, authorIds: Array<number>, addonType?: string): boolean | null => {
...
export const getCountForAuthorIds = (addonsByAuthorsState: AddonsByAuthorsState, authorIds: Array<number>, addonType?: string): null | number => {
  return addonsByAuthorsState.countFor[joinAuthorIdsAndAddonType(authorIds, addonType)] || null;
};
export const getAddonsForSlug = (addonsByAuthorsState: AddonsByAuthorsState, slug: string): Array<AddonType> | null => {
  const ids = addonsByAuthorsState.byAddonSlug[slug];
  return ids ? ids.map((id) => addonsByAuthorsState.byAddonId[id]) : null;
};
export const getAddonsForAuthorIds = (addonsByAuthorsState: AddonsByAuthorsState, authorIds: Array<number>, addonType?: string, excludeSlug?: string): Array<AddonType> | null => {
  invariant(authorIds && authorIds.length, 'At least one authorId is required.');
  const ids = authorIds.map((authorId) => {
...
const reducer = (state: AddonsByAuthorsState = initialState, action: Action): AddonsByAuthorsState => {
  switch (action.type) {
    case SET_LANG:
      return { ...state,
    ...
    case FETCH_ADDONS_BY_AUTHORS:
      {
        ...
        if (forAddonSlug) {
          newState.byAddonSlug = { ...newState.byAddonSlug,
        ...
        if (authorIds.length) {
          ...
          const addonsToRemove = getAddonsForAuthorIds(newState, authorIds, addonType);

          if (addonsToRemove) {
            for (const addonToRemove of addonsToRemove) {
              if (addonToRemove.authors) {
                for (const author of addonToRemove.authors) {
                  newState.byAuthorId[author.id] = newState.byAuthorId[author.id].filter((id) => id !== addonToRemove.id);
                }
              }
            }
          }
        ...
        const authorIdsWithAddonType = joinAuthorIdsAndAddonType(authorIds, addonType);
    ...
    case LOAD_ADDONS_BY_AUTHORS:
      {
        ...
        if (forAddonSlug) {
          newState.byAddonSlug = { ...newState.byAddonSlug,
            [forAddonSlug]: addons.slice(0, Number(pageSize)).map((addon) => addon.id),
          };
        }
        ...
        const internalAddons = addons.map((addon) => createInternalAddon(addon, state.lang));
