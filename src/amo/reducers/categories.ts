...
export type CategoryEntry = Record<string, ExternalCategory>;
export type CategoryMapType = Record<string, Record<string, CategoryEntry>>;
...
type EmptyCategoryListType = Record<string, Record<string, Array<ExternalCategory>>>;
    ...
    case LOAD_CATEGORIES:
      {
        ...
        Object.keys(categoryList).forEach((appName) => {
          ...
          Object.keys(categoryList[appName]).forEach((addonType) => {
            ...
            categories[appName][addonType] = categoryList[appName][addonType].sort((a, b) => a.name.localeCompare(b.name)).reduce((object, value) => ({ ...object,
              [value.slug]: value,
            }), {});
          });
        });
