...
import { createInternalAddon, selectLocalizedUrlWithOutgoing } from 'amo/reducers/addons';
...
import type { AddonType, ExternalAddonType, PartialExternalAddonType } from 'amo/types/addons';
import type { LocalizedString, LocalizedUrlWithOutgoing, UrlWithOutgoing } from 'amo/types/api';

export const ABORT_FETCH_HOME_DATA: 'ABORT_FETCH_HOME_DATA' = 'ABORT_FETCH_HOME_DATA';
...
type ExternalPrimaryHeroShelfWithAddonType = BaseExternalPrimaryHeroShelfType & {
  addon: PartialExternalAddonType;
  external: void;
};
type ExternalPrimaryHeroShelfWithExternalType = BaseExternalPrimaryHeroShelfType & {
  addon: void;
  external: PrimaryHeroShelfExternalAddonType;
};
export type ExternalPrimaryHeroShelfType = ExternalPrimaryHeroShelfWithAddonType | ExternalPrimaryHeroShelfWithExternalType | null;
type BasePrimaryHeroShelfType = {
  gradient: HeroGradientType;
  featuredImage: string | null;
  description: string | null;
};
type PrimaryHeroShelfWithAddonType = BasePrimaryHeroShelfType & {
  addon: AddonType;
  external: void;
};
type PrimaryHeroShelfWithExternalType = BasePrimaryHeroShelfType & {
  addon: void;
  external: InternalPrimaryHeroShelfExternalAddonType;
};
export type PrimaryHeroShelfType = PrimaryHeroShelfWithAddonType | PrimaryHeroShelfWithExternalType | null;
...
export type HomeState = {
  ...
  shelves: Record<string, Array<AddonType>>;
};
...
export type AbortFetchHomeDataAction = {
  type: typeof ABORT_FETCH_HOME_DATA;
};
export const abortFetchHomeData = (): AbortFetchHomeDataAction => {
  return {
    type: ABORT_FETCH_HOME_DATA,
  };
};
...
type LoadHomeDataParams = {
  ...
  shelves: Record<string, ApiAddonsResponse>;
};
...
type Action = AbortFetchHomeDataAction | FetchHomeDataAction | LoadHomeDataAction | SetClientAppAction;

const createInternalAddons = (response: ApiAddonsResponse, lang: string): Array<AddonType> => {
  return response.results.map((addon) => createInternalAddon(addon, lang));
};

export const createInternalPrimaryHeroShelfExternalAddon = (external: PrimaryHeroShelfExternalAddonType, lang: string): InternalPrimaryHeroShelfExternalAddonType => {
  const {
    id,
    guid,
    homepage,
...
export const createInternalLinkWithText = (obj: ExternalLinkWithTextType, lang: string): LinkWithTextType => {
  return {
    url: obj.url,
...
export const createInternalSecondaryHeroModule = (module: ExternalSecondaryHeroModuleType, lang: string): SecondaryHeroModuleType => {
  return {
    icon: module.icon,
...
export const createInternalShelf = (result: ExternalResultShelfType, lang: string): ResultShelfType => {
...
export const createInternalHomeShelves = (homeShelves: ExternalHomeShelvesType, lang: string): HomeShelvesType => {
  const {
    results,
    primary,
    secondary,
  } = homeShelves;
  const customShelves: Array<ResultShelfType> = results.map((result) => createInternalShelf(result, lang));
  ...
  if (secondary !== null) {
    secondaryShelf = {
      ...
      cta: secondary.cta ? createInternalLinkWithText(secondary.cta, lang) : null,
      modules: secondary.modules.map((module) => createInternalSecondaryHeroModule(module, lang)),
    };
  }
  ...
  invariant(primary.addon || primary.external, 'Either primary.addon or primary.external is required');
  ...
  if (primary.addon) {
    const primaryShelf: PrimaryHeroShelfWithAddonType = { ...basePrimaryShelf,
  ...
  const primaryShelf: PrimaryHeroShelfWithExternalType = { ...basePrimaryShelf,
    addon: undefined,
    external: createInternalPrimaryHeroShelfExternalAddon(primary.external, lang),
  };
  return {
    results: customShelves,
    primary: primaryShelf,
...
const reducer = (state: HomeState = initialState, action: Action, _config: typeof config = config): HomeState => {
  switch (action.type) {
    case SET_LANG:
      return { ...state,
    ...
    case LOAD_HOME_DATA:
      {
        ...
        return { ...state,
          homeShelves: homeShelves ? createInternalHomeShelves(homeShelves, state.lang) : null,
          isLoading: false,
          resultsLoaded: true,
          ...
          shelves: Object.keys(shelves).reduce((shelvesToLoad, shelfName) => {
            ...
            return { ...shelvesToLoad,
              [shelfName]: response ? createInternalAddons(response, state.lang) : [],
            };
          }, {}),
        };
      }
