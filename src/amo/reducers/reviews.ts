...
import { BEGIN_DELETE_ADDON_REVIEW, CANCEL_DELETE_ADDON_REVIEW, DELETE_ADDON_REVIEW, FETCH_REVIEW, FETCH_REVIEW_PERMISSIONS, FETCH_REVIEWS, FLASH_REVIEW_MESSAGE, HIDE_EDIT_REVIEW_FORM, HIDE_FLASHED_REVIEW_MESSAGE, HIDE_REPLY_TO_REVIEW_FORM, SEND_REPLY_TO_REVIEW, SEND_REVIEW_FLAG, SET_ADDON_REVIEWS, SET_INTERNAL_REVIEW, SET_LATEST_REVIEW, SET_REVIEW, SET_REVIEW_PERMISSIONS, SET_REVIEW_REPLY, SET_REVIEW_WAS_FLAGGED, SET_USER_REVIEWS, SHOW_EDIT_REVIEW_FORM, SHOW_REPLY_TO_REVIEW_FORM, UNLOAD_ADDON_REVIEWS, createInternalReview } from 'amo/actions/reviews';
import { addQueryParams, getQueryParametersForAttribution } from 'amo/utils/url';
import type { BeginDeleteAddonReviewAction, CancelDeleteAddonReviewAction, DeleteAddonReviewAction, FetchReviewAction, FetchReviewPermissionsAction, FetchReviewsAction, FlagReviewAction, FlashMessageType, HideEditReviewFormAction, HideFlashedReviewMessageAction, HideReplyToReviewFormAction, ReviewWasFlaggedAction, SendReplyToReviewAction, SetAddonReviewsAction, SetInternalReviewAction, SetLatestReviewAction, FlashReviewMessageAction, SetReviewAction, SetReviewPermissionsAction, SetReviewReplyAction, SetUserReviewsAction, ShowEditReviewFormAction, ShowReplyToReviewFormAction, UnloadAddonReviewsAction, UpdateRatingCountsAction, UserReviewType } from 'amo/actions/reviews';
...
type ReviewsById = Record<number, UserReviewType>;
type StoredReviewsData = {
  pageSize: string;
  reviewCount: number;
  reviews: Array<number>;
};
type ReviewsData = StoredReviewsData & {
  reviews: Array<UserReviewType>;
};
type ReviewsByAddon = Record<string, {
  data: StoredReviewsData;
  page: string;
  score: string | null;
} | void>;
type ReviewsByUserId = Record<UserId, StoredReviewsData | null | undefined>;
...
export type ReviewsState = {
  permissions: Record<string, ReviewPermissions | void>;
  byAddon: ReviewsByAddon;
  byId: ReviewsById;
  byUserId: ReviewsByUserId;
  latestUserReview: Record<string, number | null>;
  view: Record<number, ViewStateByReviewId>;
  // Short-lived messages about reviews.
  flashMessage?: FlashMessageType;
  loadingForSlug: Record<string, boolean>;
};
...
export const selectReview = (reviewsState: ReviewsState, id: number): UserReviewType | void => {
...
export function selectReviewPermissions({
  ...
  return reviewsState.permissions[makePermissionsKey({
    addonId,
    userId,
  })];
}
...
export const getReviewsByUserId = (reviewsState: ReviewsState, userId: UserId): ReviewsData | null => {
...
export const addReviewToState = ({
  ...
  const byUserId = ratingOrReviewExists ? state.byUserId : { ...state.byUserId,
    // For any new rating object, reset the list
    // to trigger a re-fetch from the server.
    [review.userId]: undefined,
  };
  const byAddon = isReviewUpdate ? state.byAddon : { ...state.byAddon,
...
export const reviewsAreLoading = (state: AppState, addonSlug: string): boolean => {
  return Boolean(state.reviews.loadingForSlug[addonSlug]);
};
type ReviewActionType = BeginDeleteAddonReviewAction | CancelDeleteAddonReviewAction | DeleteAddonReviewAction | FetchReviewAction | FetchReviewPermissionsAction | FetchReviewsAction | FlagReviewAction | FlashReviewMessageAction | UnloadAddonReviewsAction | HideEditReviewFormAction | HideFlashedReviewMessageAction | HideReplyToReviewFormAction | ReviewWasFlaggedAction | SendReplyToReviewAction | SetAddonReviewsAction | SetInternalReviewAction | SetLatestReviewAction | SetReviewAction | SetReviewPermissionsAction | SetReviewReplyAction | SetUserReviewsAction | ShowEditReviewFormAction | ShowReplyToReviewFormAction | UpdateRatingCountsAction;
export default function reviewsReducer(state: ReviewsState = initialState, action: ReviewActionType, {
    ...
    case SET_LATEST_REVIEW:
      {
        ...
        if (review && !selectReview(state, review.id)) {
          throw new Error(`Cannot handle SET_LATEST_REVIEW because review ${review.id} has not been set`);
        }

        return { ...state,
          latestUserReview: { ...state.latestUserReview,
    ...
    case FETCH_REVIEWS:
      {
        ...
        return { ...state,
          loadingForSlug: { ...state.loadingForSlug,
            [addonSlug]: true,
          },
        };
      }
    ...
    case SET_ADDON_REVIEWS:
      {
        ...
        const reviews = payload.reviews.map((review) => createInternalReview(review));
        return { ...state,
          byId: storeReviewObjects({
            state,
            reviews,
          }),
          byAddon: { ...state.byAddon,
            [payload.addonSlug]: {
              data: {
    ...
    case UNLOAD_ADDON_REVIEWS:
      {
        ...
        if (reviewData) {
          ...
          return { ...newState,
            byAddon: { ...newState.byAddon,
              [reviewAddon.slug]: undefined,
            },
            byId: { ...newState.byId,
              [reviewId]: undefined,
            },
            byUserId: { ...newState.byUserId,
              [userId]: undefined,
            },
