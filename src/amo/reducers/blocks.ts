...
export type BlockType = ExternalBlockType & {
  name: string;
};
...
export const loadBlock = ({
  block,
}: LoadBlockParams): LoadBlockAction => {
...
export const createInternalBlock = (block: ExternalBlockType, lang: string): BlockType => {
  return { ...block,
...
const reducer = (state: BlocksState = initialState, action: Action): BlocksState => {
  switch (action.type) {
