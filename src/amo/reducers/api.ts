...
type Action = SetAuthTokenAction | SetLangAction | SetClientAppAction | SetRequestIdAction | SetUserAgentAction | LogOutUserAction;
export default function api(state: Exact<ApiState> = initialApiState, action: Action): Exact<ApiState> {
  switch (action.type) {
    case SET_AUTH_TOKEN:
      return { ...state,
    ...
    case SET_CLIENT_APP:
      return { ...state,
        clientApp: action.payload.clientApp,
      };
    ...
    case SET_USER_AGENT:
      {
        const {
          ...
          device,
          os,
        } = UAParser(action.payload.userAgent);
        return { ...state,
