...
import type { ExternalLanguageToolType, LanguageToolType } from 'amo/types/addons';

export const FETCH_LANGUAGE_TOOLS: 'FETCH_LANGUAGE_TOOLS' = 'FETCH_LANGUAGE_TOOLS';
export const LOAD_LANGUAGE_TOOLS: 'LOAD_LANGUAGE_TOOLS' = 'LOAD_LANGUAGE_TOOLS';
export type LanguageToolsState = {
  byID: Record<string, LanguageToolType>;
  lang: string;
};
...
export const getAllLanguageTools = (state: AppState): Array<LanguageToolType> => {
  const {
    byID,
  } = state.languageTools;
...
export const createInternalLanguageTool = (languageTool: ExternalLanguageToolType, lang: string): LanguageToolType => ({ ...languageTool,
  name: selectLocalizedContent(languageTool.name, lang),
});
type Action = FetchLanguageToolsAction | LoadLanguageToolsAction;
export default function languageToolsReducer(state: LanguageToolsState = initialState, action: Action): LanguageToolsState {
  switch (action.type) {
