...
export const ABORT_FETCH_RECOMMENDATIONS: 'ABORT_FETCH_RECOMMENDATIONS' = 'ABORT_FETCH_RECOMMENDATIONS';
export const FETCH_RECOMMENDATIONS: 'FETCH_RECOMMENDATIONS' = 'FETCH_RECOMMENDATIONS';
export const LOAD_RECOMMENDATIONS: 'LOAD_RECOMMENDATIONS' = 'LOAD_RECOMMENDATIONS';
...
export type OutcomeType = typeof OUTCOME_CURATED | typeof OUTCOME_RECOMMENDED | typeof OUTCOME_RECOMMENDED_FALLBACK;
...
export type RecommendationsState = {
  byGuid: Record<string, Recommendations>;
  lang: string;
};
...
type Action = AbortFetchRecommendationsAction | FetchRecommendationsAction | LoadRecommendationsAction;

const reducer = (state: RecommendationsState = initialState, action: Action): RecommendationsState => {
  switch (action.type) {
    case SET_LANG:
      return { ...state,
    ...
    case LOAD_RECOMMENDATIONS:
      {
        ...
        const addons = action.payload.addons.map((addon) => createInternalAddon(addon, state.lang));
        return { ...state,
          byGuid: { ...state.byGuid,
            [guid]: {
              addons,
