...
import { UNLOAD_ADDON_REVIEWS, UPDATE_RATING_COUNTS } from 'amo/actions/reviews';
import type { UnloadAddonReviewsAction, UpdateRatingCountsAction } from 'amo/actions/reviews';
...
import type { AddonType, ExternalAddonType, ExternalPreviewType, GroupedRatingsType, PartialExternalAddonType, PreviewType } from 'amo/types/addons';
...
export type AddonsState = {
  ...
  byID: Record<string, AddonType>;
  byIdInURL: Record<string, AddonID>;
  byGUID: Record<string, AddonID>;
  bySlug: Record<string, AddonID>;
  infoBySlug: Record<string, {
    info: AddonInfoType;
    loading: boolean;
  }>;
  lang: string;
  loadingByIdInURL: Record<string, boolean>;
};
...
export const createInternalPreviews = (previews: Array<ExternalPreviewType>, lang: string): Array<PreviewType> => {
...
export const selectLocalizedUrlWithOutgoing = (url: LocalizedUrlWithOutgoing | null, lang: string): UrlWithOutgoing | null => {
...
export function createInternalAddon(apiAddon: ExternalAddonType | PartialExternalAddonType, lang: string): AddonType {
  const addon: AddonType = {
    ...
    developer_comments: selectLocalizedContent(apiAddon.developer_comments, lang),
    ...
    previews: apiAddon.previews ? createInternalPreviews(apiAddon.previews, lang) : undefined,
    ...
    currentVersionId: apiAddon.current_version ? apiAddon.current_version.id : null,
    isMozillaSignedExtension: false,
  };
  const currentVersion = apiAddon.current_version;

  if (currentVersion) {
    addon.isMozillaSignedExtension = currentVersion.file.is_mozilla_signed_extension;
  }

  return addon;
}
export const getAddonByID = (addons: AddonsState, id: AddonID): AddonType | null => {
  return addons.byID[`${id}`] || null;
};
export const getAddonByIdInURL = (addons: AddonsState, id: string): AddonType | null => {
...
export const createInternalAddonInfo = (addonInfo: ExternalAddonInfoType, lang: string): AddonInfoType => {
  return {
    eula: selectLocalizedContent(addonInfo.eula, lang),
    privacyPolicy: selectLocalizedContent(addonInfo.privacy_policy, lang),
  };
};
export function createGroupedRatings(grouping: $Shape<GroupedRatingsType> = {}): GroupedRatingsType {
  return {
...
type Action = FetchAddonAction | FetchAddonInfoAction | LoadAddonInfoAction | LoadAddonAction | UnloadAddonReviewsAction | UpdateRatingCountsAction;
export default function addonsReducer(state: AddonsState = initialState, action: Action): AddonsState {
  switch (action.type) {
    case SET_LANG:
      return { ...state,
    ...
    case FETCH_ADDON:
      {
        ...
        return { ...state,
          loadingByIdInURL: { ...state.loadingByIdInURL,
            [slug]: true,
          },
        };
      }
    ...
    case UPDATE_RATING_COUNTS:
      {
        ...
        const newGroupedRatings = ratings ? { ...ratings.grouped_counts,
        } : createGroupedRatings();

        if (oldReview && oldReview.score && newGroupedRatings[oldReview.score] > 0) {
        ...
        if (average && countForAverage && oldReview && oldReview.score) {
          ...
          if (countAfterRemoval === 0) {
            ...
            average = (average * countForAverage - oldReview.score) / countAfterRemoval;
          }
        ...
        // and recalculate the average.
        average = (average * countForAverage + Number(newReview.score)) / (countForAverage + 1);
