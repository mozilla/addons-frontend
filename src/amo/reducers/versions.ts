...
import { LOAD_COLLECTION_ADDONS, LOAD_CURRENT_COLLECTION, LOAD_CURRENT_COLLECTION_PAGE } from 'amo/reducers/collections';
...
export type AddonCompatibilityType = Record<string, {
  min: string;
  max: string;
}>;
...
export type ExternalVersionLicenseType = PartialExternalVersionLicenseType & {
  is_custom: boolean;
};
...
export type VersionLicenseType = PartialVersionLicenseType & {
  isCustom: boolean;
};
export type ExternalAddonVersionType = PartialExternalAddonVersionType & {
...
export type VersionsState = {
  byId: Record<number, AddonVersionType>;
...
export const createInternalVersion = (version: ExternalAddonVersionType, lang: string): AddonVersionType => {
  ...
  return {
    compatibility,
    id: version.id,
    isStrictCompatibilityEnabled: Boolean(version.is_strict_compatibility_enabled),
    license: version.license ? {
      isCustom: version.license.is_custom,
      name: selectLocalizedContent(version.license.name, lang),
      text: version.license.text === undefined ? undefined : selectLocalizedContent(version.license.text, lang),
...
export const getVersionsBySlug = ({
  ...
  if (infoForSlug && infoForSlug.versionIds) {
    return infoForSlug.versionIds.map((versionId) => {
      ...
      invariant(version, `missing version for slug ${slug} and versionId ${versionId}`);
      return version;
    });
  }
...
export const getVersionInfo = ({
  ...
  if (version) {
    ...
    const noMaxString = i18n.gettext('%(application)s %(minVersion)s and later');
    // translators: This is application compatibility information, such as "firefox 41 to 45"
    const maxAndMinString = i18n.gettext('%(application)s %(minVersion)s to %(maxVersion)s');
    const appInfo = Object.keys(version.compatibility).map((application) => {
      const {
        max,
        min,
      } = version.compatibility[application];

      if (max === '*') {
        return i18n.sprintf(noMaxString, {
          application,
          minVersion: min,
        });
      }
    ...
    const compatibilityString = i18n.sprintf( // eslint-disable-next-line max-len
...
const reducer = (state: VersionsState = initialState, action: Action): VersionsState => {
  switch (action.type) {
    case SET_LANG:
      return { ...state,
          ...
          if (addonToUse.current_version) {
            ...
            if (!apiVersion.license || !apiVersion.release_notes) {
              ...
              if (existingVersion) {
                ...
                version.releaseNotes = version.releaseNotes || existingVersion.releaseNotes;
              }
            }
    ...
    case LOAD_LANDING:
      {
        ...
        for (const apiResponse of [recommended, highlyRated, trending]) {
          for (const addon of apiResponse.results) {
            if (addon.current_version) {
              const version = createInternalVersion(addon.current_version, state.lang);
              newVersions[version.id] = version;
            }
          }
        }
