...
export type ExternalReviewReplyType = ExternalReviewTypeBase;
export type ExternalReviewType = ExternalReviewTypeBase & {
  score: number;
...
export type GetReviewsApiResponse = PaginatedApiResponse<ExternalReviewType> & {
...
export const flagReview = ({
  ...
  return new Promise((resolve) => {
    ...
    if (reason === REVIEW_FLAG_REASON_OTHER && !note) {
      throw new Error(`When reason is ${reason}, the note parameter is required`);
    }
