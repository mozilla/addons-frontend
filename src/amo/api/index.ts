...
import { addVersionCompatibilityToFilters, convertFiltersToQueryParams, fixFiltersForClientApp } from 'amo/searchUtils';
...
export function makeQueryString(query: Record<string, string | null>): string {
...
export function createApiError({
  ...
  const apiError = new Error(`Error calling: ${urlId} (status: ${response.status})`);
...
export function callApi({
  ...
  _log = log,
}: CallApiParams): Promise<Record<string, any>> {
  if (!endpoint) {
    return Promise.reject(new Error(`endpoint URL cannot be falsy: "${endpoint}"`));
  }
  ...
  if (!parsedUrl.host) {
    ...
    return Promise.reject(new Error(`Absolute URL "${endpoint}" has an unexpected prefix.`));
  }

  // Preserve the original query string if there is one.
  // This might happen when we parse `next` URLs returned by the API.
  const queryString = makeQueryString({ ...parsedUrl.query,
    ...params,
    lang: apiState.lang,
  });
  const options = {
  ...
  adjustedEndpoint = adjustedEndpoint.endsWith('/') ? adjustedEndpoint : `${adjustedEndpoint}/`;
  ...
  return fetch(apiURL, options).then((response) => {
    ...
    return response.text().then((text) => {
      ...
      _log.warn(oneLine`Response from API was not JSON (was Content-Type:
          ${contentType}) %o`, {
...
export function fetchAddon({
  ...
  if (!appVersion) {
    log.debug(`Failed to parse appversion for client app ${clientApp || '[empty]'}`);
  }
...
export function startLoginUrl({
  ...
  const params = {
    config: fxaConfig,
    to: fxaConfig === 'local' && _window ? _window.location.href : url.format({ ...location,
    }),
  };
  const query = makeQueryString(params);
  return `${API_BASE}${_config.get('apiVersion')}/accounts/login/start/${query}`;
}
...
export function autocomplete({
  ...
  const filtersWithAppVersion = addVersionCompatibilityToFilters({
    filters: _fixFiltersForClientApp({
      api,
      filters,
    }),
...
type GetNextResponseType = (nextURL?: string) => Promise<PaginatedApiResponse<any>>;
type AllPagesOptions = {
  pageLimit: number;
};
export const allPages = async (getNextResponse: GetNextResponseType, {
  pageLimit = 100,
}: AllPagesOptions = {}): Promise<PaginatedApiResponse<any>> => {
  let results = [];
  let nextURL;
  let count = 0;
  let pageSize = 0;
