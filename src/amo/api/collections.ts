...
import type { CollectionFilters, ExternalCollectionAddon, ExternalCollectionDetail } from 'amo/reducers/collections';
...
export type GetCollectionAddonsParams = GetCollectionParams & {
  nextURL?: string;
...
export const getCollectionAddons = ({
  ...
  slug,
  userId,
}: GetCollectionAddonsParams): Promise<PaginatedApiResponse<ExternalCollectionAddon>> => {
  invariant(slug, 'slug is required');
  invariant(userId, 'userId is required');
  const request = {
    auth: true,
    endpoint: nextURL || `accounts/account/${userId}/collections/${slug}/addons`,
    params: undefined,
    apiState: api,
  };
...
type GetAllCollectionAddonsParams = GetCollectionParams & {
...
export const getAllCollectionAddons = async ({
  ...
  const {
    results,
  } = await _allPages((nextURL) => _getCollectionAddons({
    api,
    nextURL,
    slug,
    userId,
...
export const listCollections = ({
  ...
  nextURL,
  userId,
}: ListCollectionsParams): Promise<PaginatedApiResponse<ExternalCollectionDetail>> => {
  invariant(userId, 'userId is required');
...
export type GetAllUserCollectionsParams = ListCollectionsParams & {
  _allPages?: typeof allPages;
...
export const getAllUserCollections = async ({
  ...
  const {
    results,
  } = await _allPages((nextURL) => _listCollections({
    api,
    nextURL,
    userId,
  }));
...
export type UpdateCollectionParams = ModifyCollectionParams & {
...
export type CreateCollectionParams = ModifyCollectionParams & {
  name: LocalizedString;
  slug: string;
};
export const modifyCollection = (action: 'create' | 'update', params: ModifyCollectionParams & {
  collectionSlug?: string;
  name?: LocalizedString | null | undefined;
  slug?: string | null | undefined;
}): Promise<void> => {
  ...
  if (creating) {
    ...
    invariant(collectionSlug, 'The collectionSlug parameter is required when updating');
  }
  ...
  return callApi({
    ...
    body: {
      ...
      description,
      name,
      slug, // The public=true|false flag is not sent to the API. This is
      // because collections are always public. Omitting this parameter
      // should cut down on unexpected bugs.

    },
    endpoint: `accounts/account/${userId}/collections/${creating ? '' : collectionSlug}`,
...
type ModifyCollectionAddonBaseParams = {
  ...
  slug: string;
  userId: string;
  _modifyCollectionAddon?: (arg0: any) => Promise<void>;
};
type CollectionAddonNotes = LocalizedString | null;
export type CreateCollectionAddonParams = ModifyCollectionAddonBaseParams & {
  notes?: CollectionAddonNotes;
};
export type UpdateCollectionAddonParams = ModifyCollectionAddonBaseParams & {
  notes: CollectionAddonNotes;
};
type ModifyCollectionAddonParams = (CreateCollectionAddonParams & {
  action: 'create';
}) | (UpdateCollectionAddonParams & {
  action: 'update';
});
export const modifyCollectionAddon = (params: ModifyCollectionAddonParams): Promise<void> => {
  const {
    action,
    addonId,
    api,
