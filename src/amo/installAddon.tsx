...
import { DOWNLOAD_FAILED, DOWNLOAD_PROGRESS, ENABLE_ACTION, ERROR, ERROR_CORRUPT_FILE, FATAL_ERROR, FATAL_INSTALL_ERROR, FATAL_UNINSTALL_ERROR, INSTALLING, INSTALL_ACTION, INSTALL_CANCELLED, INSTALL_CANCELLED_ACTION, INSTALL_DOWNLOAD_FAILED_ACTION, INSTALL_FAILED, INSTALL_STARTED_ACTION, SET_ENABLE_NOT_AVAILABLE, START_DOWNLOAD, UNINSTALLED, UNINSTALLING, UNINSTALL_ACTION } from 'amo/constants';
...
export function makeProgressHandler({
  ...
  return (addonInstall: AddonInstallType, event: EventType) => {
    if (addonInstall.state === 'STATE_DOWNLOADING') {
      const downloadProgress = parseInt(100 * addonInstall.progress / addonInstall.maxProgress, 10);
      dispatch({
...
type WithInstallHelpersInternalProps = WithInstallHelpersProps & WithInstallHelpersPropsFromState & WithInstallHelpersDefaultProps & {
  dispatch: DispatchFunc;
};
...
export type WithInstallHelpersInjectedProps = {
  ...
  uninstall: (arg0: UninstallParams) => Promise<any>;
};
export class WithInstallHelpers extends React.Component<WithInstallHelpersInternalProps> {
  ...
  setCurrentStatus(): Promise<void> {
    ...
    return _addonManager.getAddon(guid).then((clientAddon) => {
  ...
  enable(): Promise<void> {
    ...
    return _addonManager.enable(guid).then(() => {
      _tracking.sendEvent({
        action: getAddonTypeForTracking(type),
  ...
  uninstall({
    ...
    return _addonManager.uninstall(guid).then(() => {
      _tracking.sendEvent({
        action,
...
export const withInstallHelpers = (WrappedComponent: React.ComponentType<unknown>): React.ComponentType<unknown> => {
  // eslint-disable-next-line react/static-property-placement
  WithInstallHelpers.displayName = `WithInstallHelpers(${getDisplayName(WrappedComponent)})`;

  const mapStateToProps = (state: AppState, ownProps: WithInstallHelpersProps): WithInstallHelpersPropsFromState => {
    const {
      addon,
    } = ownProps;
    ...
    if (!currentVersion) {
      currentVersion = addon && addon.currentVersionId ? getVersionById({
