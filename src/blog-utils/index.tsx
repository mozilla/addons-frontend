...
const render = ({
  ...
  return renderToStaticMarkup(<I18nProvider i18n={i18n}>
...
export const buildStaticAddonCard = async ({
  ...
  const app = 'firefox';
  const lang = 'en-US';
  const response = await fetch(`${baseURL}/api/v5/addons/addon/${addonId}/?lang=${lang}&app=${app}`);
  const apiAddon = await response.json();
  const addon = createInternalAddon(apiAddon, lang);
  return render({
    app,
    lang,
    component: <StaticAddonCard addon={addon} />,
  });
};