...
describe('CurrentSearchPage.mapStateToProps()', () => {
  ...
  it('passes the admin search state', () => {
...
describe('CurrentSearchPage.loadSearchResultsIfNeeded()', () => {
  it('does not dispatch on undefined query', () => {
    ...
    const store = { dispatch: dispatchSpy, getState: () => ({ admin: state }) };
  ...
  it('returns right away when loaded', () => {
    ...
    const store = { dispatch: sinon.spy(), getState: () => ({ admin: state }) };
    const location = { query: { page, q: query } };
    assert.strictEqual(loadSearchResultsIfNeeded({ store, location }), true);
  });

  it('loads the admin search results when needed', () => {
    const page = 10;
    const query = 'no ads';
    const state = {
      api: { token: 'a.jwt.token' },
      admin: { loading: false, page, query: 'old query' },
    };
    ...
    return loadSearchResultsIfNeeded({ store, location }).then(() => {
      mockApi.verify();
      assert(
        dispatch.firstCall.calledWith(actions.adminSearchStart(query, page)),
        'adminSearchStart not called');
      assert(
        dispatch.secondCall.calledWith(actions.adminSearchLoad({ query, entities, result })),
        'adminSearchLoad not called');
    });
  });

  it('triggers adminSearchFail when it fails', () => {
    const page = 11;
    const query = 'no ads';
    const state = {
      api: {},
    ...
    return loadSearchResultsIfNeeded({ store, location }).then(() => {
      ...
      assert(
        ...
        'adminSearchStart not called');
      assert(
        dispatch.secondCall.calledWith(actions.adminSearchFail({
          page, query,
        })),
        'adminSearchFail not called');
    });
  });
});
