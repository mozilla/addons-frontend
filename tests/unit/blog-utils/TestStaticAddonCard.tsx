...
import { ADDON_TYPE_STATIC_THEME, CLIENT_APP_FIREFOX, RECOMMENDED } from 'amo/constants';
import StaticAddonCard from 'blog-utils/StaticAddonCard';
import { fakeAddon, createLocalizedString, createInternalAddonWithLang, dispatchClientMetadata, render as defaultRender, screen, userAgents } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const render = ({
    ...
    return defaultRender(<StaticAddonCard addon={addon} />, {
      store,
    });
  };
  ...
  it('renders a static add-on card', () => {
    ...
    expect(screen.getByClassName('StaticAddonCard')).toHaveAttribute('data-addon-id', String(addon.id));
    expect(screen.getByRole('heading')).toHaveTextContent(name);
    expect(screen.getByRole('link', {
      name,
    })).toHaveAttribute('href', `/en-US/${clientApp}/addon/${addon.slug}/`);
    expect(screen.getByRole('link', {
      name: addon.authors[0].name,
    })).toHaveAttribute('href', `/en-US/${clientApp}/user/${addon.authors[0].id}/`);
    // Promoted badge
    expect(screen.getByRole('link', {
    ...
    expect(screen.getByText('This extension is not currently available.')).toBeInTheDocument();
  });
  ...
  it('sanitizes the summary', () => {
    ...
    const scriptHTML = createLocalizedString(`${plainText}<script>alert(document.cookie);</script>`);
    render({
    ...
    expect(screen.getByClassName('StaticAddonCard-summary')).not.toHaveTextContent('<script>');
  });
  ...
  it('renders a theme image preview when add-on is a theme', () => {
    ...
    expect(screen.getByClassName('StaticAddonCard')).toHaveClass('StaticAddonCard--is-theme');
    expect(screen.getByAltText(`Preview of ${name}`)).toHaveAttribute('src', addon.previews[0].src);
    expect(screen.queryByClassName('StaticAddonCard-icon')).not.toBeInTheDocument();
  });
  it('overrides some query parameters in the download FF link', () => {
    const addon = createInternalAddonWithLang(fakeAddon);
    const expectedHref = ['https://www.mozilla.org/firefox/download/thanks/?s=direct', `utm_campaign=amo-blog-fx-cta-${addon.id}`, 'utm_content=rta%3AMTIzNEBteS1hZGRvbnMuZmlyZWZveA', 'utm_medium=referral', 'utm_source=addons.mozilla.org'].join('&');
    render({
      addon,
    });
    expect(screen.getByRole('link', {
