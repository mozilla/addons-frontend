...
describe(__filename, () => {
  ...
  describe('buildStaticAddonCard', () => {
    ...
    it('calls the API to fetch an add-on', async () => {
      const addonId = 123;
      const fetch = mockFetch({ ...fakeAddon,
        id: addonId,
      });
      await buildStaticAddonCard({
        addonId,
      });
      expect(fetch).toHaveBeenCalledWith(`https://addons.mozilla.org/api/v5/addons/addon/${addonId}/?lang=en-US&app=firefox`);
    });
    it('renders a StaticAddonCard', async () => {
      ...
      const html = cheerio.load(await buildStaticAddonCard({
        addonId,
      }));
    ...
    it('lets the caller catch and handle errors', async () => {
      ...
      await expect(buildStaticAddonCard({
        addonId,
