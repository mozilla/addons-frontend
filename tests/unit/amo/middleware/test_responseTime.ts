...
describe(__filename, () => {
  ...
  describe('integration', () => {
    ...
    it('records timing for GET responses', async () => {
      ...
      sinon.assert.calledWith(hotShotsClient.timing, 'response_time.GET_301', sinon.match.number);
    });
    it('records timing for POST responses', async () => {
      await testClient().post('/en-US/firefox/something/', {});
      sinon.assert.calledWith(hotShotsClient.timing, 'response_time.POST_200', sinon.match.number);
    });
