...
describe(__filename, () => {
  ...
  beforeEach(() => {
    ...
    fakeConfig.set('validClientAppUrlExceptions', ['about', 'developers', 'robots.txt', 'validprefix']);
  ...
  it('should call res.redirect if changing the case', () => {
    ...
    prefixMiddleware(fakeReq, fakeRes, fakeNext, {
      _config: fakeConfig,
    });
  ...
  it('should prepend lang when missing but keep clientAppUrlException', () => {
    ...
    sinon.assert.calledWith(fakeRes.redirect, 301, '/en-US/validprefix/whatever');
    sinon.assert.calledWith(fakeRes.set, 'Cache-Control', ['max-age=31536000']);
    sinon.assert.calledWith(fakeRes.vary, 'accept-language');
  });
  ...
  it('should not mangle a query string for a redirect', () => {
    ...
    sinon.assert.calledWith(fakeRes.redirect, 301, '/en-US/firefox/foo/bar?test=1&bar=2');
    sinon.assert.calledWith(fakeRes.set, 'Cache-Control', ['max-age=31536000']);
  });
});