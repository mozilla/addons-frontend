...
import { ADDON_INFO_TYPE_CUSTOM_LICENSE, ADDON_INFO_TYPE_EULA, ADDON_INFO_TYPE_PRIVACY_POLICY, extractId } from 'amo/pages/AddonInfo';
import { FETCH_ADDON, FETCH_ADDON_INFO, fetchAddon, fetchAddonInfo, loadAddonInfo, loadAddon } from 'amo/reducers/addons';
import { FETCH_VERSION, fetchVersion, loadVersions } from 'amo/reducers/versions';
import { changeLocation, createFailedErrorHandler, createFakeAddonInfo, createFakeErrorHandler, createLocalizedString, dispatchClientMetadata, fakeAddon, fakeVersion, getElement, renderPage as defaultRender, screen, within } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it.each([ADDON_INFO_TYPE_EULA, ADDON_INFO_TYPE_PRIVACY_POLICY])('fetches an addon and addonInfo for %s when requested by slug', (infoType) => {
    ...
    expect(dispatch).toHaveBeenCalledWith(fetchAddonInfo({
  ...
  it.each([ADDON_INFO_TYPE_EULA, ADDON_INFO_TYPE_PRIVACY_POLICY])(`fetches an addon and addonInfo for %s when the slug changes`, async (infoType) => {
  ...
  it.each([ADDON_INFO_TYPE_EULA, ADDON_INFO_TYPE_PRIVACY_POLICY])(`does not fetch addonInfo for %s if it is already loaded`, (infoType) => {
    _loadAddonInfo();
  ...
  it.each([ADDON_INFO_TYPE_EULA, ADDON_INFO_TYPE_PRIVACY_POLICY])(`does not fetch addonInfo for %s if it is already loading`, (infoType) => {
    store.dispatch(fetchAddonInfo({
      errorHandlerId: getErrorHandlerId(),
      slug: defaultSlug,
    }));
    const dispatch = jest.spyOn(store, 'dispatch');
  ...
  describe('ADDON_INFO_TYPE_CUSTOM_LICENSE', () => {
    ...
    it('fetches an addonVersion when the loaded version has no license text', () => {
      ...
      expect(dispatch).toHaveBeenCalledWith(fetchVersion({
        errorHandlerId: getErrorHandlerId({
          infoType,
        }),
        slug: defaultSlug,
  ...
  it('renders LoadingText without content', () => {
    render();
    expect(within(screen.getByClassName('AddonInfo-info')).getByRole('alert')).toBeInTheDocument();
  });
  it('renders an AddonSummaryCard with an addon', () => {
    ...
    expect(screen.getByRole('heading', {
      name: `Privacy policy for ${name}`,
    })).toBeInTheDocument();
    expect(screen.getByRole('link', {
      name,
    })).toHaveAttribute('href', `/${lang}/${clientApp}/addon/${defaultSlug}/`);
  });
  it('renders an AddonSummaryCard without an addon', () => {
    render();
    expect(within(screen.getByClassName('AddonSummaryCard')).getAllByRole('alert')).toHaveLength(12);
  });
  it('renders an HTML title', async () => {
    ...
    await waitFor(() => expect(getElement('title')).toHaveTextContent(`Privacy policy for ${name}`));
  });
  it('does not render an HTML title when there is no add-on', async () => {
    render();
    await waitFor(() => expect(getElement('title')).toHaveTextContent(`Add-ons for Firefox (${lang})`));
  });
  ...
  it('renders a privacy policy page', () => {
    ...
    const addonInfo = createFakeAddonInfo({
      privacyPolicy,
    });
  ...
  it('renders a EULA page', () => {
    ...
    expect(screen.getAllByText(`End-User License Agreement for ${name}`)).toHaveLength(2);
  ...
  it('renders a License page', () => {
    ...
    const name = 'My addon';
    addon.name = createLocalizedString(name);
    const addonVersion = { ...fakeVersion,
      license: { ...fakeVersion.license,
  ...
  it('renders an empty licence when license.text is null', () => {
    ...
    expect(screen.queryByClassName('AddonInfo-info-html')).not.toBeInTheDocument();
  });
  it('renders licence in a loading state when the version has not been loaded', () => {
    ...
    expect(within(screen.getByClassName('AddonInfo')).getByRole('alert')).toBeInTheDocument();
  });
  it('sanitizes the html content', () => {
    const privacyPolicy = 'Some privacy <script>alert(document.cookie);</script> policy text';
    const addonInfo = createFakeAddonInfo({
      privacyPolicy,
    });
    ...
    within(screen.getByClassName('AddonInfo-info-html')).queryByTagName('script')).not.toBeInTheDocument();
  });
  it('adds <br> tags for newlines in the html content', () => {
    ...
    expect(within(screen.getByClassName('AddonInfo-info-html')).getAllByTagName('br')).toHaveLength(1);
  });
  it('allows some HTML tags', () => {
    ...
    expect(within(screen.getByClassName('AddonInfo-info-html')).getByTagName('b')).toBeInTheDocument();
    expect(within(screen.getByClassName('AddonInfo-info-html')).getByTagName('i')).toBeInTheDocument();
    expect(screen.getByRole('link', {
      name: 'bug fixes',
    })).toBeInTheDocument();
    expect(screen.getByClassName('AddonInfo-info-html')).toHaveTextContent('lots of bug fixes');
  });
  describe('extractId', () => {
