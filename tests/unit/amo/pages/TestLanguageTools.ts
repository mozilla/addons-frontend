...
import { ADDON_TYPE_DICT, ADDON_TYPE_LANG, CLIENT_APP_FIREFOX } from 'amo/constants';
import { loadLanguageTools } from 'amo/reducers/languageTools';
import { getCanonicalURL } from 'amo/utils';
import { createFakeLanguageTool, dispatchClientMetadata, getElement, renderPage as defaultRender, screen, within } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const languageTools = [createFakeLanguageTool({
    id: 1,
  ...
  it('renders language tools in your locale', () => {
    ...
    expect(screen.getByRole('heading', {
      name: 'Available for your locale',
    })).toBeInTheDocument();
    const dictionary = screen.getByClassName(`LanguageTools-in-your-locale-list-item--${ADDON_TYPE_DICT}`);
    expect(within(dictionary).getByText('le French Dictionary')).toBeInTheDocument();
    const langPack = screen.getByClassName(`LanguageTools-in-your-locale-list-item--${ADDON_TYPE_LANG}`);
    expect(within(langPack).getByText('French Language Pack')).toBeInTheDocument();
  });
  ...
  it('renders language packs in the table view for the right language', () => {
    ...
    render();
    expect(screen.getByText('Scottish Language Pack (with Irn-Bru)')).toBeInTheDocument();
    expect(screen.getByText('English Language Pack with Extra Us')).toBeInTheDocument();
  ...
  it('renders multiple language tools in a list', () => {
    ...
    render();
    const dictionaryList = within(screen.getByClassName('LanguageTools-lang-en-GB-dictionaries')).getAllByRole('listitem');
    const languagePackList = within(screen.getByClassName('LanguageTools-lang-en-GB-languagePacks')).getAllByRole('listitem');
  ...
  it('does not render languages we know of but do not have languages for', () => {
    ...
    render();
    expect(screen.queryByClassName('LanguageTools-lang-es')).not.toBeInTheDocument();
  });
  it('renders an HTML title', async () => {
    ...
    render();
    await waitFor(() => expect(getElement('title')).toHaveTextContent('Dictionaries and Language Packs – Add-ons for Firefox (pt-BR)'));
  });
  it('renders add-ons for all variants of a short locale on a single row (only one supported language)', () => {
    ...
    const addons = [createFakeLanguageTool({
      id: 1,
      lang,
    ...
    store.dispatch(loadLanguageTools({
      languageTools: addons,
    }));
    render();
    // We expect only one row with all the add-ons in it.
    const row = screen.getByClassName('LanguageTools-table-row');
    expect(row).toBeInTheDocument();
    const langPacks = within(row).getByClassName('LanguageTools-lang-az-languagePacks');
    expect(within(langPacks).getByText('Azərbaycanca (AZ) Language Pack')).toBeInTheDocument();
    const dictionaries = within(row).getByClassName('LanguageTools-lang-az-dictionaries');
    expect(within(dictionaries).getByText('Azerbaijani Spell Checker')).toBeInTheDocument();
    expect(within(dictionaries).getByText('Azerbaijani Foo')).toBeInTheDocument();
  });
  ...
  it('only renders supported languages', () => {
    ...
    expect(screen.queryByClassName('LanguageTools-table-row')).not.toBeInTheDocument();
  });
  it('renders add-ons on two different rows corresponding to two supported languages', () => {
    ...
    expect(within(rows[1]).getByText('Persian (IR) Dictionary')).toBeInTheDocument();
    expect(within(rows[1]).getByText('Lilak, Persian Spell Checker Dictionary')).toBeInTheDocument();
  });
  it('renders meta tags', async () => {
    render();
    await waitFor(() => expect(getElement('meta[name="description"]')).toHaveAttribute('content', expect.stringContaining('Download Firefox dictionaries and language')));
  });
