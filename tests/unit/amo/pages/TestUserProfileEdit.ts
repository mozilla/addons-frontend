...
import { CLIENT_APP_FIREFOX, USERS_EDIT, VIEW_CONTEXT_HOME } from 'amo/constants';
import { clearError } from 'amo/reducers/errors';
import { FETCH_USER_ACCOUNT, FETCH_USER_NOTIFICATIONS, LOG_OUT_USER, deleteUserAccount, deleteUserPicture, fetchUserAccount, fetchUserNotifications, finishUpdateUserAccount, getCurrentUser, getUserById, loadUserAccount, loadUserNotifications, logOutUser, updateUserAccount } from 'amo/reducers/users';
import { getNotificationDescription } from 'amo/utils/notifications';
import { changeLocation, createFailedErrorHandler, createFakeLocation, createUserAccountResponse, createUserNotificationsResponse, dispatchClientMetadata, dispatchSignInActionsWithStore, fakeAuthors, fakeI18n, getElement, renderPage as defaultRender, screen, within } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const getErrorHandlerId = (userId) => `src/amo/pages/UserProfileEdit/index.js-${userId}`;
  ...
  const renderForOtherUser = ({
    ...
    const userId = loggedIn ? signInUserWithProps({
  ...
  it('renders user profile page for current logged-in user', () => {
    ...
    expect(screen.getByText(`Tell users a bit more information about yourself. Most fields are ` + `optional, but they'll help other users get to know you better.`)).toBeInTheDocument();
    expect(screen.getByText('Introduce yourself to the community if you like')).toBeInTheDocument();
    expect(screen.getByRole('link', {
      name: 'Manage Firefox Accounts…',
    })).toBeInTheDocument();
    expect(screen.getByText(`From time to time, Mozilla may send you email about upcoming ` + `releases and add-on events. Please select the topics you are ` + `interested in.`)).toBeInTheDocument();
  ...
  it('dispatches fetchUserAccount and fetchUserNotifications actions if userId is not found', () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(fetchUserAccount({
      errorHandlerId: getErrorHandlerId(userId),
      userId: defaultUserId,
    }));
    expect(dispatch).toHaveBeenCalledWith(fetchUserNotifications({
      errorHandlerId: getErrorHandlerId(userId),
      userId: defaultUserId,
    }));
  });
  ...
  it('always renders a disabled "email" input field', () => {
    ...
    expect(emailTextBox).toHaveAttribute('title', 'Email address cannot be changed here');
  });
  it('renders help sections for some fields', () => {
    renderForCurrentUser();
    expect(screen.getByTextAcrossTags('You can change your email address on Firefox Accounts. Need help?')).toBeInTheDocument();
    expect(screen.getByRole('link', {
      name: 'Need help?',
    })).toHaveAttribute('href', 'https://support.mozilla.org/kb/change-primary-email-address-firefox-accounts');
    expect(screen.getByText('This URL will only be visible for users who are developers.')).toHaveClass('UserProfileEdit-homepage--help');
    expect(screen.getByClassName('UserProfileEdit-biography--help')).toHaveTextContent(`Some HTML supported: <abbr title> <acronym title> <b> <blockquote> ` + `<code> <em> <i> <li> <ol> <strong> <ul>. Links are forbidden.`);
    expect(screen.queryByText(`Mozilla reserves the right to contact you individually about ` + `specific concerns with your hosted add-ons.`)).not.toBeInTheDocument();
  });
  it('renders a help text about add-on notifications for users who are developers', () => {
    renderForCurrentUser({
      is_addon_developer: true,
    });
    expect(screen.getByText(`Mozilla reserves the right to contact you individually about ` + `specific concerns with your hosted add-ons.`)).toBeInTheDocument();
  });
  ...
  it.each([['biography', () => screen.getByClassName('UserProfileEdit-biography')], ['display_name', () => screen.getByLabelText('Display Name *')], ['homepage', () => screen.getByRole('textbox', {
    name: 'Homepage',
  })], ['location', () => screen.getByRole('textbox', {
    name: 'Location',
  })], ['occupation', () => screen.getByRole('textbox', {
    name: 'Occupation',
  ...
  it('dispatches updateUserAccount action with all fields on submit', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(updateUserAccount({
  ...
  it('renders a create button with a different text when updating', async () => {
    ...
    await userEvent.type(screen.getByLabelText('Display Name *'), 'Some display name');
    await userEvent.click(screen.getByRole('button', {
      name: 'Create My Profile',
    }));
    expect(screen.getByRole('button', {
  ...
  it('allows to edit another user if logged-in user has USERS_EDIT permission', () => {
    ...
    expect(screen.getByLabelText('Email Address')).toHaveValue(defaultOtherUserEmail);
    expect(screen.getByRole('link', {
    ...
    expect(screen.queryByText(`Mozilla reserves the right to contact you individually about ` + `specific concerns with your hosted add-ons.`)).not.toBeInTheDocument();
    expect(screen.getByText(`Tell users a bit more information about this user. Most fields are ` + `optional, but they'll help other users get to know ${user.name} ` + `better.`)).toBeInTheDocument();
    // We do not render this link when user is not the current logged-in user.
    expect(screen.queryByRole('link', {
      name: 'Manage Firefox Accounts…',
    })).not.toBeInTheDocument();
    expect(screen.getByText(`Introduce ${user.name} to the community`)).toBeInTheDocument();
    expect(screen.getByText(`From time to time, Mozilla may send this user email about upcoming ` + `releases and add-on events. Please select the topics this user may ` + `be interested in.`)).toBeInTheDocument();
  });
  it('renders errors', () => {
  ...
  it('displays an AuthenticateButton if current user is not logged-in', () => {
    render();
    expect(screen.getByRole('link', {
      name: 'Log in to edit the profile',
    })).toBeInTheDocument();
  });
  ...
  it('displays a message when user has deleted their profile picture', async () => {
    ...
    const user = getCurrentUser(store.getState().users);
    expect(screen.getByAltText(`Profile picture for ${user.display_name}`)).toHaveAttribute('src', pictureUrl);
    ...
    expect(await screen.findByText('Picture successfully deleted')).toBeInTheDocument();
    expect(screen.getByClassName('Notice-success')).toBeInTheDocument();
    expect(screen.queryByAltText(`Profile picture for ${user.display_name}`)).not.toBeInTheDocument();
  ...
  it('displays a modal when user clicks the delete profile button', () => {
    ...
    expect(screen.getByText('IMPORTANT: Deleting your Firefox Add-ons profile is irreversible.')).toBeInTheDocument();
    expect(screen.getByClassName('Overlay')).toHaveClass('Overlay--visible');
    expect(screen.getByText(`Your data will be permanently removed, including ` + `profile details (picture, user name, display name, location, home ` + `page, biography, occupation), notification preferences, reviews, and ` + `collections.`)).toBeInTheDocument();
    expect(screen.getByText(`If you authored any add-ons they will also be deleted, unless you ` + `share ownership with other authors. In that case, you will be ` + `removed as an author and the remaining authors will maintain ` + `ownership of the add-on.`)).toBeInTheDocument();
    expect(screen.getByText(`When you use this email address to log in again to ` + `addons.mozilla.org, your profile on Firefox Add-ons will not have ` + `access to any of its previous content.`)).toBeInTheDocument();
  ...
  it('renders different information in the modal when user to be deleted is not the current logged-in user', async () => {
    ...
    expect(screen.getByText('IMPORTANT: Deleting this Firefox Add-ons profile is irreversible.')).toBeInTheDocument();
    expect(screen.getByText(`If the user authored any add-ons they will also be deleted, unless ` + `ownership is shared with other authors. In that case, the user will ` + `be removed as an author and the remaining authors will maintain ` + `ownership of the add-on.`)).toBeInTheDocument();
    expect(screen.getAllByRole('button', {
      name: 'Delete Profile',
    })[1]).not.toBeDisabled();
  });
  it('closes the modal when user clicks the cancel button', async () => {
    ...
    expect(preventDefaultWatcher).toHaveBeenCalled();
    expect(screen.queryByText('IMPORTANT: Deleting your Firefox Add-ons profile is irreversible.')).not.toBeInTheDocument();
  });
  it('dispatches deleteUserAccount and logOutUser when current logged-in user confirms account deletion', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(deleteUserAccount({
      errorHandlerId: getErrorHandlerId(userId),
      userId,
    }));
  ...
  it('clears the error handler when unmounting', () => {
    ...
    unmount();
    expect(dispatch).toHaveBeenCalledWith(clearError(getErrorHandlerId(userId)));
  });
  ...
  it('renders without a user loaded when current logged-in user is an admin', () => {
    ...
    expect(screen.getByText('Account')).toBeInTheDocument();
    expect(within(screen.getByClassName('UserProfileEdit-profile-aside')).getAllByRole('alert')).toHaveLength(2);
    expect(within(getElement('.UserProfileEdit--label[for="biography"]')).getByRole('alert')).toBeInTheDocument();
  });
  ...
  describe('Tests for UserProfileEditNotifications', () => {
    it('renders loading notifications without a user', () => {
      ...
      const notifications = screen.getAllByClassName('UserProfileEditNotification');
      expect(notifications).toHaveLength(2);
      notifications.forEach((notification, index) => {
        expect(notification).toHaveClass('UserProfileEditNotification--disabled');
        const checkbox = within(notification).getByRole('checkbox');
        expect(checkbox).toBeDisabled();
        expect(checkbox).toHaveAttribute('name', `loading-notification-${index}`);
    ...
    it(`renders notifications when the user's notifications are loaded`, () => {
      ...
      userNotifications.forEach((notification, index) => {
        ...
        expect(within(notificationElement).queryByRole('alert')).not.toBeInTheDocument();
        ...
        if (notification.mandatory) {
          expect(notificationElement).toHaveClass('UserProfileEditNotification--disabled');
          expect(checkbox).toBeDisabled();
        } else {
          expect(notificationElement).not.toHaveClass('UserProfileEditNotification--disabled');
    ...
    it('does not render a notification if there is no corresponding label', () => {
      ...
      const userNotifications = [{
        name: 'invalid-notification-name',
        enabled: true,
        mandatory: false,
      }];
      store.dispatch(loadUserNotifications({
      ...
      render();
      expect(screen.queryByClassName('UserProfileEditNotification')).not.toBeInTheDocument();
    });
  });
  describe('Tests for UserProfileEditPicture', () => {
    it('renders without a user', () => {
      ...
      const fileInput = screen.getByClassName('UserProfileEditPicture-file-input');
    ...
    it('specifies the alt text of the UserAvatar component when a user is passed', () => {
      ...
      expect(screen.getByAltText(`Profile picture for ${defaultDisplayName}`)).toHaveAttribute('src', pictureUrl);
    });
    it('enables the input file and select button when a user is supplied', () => {
      renderForCurrentUser();
      expect(screen.getByClassName('UserProfileEditPicture-file-input')).not.toBeDisabled();
      expect(screen.getByText('Choose Photo…')).not.toHaveClass('Button--disabled');
    });
    it('calls the onSelect() prop when a user selects a picture file', async () => {
      ...
      await waitFor(() => expect(screen.getByAltText(`Profile picture for ${defaultDisplayName}`)).toHaveAttribute('src', expect.stringContaining('data:')));
    });
    it('renders a "delete" ConfirmButton when user has a picture URL', async () => {
      ...
      expect(screen.getByText('Do you really want to delete this picture?')).toBeInTheDocument();
    });
    ...
    it('calls the onDelete() prop when a user deletes the picture', async () => {
      ...
      fireEvent(button, clickEvent);
      expect(preventDefaultWatcher).toHaveBeenCalled();
      expect(dispatch).toHaveBeenCalledWith(deleteUserPicture({
        errorHandlerId: getErrorHandlerId(userId),
        userId,
      }));
    });
    it('adds and removes a CSS class when file input has/looses focus', async () => {
      renderForCurrentUser();
      expect(screen.getByClassName('UserProfileEditPicture-file')).not.toHaveClass('UserProfileEditPicture-file--has-focus');
      await userEvent.click(screen.getByClassName('UserProfileEditPicture-file-input'));
      expect(screen.getByClassName('UserProfileEditPicture-file')).toHaveClass('UserProfileEditPicture-file--has-focus');
      await userEvent.tab();
      expect(screen.getByClassName('UserProfileEditPicture-file')).not.toHaveClass('UserProfileEditPicture-file--has-focus');
    });
  });
