...
import { FETCH_BLOCK, abortFetchBlock, fetchBlock, loadBlock } from 'amo/reducers/blocks';
import { createApiError } from 'amo/api';
import { createFailedErrorHandler, createFakeBlockResult, createLocalizedString, dispatchClientMetadata, fakeI18n, getElement, renderPage as defaultRender, screen, within } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const getErrorHandlerId = (guid = defaultGuid) => `src/amo/pages/Block/index.js-${guid}`;

  const getLocation = (guid = defaultGuid) => `/${lang}/${clientApp}/blocked-addon/${guid}/`;
  ...
  it('dispatches fetchBlock when block is undefined', () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(fetchBlock({
      guid: defaultGuid,
  ...
  it('does not fetch a block if fetch was aborted', () => {
    store.dispatch(abortFetchBlock({
      guid: defaultGuid,
    }));
    const dispatch = jest.spyOn(store, 'dispatch');
    render();
  ...
  it('fetches a block if it has not already been loaded', () => {
    ...
    const block1 = _createFakeBlockResult({
      guid: guid1,
    });
    ...
    expect(dispatch).toHaveBeenCalledWith(fetchBlock({
      guid: guid2,
      errorHandlerId: getErrorHandlerId(guid2),
    }));
  });
  ...
  it('renders a page with loading indicators when a block has not been loaded yet', () => {
    render();
    expect(within(screen.getByClassName('Block-reason')).getAllByRole('alert')).toHaveLength(1);
    // 1. versions blocked
    // 2. date and URL
    expect(within(screen.getByClassName('Block-metadata')).getAllByRole('alert')).toHaveLength(2);
  });
  it('renders a generic header/title when the block has no add-on name', async () => {
    ...
    render();
    await waitFor(() => expect(getElement('title')).toHaveTextContent(`${title} – Add-ons for Firefox (${lang})`));
  ...
  it('renders "all versions" when min is 0 and max is *', () => {
    ...
    // by a </br>.
    expect(screen.getByTextAcrossTags(`Versions blocked: all versions.Blocked on ${i18n.moment(block.created).format('ll')}.`)).toBeInTheDocument();
  });
  it('renders the min/max versions if min is not 0 and max is not *', () => {
    ...
    // The version info and the block date are inside the same tag, separated
    // by a </br>.
    expect(screen.getByTextAcrossTags(`Versions blocked: ${min} to ${max}.Blocked on ${i18n.moment(block.created).format('ll')}.`)).toBeInTheDocument();
  });
  it('renders the reason with HTML tags removed', () => {
    ...
    render();
    expect(screen.getByText('this is a reason for a block')).toBeInTheDocument();
  });
  ...
  it('does not render any link when there is no block URL', () => {
    ...
    within(screen.getByClassName('Block-metadata')).queryByRole('link')).not.toBeInTheDocument();
  });
  ...
  it('renders a block page when a versionId is present in the URL', () => {
    ...
    expect(screen.getByText(`${name} has been blocked for your protection.`)).toBeInTheDocument();
  });
  describe('extractId', () => {
