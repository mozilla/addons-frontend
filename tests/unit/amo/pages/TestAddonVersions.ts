...
import { FETCH_VERSIONS, loadVersions, fetchVersions } from 'amo/reducers/versions';
import { ADDON_TYPE_STATIC_THEME, CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX, FATAL_ERROR, INCOMPATIBLE_ANDROID_UNSUPPORTED, INCOMPATIBLE_FIREFOX_FOR_IOS, INCOMPATIBLE_NOT_FIREFOX, INCOMPATIBLE_OVER_MAX_VERSION, INCOMPATIBLE_UNDER_MIN_VERSION, INCOMPATIBLE_UNSUPPORTED_PLATFORM, INSTALLING, STRATEGIC, VERIFIED } from 'amo/constants';
...
import { changeLocation, createFailedErrorHandler, createFakeClientCompatibility, createFakeErrorHandler, createInternalAddonWithLang, createInternalVersionWithLang, createLocalizedString, dispatchClientMetadata, fakeAddon, fakeFile, fakeI18n, fakeVersion, getElement, renderPage as defaultRender, screen, userAgents, userAgentsByPlatform, within } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const getLocation = (slug = defaultSlug) => `/${lang}/${clientApp}/addon/${slug}/versions/`;
  ...
  it('fetches versions when versions are not loaded', () => {
    const dispatch = jest.spyOn(store, 'dispatch');
    render();
    expect(dispatch).toHaveBeenCalledWith(fetchVersions({
  ...
  it('generates an empty header when no add-on is loaded', async () => {
    ...
    expect(screen.getByClassName('AddonSummaryCard-header-text')).toHaveTextContent('');
  });
  it('generates an empty header when no versions have loaded', async () => {
    ...
    render();
    await waitFor(() => expect(getElement('title')).toHaveTextContent('Add-ons for Firefox (en-US)'));
    expect(within(screen.getByClassName('AddonSummaryCard-header-text')).getByClassName('visually-hidden')).toHaveTextContent('');
  });
  it('generates a header with add-on name and version count when versions have loaded', async () => {
    ...
    render();
    await waitFor(() => expect(getElement('title')).toHaveTextContent(expectedHeader));
  ...
  it('passes an add-on to the AddonSummaryCard', () => {
    ...
    render();
    expect(screen.getByAltText('Add-on icon')).toHaveAttribute('src', fakeAddon.icon_url);
  });
  it('passes a LoadingText component to the CardList header if the header is blank', () => {
    render();
    expect(within(within(screen.getByClassName('CardList')).getByClassName('Card-header-text')).getByRole('alert')).toBeInTheDocument();
  });
  ...
  it('passes the first found version into the AddonVersionCard', () => {
    ...
    expect(within(latestVersionCard).getByRole('heading', {
      name: `Version ${versionNumber}`,
    })).toBeInTheDocument();
    expect(within(latestVersionCard).getByText(`Works with ${app} ${min} to ${max}`)).toBeInTheDocument();
    expect(within(latestVersionCard).getByText(`Released ${i18n.moment(created).format('ll')} - ${formatFilesize({
      i18n,
      size,
    })}`)).toBeInTheDocument();
    expect(within(latestVersionCard).getByText(releaseNotes)).toBeInTheDocument();
    expect(within(latestVersionCard).getByTextAcrossTags(`Source code released under ${licenseName}`)).toBeInTheDocument();
    expect(within(latestVersionCard).getByRole('link', {
      name: licenseName,
  ...
  it('does not render file info when created date is missing', () => {
    ...
    expect(screen.queryByClassName('AddonVersionCard-fileInfo')).not.toBeInTheDocument();
  });
  it('passes the correct versions into multiple AddonVersionCards', () => {
    ...
    expect(within(versionCards[0]).getByText('Version 1.0')).toBeInTheDocument();
    expect(within(versionCards[1]).getByText('Version 2.0')).toBeInTheDocument();
    expect(within(versionCards[2]).getByText('Version 3.0')).toBeInTheDocument();
  });
  it('displays expected headings for latest and older versions', () => {
    ...
    render();
    const versionCards = allVersionCards();
    expect(within(versionCards[0]).getByRole('heading', {
      name: 'Latest version',
    })).toBeInTheDocument();
    expect(within(versionCards[1]).getByRole('heading', {
      name: 'Older versions',
    })).toBeInTheDocument();
    expect(within(versionCards[1]).queryByRole('heading', {
      name: 'Latest version',
    })).not.toBeInTheDocument();
    expect(within(versionCards[2]).queryByRole('heading', {
      name: 'Older versions',
    })).not.toBeInTheDocument();
    expect(within(versionCards[2]).queryByRole('heading', {
      name: 'Latest version',
    })).not.toBeInTheDocument();
    expect(within(versionCards[3]).queryByRole('heading', {
      name: 'Older versions',
    })).not.toBeInTheDocument();
    expect(within(versionCards[3]).queryByRole('heading', {
      name: 'Latest version',
  ...
  describe('Tests for AddonVersionCard', () => {
    it('returns a card with a message if version is null', () => {
      ...
      expect(screen.getByRole('heading', {
        level: 2,
      })).toHaveTextContent('No version found');
    });
    it('returns a card with LoadingText if version is undefined', () => {
      _loadAddon();

      render();
      expect(within(screen.getByClassName('AddonVersionCard')).getAllByRole('alert')).toHaveLength(3);
    });
    it('renders nothing for compatibility when no version is loaded', () => {
      _loadAddon();

      render();
      expect(screen.queryByClassName('AddonVersionCard-compatibility')).not.toBeInTheDocument();
    });
    it('strips illegal HTML from release notes', () => {
      ...
      expect(within(screen.getByClassName('AddonVersionCard-releaseNotes')).getByTagName('b')).toBeInTheDocument();
      expect(within( // eslint-disable-next-line testing-library/prefer-presence-queries
      screen.getByClassName('AddonVersionCard-releaseNotes')).queryByTagName('script')).not.toBeInTheDocument();
    });
    ...
    it('renders a link to a custom license', () => {
      ...
      expect(screen.getByRole('link', {
        name: licenseName,
      })).toHaveAttribute('href', `/en-US/firefox/addon/${defaultSlug}/license/`);
    });
    it('does not render license info if there is no license', () => {
      ...
      render();
      expect(screen.queryByClassName('AddonVersionCard-license')).not.toBeInTheDocument();
    });
    it('renders plain text when license has no URL', () => {
      ...
      render();
      expect(screen.getByText(`Source code released under ${licenseName}`)).toBeInTheDocument();
    });
    ...
    it('does not render an install error if there is no error', () => {
      renderWithAddonAndVersions();
      render();
      expect(screen.queryByClassName('AddonInstallError')).not.toBeInTheDocument();
    });
    ...
    it('does not render an AddonInstallError if it is not the current version', () => {
      ...
      _loadVersions({
        versions: [fakeVersion, { ...fakeVersion,
          id: 2,
        }],
      });
      ...
      render();
      const versionCards = allVersionCards();
      expect(within(versionCards[0]).getByText('An unexpected error occurred.')).toBeInTheDocument();
      expect(within(versionCards[1]).queryByText('An unexpected error occurred.')).not.toBeInTheDocument();
    });
    it('does not render an AddonCompatibilityError if there is no version', () => {
      ...
      expect(screen.queryByClassName('AddonCompatibilityError')).not.toBeInTheDocument();
    });
    it('does not render an AddonCompatibilityError if it is not the current version', () => {
      ...
      const versionCards = allVersionCards();
      expect(within(versionCards[0]).getByText('This add-on is not compatible with your version of Firefox.')).toBeInTheDocument();
      expect(within(versionCards[1]).queryByText('This add-on is not compatible with your version of Firefox.')).not.toBeInTheDocument();
    });
    ...
    it('passes showLinkInsteadOfButton to InstallButtonWrapper for a non-current version', () => {
      ...
      expect(within(versionCards[0]).getByRole('link', {
        name: 'Add to Firefox',
      })).toBeInTheDocument();
      expect(within(versionCards[1]).getByRole('link', {
        name: 'Download file',
      })).toBeInTheDocument();
    });
  ...
  describe('Tests for AddonCompatibilityError', () => {
    ...
    it('renders a notice if add-on is over maxVersion/compat is strict', () => {
      ...
      renderWithAddonAndVersions();
      expect(screen.getByClassName('AddonCompatibilityError')).toBeInTheDocument();
      expect(screen.getByText('This add-on is not compatible with your version of Firefox.')).toBeInTheDocument();
    });
    ...
    it.each([INCOMPATIBLE_ANDROID_UNSUPPORTED, INCOMPATIBLE_FIREFOX_FOR_IOS, INCOMPATIBLE_NOT_FIREFOX, INCOMPATIBLE_UNDER_MIN_VERSION, 'unknown reason'])('renders nothing if the incompatibility reason is %s', (reason) => {
      getClientCompatibility.mockReturnValue(createFakeClientCompatibility({
        compatible: false,
        reason,
      }));
  ...
  describe('Tests for InstallWarning', () => {
    const getInstallWarning = () => screen.getByText(`This add-on is not actively monitored for security by Mozilla. ` + `Make sure you trust it before installing.`);

    it('contains a correct link', () => {
      renderWithAddonAndVersions();
      expect(screen.getByRole('link', {
        name: 'Learn more',
      })).toHaveAttribute('href', getPromotedBadgesLinkUrl({
