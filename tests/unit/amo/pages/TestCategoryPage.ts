...
import { ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME, CLIENT_APP_FIREFOX, SEARCH_SORT_POPULAR, SEARCH_SORT_RECOMMENDED, SEARCH_SORT_TRENDING } from 'amo/constants';
...
import { dispatchClientMetadata, dispatchSearchResults, fakeAddon, fakeCategory, getElement, getSearchErrorHandlerId, renderPage as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  function render({
    ...
    const initialEntry = location || `/${lang}/${clientApp}/${addonType}/category/${category}/`;
    const renderOptions = {
  ...
  it('does not override an existing sort filter', async () => {
    const dispatch = jest.spyOn(store, 'dispatch');
    render({
      location: `${defaultLocation}?sort=${SEARCH_SORT_POPULAR}`,
    });
    await waitFor(() => expect(dispatch).toHaveBeenCalledWith(searchStart({
      errorHandlerId: getSearchErrorHandlerId(),
      filters: {
        addonType: ADDON_TYPE_EXTENSION,
        category: defaultCategory,
  ...
  it('configures pagination using filters and the category/type', async () => {
    ...
    render({
      location: `/${lang}/${clientApp}/${visibleAddonType(addonType)}/category/${category}/?page=${page}&sort=${sort}`,
    });
    ...
    expect(screen.getByRole('link', {
      name: 'Previous',
    })).toHaveAttribute('href', `/${lang}/${clientApp}/${visibleAddonType(addonType)}/category/${category}/?page=1&sort=${SEARCH_SORT_POPULAR}`);
  });
  ...
  it.each([[ADDON_TYPE_EXTENSION, 'Extensions'], [ADDON_TYPE_STATIC_THEME, 'Themes']])('sets the expected title for type: %s', async (type, expectedTitle) => {
    ...
    render({
      addonType: visibleAddonType(type),
    });
    await waitFor(() => expect(getElement('title')).toHaveTextContent(`${expectedTitle} in ${name} – Add-ons for Firefox (en-US)`));
  });
  it.each([[ADDON_TYPE_EXTENSION, 'Extensions'], [ADDON_TYPE_STATIC_THEME, 'Themes']])('sets the expected title for type: %s without a category name', async (type, expectedTitle) => {
    render({
      addonType: visibleAddonType(type),
    });
    await waitFor(() => expect(getElement('title')).toHaveTextContent(`${expectedTitle} – Add-ons for Firefox (en-US)`));
  });
