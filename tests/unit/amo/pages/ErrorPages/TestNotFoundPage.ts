...
import { dispatchClientMetadata, fakeI18n, renderPage as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const render = (i18n = fakeI18n()) => defaultRender({
  ...
  it('renders a NotFound Page', () => {
    ...
    expect(screen.getByTextAcrossTags(`Try visiting the page later, as the theme or extension may become ` + `available again. Alternatively, you may be able to find what you’re ` + `looking for in one of the available extensions or themes, or by ` + `asking for help on our community forums.`)).toBeInTheDocument();
