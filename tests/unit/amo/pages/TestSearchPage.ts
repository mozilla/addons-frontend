...
import { ADDON_TYPE_EXTENSION, ADDON_TYPE_LANG, ADDON_TYPE_STATIC_THEME, CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX, DEFAULT_CATEGORY_SORT, LINE, RECOMMENDED, REVIEWED_FILTER, SEARCH_SORT_RANDOM, SEARCH_SORT_RECOMMENDED, SEARCH_SORT_RELEVANCE, SEARCH_SORT_TRENDING, SEARCH_SORT_TOP_RATED, SEARCH_SORT_POPULAR, SET_VIEW_CONTEXT, VERIFIED_FILTER, VIEW_CONTEXT_HOME } from 'amo/constants';
...
import { createFailedErrorHandler, dispatchClientMetadata, dispatchSearchResults, fakeAddon, fakeCategory, getElement, getElements, getSearchErrorHandlerId, renderPage as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const _dispatchSearchResults = async (args = {}) => dispatchSearchResults({
    store,
    ...args,
  });
  ...
  it('preserves category in paginationQueryParams', async () => {
    ...
    expect(screen.getByRole('link', {
      name: 'Previous',
    })).toHaveAttribute('href', `${defaultLocation}?category=${category}&page=1&q=${query}`);
  });
  it('dispatches a server redirect when `atype` parameter is "1"', () => {
    const fakeDispatch = jest.spyOn(store, 'dispatch');
    render({
      location: `${defaultLocation}?atype=1`,
    });
    expect(fakeDispatch).toHaveBeenCalledWith(sendServerRedirect({
      status: 301,
  ...
  it('uses the clientApp from the API and not the location', () => {
    ...
    const fakeDispatch = jest.spyOn(store, 'dispatch');
    render({
      location: `/${lang}/${CLIENT_APP_FIREFOX}/search/?atype=1`,
    });
    expect(fakeDispatch).toHaveBeenCalledWith(sendServerRedirect({
      status: 301,
  ...
  it('uses the lang from the API and not the location', () => {
    ...
    const fakeDispatch = jest.spyOn(store, 'dispatch');
    render({
      location: `/en-US/${CLIENT_APP_FIREFOX}/search/?atype=1`,
    });
    expect(fakeDispatch).toHaveBeenCalledWith(sendServerRedirect({
      status: 301,
  ...
  it('dispatches a server redirect when `atype` parameter is "3"', () => {
    const fakeDispatch = jest.spyOn(store, 'dispatch');
    render({
      location: `${defaultLocation}?atype=3`,
    });
    expect(fakeDispatch).toHaveBeenCalledWith(sendServerRedirect({
      status: 301,
  ...
  it('dispatches a server redirect when `atype` parameter is "5"', () => {
    const fakeDispatch = jest.spyOn(store, 'dispatch');
    render({
      location: `${defaultLocation}?atype=5`,
    });
    expect(fakeDispatch).toHaveBeenCalledWith(sendServerRedirect({
      status: 301,
  ...
  it('does not dispatch a server redirect when `atype` has no mapping', () => {
    ...
    render({
      location: `${defaultLocation}?atype=123`,
    });
    expect(fakeDispatch).not.toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('dispatches a server redirect when `category` and `addonType` are set', () => {
    ...
    const page = '123';
    const fakeDispatch = jest.spyOn(store, 'dispatch');
    const location = [`${defaultLocation}?category=${category}`, `page=${page}`, `sort=${SEARCH_SORT_RECOMMENDED}`, `type=${ADDON_TYPE_EXTENSION}`].join('&');
    render({
      location,
    });
    const url = [`/${lang}/${clientApp}/extensions/category/${category}/?page=${page}`, `sort=${SEARCH_SORT_RECOMMENDED}`].join('&');
    expect(fakeDispatch).toHaveBeenCalledWith(sendServerRedirect({
      status: 301,
      url,
    }));
  ...
  it('does not dispatch a server redirect when category is set along with an invalid `addonType`', () => {
    ...
    const page = '123';
    const fakeDispatch = jest.spyOn(store, 'dispatch');
    const location = [`${defaultLocation}?category=${category}`, `page=${page}`, `sort=${SEARCH_SORT_RECOMMENDED}`, 'type=invalid'].join('&');
    render({
      location,
    });
    expect(fakeDispatch).not.toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('removes the default category sort, when present, when redirecting', () => {
    ...
    const page = '123';
    const fakeDispatch = jest.spyOn(store, 'dispatch');
    const location = [`${defaultLocation}?category=${category}`, `page=${page}`, `sort=${DEFAULT_CATEGORY_SORT}`, `type=${ADDON_TYPE_EXTENSION}`].join('&');
    render({
      location,
    });
    expect(fakeDispatch).toHaveBeenCalledWith(sendServerRedirect({
  ...
  it('dispatches a server redirect when `tag` is set', () => {
    ...
    const page = '123';
    const fakeDispatch = jest.spyOn(store, 'dispatch');
    const location = [`${defaultLocation}?tag=${tag}`, `page=${page}`, `sort=${SEARCH_SORT_RECOMMENDED}`, `type=${ADDON_TYPE_EXTENSION}`].join('&');
    render({
      location,
    });
    const url = [`/${lang}/${clientApp}/tag/${tag}/?page=${page}`, `sort=${SEARCH_SORT_RECOMMENDED}`, `type=${ADDON_TYPE_EXTENSION}`].join('&');
    expect(fakeDispatch).toHaveBeenCalledWith(sendServerRedirect({
      status: 301,
      url,
    }));
  ...
  it('removes the default tag sort, when present, when redirecting', () => {
    ...
    const page = '123';
    const fakeDispatch = jest.spyOn(store, 'dispatch');
    const location = [`${defaultLocation}?tag=${tag}`, `page=${page}`, `sort=${DEFAULT_CATEGORY_SORT}`].join('&');
    render({
      location,
    });
    expect(fakeDispatch).toHaveBeenCalledWith(sendServerRedirect({
  ...
  it('dispatches a server redirect when `platform` is set', () => {
    const fakeDispatch = jest.spyOn(store, 'dispatch');
    render({
      location: `${defaultLocation}?platform=whatever`,
    });
    expect(fakeDispatch).toHaveBeenCalledWith(sendServerRedirect({
      status: 301,
      url: defaultLocation,
  ...
  it('redirects without affecting the other parameters', () => {
    ...
    const page = '123';
    render({
      location: `${defaultLocation}?page=${page}&platform=whatever`,
    });
    expect(fakeDispatch).toHaveBeenCalledWith(sendServerRedirect({
      status: 301,
  ...
  describe('Tests for Search', () => {
    ...
    it('passes a Paginate component to the SearchResults component', async () => {
      ...
      expect(screen.getByRole('link', {
        name: 'Previous',
      })).toHaveAttribute('href', `${defaultLocation}?page=1&q=${query}`);
    });
    it('dispatches the search on mount', () => {
      ...
      expect(dispatch).toHaveBeenCalledWith(searchStart({
        errorHandlerId: getSearchErrorHandlerId(),
        filters: {
          query,
        },
    ...
    it.each([[{}, 'Search results'], [{
      ...
      q: 'search term',
    }, 'Search results for "search term"']])('renders an HTML title for %s', async (filterProps, title) => {
      let location = `${defaultLocation}?`;
      ...
      await waitFor(() => expect(getElement('title')).toHaveTextContent(`${title} – Add-ons for Firefox (en-US)`));
    });
  ...
  describe('Tests for SearchFilters', () => {
    ...
    it('selects the sort criterion in the sort select', () => {
      ...
      render({
        location: `${defaultLocation}?sort=${sort}`,
      });
      expect(screen.getByRole('option', {
        name: 'Trending',
      }).selected).toEqual(true);
    });
    it.each([`${SEARCH_SORT_RECOMMENDED},${SEARCH_SORT_TRENDING}`, `${SEARCH_SORT_TRENDING},${SEARCH_SORT_RECOMMENDED}`, `${SEARCH_SORT_TRENDING},${SEARCH_SORT_RECOMMENDED},${SEARCH_SORT_RELEVANCE}`])('selects the first non-recommended sort criterion in the sort select: %s', (sort) => {
      render({
        location: `${defaultLocation}?sort=${sort}`,
      });
      expect(screen.getByRole('option', {
    ...
    it('selects the promoted criterion in the promoted select', () => {
      render({
        location: `${defaultLocation}?promoted=${RECOMMENDED}`,
      });
      expect(screen.getByRole('option', {
        name: 'Recommended',
      }).selected).toEqual(true);
    ...
    it('does not display the addonType or badging filters on Android', () => {
      ...
      render({
        location: `/${lang}/${CLIENT_APP_ANDROID}/search/`,
      });
  ...
  describe('Tests for SearchContextCard', () => {
    ...
    it('should not fetch categories if there is no category filter', () => {
      const dispatch = jest.spyOn(store, 'dispatch');
      render();
      expect(dispatch).not.toHaveBeenCalledWith(fetchCategories({
        errorHandlerId: thisErrorHandlerId,
      }));
    });
    ...
    [{
      count: 1,
      query: 'test',
      expected: '1 result found for "test"',
    }], // Multiple results, just a query.
    [{
      count: 2,
      query: 'test',
      expected: '2 results found for "test"',
    }]])('should (old) render the expected header for %s results, category: %s, query: %s, tag: %s', async ({
      count,
      query,
      expected,
    }) => {
    ...
    const testData = (type, singular, plural) => [// Only type filter.
    {
      filters: {
        addonType: type,
      },
    ...
    const testWithFilters = async (filters, expectedSingular, expectedPlural) => {
      // Category is only displayed for extensions and themes, so skip if not
      // one of those.
      if (![ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME].includes(filters.addonType) && filters.category) {
        return true;
      }
    ...
    it.each(testData(ADDON_TYPE_EXTENSION, 'extension', 'extensions'))('should render the expected headers for extensions for %s', async ({
      filters,
      expectedSingular,
      expectedPlural,
    }) => {
      ...
      expect(await testWithFilters(filters, expectedSingular, expectedPlural)).toBeTruthy();
    });
    it.each(testData(ADDON_TYPE_STATIC_THEME, 'theme', 'themes'))('should render the expected headers for themes for %s', async ({
      filters,
      expectedSingular,
      expectedPlural,
    }) => {
    ...
    it.each(testData(ADDON_TYPE_LANG, 'result', 'results'))('should render the expected headers for lang packs for %s', async ({
      filters,
      expectedSingular,
      expectedPlural,
    }) => {
