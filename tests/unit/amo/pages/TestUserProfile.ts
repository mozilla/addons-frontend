...
import { fetchUserReviews, setUserReviews, FETCH_USER_REVIEWS } from 'amo/actions/reviews';
import createLocalState from 'amo/localState';
import { extractId } from 'amo/pages/UserProfile';
import { EXTENSIONS_BY_AUTHORS_PAGE_SIZE, FETCH_ADDONS_BY_AUTHORS, THEMES_BY_AUTHORS_PAGE_SIZE, fetchAddonsByAuthors } from 'amo/reducers/addonsByAuthors';
import { hideUserAbuseReportUI, loadUserAbuseReport, sendUserAbuseReport, showUserAbuseReportUI } from 'amo/reducers/userAbuseReports';
import { fetchUserAccount, getCurrentUser, loadUserAccount, FETCH_USER_ACCOUNT } from 'amo/reducers/users';
import { DEFAULT_API_PAGE_SIZE, createApiError } from 'amo/api';
import { ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME, API_ERROR_AUTHENTICATION_EXPIRED, CLIENT_APP_FIREFOX, SEARCH_SORT_POPULAR, USERS_EDIT, VIEW_CONTEXT_HOME } from 'amo/constants';
import { sendServerRedirect } from 'amo/reducers/redirectTo';
import { changeLocation, createFailedErrorHandler, createFakeUserAbuseReport, createUserAccountResponse, dispatchClientMetadata, dispatchSignInActionsWithStore, fakeAuthors, fakeReview, getElement, getElements, loadAddonsByAuthors, renderPage as defaultRender, screen, within } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  function renderUserProfile({
    ...
    const renderOptions = {
      initialEntries: [location || getLocation({
        userId,
      })],
      store,
    };
  ...
  it('dispatches fetchUserAccount action if userId is not found', async () => {
    ...
    await waitFor(() => expect(dispatch).toHaveBeenCalledWith(fetchUserAccount({
  ...
  it('renders the user avatar', () => {
    ...
    expect(screen.getByAltText('User Avatar')).toHaveAttribute('src', pictureUrl);
  });
  ...
  it('renders LoadingText when user has not been loaded yet', () => {
    renderUserProfile();
    expect(within(screen.getByClassName('UserProfile-name')).getByRole('alert')).toBeInTheDocument();
    expect(within(screen.getByClassName('UserProfile-user-since')).getByRole('alert')).toBeInTheDocument();
    expect(within(screen.getByClassName('UserProfile-number-of-addons')).getByRole('alert')).toBeInTheDocument();
    expect(within(screen.getByClassName(`UserProfile-rating-average`)).getByRole('alert')).toBeInTheDocument();
  });
  ...
  it("renders the user's average add-on rating", () => {
    ...
    expect(screen.getByText('Average rating of developer’s add-ons')).toBeInTheDocument();
  ...
  it("renders the user's biography", () => {
    ...
    expect(within(screen.getByClassName('UserProfile-biography')).getByTagName('blockquote')).toHaveTextContent(biographyText);
  });
  ...
  it('does not render a report abuse button if user is the current logged-in user', () => {
    ...
    expect(screen.queryByRole('button', {
      name: 'Report this user for abuse',
  ...
  it('shows Login Expired if api returns 401 auth expired', async () => {
    ...
    expect(screen.getByRole('link', {
      name: 'Edit profile',
    })).toHaveAttribute('href', `/${lang}/${clientApp}/users/edit`);
    createFailedErrorHandler({
      error: createApiError({
        response: {
          status: 401,
  ...
  it('renders an edit link if user has sufficient permission', () => {
    ...
    expect(screen.getByRole('link', {
      name: 'Edit profile',
    })).toHaveAttribute('href', `/${lang}/${clientApp}/user/${anotherUserId}/edit/`);
  });
  ...
  it('renders an admin link if user has sufficient permission', () => {
    ...
    expect(screen.getByRole('link', {
      name: 'Admin user',
    })).toHaveAttribute('href', `/admin/models/users/userprofile/${userId}/`);
  });
  ...
  it('fetches reviews if not loaded and userId does not change', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(fetchUserReviews({
      errorHandlerId: createErrorHandlerId(),
      page: '1',
      userId: defaultUserId,
    }));
  ...
  it(`displays the user's reviews`, () => {
    ...
    expect(within(paginator).queryByRole('link', {
      name: 'Next',
    })).not.toBeInTheDocument();
  });
  it(`displays the user's reviews with pagination when there are more reviews than the default API page size`, () => {
    ...
    expect(within(paginator).getByRole('link', {
      name: 'Next',
    })).toHaveAttribute('href', getLocation({
      search: `?page=2`,
    }));
  ...
  it('renders a user profile when URL contains a user ID', () => {
    ...
    renderUserProfile();
    expect(within(screen.getByClassName('UserProfile')).getByText(name)).toBeInTheDocument();
    expect(screen.getByText(`Extensions by ${name}`)).toBeInTheDocument();
    expect(screen.getByText(`Themes by ${name}`)).toBeInTheDocument();
    expect(screen.getByText('Next')).toHaveAttribute('href', getLocation({
      search: `?page=2`,
    }));
  });
  it('renders a UserProfileHead component when user is a developer', async () => {
    ...
    await waitFor(() => {
      expect(getElement('meta[name="description"]')).toHaveAttribute('content', `The profile of ${name}, Firefox extension author. Find other extensions by ${name}, including average ratings, tenure, and the option to report issues.`);
    });
  });
  it('renders a UserProfileHead component when user is an artist', async () => {
    ...
    await waitFor(() => {
      expect(getElement('meta[name="description"]')).toHaveAttribute('content', `The profile of ${name}, Firefox theme author. Find other themes by ${name}, including average ratings, tenure, and the option to report issues.`);
    });
  });
  it('renders a UserProfileHead component when user is a developer and an artist', async () => {
    ...
    await waitFor(() => {
      expect(getElement('meta[name="description"]')).toHaveAttribute('content', `The profile of ${name}, a Firefox extension and theme author. Find other apps by ${name}, including average ratings, tenure, and the option to report issues.`);
    });
  });
  ...
  describe('Tests for AddonsByAuthorsCard', () => {
    ...
    it('renders a LoadingText header when authorIds is null', () => {
      renderUserProfile();
      expect(within(within(screen.getAllByClassName('AddonsByAuthorsCard')[0]).getByClassName('Card-header-text')).getByRole('alert')).toBeInTheDocument();
    });
    ...
    describe('with pagination', () => {
      ...
      it('should dispatch a fetch action if page changes', async () => {
        ...
        expect(dispatch).not.toHaveBeenCalledWith(fetchAddonsByAuthors({
          addonType: ADDON_TYPE_STATIC_THEME,
          authorIds: [defaultUserId],
          errorHandlerId: createErrorHandlerId(),
          page: '2',
  ...
  describe('Tests for UserProfileHead', () => {
    it.each([['no query parameters', {}, ''], ['other query parameters', {
      foo: '123',
    }, ''], ['only page_e provided', {
      page_e: '123',
    }, '?page_e=123'], ['only page_t provided', {
      page_t: '123',
    }, '?page_t=123'], ['page_t and another param', {
      page_t: '123',
      foo: 'bar',
    }, '?page_t=123'], ['page_e = 1', {
      page_e: '1',
      ...
      page_t: '1',
    }, ''], ['page_e = 1 and page_t = 1', {
      page_e: '1',
      page_t: '1',
    }, ''], ['page_t = 1 and page_e = 1', {
      page_t: '1',
      page_e: '1',
    }, ''], ['page_e = 1 and page_t = 2', {
      page_e: '1',
      page_t: '2',
    }, '?page_t=2'], ['page_t = 1 and page_e = 2', {
      page_t: '1',
      page_e: '2',
    }, '?page_e=2'], ['page_e = 2 and page_t = 1', {
      page_e: '2',
      page_t: '1',
    }, '?page_e=2'], ['page_t = 2 and page_e = 1', {
      page_t: '2',
      page_e: '1',
    }, '?page_t=2'], ['page_e = 2 and page_t = 2', {
      page_e: '2',
      page_t: '2',
    }, '?page_e=2'], ['page_t = 2 and page_e = 2', {
      page_t: '2',
      page_e: '2',
    }, '?page_t=2'], ['page_e = 3 and page_t = 4', {
      page_e: '3',
      page_t: '4',
    }, '?page_t=4'], ['page_e = 4 and page_t = 3', {
      page_e: '4',
      page_t: '3',
    }, '?page_e=4']])('passes the expected queryString to HeadMetaTags and HeadLinks with %s', async (feature, query, expectedQueryString) => {
      ...
      await waitFor(() => expect(getElement('link[rel="canonical"]')).toHaveAttribute('href', `${config.get('baseURL')}${getLocation({
        search: expectedQueryString,
      })}`));
      expect(getElement('meta[property="og:url"]')).toHaveAttribute('content', `${config.get('baseURL')}${getLocation({
        search: expectedQueryString,
      })}`);
    });
    it('passes the expected title to HeadMetaTags when not logged in', async () => {
      renderUserProfile();
      await waitFor(() => expect(getElement('meta[property="og:title"]')).toHaveAttribute('content', `User Profile – Add-ons for Firefox (${lang})`));
    });
    it('passes the expected title to HeadMetaTags when logged in', async () => {
      ...
      await waitFor(() => expect(getElement('meta[property="og:title"]')).toHaveAttribute('content', `User Profile for ${displayName} – Add-ons for Firefox (${lang})`));
    });
  });
  describe('Tests for ReportUserAbuse', () => {
    ...
    it('shows the preview content when first rendered', () => {
      renderUserProfile();
      expect(screen.getByClassName('ReportUserAbuse')).not.toHaveClass('ReportUserAbuse--is-expanded');
      expect(screen.getByRole('button', {
        name: 'Report this user for abuse',
      })).toBeInTheDocument();
    });
    it('shows more content when the button is clicked', async () => {
      ...
      expect(dispatch).toHaveBeenCalledWith(showUserAbuseReportUI({
        userId,
      }));
      expect(screen.getByClassName('ReportUserAbuse')).toHaveClass('ReportUserAbuse--is-expanded');
    ...
    it('hides more content when the cancel button is clicked', async () => {
      ...
      expect(dispatch).toHaveBeenCalledWith(hideUserAbuseReportUI({
        userId,
      }));
      expect(screen.getByClassName('ReportUserAbuse')).not.toHaveClass('ReportUserAbuse--is-expanded');
    });
    it('dispatches the send abuse report action', async () => {
      ...
      await userEvent.type(screen.getByPlaceholderText('Explain how this user is violating our policies.'), message);
      await userEvent.click(screen.getByRole('button', {
        name: 'Send abuse report',
      }));
      expect(dispatch).toHaveBeenCalledWith(sendUserAbuseReport({
        errorHandlerId,
        message,
        userId,
      }));
      expect(screen.getByRole('button', {
        name: 'Sending abuse report',
      })).toBeDisabled();
    });
    ...
    describe('Tests for DismissibleTextForm', () => {
      const getLocalStateId = (id) => `src/amo/components/ReportUserAbuse/index.js-${id}`;
      ...
      it('recreates LocalState on update when the ID changes', async () => {
        ...
        expect(createLocalState).toHaveBeenCalledWith(getLocalStateId(anotherUserId));
      });
