...
import { PRIMARY_HERO_CLICK_ACTION, PRIMARY_HERO_CLICK_CATEGORY, PRIMARY_HERO_EXTERNAL_LABEL, PRIMARY_HERO_IMPRESSION_ACTION, PRIMARY_HERO_IMPRESSION_CATEGORY, PRIMARY_HERO_SRC } from 'amo/components/HeroRecommendation';
import { HOMESHELVES_ENDPOINT_COLLECTIONS, HOMESHELVES_ENDPOINT_SEARCH, HOMESHELVES_ENDPOINT_RANDOM_TAG } from 'amo/components/HomepageShelves';
import { SECONDARY_HERO_CLICK_ACTION, SECONDARY_HERO_CLICK_CATEGORY, SECONDARY_HERO_SRC, makeCallToActionURL } from 'amo/components/SecondaryHero';
import { ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME, CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX, DEFAULT_UTM_MEDIUM, DEFAULT_UTM_SOURCE, INSTALL_SOURCE_FEATURED_COLLECTION, INSTALL_SOURCE_FEATURED, INSTALL_SOURCE_TAG_SHELF_PREFIX, LANDING_PAGE_EXTENSION_COUNT, LANDING_PAGE_THEME_COUNT, LINE, RECOMMENDED, SPONSORED, VIEW_CONTEXT_HOME, VERIFIED } from 'amo/constants';
import { FETCH_HOME_DATA, fetchHomeData, loadHomeData } from 'amo/reducers/home';
...
import { changeLocation, createAddonsApiResult, createFailedErrorHandler, createHomeShelves, createLocalizedString, createPrimaryHeroShelf, dispatchClientMetadata, fakeAddon, fakeExternalShelf, fakePrimaryHeroShelfExternalAddon, getElement, renderPage as defaultRender, screen, within } from 'tests/unit/helpers';

jest.mock('amo/utils', () => ({ ...jest.requireActual('amo/utils'),
  checkInternalURL: jest.fn().mockReturnValue({
    isInternal: false,
  }),
...
describe(__filename, () => {
  ...
  const _loadHomeData = ({
    ...
    store.dispatch(loadHomeData({
      homeShelves: homeShelves || _createHomeShelves({
        primaryProps,
        resultsProps,
        secondaryProps,
      }),
  ...
  describe('Tests for SecondaryHero', () => {
    it('renders a message with an internal link', () => {
      ...
      expect(link).toHaveAttribute('href', `/${defaultLang}/${defaultClientApp}${addQueryParams(cta.url, {
    ...
    it('renders a message with an external link', () => {
      ...
      expect(link).toHaveAttribute('href', addQueryParams(cta.url, {
        utm_source: DEFAULT_UTM_SOURCE,
        utm_medium: DEFAULT_UTM_MEDIUM,
        utm_content: SECONDARY_HERO_SRC,
      }));
    ...
    it('renders a message without a link', () => {
      ...
      within(screen.getByClassName('SecondaryHero-message')).queryByRole('link')).not.toBeInTheDocument();
    });
    ...
    it('renders in a loading state', () => {
      render();
      expect(within(screen.getByClassName('SecondaryHero')).getAllByRole('alert')).toHaveLength(11);
    });
    ...
    describe('modules', () => {
      ...
      it.each([[0, 'internal', module1], [1, 'undefined', module2], [2, 'external', module3]])('renders the module at position "%s" with an %s link', (moduleIndex, linkType, moduleData) => {
        checkInternalURL.mockReturnValue({
          isInternal: linkType === 'internal',
          relativeURL: moduleData.cta && moduleData.cta.url,
        });
        renderWithHomeData({
          secondaryProps: secondaryPropsWithModules,
        });
        const module = screen.getAllByClassName('SecondaryHero-module')[moduleIndex];
        expect(screen.getByAltText(moduleData.description)).toHaveAttribute('src', moduleData.icon);
        expect(screen.getByText(moduleData.description)).toBeInTheDocument();
        expect(within(module).queryAllByRole('link')).toHaveLength(moduleData.cta ? 1 : 0);
        ...
        if (moduleData.cta) {
          ...
          if (linkType === 'external') {
            // eslint-disable-next-line jest/no-conditional-expect
            expect(link).toHaveAttribute('href', addQueryParams(moduleData.cta.url, {
              utm_source: DEFAULT_UTM_SOURCE,
              utm_medium: DEFAULT_UTM_MEDIUM,
              utm_content: SECONDARY_HERO_SRC,
            }));
            ...
            // eslint-disable-next-line jest/no-conditional-expect
            expect(link).toHaveAttribute('href', `/${defaultLang}/${defaultClientApp}${addQueryParams(moduleData.cta.url, {
              utm_source: DEFAULT_UTM_SOURCE,
              utm_medium: DEFAULT_UTM_MEDIUM,
              utm_content: SECONDARY_HERO_SRC,
            })}`);
      ...
      it('sends a tracking event when the cta is clicked', async () => {
        ...
        tracking.sendEvent.mockClear();
        await userEvent.click(screen.getByRole('link', {
          name: module1.cta.text,
        }));
        expect(stripLangFromAmoUrl).toHaveBeenCalledWith({
          urlString: expect.stringContaining(makeCallToActionURL(module1.cta.url)),
        });
        expect(tracking.sendEvent).toHaveBeenCalledTimes(1);
        expect(tracking.sendEvent).toHaveBeenCalledWith({
          action: SECONDARY_HERO_CLICK_ACTION,
  ...
  describe('Tests for HomepageShelves', () => {
    it('renders shelves in a loading state', () => {
      render();
      const loadingShelves = screen.getAllByClassName('HomepageShelves-loading-card');
    ...
    it('renders shelves in a loaded state', () => {
      ...
      const shelf1Data = { ...fakeExternalShelf,
      ...
      const shelf2Data = { ...fakeExternalShelf,
        addons: new Array(LANDING_PAGE_THEME_COUNT).fill({ ...fakeAddon,
      ...
      renderWithHomeData({
        resultsProps: [shelf1Data, shelf2Data],
      });
      const shelves = screen.getAllByClassName('LandingAddonsCard');
      expect(shelves).toHaveLength(2);
      const shelf1 = shelves[0];
      expect(within(shelf1).getAllByRole('link', {
        name: shelf1AddonName,
      ...
      expect(within(shelf1).getByClassName('Card-shelf-header')).toBeInTheDocument();
      expect(within(shelf1).getByClassName('Card-shelf-footer-in-header')).toBeInTheDocument();
      const shelf2 = shelves[1];
      expect(within(shelf2).getAllByRole('link', {
        name: shelf2AddonName,
    ...
    it.each([[INSTALL_SOURCE_FEATURED_COLLECTION, HOMESHELVES_ENDPOINT_COLLECTIONS], [INSTALL_SOURCE_FEATURED, HOMESHELVES_ENDPOINT_SEARCH]])('passes addonInstallSource as %s when endpoint is %s', (addonInstallSource, endpoint) => {
      ...
      expect(screen.getByRole('link', {
        name: addonName,
      })).toHaveAttribute('href', [`/${defaultLang}/${defaultClientApp}/addon/${slug}/?utm_source=${DEFAULT_UTM_SOURCE}`, `utm_medium=${DEFAULT_UTM_MEDIUM}`, `utm_content=${addonInstallSource}`].join('&'));
    });
    it('passes addonInstallSource as tag-shelf-{tag} when endpoint is random-tag', () => {
      ...
      expect(screen.getByRole('link', {
        name: addonName,
      })).toHaveAttribute('href', [`/${defaultLang}/${defaultClientApp}/addon/${slug}/?utm_source=${DEFAULT_UTM_SOURCE}`, `utm_medium=${DEFAULT_UTM_MEDIUM}`, `utm_content=${INSTALL_SOURCE_TAG_SHELF_PREFIX}${tagName}`].join('&'));
    });
    it('generates a default footerText', () => {
      ...
      renderWithHomeData({
        resultsProps: [{ ...fakeExternalShelf,
          addons: new Array(LANDING_PAGE_EXTENSION_COUNT).fill(fakeAddon),
          addon_type: ADDON_TYPE_EXTENSION,
          footer: { ...fakeExternalShelf.footer,
            text: '',
          },
    ...
    it('passes a relative URL for an internal link', () => {
      ...
      expect(screen.getAllByRole('link', {
        name: text,
      })[0]).toHaveAttribute('href', `/${defaultLang}/${defaultClientApp}${fixedURL}`);
      expect(checkInternalURL).toHaveBeenCalledWith({
        urlString: url,
      });
    });
  ...
  describe('Tests for HeroRecommendation', () => {
    describe('for an addon', () => {
      ...
      it('renders a link', () => {
        ...
        })).toHaveAttribute('href', addQueryParams(`/${defaultLang}/${defaultClientApp}/addon/${slug}/`, {
          utm_source: DEFAULT_UTM_SOURCE,
          utm_medium: DEFAULT_UTM_MEDIUM,
          utm_content: PRIMARY_HERO_SRC,
        }));
      });
      it.each([[LINE, 'BY FIREFOX'], [RECOMMENDED, 'RECOMMENDED'], [SPONSORED, 'SPONSORED'], [VERIFIED, 'SPONSORED'], ['unknown category', 'SPONSORED']])('displays the expected title for %s add-ons', (category, title) => {
        renderWithHomeData({
          primaryProps: {
            addon: addonForPromotedCategory(category),
          },
        });
        expect(screen.getByText(title)).toHaveClass('HeroRecommendation-title-text');
      });
      it.each([SPONSORED, VERIFIED, 'unknown category'])('displays an additional link for %s add-ons', (category) => {
        renderWithHomeData({
          primaryProps: {
            addon: addonForPromotedCategory(category),
          },
        ...
        expect(screen.getByRole('link', {
          name: `Firefox only recommends extensions that meet our ` + `standards for security and performance.`,
        })).toBeInTheDocument();
      });
      ...
      it.each([LINE, RECOMMENDED])('does not display an additional link for %s add-ons', (category) => {
        renderWithHomeData({
          primaryProps: {
            addon: addonForPromotedCategory(category),
          },
    ...
    describe('for an external item', () => {
      ...
      it('renders a link', () => {
        ...
        expect(screen.getByRole('link', {
          name: 'Get the extension',
        })).toHaveAttribute('href', addQueryParams(url, {
          utm_source: DEFAULT_UTM_SOURCE,
          utm_medium: DEFAULT_UTM_MEDIUM,
          utm_content: PRIMARY_HERO_SRC,
        }));
    ...
    it('renders with an image', () => {
      ...
      expect(screen.getByClassName('HeroRecommendation')).not.toHaveClass('HeroRecommendation--no-image');
      expect(screen.getByClassName('HeroRecommendation-image')).toHaveAttribute('src', featuredImage);
    });
    it('renders without an image', () => {
      ...
      expect(screen.getByClassName('HeroRecommendation')).toHaveClass('HeroRecommendation--no-image');
      expect(screen.queryByClassName('HeroRecommendation-image')).not.toBeInTheDocument();
    });
    it('assigns a className based on the gradient', () => {
      ...
      expect(screen.getByClassName('HeroRecommendation')).toHaveClass(`HeroRecommendation-${gradient.start}-${gradient.end}`);
    });
    it('renders a body', () => {
    ...
    it('allows some html tags in the body', () => {
      ...
      expect(within(screen.getByClassName('HeroRecommendation-body')).getByTagName('b')).toBeInTheDocument();
      expect(within(screen.getByClassName('HeroRecommendation-body')).getByTagName('blockquote')).toBeInTheDocument();
    });
    ...
    it.each([{
      ...
      readOnly: true,
      notice: 'some notice',
    }])('assigns the expected class when an AppBanner is present, status: %s', (status) => {
      store.dispatch(loadSiteStatus(status));
      render();
      expect(screen.getByClassName('HeroRecommendation')).toHaveClass('HeroRecommendation--height-with-notice');
    });
    it('assigns the expected class when an AppBanner is not present', () => {
      ...
      render();
      expect(screen.getByClassName('HeroRecommendation')).toHaveClass('HeroRecommendation--height-without-notice');
    });
    it('renders in a loading state', () => {
      render();
      expect(within(screen.getByClassName('HeroRecommendation')).getAllByRole('alert')).toHaveLength(5);
    });
    it('renders nothing if shelfData is null', () => {
      renderWithHomeData({
        homeShelves: {
      ...
      expect(screen.queryByClassName('HeroRecommendation')).not.toBeInTheDocument();
    });
    describe('tracking', () => {
      ...
      it.each([['addon', withAddonShelfData], ['external', withExternalShelfData]])('sends a tracking event when the cta is clicked for %s', async (feature, shelfData) => {
        ...
        expect(tracking.sendEvent).toHaveBeenCalledWith({
          ...
          label: feature === 'addon' ? shelfData.primaryProps.addon.guid : PRIMARY_HERO_EXTERNAL_LABEL,
        });
      });
      it.each([['addon', withAddonShelfData], ['external', withExternalShelfData]])('sends a tracking event for the impression on mount for %s', (feature, shelfData) => {
        renderWithHomeData(shelfData);
        expect(tracking.sendEvent).toHaveBeenCalledTimes(1);
        expect(tracking.sendEvent).toHaveBeenCalledWith({
      ...
      it.each([['addon', withAddonShelfData], ['external', withExternalShelfData]])('sends a tracking event for the impression on update for %s', async (feature, shelfData) => {
        render();
  ...
  it('renders a shelf with curated themes on desktop', () => {
    const expectedThemes = ['abstract', 'nature', 'film-and-tv', 'scenery', 'music', 'seasonal'];
    render();
    expect(screen.getByRole('heading', {
    ...
    expectedThemes.forEach((slug, index) => {
      expect(themeLinks[index]).toHaveAttribute('href', `/${defaultLang}/${defaultClientApp}${getCategoryResultsPathname({
  ...
  it('renders only the Recommended extensions shelf on android', () => {
    ...
    expect(within(screen.getByClassName('Home-RecommendedExtensions')).getAllByRole('alert')).toHaveLength(40);
  });
  it('renders the Recommended extensions shelf with data loaded on android', () => {
    ...
    expect( // eslint-disable-next-line testing-library/prefer-presence-queries
    within(screen.getByClassName('Home-RecommendedExtensions')).queryByRole('alert')).not.toBeInTheDocument();
    expect(screen.getByRole('link', {
      name: addonName,
    })).toHaveAttribute('href', `/${defaultLang}/${clientApp}${addQueryParams(`/addon/${addon.slug}/`, {
      utm_source: DEFAULT_UTM_SOURCE,
      utm_medium: DEFAULT_UTM_MEDIUM,
  ...
  it('dispatches an action to fetch the add-ons to display', () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(fetchHomeData({
      errorHandlerId,
      isDesktopSite: true,
    }));
  });
  ...
  it('displays an error in the expected place on Android', () => {
    ...
    expect(screen.getByClassName('Home-noHeroError')).toHaveTextContent(message);
  });
  it('displays an error in the expected place on desktop', () => {
    ...
    expect(screen.getByClassName('HeroRecommendation')).toHaveTextContent(message);
  });
  it('renders a HeadMetaTags component', async () => {
    render();
    // Without the waitFor, the meta tags have not rendered into the head yet.
    await waitFor(() => expect(getElement('meta[name="description"]')).toHaveAttribute('content', `Download Firefox extensions and themes. They’re like apps for your ` + `browser. They can block annoying ads, protect passwords, change ` + `browser appearance, and more.`));
  });
  ...
  it('renders the heroHeader for Android in a loading state if shelves are not loaded', () => {
    ...
    expect(within(screen.getByClassName('Home-heroHeader')).getAllByRole('alert')).toHaveLength(2);
  });
