...
import { CONTRIBUTE_BUTTON_CLICK_ACTION, CONTRIBUTE_BUTTON_CLICK_CATEGORY } from 'amo/components/ContributeCard';
import { ADDON_TYPE_DICT, ADDON_TYPE_EXTENSION, ADDON_TYPE_LANG, ADDON_TYPE_STATIC_THEME, ADDONS_CONTENT_REVIEW, ADDONS_EDIT, ADDONS_REVIEW, CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX, DEFAULT_UTM_SOURCE, FATAL_ERROR, INCOMPATIBLE_UNSUPPORTED_PLATFORM, INSTALLING, RECOMMENDED, SET_VIEW_CONTEXT, STATIC_THEMES_REVIEW } from 'amo/constants';
import { EXPERIMENT_CONFIG, VARIANT_SHOW } from 'amo/experiments/20210714_amo_vpn_promo';
import { ADDONS_BY_AUTHORS_COUNT, extractId } from 'amo/pages/Addon';
import { FETCH_ADDON, LOAD_ADDON, fetchAddon, getAddonByIdInURL, loadAddon } from 'amo/reducers/addons';
import { EXTENSIONS_BY_AUTHORS_PAGE_SIZE, FETCH_ADDONS_BY_AUTHORS, fetchAddonsByAuthors } from 'amo/reducers/addonsByAuthors';
...
import { FETCH_RECOMMENDATIONS, OUTCOME_CURATED, OUTCOME_RECOMMENDED, OUTCOME_RECOMMENDED_FALLBACK, fetchRecommendations, loadRecommendations } from 'amo/reducers/recommendations';
import { SEND_SERVER_REDIRECT, sendServerRedirect } from 'amo/reducers/redirectTo';
...
import { correctedLocationForPlatform, getClientCompatibility } from 'amo/utils/compatibility';
import { changeLocation, createCapturedErrorHandler, createFailedErrorHandler, createFakeClientCompatibility, createFakeErrorHandler, createLocalizedString, createVPNExperimentCookie, dispatchClientMetadata, dispatchSignInActionsWithStore, fakeAddon, fakeAuthor, fakeAuthors, fakeFile, fakePreview, fakeRecommendations, fakeVersion, getElement, getMockConfig, loadAddonsByAuthors, renderPage as defaultRender, screen, within } from 'tests/unit/helpers';
...
jest.mock('amo/utils/compatibility', () => ({ ...jest.requireActual('amo/utils/compatibility'),
  correctedLocationForPlatform: jest.fn().mockReturnValue('a/different/location/'),
  getClientCompatibility: jest.fn().mockReturnValue({
    compatible: true,
    reason: null,
  }),
}));
jest.mock('amo/addonManager', () => ({ ...jest.requireActual('amo/addonManager'),
...
jest.mock('amo/tracking', () => ({ ...jest.requireActual('amo/tracking'),
  sendEvent: jest.fn(),
...
describe(__filename, () => {
  ...
  const getLocation = ({
    ...
    return `/${lang}/${clientApp}/addon/${slug}/${page ? `?page=${page}/` : ''}`;
  };

  const getErrorHandlerId = (slug = defaultSlug) => `src/amo/pages/Addon/index.js-${slug}`;

  const fakeErrorHandler = createFakeErrorHandler({
    id: getErrorHandlerId(),
  });

  const mockClientHeight = (height) => Object.defineProperty(HTMLElement.prototype, 'clientHeight', {
    configurable: true,
    value: height,
  });
  ...
  beforeEach(() => {
    addon = { ...fakeAddon,
      // By default loads an add-on with an expected author.
  ...
  it('updates the ViewContext on update', async () => {
    ...
    expect(screen.getByRole('link', {
      name: 'Extensions',
    })).toHaveClass('SectionLinks-link--active');
  ...
  it('renders a WrongPlatformWarning component', () => {
    renderWithAddon();
    expect(screen.getByClassName('Addon-WrongPlatformWarning')).toBeInTheDocument();
    expect(screen.getByRole('link', {
      name: 'Browse add-ons for Firefox on desktop',
    })).toHaveAttribute('href', 'a/different/location/');
    expect(screen.getByText(/This listing is not intended for this platform./)).toBeInTheDocument();
  });
  it('does not render a WrongPlatformWarning component without an addon', () => {
    render();
    expect(screen.queryByClassName('Addon-WrongPlatformWarning')).not.toBeInTheDocument();
  });
  ...
  it('fetches an add-on when rendering without an add-on', () => {
    const dispatch = jest.spyOn(store, 'dispatch');
    render();
    expect(dispatch).toHaveBeenCalledWith(fetchAddon({
  ...
  it('passes props to the Page component', () => {
    ...
    expect(screen.getByClassName('Addon-WrongPlatformWarning')).toBeInTheDocument();
    expect(screen.queryByClassName('Page-WrongPlatformWarning')).not.toBeInTheDocument();
  });
  describe('VPNPromoBanner integration', () => {
    ...
    it('passes showVPNPromo as `true` to Page if the add-on is an extension', () => {
      renderWithAddon();
      expect(screen.getByRole('link', {
        name: 'Get Mozilla VPN',
      })).toBeInTheDocument();
    });
    it.each([ADDON_TYPE_DICT, ADDON_TYPE_LANG, ADDON_TYPE_STATIC_THEME])('passes showVPNPromo as `false` to Page if the add-on is a %s', (type) => {
      addon.type = type;
      renderWithAddon();
  ...
  const testServerRedirect = ({
    ...
    expect(dispatch).toHaveBeenCalledWith(sendServerRedirect({
      status: 301,
  ...
  it('dispatches a server redirect when slug is a numeric ID', () => {
    const dispatch = testServerRedirect({
      slugInURL: addon.id,
    });
  ...
  it('dispatches a server redirect when slug has trailing spaces', () => {
    expect(testServerRedirect({
      slugInURL: `${defaultSlug}  `,
    })).toBeTruthy();
  });
  ...
  it(`dispatches a server redirect when slug is the add-on's GUID using a different case`, () => {
    ...
    expect(testServerRedirect({
      slugInURL: addon.guid.toUpperCase(),
    })).toBeTruthy();
  });
  it('sanitizes a summary', () => {
    const summaryText = 'some summary text';
    addon.summary = createLocalizedString(`${summaryText}<script>alert(document.cookie);</script>`);
    renderWithAddon();
    ...
    expect(within(addonSummary).queryByTagName('script')).not.toBeInTheDocument();
  });
  ...
  it('sanitizes bad description HTML', () => {
    ...
    addon.summary = createLocalizedString(`${descriptionText}<script>alert(document.cookie);</script>`);
    renderWithAddon();
    // Verify that the summary text exists without the script tag.
    expect(screen.getByText(descriptionText)).toBeInTheDocument();
    // Verify that no script tags exist in the summary.
    const addonDescription = screen.getByClassName('AddonDescription');
    expect(within(addonDescription).queryByTagName('script')).not.toBeInTheDocument();
  });
  ...
  const testContentId = async ({
    ...
    await userEvent.click(within(card).getByRole('link', {
  ...
  it('passes the expected contentId to ShowMoreCard for description', async () => {
    expect(await testContentId({
      addonProp: 'id',
      addonPropValue: addon.id + 1,
      cardClassName: 'AddonDescription',
    })).toBeTruthy();
  });
  it.each([[ADDON_TYPE_EXTENSION, 'extension'], [ADDON_TYPE_STATIC_THEME, 'theme'], [ADDON_TYPE_DICT, 'dictionary'], [ADDON_TYPE_LANG, 'language pack'], ['generic-type', 'add-on']])('sets a title for the description of a(n) %s', (type, title) => {
    addon.type = type;
    renderWithAddon();
  ...
  it.each([[ADDON_TYPE_EXTENSION, 'extension'], [ADDON_TYPE_STATIC_THEME, 'theme'], [ADDON_TYPE_DICT, 'dictionary'], [ADDON_TYPE_LANG, 'language pack'], ['generic-type', 'add-on']])('does not display a description if a(n) %s has no description', (type) => {
    addon.type = type;
  ...
  it('allows some HTML tags in the description', () => {
    addon.description = createLocalizedString('<b>super</b> <i>cool</i> <blink>add-on</blink>');
    renderWithAddon();
    const addonDescription = screen.getByClassName('AddonDescription');
    ...
    expect(within(addonDescription).queryByTagName('blink')).not.toBeInTheDocument();
  ...
  it('strips dangerous HTML tag attributes from description', () => {
    ...
    addon.description = createLocalizedString(`<a href="javascript:alert(document.cookie)" onclick="sneaky()">${placeholder}</a>`);
    renderWithAddon();
  ...
  it('passes the expected contentId to ShowMoreCard for developer comments', async () => {
    addon.developer_comments = createLocalizedString('some awesome developers comments');
    expect(await testContentId({
      addonProp: 'id',
      addonPropValue: addon.id + 1,
  ...
  it('allows some HTML tags in the developer comments', () => {
    addon.developer_comments = createLocalizedString('<b>super</b> <i>cool</i> <blink>comments</blink>');
    renderWithAddon();
    ...
    expect(within(developerComments).queryByTagName('blink')).not.toBeInTheDocument();
    expect(screen.getByTextAcrossTags('super cool comments')).toBeInTheDocument();
  });
  it('configures the RatingManager', async () => {
    ...
    store.dispatch(setLatestReview({
      addonId: addon.id,
      review: null,
      userId: authorUserId,
    }));
    ...
    expect(dispatch).toHaveBeenCalledWith(createAddonReview({
      addonId: addon.id,
  ...
  it('does not show a RatingManager without a version', () => {
    addon.current_version = null;
    renderWithAddon();
    expect(screen.getByClassName('Addon-no-rating-manager')).toBeInTheDocument();
  });
  ...
  it('renders a summary with links', () => {
    ...
    addon.summary = createLocalizedString(`${summaryText} <a href="http://foo.com/">${linkText}</a>`);
    renderWithAddon();
  ...
  it('renders an amo icon image', () => {
    ...
    renderWithAddon();
    expect(screen.getByAltText(`Preview of ${addonName}`)).toHaveAttribute('src', addon.icon_url);
  });
  ...
  it('passes the addon to AddonCompatibilityError', () => {
    getClientCompatibility.mockReturnValue(createFakeClientCompatibility({
      compatible: false,
      reason: INCOMPATIBLE_UNSUPPORTED_PLATFORM,
    }));
    renderWithAddon();
    expect(screen.getByText('This add-on is not available on your platform.')).toBeInTheDocument();
  });
  it('renders an AddonMoreInfo component when there is an add-on', () => {
    ...
    renderWithAddon();
    expect(screen.getByText('Homepage')).toHaveAttribute('href', 'https://outgoing.mozilla.org/hamster');
  });
  ...
  describe('read reviews footer', () => {
    const renderWithRatings = (count) => {
      addon.ratings = { ...fakeAddon.ratings,
        count,
      };
      renderWithAddon();
    };
    ...
    it('only links to reviews when they exist', () => {
      ...
      expect(within(readReviewsFooter).queryByRole('link')).not.toBeInTheDocument();
    });
    ...
    it('prompts you to read many reviews', () => {
      renderWithRatings(5);
      expect(screen.getByRole('link', {
        name: 'Read all 5 reviews',
      })).toBeInTheDocument();
    });
    it('localizes the review count', () => {
      renderWithRatings(10000);
      expect(screen.getByRole('link', {
        name: 'Read all 10,000 reviews',
      })).toBeInTheDocument();
    });
    it('links to all reviews', () => {
      renderWithRatings(2);
      expect(screen.getByRole('link', {
        name: 'Read all 2 reviews',
      })).toHaveAttribute('href', `/${lang}/${clientApp}${reviewListURL({
  ...
  describe('version release notes', () => {
    const renderWithVersion = (props = {}) => {
      addon.current_version = { ...fakeVersion,
        ...props,
      };
      renderWithAddon();
    };
    ...
    it('is hidden when an add-on has not loaded yet', () => {
      render();
      expect(screen.queryByClassName('AddonDescription-version-notes')).not.toBeInTheDocument();
    });
    ...
    it('allows some HTML tags', () => {
      ...
      expect(screen.getByTextAcrossTags('lots of bug fixes')).toBeInTheDocument();
    ...
    it('allows some ul-li tags', () => {
      ...
      const notesCard = screen.getByClassName('AddonDescription-version-notes');
      expect(within(notesCard).getByTextAcrossTags('The Listonetwo')).toBeInTheDocument();
      expect(within(notesCard).getByRole('list')).toBeInTheDocument();
      expect(within(notesCard).getAllByRole('listitem')[0]).toHaveTextContent('one');
      expect(within(notesCard).getAllByRole('listitem')[1]).toHaveTextContent('two');
    });
  });
  it('renders the site identifier as a data attribute', () => {
    renderWithAddon();
    expect(screen.getByClassName('Addon')).toHaveAttribute('data-site-identifier', String(addon.id));
  });
  ...
  const nonPublicNotice = `This is not a public listing. You are only seeing it because of ` + `elevated permissions.`;
  ...
  it('passes an error to the AddonInstallError component', () => {
    ...
    store.dispatch(setInstallError({
      error: FATAL_ERROR,
      guid: addon.guid,
    }));
    renderWithAddon();
    expect(within(screen.getByClassName('AddonInstallError')).getByText('An unexpected error occurred.')).toBeInTheDocument();
  });
  describe('InstallWarning', () => {
    it('renders the InstallWarning if an add-on exists', () => {
      correctedLocationForPlatform.mockReturnValue('');
      renderWithAddon();
      expect(screen.getByText(`This add-on is not actively monitored for security by Mozilla. ` + `Make sure you trust it before installing.`)).toBeInTheDocument();
    });
    it('does not render the InstallWarning if an add-on does not exist', () => {
      correctedLocationForPlatform.mockReturnValue('');
      render();
      expect(screen.queryByText(`This add-on is not actively monitored for security by Mozilla. ` + `Make sure you trust it before installing.`)).not.toBeInTheDocument();
    });
  ...
  describe('Tests for ThemeImage', () => {
    ...
    it('does not render a theme image when there is no add-on', () => {
      render();
      expect(screen.queryByClassName('ThemeImage-image')).not.toBeInTheDocument();
    });
    ...
    it('renders a theme image with rounded corners', () => {
      ...
      addon.type = ADDON_TYPE_STATIC_THEME;
      renderWithAddon();
      expect(screen.getByRole('presentation')).toHaveClass('ThemeImage--rounded-corners');
    });
    it('displays a preview with 720 width', () => {
      ...
      renderWithAddon();
      expect(screen.getByAltText(`Preview of ${addonName}`)).toHaveAttribute('src', fullImage720);
    });
  });
  describe('Tests for AddonAdminLinks', () => {
    ...
    it('shows edit and admin add-on links if the user has permission', () => {
      ...
      expect(screen.getByRole('link', {
        name: 'Edit add-on',
      })).toHaveAttribute('href', `/developers/addon/${defaultSlug}/edit`);
      expect(screen.getByRole('link', {
        name: 'Admin add-on',
    ...
    it('shows a content review link if the user has permission', () => {
      renderWithPermissions({
        permissions: ADDONS_CONTENT_REVIEW,
      });
      expect(screen.getByRole('link', {
        name: 'Content review add-on',
  ...
  describe('Tests for AddonHead', () => {
    ...
    it('renders links via the HeadLinks component', async () => {
      renderWithAddon();
      await waitFor(() => expect(getElement('link[rel="canonical"]')).toHaveAttribute('href', getCanonicalURL({
        locationPathname: getLocation(),
      })));
    });
    it('renders meta tags via the HeadMetaTags component', async () => {
      ...
      addon.summary = createLocalizedString(summary);
      renderWithAddon();
      await waitFor(() => expect(getElement('meta[property="og:type"]')).toBeInTheDocument());
      expect(getElement('meta[name="date"]')).toHaveAttribute('content', String(addon.created));
      expect(getElement('meta[name="description"]')).toHaveAttribute('content', `Download ${addonName} for Firefox. ${summary}`);
      expect(getElement(`meta[property="og:image"]`)).toHaveAttribute('content', addon.previews[0].src);
      expect(getElement('meta[name="last-modified"]')).toHaveAttribute('content', String(addon.created));
      expect(getElement('meta[name="twitter:site"]')).toHaveAttribute('content', '@mozamo');
    });
    it.each([[ADDON_TYPE_DICT, 'Dictionary'], [ADDON_TYPE_EXTENSION, 'Extension'], [ADDON_TYPE_LANG, 'Language Pack'], [ADDON_TYPE_STATIC_THEME, 'Theme']])('renders an HTML title for Firefox (add-on type: %s)', async (type, name) => {
      addon.name = createLocalizedString(addonName);
      addon.type = type;
      renderWithAddon();
      await waitFor(() => expect(getElement('title')).toHaveTextContent(`${addonName} – Get this ${name} for 🦊 Firefox (${lang})`));
    });
    it.each([[ADDON_TYPE_DICT, 'Dictionary'], [ADDON_TYPE_EXTENSION, 'Extension'], [ADDON_TYPE_LANG, 'Language Pack'], [ADDON_TYPE_STATIC_THEME, 'Theme']])('renders an HTML title for Android (add-on type: %s)', async (type, name) => {
      addon.name = createLocalizedString(addonName);
      addon.type = type;
      ...
      await waitFor(() => expect(getElement('title')).toHaveTextContent(`${addonName} – Get this ${name} for 🦊 Firefox Android (${lang})`));
    });
    it('renders JSON linked data', async () => {
      _loadAddon();

      const internalAddon = getAddonByIdInURL(store.getState().addons, defaultSlug);
      const currentVersion = getVersionById({
        id: internalAddon.currentVersionId,
        state: store.getState().versions,
      });
      render();
      await waitFor(() => expect(getElement('script[type="application/ld+json"]')).toBeInTheDocument());
      expect(getElement('script[type="application/ld+json"]')).toHaveTextContent(serialize(getAddonJsonLinkedData({
        addon: internalAddon,
        currentVersion,
      }), {
        isJSON: true,
    ...
    it('escapes JSON linked data', async () => {
      ...
      renderWithAddon();
      await waitFor(() => expect(getElement('script[type="application/ld+json"]')).toBeInTheDocument());
      expect(getElement('script[type="application/ld+json"]')).toHaveTextContent(/\\u003Cscript\\u003E/);
    });
  });
  describe('Tests for ContributeCard', () => {
    ...
    it('displays content for an extension developer', () => {
      ...
      expect(screen.getByText(`The developer of this extension asks that you help support its ` + `continued development by making a small contribution.`)).toBeInTheDocument();
    });
    it('displays content for multiple extension developers', () => {
      ...
      expect(screen.getByText(`The developers of this extension ask that you help support its ` + `continued development by making a small contribution.`)).toBeInTheDocument();
    });
    it('displays content for a theme artist', () => {
      ...
      expect(screen.getByText(`The artist of this theme asks that you help support its continued ` + `creation by making a small contribution.`)).toBeInTheDocument();
    });
    it('displays content for multiple theme artists', () => {
      ...
      expect(screen.getByText(`The artists of this theme ask that you help support its continued ` + `creation by making a small contribution.`)).toBeInTheDocument();
    });
    it('displays content for a add-on author', () => {
      ...
      expect(screen.getByText(`The author of this add-on asks that you help support its ` + `continued work by making a small contribution.`)).toBeInTheDocument();
    });
    it('displays content for multiple add-on authors', () => {
      ...
      expect(screen.getByText(`The authors of this add-on ask that you help support its ` + `continued work by making a small contribution.`)).toBeInTheDocument();
    });
  ...
  describe('Tests for PermissionsCard', () => {
    ...
    describe('with permissions', () => {
      ...
      it('renders learn more link in header', () => {
        ...
        const learnMoreLink = within(getPermissionsCard()).getByText('Learn more');
        expect(learnMoreLink).toHaveAttribute('href', 'https://support.mozilla.org/kb/permission-request-messages-firefox-extensions');
        expect(within(learnMoreLink).getByClassName('Icon-external-dark')).toBeInTheDocument();
      });
      it('renders required permissions only', () => {
        ...
        renderWithAddon();
        expect(screen.getByText('This add-on needs to:')).toHaveClass('PermissionsCard-subhead--required');
        expect(within(getPermissionsCard()).getByTagName('ul')).toHaveClass('PermissionsCard-list--required');
        expect(screen.getByClassName('Icon-permission-bookmarks')).toBeInTheDocument();
        expect(screen.queryByClassName('PermissionsCard-subhead--optional')).not.toBeInTheDocument();
        expect(screen.queryByClassName('PermissionsCard-list--optional')).not.toBeInTheDocument();
      });
      it('renders optional permissions only', () => {
        ...
        renderWithAddon();
        expect(screen.getByText('This add-on may also ask to:')).toHaveClass('PermissionsCard-subhead--optional');
        expect(within(getPermissionsCard()).getByTagName('ul')).toHaveClass('PermissionsCard-list--optional');
        expect(screen.getByClassName('Icon-permission-bookmarks')).toBeInTheDocument();
        expect(screen.queryByClassName('PermissionsCard-subhead--required')).not.toBeInTheDocument();
        expect(screen.queryByClassName('PermissionsCard-list--required')).not.toBeInTheDocument();
      });
      it('renders both optional and required permissions', () => {
        ...
        expect(screen.getByText('This add-on may also ask to:')).toHaveClass('PermissionsCard-subhead--optional');
        expect(screen.getByClassName('Icon-permission-bookmarks')).toBeInTheDocument();
        expect(screen.getByClassName('Icon-permission-history')).toBeInTheDocument();
        expect(screen.getByClassName('PermissionsCard-list--required')).toBeInTheDocument();
        expect(screen.getByClassName('PermissionsCard-list--optional')).toBeInTheDocument();
      });
    });
  });
  describe('Tests for HostPermissions', () => {
    it('formats domain permissions', () => {
      addon.current_version = createVersionWithPermissions({
        required: ['*://*.mozilla.org/*', '*://*.mozilla.com/*', '*://*.mozilla.ca/*', '*://*.mozilla.us/*', '*://*.mozilla.co.nz/*', '*://*.mozilla.co.uk/*'],
      });
      renderWithAddon();
      expect(screen.getAllByClassName('Icon-permission-hostPermission')).toHaveLength(6);
      expect(screen.getByText('Access your data for sites in the mozilla.org domain')).toBeInTheDocument();
      expect(screen.getByText('Access your data for sites in the mozilla.com domain')).toBeInTheDocument();
      expect(screen.getByText('Access your data for sites in the mozilla.ca domain')).toBeInTheDocument();
      expect(screen.getByText('Access your data for sites in the mozilla.us domain')).toBeInTheDocument();
      expect(screen.getByText('Access your data for sites in the mozilla.co.nz domain')).toBeInTheDocument();
      expect(screen.getByText('Access your data for sites in the mozilla.co.uk domain')).toBeInTheDocument();
    });
    it('formats site permissions', () => {
      addon.current_version = createVersionWithPermissions({
        required: ['*://developer.mozilla.org/*', '*://addons.mozilla.org/*', '*://www.mozilla.org/*', '*://testing.mozilla.org/*', '*://awesome.mozilla.org/*'],
      });
      renderWithAddon();
      expect(screen.getAllByClassName('Icon-permission-hostPermission')).toHaveLength(5);
      expect(screen.getByText('Access your data for developer.mozilla.org')).toBeInTheDocument();
      expect(screen.getByText('Access your data for addons.mozilla.org')).toBeInTheDocument();
      expect(screen.getByText('Access your data for www.mozilla.org')).toBeInTheDocument();
      expect(screen.getByText('Access your data for testing.mozilla.org')).toBeInTheDocument();
      expect(screen.getByText('Access your data for awesome.mozilla.org')).toBeInTheDocument();
    });
    it('returns a single host permission for all urls', () => {
      ...
      for (const allUrlsPermission of ['<all_urls>', '*://*/']) {
        ...
        renderWithAddon();
        expect(screen.getAllByClassName('Icon-permission-hostPermission')).toHaveLength(1);
        expect(screen.getByText('Access your data for all websites')).toBeInTheDocument();
        cleanup();
      }
    });
    ...
    it('deduplicates domain and site permissions', () => {
      addon.current_version = createVersionWithPermissions({
        required: ['https://*.okta.com/', 'https://*.okta.com/login/login.htm*', 'https://*.okta.com/signin/verify/okta/push', 'https://*.okta.com/signin/verify/okta/sms', 'https://trishulgoel.com/about', 'https://trishulgoel.com/speaker', '*://*.mozilla.org/*', '*://*.mozilla.com/*', '*://*.mozilla.ca/*', '*://*.mozilla.us/*', '*://*.mozilla.co.nz/*', '*://*.mozilla.co.uk/*'],
      });
      renderWithAddon();
      expect(screen.getAllByClassName('Icon-permission-hostPermission')).toHaveLength(8);
      expect(screen.getByText('Access your data for sites in the okta.com domain')).toBeInTheDocument();
      expect(screen.getByText('Access your data for sites in the mozilla.org domain')).toBeInTheDocument();
      expect(screen.getByText('Access your data for sites in the mozilla.com domain')).toBeInTheDocument();
      expect(screen.getByText('Access your data for sites in the mozilla.ca domain')).toBeInTheDocument();
      expect(screen.getByText('Access your data for sites in the mozilla.us domain')).toBeInTheDocument();
      expect(screen.getByText('Access your data for sites in the mozilla.co.nz domain')).toBeInTheDocument();
      expect(screen.getByText('Access your data for sites in the mozilla.co.uk domain')).toBeInTheDocument();
      expect(screen.getByText('Access your data for trishulgoel.com')).toBeInTheDocument();
    });
  });
  describe('Tests for AddonRecommendations', () => {
    ...
    it('renders nothing without an addon', () => {
      render();
      expect(screen.queryByText('Other users with this extension also installed')).not.toBeInTheDocument();
    });
    it('renders nothing if there is an error', () => {
      createFailedErrorHandler({
        id: thisErrorHandlerId,
        store,
      });
    ...
    it('renders an AddonCard when recommendations are loaded', () => {
      ...
      renderWithAddon();
      const expectedLink = [`/${lang}/${clientApp}/addon/${slug}/?utm_source=${DEFAULT_UTM_SOURCE}`, 'utm_medium=referral', `utm_content=${outcome}`].join('&');
      ...
      expect(screen.getByText('Other users with this extension also installed')).toBeInTheDocument();
    ...
    it('renders an AddonCard when recommendations are loading', () => {
      ...
      renderWithAddon();
      expect(within(within(screen.getByClassName('AddonRecommendations')).getByClassName('Card-header')).getByRole('alert')).toBeInTheDocument();
      expect(within(screen.getByClassName('AddonRecommendations')).getAllByAltText('')).toHaveLength(4);
    });
    ...
    it.each([ADDON_TYPE_DICT, ADDON_TYPE_LANG, ADDON_TYPE_STATIC_THEME])('does not render recommendations if the add-on is a %s', (addonType) => {
      ...
      renderWithAddon();
      expect(screen.queryByText('Other popular extensions')).not.toBeInTheDocument();
    });
    it('should dispatch a fetch action if no recommendations exist', () => {
      const dispatch = jest.spyOn(store, 'dispatch');
      renderWithAddon();
      expect(dispatch).toHaveBeenCalledWith(fetchRecommendations({
        errorHandlerId: thisErrorHandlerId,
        guid: addon.guid,
      }));
    });
  ...
  describe('more add-ons by authors', () => {
    it('puts "add-ons by author" in main content if type is theme', () => {
      ...
      expect(screen.getByClassName('AddonsByAuthorsCard')).toHaveClass('Addon-MoreAddonsCard');
      expect(screen.getAllByClassName('AddonsByAuthorsCard')).toHaveLength(1);
      expect(within(screen.getByClassName('Addon-main-content')).getByText(`More themes by ${authorName}`)).toBeInTheDocument();
    });
    it('puts "add-ons by author" outside main if type is not theme', () => {
      ...
      expect(screen.getAllByClassName('AddonsByAuthorsCard')).toHaveLength(1);
      expect( // eslint-disable-next-line testing-library/prefer-presence-queries
      within(screen.getByClassName('Addon-main-content')).queryByText(`More themes by ${authorName}`)).not.toBeInTheDocument();
      expect(screen.getByText(`More extensions by ${authorName}`)).toBeInTheDocument();
    });
    it('is hidden when an add-on has not loaded yet', () => {
      render();
      expect(screen.queryByClassName('AddonsByAuthorsCard')).not.toBeInTheDocument();
    });
    ...
    it('displays more add-ons by authors for an extension', async () => {
      ...
      expect(await within(addonsByAuthorsCard).findByText(`More extensions by ${authorName}`)).toBeInTheDocument();
      expect(within(addonsByAuthorsCard).getByRole('link', {
      ...
      expect(within(addonsByAuthorsCard).queryByClassName('SearchResult-summary')).not.toBeInTheDocument();
    ...
    it('displays more add-ons by authors for a theme', async () => {
      ...
      const addonsByAuthorsCard = screen.getByClassName('AddonsByAuthorsCard');
      expect(await within(addonsByAuthorsCard).findByText(`More themes by ${authorName}`)).toBeInTheDocument();
      expect(within(addonsByAuthorsCard).getByRole('link', {
        name: `${moreAddonName}-0`,
      })).toBeInTheDocument();
      expect(within(addonsByAuthorsCard).getByRole('link', {
    ...
    it('adds a CSS class to the main component when there are add-ons', async () => {
      ...
      await waitFor(() => expect(addonComponent).toHaveClass('Addon--has-more-than-0-addons'));
  ...
  describe('Tests for AddonsByAuthorsCard', () => {
    ...
    it('should render nothing if there are no add-ons', async () => {
      ...
      await waitFor(() => expect(screen.queryByClassName('AddonsByAuthorsCard')).not.toBeInTheDocument());
    });
    it('should render a loading state on first instantiation', () => {
      renderWithAddon();
      // Expect 6 placeholders with 4 LoadingText each.
      expect(within(screen.getByClassName('AddonsByAuthorsCard')).getAllByRole('alert')).toHaveLength(24);
    });
    ...
    it('should always fetch addons by authors', () => {
      const dispatch = jest.spyOn(store, 'dispatch');
      renderWithAddon();
      expect(dispatch).toHaveBeenCalledWith(fetchAddonsByAuthors({
        addonType: ADDON_TYPE_EXTENSION,
        authorIds: [authorUserId],
        errorHandlerId: getThisErrorHandlerId(ADDON_TYPE_EXTENSION),
        forAddonSlug: defaultSlug,
    ...
    it.each([ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME])('should display at most numberOfAddons for %s', (type) => {
      addon.type = type;
      renderWithAddon();
      loadAddonsByAuthors({
        addonType: ADDON_TYPE_EXTENSION,
      ...
      expect(within(screen.getByClassName('AddonsByAuthorsCard')).getAllByRole('listitem')).toHaveLength(ADDONS_BY_AUTHORS_COUNT);
    });
    it('should add a theme class if it is a static theme', () => {
      ...
      expect(screen.getByClassName('AddonsByAuthorsCard')).toHaveClass('AddonsByAuthorsCard--theme');
    });
    it.each([[ADDON_TYPE_DICT, `More dictionaries by ${authorName}`], [ADDON_TYPE_EXTENSION, `More extensions by ${authorName}`], [ADDON_TYPE_LANG, `More language packs by ${authorName}`], [ADDON_TYPE_STATIC_THEME, `More themes by ${authorName}`], ['unknown-type', `More add-ons by ${authorName}`]])('shows expected header for %s', (type, header) => {
      addon.type = type;
      renderWithAddon();
      loadAddonsByAuthors({
        addonType: type,
    ...
    it.each([[ADDON_TYPE_DICT, 'More dictionaries by these translators'], [ADDON_TYPE_EXTENSION, 'More extensions by these developers'], [ADDON_TYPE_LANG, 'More language packs by these translators'], [ADDON_TYPE_STATIC_THEME, 'More themes by these artists'], ['unknown-type', 'More add-ons by these developers']])('shows expected header for %s with multiple authors', (type, header) => {
      addon.type = type;
  ...
  describe('Tests for PromotedBadge', () => {
    ...
    it('can be rendered as large', () => {
      ...
      expect(screen.getByClassName('PromotedBadge')).toHaveClass('PromotedBadge-large');
      expect(screen.getByClassName('IconPromotedBadge')).toHaveClass('IconPromotedBadge-large');
    });
    it.each([['line', 'Official add-on built by Mozilla Firefox. Meets security and performance standards.', 'By Firefox'], ['recommended', 'Firefox only recommends add-ons that meet our standards for security and performance.', 'Recommended'], ['verified', 'This add-on has been reviewed to meet our standards for security and performance.', 'Verified']])('renders the category "%s" as expected', (category, linkTitle, label) => {
      renderWithPromotedCategory(category);
      expect(screen.getByClassName('PromotedBadge')).toHaveClass(`PromotedBadge--${category}`);
      const link = screen.getByTitle(linkTitle);
      expect(link).toHaveAttribute('href', getPromotedBadgesLinkUrl({
        utm_content: 'promoted-addon-badge',
      }));
      expect(link).toHaveClass(`PromotedBadge-link--${category}`);
      expect(screen.getByText(label)).toHaveClass(`PromotedBadge-label--${category}`);

      if (category !== 'line') {
        // eslint-disable-next-line jest/no-conditional-expect
        expect(screen.getByClassName('IconPromotedBadge-iconPath')).toHaveClass(`IconPromotedBadge-iconPath--${category}`);
      }
    });
    ...
    it('does not pass an alt property to IconPromotedBadge', () => {
      renderWithPromotedCategory();
      expect( // eslint-disable-next-line testing-library/prefer-presence-queries
      within(screen.getByClassName('PromotedBadge')).queryByClassName('visually-hidden')).not.toBeInTheDocument();
    });
  });
  describe('Tests for AddonBadges', () => {
    ...
    it('displays no badges when none are called for', () => {
      renderWithAddon();
      expect( // eslint-disable-next-line testing-library/prefer-presence-queries
      within(screen.getByClassName('AddonBadges')).queryByTagName('div')).not.toBeInTheDocument();
    });
    it('displays a badge when the addon is experimental', () => {
      addon.is_experimental = true;
      renderWithAddon();
      expect(screen.getByClassName('Badge-experimental')).toHaveTextContent('Experimental');
    });
    it('displays a badge when the addon requires payment', () => {
      addon.requires_payment = true;
      renderWithAddon();
      expect(screen.getByClassName('Badge-requires-payment')).toHaveTextContent('Some features may require payment');
    });
  });
