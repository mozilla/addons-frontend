...
import { CLIENT_APP_FIREFOX, SEARCH_SORT_RECOMMENDED, SEARCH_SORT_TRENDING, SEARCH_SORT_POPULAR } from 'amo/constants';
import { searchStart } from 'amo/reducers/search';
import { convertFiltersToQueryParams } from 'amo/searchUtils';
import { dispatchClientMetadata, dispatchSearchResults, fakeAddon, getElement, getSearchErrorHandlerId, renderPage as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('configures pagination using filters and the tag', async () => {
    ...
    expect(screen.getByRole('link', {
      name: 'Previous',
    })).toHaveAttribute('href', `/${lang}/${clientApp}/tag/${tag}/?page=1&sort=${SEARCH_SORT_POPULAR}`);
  });
  it('sets the expected title for the tag', async () => {
    render();
    await waitFor(() => expect(getElement('title')).toHaveTextContent(`Add-ons tagged with ${defaultTag} – Add-ons for Firefox (en-US)`));
  });
});