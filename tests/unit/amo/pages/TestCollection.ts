...
import { extractId as collectionAddAddonExtractId, MESSAGE_RESET_TIME } from 'amo/components/CollectionAddAddon';
...
import { ADDON_TYPE_STATIC_THEME, CLIENT_APP_FIREFOX, COLLECTION_SORT_DATE_ADDED_DESCENDING, COLLECTION_SORT_NAME, FEATURED_THEMES_COLLECTION_EDIT, FEATURED_THEMES_COLLECTION_SLUG, MOZILLA_COLLECTIONS_EDIT } from 'amo/constants';
import { FETCH_CURRENT_COLLECTION, FETCH_CURRENT_COLLECTION_PAGE, addAddonToCollection, addonAddedToCollection, addonRemovedFromCollection, beginCollectionModification, collectionName, createCollection, deleteCollection, deleteCollectionAddonNotes, fetchCurrentCollection, fetchCurrentCollectionPage, finishEditingCollectionDetails, loadCurrentCollection, removeAddonFromCollection, updateCollection, updateCollectionAddon } from 'amo/reducers/collections';
import { SEND_SERVER_REDIRECT, sendServerRedirect } from 'amo/reducers/redirectTo';
import { DEFAULT_ADDON_PLACEHOLDER_COUNT, extractId } from 'amo/pages/Collection';
import { changeLocation, createFakeAutocompleteResult, createFakeCollectionAddon, createFailedErrorHandler, createFakeCollectionDetail, createInternalCollectionWithLang, createLocalizedString, createFakeCollectionAddonsListResponse, dispatchAutocompleteResults, dispatchClientMetadata, dispatchSignInActionsWithStore, fakeAddon, fakeI18n, fakePreview, getElement, renderPage as defaultRender, screen, within } from 'tests/unit/helpers';

jest.mock('amo/localState', () => jest.fn(() => {
  return {
...
describe(__filename, () => {
  ...
  const editableCollectionAddonErrorHandlerId = 'src/amo/components/EditableCollectionAddon/index.js-editable-collection-addon-1234';
  const lang = 'en-US';
  ...
  } = {}) => `/${lang}/${clientApp}/collections/${userId}/${slug}/${editing ? 'edit/' : ''}`;
  ...
  const render = ({
    ...
    const initialEntry = location || getLocation({
      editing,
      slug,
      userId,
    });
  ...
  const renderWithCollection = ({
    ...
    _loadCurrentCollection({
      addonsResponse,
      detail: { ..._createFakeCollectionDetail(detailProps),
        count: addons.length,
      },
    });
  ...
  const renderInAddMode = ({
    ...
    return render({
      location: `/${lang}/${clientApp}/collections/add/${withAddonId ? `?include_addon_id=${withAddonId}` : ''}`,
    });
  };
  ...
  const clickEditButton = async () => userEvent.click(screen.getByRole('link', {
    name: 'Edit this collection',
  }));
  ...
  */
  it('renders placeholder text if there are no add-ons', () => {
    ...
    expect(screen.getByText('Search for extensions and themes to add to your collection.')).toBeInTheDocument();
  });
  it('renders placeholder text when creating a collection', () => {
    renderInAddMode();
    expect(screen.getByText('First, create your collection. Then you can add extensions and themes.')).toBeInTheDocument();
  });
  it('hides placeholder text when creating a collection if not logged in', () => {
    renderInAddMode({
      loggedIn: false,
    });
    expect(screen.queryByText('First, create your collection. Then you can add extensions and themes.')).not.toBeInTheDocument();
  });
  it('hides placeholder text if there are add-ons', () => {
    ...
    expect(screen.queryByText('Search for extensions and themes to add to your collection.')).not.toBeInTheDocument();
  });
  ...
  it('dispatches fetchCurrentCollection on mount', () => {
    ...
    render();
    expect(dispatch).toHaveBeenCalledWith(fetchCurrentCollection({
  ...
  it('dispatches fetchCurrentCollectionPage when page has changed', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(fetchCurrentCollectionPage({
      errorHandlerId: getCollectionPageErrorHandlerId({
        page,
      }),
      filters: { ...defaultFilters,
  ...
  it('renders collection add-ons', () => {
    ...
    expect(screen.getByRole('link', {
      name: addonName,
    })).toHaveAttribute('href', `/${lang}/${clientApp}/addon/${fakeAddon.slug}/?utm_source=addons.mozilla.org&utm_medium=referral&utm_content=collection`);
    expect(screen.getByAltText(addonName)).toHaveAttribute('src', fakePreview.image_url);
  });
  it('sets placeholder counts for the AddonsCard as expected', async () => {
    render();
    // Each SearchResult will have 4 instances of LoadingText.
    expect(within(screen.getByClassName('AddonsCard')).getAllByRole('alert')).toHaveLength(DEFAULT_ADDON_PLACEHOLDER_COUNT * 4);
    ...
    expect(within(screen.getByClassName('AddonsCard')).getAllByRole('alert')).toHaveLength(4);
  });
  it('renders a collection with pagination', () => {
    ...
    expect(screen.getByRole('link', {
      name: 'Previous',
    })).toHaveAttribute('href', `${defaultLocation}?page=1&collection_sort=${sort}`);
    expect(screen.getByText('Next')).toBeInTheDocument();
  });
  it('renders a create collection page', () => {
    renderInAddMode();
    const expectedUrlPrefix = `${config.get('apiHost')}/${lang}/${clientApp}/collections/${defaultUserId}/`;
    ...
    expect(screen.getByTitle(expectedUrlPrefix)).toHaveTextContent(expectedUrlPrefix);
    expect(screen.getByRole('button', {
      name: 'Create collection',
    })).toHaveProperty('disabled', true);
    // Sort controls should be absent.
    expect(screen.queryByRole('combobox', {
      name: 'Sort add-ons by',
  ...
  it('renders loading indicator on add-ons when fetching next page', async () => {
    ...
    expect(within(screen.getByClassName('AddonsCard')).getAllByRole('alert')).toHaveLength(numberOfAddons * 4);
  ...
  it('renders 404 page for missing collection', () => {
    ...
    render();
    expect(screen.getByText('Oops! We can’t find that page')).toBeInTheDocument();
  });
  ...
  it('renders an HTML title', async () => {
    renderWithCollection();
    await waitFor(() => expect(getElement('title')).toHaveTextContent(`${defaultCollectionName} – Add-ons for Firefox (en-US)`));
  });
  it('renders an HTML title for a collection with a missing name', async () => {
    ...
    await waitFor(() => expect(getElement('title')).toHaveTextContent(`${collectionName({
      name: null,
      i18n: fakeI18n(),
  ...
  it('renders the default HTML title when there is no collection loaded', async () => {
    render();
    await waitFor(() => expect(getElement('title')).toHaveTextContent('Add-ons for Firefox (en-US)'));
  });
  ...
  it('renders a CollectionAddAddon component when editing', () => {
    ...
    expect(screen.getByPlaceholderText('Find an add-on to include in this collection')).toBeInTheDocument();
  });
  it('does not render a CollectionAddAddon component when not editing', () => {
    renderWithCollectionForSignedInUser();
    expect(screen.queryByPlaceholderText('Find an add-on to include in this collection')).not.toBeInTheDocument();
  });
  it('renders AuthenticateButton when creating and not signed in', () => {
    ...
    expect(screen.getByRole('link', {
      name: 'Log in to create a collection',
    })).toBeInTheDocument();
  ...
  it('renders AuthenticateButton when editing and not signed in', () => {
    ...
    expect(screen.getByRole('link', {
      name: 'Log in to edit this collection',
    })).toBeInTheDocument();
    // Make sure the form was not rendered.
    expect(screen.queryByRole('textbox', {
      name: 'Collection name',
  ...
  it('does not update the page when removeAddon is called and there are still addons to show on the current page', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(removeAddonFromCollection({
      addonId,
      errorHandlerId: getCollectionPageErrorHandlerId({
        page,
      }),
  ...
  it('dispatches deleteCollection when the Delete collection button is clicked and confirmed', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(deleteCollection({
      errorHandlerId: getCollectionPageErrorHandlerId(),
      slug: defaultSlug,
      userId: defaultUserId,
    }));
  ...
  it('dispatches deleteCollectionAddonNotes when clicking delete on DismissibleTextForm', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(deleteCollectionAddonNotes({
  ...
  it('dispatches updateCollectionAddon when saving DismissibleTextForm', async () => {
    ...
    const input = screen.getByPlaceholderText('Add a comment about this add-on.');
    await userEvent.clear(input);
    await userEvent.type(input, newNotes);
    await userEvent.click(screen.getByRole('button', {
      name: 'Save',
    }));
    expect(dispatch).toHaveBeenCalledWith(updateCollectionAddon({
      addonId: fakeAddon.id,
      errorHandlerId: editableCollectionAddonErrorHandlerId,
      filters: defaultFilters,
  ...
  it('sends a server redirect when userId parameter is not a numeric ID', async () => {
    ...
    const dispatch = jest.spyOn(store, 'dispatch').mockImplementation(() => null);
    render({
  ...
  it('renders a "description" meta tag', async () => {
    ...
    await waitFor(() => expect(getElement('meta[name="description"]')).toHaveAttribute('content', ['Download and create Firefox collections to keep track of favorite extensions and themes.', `Explore the ${name}—${description}.`].join(' ')));
  });
  it('renders a "description" meta tag without a collection description', async () => {
    ...
    await waitFor(() => expect(getElement('meta[name="description"]')).toHaveAttribute('content', ['Download and create Firefox collections to keep track of favorite extensions and themes.', `Explore the ${name}.`].join(' ')));
  });
  it('renders a "description" meta tag for a collection with a missing name', async () => {
    ...
    await waitFor(() => expect(getElement('meta[name="description"]')).toHaveAttribute('content', ['Download and create Firefox collections to keep track of favorite extensions and themes.', `Explore the ${collectionName({
      name,
      i18n: fakeI18n(),
    })}.`].join(' ')));
  });
  ...
  describe('Tests for CollectionAddAddon', () => {
    const getErrorHandlerId = (id = '') => `src/amo/components/CollectionAddAddon/index.js-collection${id}`;

    const selectAnAddon = async ({
      addonName,
      id,
    }) => {
      await userEvent.type(screen.getByPlaceholderText('Find an add-on to include in this collection'), 'test');
    ...
    it('dispatches addAddonToCollection when selecting an add-on', async () => {
      ...
      expect(dispatch).toHaveBeenCalledWith(addAddonToCollection({
        addonId: id,
        collectionId: defaultCollectionId,
        slug: defaultSlug,
        editing: true,
    ...
    it('displays a notification for 5 seconds after an add-on has been added', async () => {
      ...
      expect(screen.getByClassName('Notice-success')).toHaveClass('CollectionAddAddon-noticePlaceholder-transition-enter');
      ...
      await waitFor(() => expect(screen.getByClassName('Notice-success')).toHaveClass('CollectionAddAddon-noticePlaceholder-transition-enter-done'));
    });
    it('displays a notification for 5 seconds after an add-on has been removed', async () => {
      ...
      expect(screen.queryByText('Removed from collection')).not.toBeInTheDocument();

      _addonRemovedFromCollection();

      expect(await screen.findByText('Removed from collection')).toBeInTheDocument();
      expect(screen.getByClassName('Notice-generic')).toHaveClass('CollectionAddAddon-noticePlaceholder-transition-enter');
      // Trigger the setTimeout behavior.
      jest.advanceTimersByTime(MESSAGE_RESET_TIME);
      // The Notice element will still be present, but the fade transition,
      // controlled by CSSTransition should have been started.
      await waitFor(() => expect(screen.getByClassName('Notice-generic')).toHaveClass('CollectionAddAddon-noticePlaceholder-transition-enter-done'));
    });
    ...
    it('removes the notification after a new add-on has been selected', async () => {
      ...
      expect(screen.getByClassName('Notice-success')).toHaveClass('CollectionAddAddon-noticePlaceholder-transition-exit');
    });
    describe('extractId', () => {
      it('generates an ID without a collection', () => {
        expect(collectionAddAddonExtractId({
          collection: null,
        })).toEqual('collection');
      });
  ...
  describe('Tests for CollectionSort', () => {
    it.each([true, false])(`calls history.push with expected pathname and query when a sort is selected and editing is %s`, async (editing) => {
  ...
  describe('Tests for EditableCollectionAddon', () => {
    it('renders a class name with its type', () => {
      ...
      expect(screen.getByClassName('EditableCollectionAddon')).toHaveClass(`EditableCollectionAddon--${type}`);
    });
    it("renders the add-on's icon", () => {
      ...
      expect(screen.getByAltText(addonName)).toHaveAttribute('src', fakeAddon.icon_url);
    });
    ...
    it('hides the leave a note button when notes exist', () => {
      renderWithNotes();
      expect(screen.getByClassName('EditableCollectionAddon-leaveNote--hidden')).toBeInTheDocument();
    });
    ...
    describe('notes area', () => {
      ...
      it('shows the read-only version of the notes area if there are notes', () => {
        ...
        expect(screen.queryByPlaceholderText('Add a comment about this add-on.')).not.toBeInTheDocument();
      });
    });
    it('renders newlines in notes', () => {
      const notes = 'Some\nnotes.';
      renderWithNotes(notes);
      const notesContent = screen.getByClassName('EditableCollectionAddon-notes-content');
    ...
    it('shows an empty notes form when the leave a note button is clicked', async () => {
      ...
      expect(screen.getByClassName('Icon-comments-blue')).toBeInTheDocument();
      expect(screen.getByPlaceholderText('Add a comment about this add-on.')).toHaveValue('');
    ...
    it('does not show <a> tag in DismissibleTextForm when editing notes', async () => {
      ...
      expect(screen.getByPlaceholderText('Add a comment about this add-on.')).toHaveValue(linkText);
      // The read-only portion should not be shown.
      expect(screen.queryByRole('heading', {
        name: `Collector's note`,
      })).not.toBeInTheDocument();
    ...
    it('hides the notes form when the cancel button is clicked', async () => {
      ...
      expect(screen.getByPlaceholderText('Add a comment about this add-on.')).toBeInTheDocument();
      await userEvent.click(screen.getByRole('button', {
        name: 'Cancel',
      }));
      expect(screen.queryByPlaceholderText('Add a comment about this add-on.')).not.toBeInTheDocument();
    ...
    describe('errorHandler - extractId', () => {
      it('returns a unique ID with an add-on', () => {
        expect(editableCollectionAddonExtractId({
          addon: fakeAddon,
        })).toEqual(`editable-collection-addon-${fakeAddon.id}`);
      });
    });
  });
  describe('Tests for CollectionManager', () => {
    const getErrorHandlerId = (slug = '') => `src/amo/components/CollectionManager/index.js-collection-${slug}`;
    ...
    const fillInDetailsScreen = async ({
      description,
      name,
      slug,
    }) => {
    ...
    it('creates a collection on submit', async () => {
      ...
      expect(dispatch).toHaveBeenCalledWith(createCollection({
        defaultLocale: lang,
        description: {
          [lang]: description,
        },
    ...
    it('updates the collection on submit', async () => {
      ...
      expect(dispatch).toHaveBeenCalledWith(updateCollection({
    ...
    it('disables submit button when the name is blank', async () => {
      ...
      expect(expectButtonDisabledStatus({
    ...
    it('trims leading and trailing spaces from slug and name before submitting', async () => {
      ...
      expect(dispatch).toHaveBeenCalledWith(updateCollection({
        ...
        defaultLocale: lang,
        description: {
          [lang]: defaultCollectionDescription,
        },
    ...
    it('autofills slug when name is entered while creating collection', async () => {
      ...
      expect(screen.getByRole('textbox', {
        name: 'Custom URL',
      })).toHaveValue('trishul-s-collection');
    });
    ...
    it('calls history.push() when creating', async () => {
      ...
      expect(pushSpy).toHaveBeenCalledWith(`/${lang}/${clientApp}/collections/`);
    });
    it('populates form state when updating to a new collection', async () => {
      ...
      expect(screen.getByRole('textbox', {
        name: 'Description',
      })).toHaveValue(defaultCollectionDescription);
    });
    it('populates form state when switching collections', async () => {
      ...
      expect(screen.getByRole('textbox', {
        name: 'Description',
      })).toHaveValue(secondDescription);
    });
    describe('extractId', () => {
      it('generates an ID without a collection', () => {
        expect(collectionManagerExtractId({
          collection: null,
        })).toEqual('collection-');
      });
      it('generates an ID with a collection', () => {
  ...
  describe('Tests for CollectionDetails', () => {
    it('renders collection details', () => {
      ...
      const addons = [createFakeCollectionAddon({
        addon: { ...fakeAddon,
          id: 1,
        },
    ...
    it('renders loading indicators when there is no collection', () => {
      render();
      expect(within(screen.getByClassName('CollectionDetails')).getAllByRole('alert')).toHaveLength(5);
    });
    ...
    it('switches into collection edit mode when the edit button is clicked', async () => {
      ...
      expect(screen.queryByRole('link', {
        name: 'Back to collection',
      })).toHaveAttribute('href', `${defaultLocation}?page=1&collection_sort=-added`);
    });
  ...
  describe('Tests for CollectionDetailsCard', () => {
    it('renders an edit button if the current user is the author', () => {
      ...
      expect(screen.getByRole('link', {
        name: 'Edit this collection',
      })).toHaveAttribute('href', `${defaultLocation}edit/?page=1&collection_sort=-added`);
    });
