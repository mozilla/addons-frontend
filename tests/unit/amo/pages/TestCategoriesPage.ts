...
import { FETCH_CATEGORIES, fetchCategories, loadCategories } from 'amo/reducers/categories';
import { ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME, CLIENT_APP_FIREFOX } from 'amo/constants';
import { getCanonicalURL, visibleAddonType } from 'amo/utils';
import { getCategoryResultsPathname } from 'amo/utils/categories';
import { changeLocation, createFailedErrorHandler, dispatchClientMetadata, fakeCategory, getElement, renderPage as defaultRender, screen, within } from 'tests/unit/helpers';

describe(__filename, () => {
  const clientApp = CLIENT_APP_FIREFOX;
  const lang = 'en-US';

  const getLocation = (addonType = ADDON_TYPE_EXTENSION) => `/${lang}/${clientApp}/${visibleAddonType(addonType)}/categories/`;

  const getErrorHandlerId = (addonType = ADDON_TYPE_EXTENSION) => `Categories-${addonType}`;
  ...
  it.each([[ADDON_TYPE_EXTENSION, 'extension'], [ADDON_TYPE_STATIC_THEME, 'theme']])('renders an HTML title for %s', async (addonType, expectedMatch) => {
    render({
      addonType,
    });
    await waitFor(() => expect(getElement('title')).toHaveTextContent(`All ${expectedMatch} categories – Add-ons for Firefox (en-US)`));
  });
  ...
  it.each([[ADDON_TYPE_EXTENSION, 'extension'], [ADDON_TYPE_STATIC_THEME, 'theme']])('renders a HeadMetaTags component for %s', async (addonType, expectedMatch) => {
    render({
      addonType,
    });
    await waitFor(() => expect(getElement(`meta[property="og:title"]`)).toHaveAttribute('content', `All ${expectedMatch} categories – Add-ons for Firefox (en-US)`));
  });
  describe('Tests for Categories', () => {
    ...
    it('does not dispatch setViewContext if addonType does not change', async () => {
      ...
      expect(dispatch).not.toHaveBeenCalledWith(setViewContext(ADDON_TYPE_EXTENSION));
    });
    ...
    it('generates an expected link for a category', () => {
      ...
      expect(screen.getByRole('link', {
        name: 'Games',
      })).toHaveAttribute('href', `/${lang}/${clientApp}${getCategoryResultsPathname({
        addonType: ADDON_TYPE_EXTENSION,
        slug: 'Games',
      })}`);
      expect(screen.getByRole('link', {
    ...
    it('sorts and renders the sorted categories', () => {
      ...
      store.dispatch(loadCategories(categoriesResponse));
      render();
      const categoryLinks = within(screen.getByClassName('Categories-list')).getAllByRole('link');
    ...
    it('renders a class name with a color for each category', () => {
      const categoriesResponse = {
        ...
        results: Array(13).fill().map((_, index) => ({ ...fakeCategory,
          application: CLIENT_APP_FIREFOX,
          id: index,
          name: `category ${index}`,
          slug: `category-${index}`,
