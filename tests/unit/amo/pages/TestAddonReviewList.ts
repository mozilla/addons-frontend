...
import { fetchReview, fetchReviews, fetchReviewPermissions, setAddonReviews, setReview, setReviewPermissions } from 'amo/actions/reviews';
...
import { ADDON_TYPE_STATIC_THEME, CLIENT_APP_FIREFOX, SET_VIEW_CONTEXT } from 'amo/constants';
import { fetchAddon, loadAddon } from 'amo/reducers/addons';
import { changeLocation, createFailedErrorHandler, createFakeErrorHandler, createLocalizedString, dispatchClientMetadata, dispatchSignInActionsWithStore, fakeAddon, fakeReview, getElement, getElements, renderPage as defaultRender, screen, within } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const getLocation = ({
    ...
    return `/${lang}/${clientApp}/addon/${slug}/reviews/${reviewId ? `${reviewId}/` : ''}${queryString}`;
  };
  ...
  const signInAndSetReviewPermissions = ({
    ...
    store.dispatch(setReviewPermissions({
      addonId,
      userId,
      ...permissions,
    }));
  ...
  describe('<AddonReviewList/>', () => {
    it('shows placeholders for reviews without an addon', () => {
      ...
      cards.forEach((card) => expect(within(card).getAllByRole('alert')).toHaveLength(2));
    });
    it('displays 4 placeholders while loading for an addon with no reviews', () => {
      addon.ratings = { ...addon.ratings,
        count: 0,
      };
      renderWithAddon();
      const cards = screen.getAllByClassName('AddonReviewCard');
    ...
    it('displays 25 placeholders for more than 25 reviews for an addon', () => {
      ...
      renderWithAddon();
      expect(screen.getAllByClassName('AddonReviewCard')).toHaveLength(DEFAULT_API_PAGE_SIZE);
    });
    it('renders an AddonSummaryCard with an addon', () => {
      const name = 'My addon';
      addon.name = createLocalizedString(name);
      renderWithAddon();
      expect(screen.getByAltText('Add-on icon')).toHaveAttribute('src', addon.icon_url);
    ...
    it('does not fetch an addon if it is already loading', () => {
      ...
      const dispatch = jest.spyOn(store, 'dispatch');
      render();
      expect(dispatch).not.toHaveBeenCalledWith(fetchAddon({
        slug: defaultSlug,
        showGroupedRatings: true,
        errorHandler,
      }));
    ...
    it('fetches reviews if needed', () => {
      const dispatch = jest.spyOn(store, 'dispatch');
      renderWithAddon();
      expect(dispatch).toHaveBeenCalledWith(_fetchReviews({
        addonSlug: defaultSlug,
        errorHandlerId: getErrorHandlerId(),
      }));
    });
    it('does not fetch reviews if they are already loading', () => {
      store.dispatch(_fetchReviews({
        addonSlug: defaultSlug,
        errorHandlerId: getErrorHandlerId(),
      }));
      const dispatch = jest.spyOn(store, 'dispatch');
      renderWithAddon();
      expect(dispatch).not.toHaveBeenCalledWith(_fetchReviews({
        addonSlug: defaultSlug,
        errorHandlerId: getErrorHandlerId(),
      }));
    });
    ...
    it('dispatches a view context for new add-on types', async () => {
      ...
      expect(dispatch).toHaveBeenCalledWith(setViewContext(ADDON_TYPE_STATIC_THEME));
    });
    it('renders an error', () => {
    ...
    it('renders content when no reviews exist', () => {
      ...
      expect(screen.getByClassName('AddonReviewList-reviewCount')).toHaveTextContent('');
    });
    it('does not include a review in the listing if the review is also featured', () => {
      ...
      // Featured review.
      expect(within(screen.getByClassName('FeaturedAddonReview')).getAllByTitle('Rated 1 out of 5')).toHaveLength(6);
      // Review listing.
      expect(within(screen.getByClassName('AddonReviewList-reviews-listing')).getAllByTitle('Rated 2 out of 5')).toHaveLength(6);
      expect(within( // eslint-disable-next-line testing-library/prefer-presence-queries
      screen.getByClassName('AddonReviewList-reviews-listing')).queryByTitle('Rated 1 out of 5')).not.toBeInTheDocument();
    });
    it('does not display a listing if the only review is also featured', () => {
      ...
      expect(screen.queryByClassName('AddonReviewList-reviews-listing')).not.toBeInTheDocument();
    });
    it('renders a class name with its type', () => {
      addon.type = ADDON_TYPE_STATIC_THEME;
      renderWithAddon();
      expect(screen.getByClassName('AddonReviewList')).toHaveClass(`AddonReviewList--${ADDON_TYPE_STATIC_THEME}`);
    });
    ...
    it('configures CardList header with LoadingText', () => {
      ...
      // 1 in the header.
      expect(within(screen.getByClassName('AddonReviewList-reviews')).getAllByRole('alert')).toHaveLength(9);
    });
    describe('with pagination', () => {
      const renderWithPagination = ({
        ...
        return renderWithAddonAndReviews({
          page,
          reviews,
          ...otherProps,
        });
      ...
      it('configures a paginator with the right URL', () => {
        ...
        expect(screen.getByRole('link', {
          name: 'Next',
        })).toHaveAttribute('href', `/en-US/firefox/addon/${defaultSlug}/reviews/?page=2`);
      });
      it('adds UTM query parameters to the reviews URL when there are some', () => {
        ...
        expect(screen.getByRole('link', {
          name: 'Next',
        })).toHaveAttribute('href', `/en-US/firefox/addon/${defaultSlug}/reviews/?utm_campaign=${utmCampaign}&page=2`);
      });
    ...
    it('renders an HTML title', async () => {
      ...
      addon.name = createLocalizedString(name);
      renderWithAddon();
      await waitFor(() => expect(getElement('title')).toHaveTextContent(`Reviews for ${name}`));
    });
    ...
    it('renders a robots meta tag when there is a featured review', async () => {
      ...
      await waitFor(() => expect(getElement('meta[name="robots"]')).toHaveAttribute('content', 'noindex, follow'));
    });
    it('dispatches fetchReviewPermissions on mount', () => {
      ...
      const dispatch = jest.spyOn(store, 'dispatch');
      renderWithAddon();
      expect(dispatch).toHaveBeenCalledWith(fetchReviewPermissions({
        addonId: addon.id,
        errorHandlerId: getErrorHandlerId(),
        userId,
      }));
    ...
    it('does not dispatch fetchReviewPermissions() when an error has occurred', () => {
      ...
      const dispatch = jest.spyOn(store, 'dispatch');
      renderWithAddon();
      expect(dispatch).not.toHaveBeenCalledWith(fetchReviewPermissions({
        addonId: addon.id,
        errorHandlerId: getErrorHandlerId(),
        userId,
      }));
    ...
    it('passes siteUserCanReply to FeaturedAddonReview', () => {
      ...
      expect(within(screen.getByClassName('FeaturedAddonReview')).getByRole('link', {
        name: 'Reply to this review',
      })).toBeInTheDocument();
    });
  });
  it('renders a "description" meta tag', async () => {
    ...
    addon.name = createLocalizedString(name);
    renderWithAddon();
    await waitFor(() => expect(getElement('meta[name="description"]')).toHaveAttribute('content', `Reviews and ratings for ${name}. Find out what other users think ` + `about ${name} and add it to your Firefox Browser.`));
  });
  describe('filterByScoreSelector', () => {
    const getSelector = () => within(screen.getByClassName('AddonReviewList-filterByScore')).getByRole('combobox');
    ...
    it('lets you select all reviews', async () => {
      ...
      expect(pushSpy).toHaveBeenCalledWith(`/en-US/firefox/addon/${defaultSlug}/reviews/`);
    });
    it.each([[5, 'Show only five-star reviews'], [4, 'Show only four-star reviews'], [3, 'Show only three-star reviews'], [2, 'Show only two-star reviews'], [1, 'Show only one-star reviews']])('lets you select only %s star reviews', async (score, option) => {
      renderWithAddonAndReviews();
      const pushSpy = jest.spyOn(history, 'push');
      await userEvent.selectOptions(getSelector(), option);
      expect(pushSpy).toHaveBeenCalledWith(`/en-US/firefox/addon/${defaultSlug}/reviews/?score=${score}`);
    });
  ...
  describe('Tests for FeaturedAddonReview', () => {
    const getThisErrorHandlerId = (reviewId) => `src/amo/components/FeaturedAddonReview/index.js-${reviewId}`;
    ...
    it('fetches a review at construction', () => {
      ...
      expect(dispatch).toHaveBeenCalledWith(fetchReview({
        reviewId,
    ...
    it('does not fetch a review if one is already loaded', () => {
      ...
      expect(dispatch).not.toHaveBeenCalledWith(fetchReview({
        reviewId,
        errorHandlerId: getThisErrorHandlerId(reviewId),
      }));
    });
    ...
    it('displays the correct header for a review', () => {
      ...
      expect(screen.getByText(`Review by ${fakeReview.user.name}`)).toBeInTheDocument();
    });
    it('displays the correct header for a reply', () => {
      ...
      expect(screen.getByText(`Response by ${fakeReview.user.name}`)).toBeInTheDocument();
    });
  });
});