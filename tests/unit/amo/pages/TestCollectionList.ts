...
import { collectionName, fetchUserCollections, loadUserCollections, FETCH_USER_COLLECTIONS } from 'amo/reducers/collections';
import { createFakeCollectionDetail, dispatchClientMetadata, dispatchSignInActions, fakeI18n, renderPage as defaultRender, screen, within } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const signInUser = () => dispatchSignInActions({
    clientApp,
    lang,
    store,
    userId,
  ...
  it('dispatches fetchUserCollections for a logged in user with no collections loaded yet', () => {
    ...
    render();
    expect(dispatch).toHaveBeenCalledWith(fetchUserCollections({
      errorHandlerId,
      userId,
    }));
  });
  ...
  it('renders an AuthenticateButton without a logged in user', () => {
    ...
    expect(screen.getByRole('link', {
      name: 'Log in to view your collections',
    })).toBeInTheDocument();
  });
  ...
  it('renders the collection listing info card', () => {
    ...
    expect(screen.getByText(`Collections make it easy to keep track of favorite add-ons and ` + `share your perfectly customized browser with others.`)).toBeInTheDocument();
    expect(screen.getByRole('link', {
  ...
  it('renders placeholder text if the user has no collections', () => {
    ...
    expect(screen.getByText('You do not have any collections.')).toBeInTheDocument();
    expect(screen.queryByClassName('CollectionList-listing')).not.toBeInTheDocument();
  });
  it('renders loading UserCollection objects if collections are loading', () => {
    ...
    render();
    expect(screen.queryByText('You do not have any collections.')).not.toBeInTheDocument();
    // Expect four placeholder collections with two LoadingText each.
    expect(within(screen.getByClassName('CollectionList-listing')).getAllByRole('alert')).toHaveLength(8);
  });
  it('renders a list of collections', () => {
    ...
    expect(screen.getByRole('link', {
      name: `${name1} 1 add-on`,
    })).toHaveAttribute('href', `/${lang}/${clientApp}/collections/${userId}/${slug1}/`);
    expect(screen.getByRole('heading', {
      name: name1,
    })).toBeInTheDocument();
    expect(screen.getByText('1 add-on')).toBeInTheDocument();
    expect(screen.getByRole('link', {
      name: `${name2} 2 add-ons`,
    })).toHaveAttribute('href', `/${lang}/${clientApp}/collections/${userId}/${slug2}/`);
    expect(screen.getByRole('heading', {
      name: name2,
    })).toBeInTheDocument();
  ...
  describe('Tests for UserCollection', () => {
    ...
    it('can render a collection that is loading', () => {
      ...
      render();
      expect(within(screen.getByClassName('UserCollection-name')).getByRole('alert')).toBeInTheDocument();
      expect(within(screen.getByClassName('UserCollection-number')).getByRole('alert')).toBeInTheDocument();
    });
  });
});