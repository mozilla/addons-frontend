...
import { ErrorHandler, withErrorHandler, withFixedErrorHandler, withRenderedErrorHandler } from 'amo/errorHandler';
import { clearError, setError, setErrorMessage } from 'amo/reducers/errors';
import { createCapturedErrorHandler, dispatchClientMetadata, render as defaultRender, screen } from 'tests/unit/helpers';
...
class SomeComponentBase extends React.Component {
  ...
  render() {
    return <div className="SomeComponent">
        <input name="inputName" onClick={this.handleError} readOnly value={this.props.errorHandler.id} />
...
describe(__filename, () => {
  ...
  const render = ({
    ...
    const ComponentWithErrorHandling = compose(translate(), decorator({
      id,
      name: 'SomeComponent',
      ...options,
    }))(SomeComponentBase);
  ...
  describe('withErrorHandler', () => {
    ...
    it('throws an error if `extractId` is not a function', () => {
      ...
      }).toThrow('`extractId` must be a function taking `ownProps` as unique argument.');
    });
    ...
    it('configures an error handler for action dispatching', async () => {
      ...
      expect(dispatch).toHaveBeenCalledWith(setError({
  ...
  describe('withRenderedErrorHandler', () => {
    ...
    it('passes a nested API response object', () => {
      ...
      expect(screen.getByRole('list')).toBeInTheDocument();
      expect(screen.getByText('{"nested":{"message":"end"}}')).toBeInTheDocument();
    });
  ...
  describe('ErrorHandler', () => {
    ...
    it('dispatches an error', () => {
      ...
      errorHandler.handle(error);
      expect(errorHandler.dispatch).toHaveBeenCalledWith(setError({
        id: errorHandler.id,
        error,
      }));
    });
    it('requires a dispatch function to handle an error', () => {
      ...
      const error = new Error();
      expect(() => handler.handle(error)).toThrow(/dispatch function has not been configured/);
    });
    it('requires a dispatch function to add a message', () => {
      const handler = new ErrorHandler({
        id: 'some-id',
      });
      expect(() => handler.addMessage('some message')).toThrow(/dispatch function has not been configured/);
    });
    it('dispatches a message', () => {
      const message = 'Name field cannot be blank';
      errorHandler.addMessage(message);
      expect(errorHandler.dispatch).toHaveBeenCalledWith(setErrorMessage({
        id: errorHandler.id,
        message,
      }));
    });
    it('lets you create an error action', () => {
      const error = new Error();
      expect(errorHandler.createErrorAction(error)).toEqual(setError({
        id: errorHandler.id,
        error,
      }));
    });
    it('clears an error', () => {
      errorHandler.clear();
      expect(errorHandler.dispatch).toHaveBeenCalledWith(clearError(errorHandler.id));
    });
    it('lets you create an error clearing action', () => {
      expect(errorHandler.createClearingAction()).toEqual(clearError(errorHandler.id));
    });
    ...
    it('returns a component if it has an error', () => {
      const handler = createCapturedErrorHandler({
        store,
      });
  ...
  describe('withFixedErrorHandler', () => {
    ...
    it('creates an error handler with a fixed ID', () => {
      ...
      expect(getInput()).toHaveValue('src/SomeComponent/index.js-unique-id-based-on-props');
    });
  });
});