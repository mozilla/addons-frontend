...
import { GET_FIREFOX_BUTTON_CLICK_ACTION, GET_FIREFOX_BUTTON_CLICK_CATEGORY, getDownloadLink, getDownloadCampaign } from 'amo/components/GetFirefoxButton';
import InstallButtonWrapper from 'amo/components/InstallButtonWrapper';
import { ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME, CLIENT_APP_FIREFOX, DISABLED, DOWNLOAD_FAILED, DOWNLOAD_FIREFOX_BASE_URL, DOWNLOAD_FIREFOX_UTM_CAMPAIGN, DOWNLOAD_PROGRESS, ENABLED, ENABLE_ACTION, ERROR, ERROR_CORRUPT_FILE, FATAL_ERROR, FATAL_INSTALL_ERROR, FATAL_UNINSTALL_ERROR, INCOMPATIBLE_ANDROID_UNSUPPORTED, INCOMPATIBLE_FIREFOX_FOR_IOS, INCOMPATIBLE_NOT_FIREFOX, INCOMPATIBLE_OVER_MAX_VERSION, INCOMPATIBLE_UNDER_MIN_VERSION, INCOMPATIBLE_UNSUPPORTED_PLATFORM, INACTIVE, INSTALLED, INSTALLING, INSTALL_ACTION, INSTALL_CANCELLED, INSTALL_CANCELLED_ACTION, INSTALL_DOWNLOAD_FAILED_ACTION, INSTALL_ERROR, INSTALL_FAILED, INSTALL_STARTED_ACTION, SET_ENABLE_NOT_AVAILABLE, START_DOWNLOAD, TRACKING_TYPE_INVALID, UNINSTALLED, UNINSTALLING, UNINSTALL_ACTION } from 'amo/constants';
...
import tracking, { getAddonTypeForTracking, getAddonEventCategory } from 'amo/tracking';
import { createFakeTracking, createInternalAddonWithLang, createInternalVersionWithLang, dispatchClientMetadata, fakeAddon, fakeFile, fakeInstalledAddon, fakeVersion, getFakeAddonManagerWrapper, getFakeLoggerWithJest as getFakeLogger, render as defaultRender, screen, userAgents, userAgentsByPlatform } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const render = ({
    ...
    return defaultRender(<InstallButtonWrapper addon={createInternalAddonWithLang(addon)} _addonManager={_addonManager} _getClientCompatibility={_getClientCompatibility} {...props} />, {
      store,
    });
  };
  ...
  it.each(['unknown reason', INCOMPATIBLE_UNDER_MIN_VERSION])('hides the install button and shows the download button for an incompatible addon when the reason is %s', (reason) => {
    const version = { ...fakeVersion,
      id: fakeVersion.id + 1,
    };
  ...
  it.each([INCOMPATIBLE_ANDROID_UNSUPPORTED, INCOMPATIBLE_FIREFOX_FOR_IOS, INCOMPATIBLE_NOT_FIREFOX, INCOMPATIBLE_OVER_MAX_VERSION, INCOMPATIBLE_UNSUPPORTED_PLATFORM])('hides the download button and shows the install button for an incompatible addon when the reason is %s', (reason) => {
    const version = { ...fakeVersion,
      id: fakeVersion.id + 1,
    };
    ...
    expect(screen.queryByText('Download the new Firefox and get the extension')).not.toBeInTheDocument();
  });
  ...
  it('passes the expected status to AMInstallButton when the add-on is installed', () => {
    store.dispatch(setInstallState({ ...fakeInstalledAddon,
      guid: addon.guid,
      status: INSTALLED,
    }));
    render({
  ...
  it('passes an add-on to GetFirefoxButton', () => {
    ...
    const expectedHref = ['https://www.mozilla.org/firefox/download/thanks/?s=direct', 'utm_campaign=amo-fx-cta-1234', `utm_content=rta%3A${encodedGUID}`, 'utm_medium=referral', 'utm_source=addons.mozilla.org'].join('&');
    render();
    expect(screen.getByRole('link', {
  ...
  it('passes a custom className to AMInstallButton', () => {
    ...
    expect(screen.getByClassName('AMInstallButton')).toHaveClass(`AMInstallButton--${className}`);
  });
  it('passes a custom className to GetFirefoxButton', () => {
    ...
    expect(screen.getByClassName('GetFirefoxButton')).toHaveClass(`GetFirefoxButton--${className}`);
  });
  ...
  it('adds a special classname when no download link is displayed', () => {
    ...
    expect(screen.getByClassName('AMInstallButton')).toHaveClass('AMInstallButton--noDownloadLink');
  });
  it('does not add a special classname when a download link is displayed', () => {
    ...
    expect(screen.getByClassName('AMInstallButton')).not.toHaveClass('AMInstallButton--noDownloadLink');
  });
  ...
  describe('Tests for GetFirefoxButton', () => {
    const getFirefoxButton = () => screen.getByRole('link', {
    ...
    describe('Not firefox', () => {
      ...
      it('has the expected callout text for an extension', () => {
        render();
        expect(screen.getByText(`You'll need Firefox to use this extension`)).toBeInTheDocument();
      });
      it('has the expected callout text for an extension, which is incompatible', () => {
        renderAsIncompatible();
        expect(screen.getByText('You need an updated version of Firefox for this extension')).toBeInTheDocument();
      });
      it('has the expected callout text for a theme', () => {
        addon.type = ADDON_TYPE_STATIC_THEME;
        render();
        expect(screen.getByText(`You'll need Firefox to use this theme`)).toBeInTheDocument();
      });
      it('has the expected callout text for a theme, which is incompatible', () => {
        addon.type = ADDON_TYPE_STATIC_THEME;
        renderAsIncompatible();
        expect(screen.getByText('You need an updated version of Firefox for this theme')).toBeInTheDocument();
      });
    ...
    describe('getDownloadCampaign', () => {
      ...
      it('returns a campaign with an addonId', () => {
        const addonId = 12345;
        expect(getDownloadCampaign({
          addonId,
        })).toEqual(`${DOWNLOAD_FIREFOX_UTM_CAMPAIGN}-${addonId}`);
      });
    });
    describe('getDownloadLink', () => {
      ...
      it('returns the expected URL for an add-on', () => {
        ...
        addon.id = addonId;
        addon.guid = guid;
        const expectedLink = [`${DOWNLOAD_FIREFOX_BASE_URL}?s=direct`, `utm_campaign=${DOWNLOAD_FIREFOX_UTM_CAMPAIGN}-${addonId}`, `utm_content=rta%3A${encode(guid)}`, `utm_medium=referral&utm_source=addons.mozilla.org`].join('&');
        expect(getDownloadLink({
          addon: createInternalAddonWithLang(addon),
  ...
  describe('Tests for withInstallHelpers', () => {
    ...
    it('does not call getAddon() if we do not have an addon', () => {
      ...
      expect(screen.queryByClassName('InstallButtonWrapper')).not.toBeInTheDocument();
    });

    const successfulGetAddonMock = (getAddonOverrides = {}) => Promise.resolve({
      isActive: true,
      isEnabled: true,
      ...getAddonOverrides,
    });
    ...
    describe('setCurrentStatus', () => {
      it.each([[ADDON_TYPE_EXTENSION, 'enabled', ENABLED, successfulGetAddonMock()], [ADDON_TYPE_EXTENSION, 'disabled', DISABLED, successfulGetAddonMock({
        isEnabled: false,
      })], [ADDON_TYPE_EXTENSION, 'disabled and inactive', DISABLED, successfulGetAddonMock({
        isActive: false,
        isEnabled: false,
      })], [ADDON_TYPE_EXTENSION, 'enabled but inactive', INACTIVE, successfulGetAddonMock({
        isActive: false,
      })], [ADDON_TYPE_STATIC_THEME, 'enabled', ENABLED, successfulGetAddonMock()], [ADDON_TYPE_STATIC_THEME, 'disabled', DISABLED, successfulGetAddonMock({
        isEnabled: false,
      })], [ADDON_TYPE_STATIC_THEME, 'disabled and inactive', DISABLED, successfulGetAddonMock({
        isActive: false,
        isEnabled: false,
      })], [ADDON_TYPE_STATIC_THEME, 'enabled but inactive', DISABLED, successfulGetAddonMock({
        isActive: false,
      })]])('when a(n) %s is %s, sets the status to %s', async (type, statusDesc, expectedStatus, getAddon) => {
        addon.type = type;
        ...
        expect(dispatch).toHaveBeenCalledWith(setInstallState({
      ...
      it('does nothing when addon is `null`', () => {
        ...
        expect(_log.debug).toHaveBeenCalledWith('no addon, aborting setCurrentStatus()');
      });
      it('does nothing when currentVersion is `null`', () => {
        ...
        expect(dispatch).not.toHaveBeenCalled();
        expect(_log.debug).toHaveBeenCalledWith('no currentVersion, aborting setCurrentStatus()');
      });
    ...
    describe('enable', () => {
      ...
      it('dispatches a FATAL_ERROR', async () => {
        ...
        expect(_addonManager.enable).toHaveBeenCalledWith(addon.guid);
        await waitFor(() => expect(dispatch).toHaveBeenCalledWith(setInstallState({
          guid: addon.guid,
          status: ERROR,
          error: FATAL_ERROR,
        })));
      ...
      it('does not dispatch a FATAL_ERROR when setEnabled is missing', async () => {
        const addonManagerOverrides = {
          enable: jest.fn().mockRejectedValue(new Error(SET_ENABLE_NOT_AVAILABLE)),
          getAddon: Promise.resolve({
            isActive: true,
            isEnabled: false,
          }),
        ...
        expect(_addonManager.enable).toHaveBeenCalledWith(addon.guid);
        expect(dispatch).not.toHaveBeenCalledWith(setInstallState({
          guid: addon.guid,
          status: ERROR,
          error: FATAL_ERROR,
        }));
    ...
    describe('install', () => {
      it('calls addonManager.install()', async () => {
        ...
        expect(_addonManager.install).toHaveBeenCalledWith(addon.current_version.file.url, expect.any(Function), {
      ...
      it('uses a version instead of the currentVersion when one exists in props', async () => {
        ...
        await waitFor(() => expect(button).not.toHaveAttribute('disabled'));
        await userEvent.click(button);
        expect(_addonManager.install).toHaveBeenCalledWith(versionInstallURL, expect.any(Function), {
          hash: versionHash,
        });
      });
      it('tracks the start of an addon install', async () => {
        ...
        expect(fakeTracking.sendEvent).toHaveBeenCalledWith({
          action: getAddonTypeForTracking(ADDON_TYPE_EXTENSION),
          category: getAddonEventCategory(ADDON_TYPE_EXTENSION, INSTALL_STARTED_ACTION),
          label: addon.guid,
        });
      });
      it('tracks an addon install', async () => {
        ...
        await userEvent.click(button);
        expect(_addonManager.install).toHaveBeenCalled();
        await waitFor(() => expect(fakeTracking.sendEvent).toHaveBeenCalledTimes(2));
        expect(fakeTracking.sendEvent).toHaveBeenCalledWith({
          action: getAddonTypeForTracking(ADDON_TYPE_EXTENSION),
          category: getAddonEventCategory(ADDON_TYPE_EXTENSION, INSTALL_STARTED_ACTION),
          label: addon.guid,
      ...
      it('tracks the start of a static theme install', async () => {
        ...
        expect(fakeTracking.sendEvent).toHaveBeenCalledWith({
          ...
          category: getAddonEventCategory(ADDON_TYPE_STATIC_THEME, INSTALL_STARTED_ACTION),
          label: addon.guid,
        });
      });
      it('tracks a static theme install', async () => {
        ...
        expect(fakeTracking.sendEvent).toHaveBeenCalledWith({
          action: getAddonTypeForTracking(ADDON_TYPE_STATIC_THEME),
          category: getAddonEventCategory(ADDON_TYPE_STATIC_THEME, INSTALL_ACTION),
          label: addon.guid,
        });
      });
      ...
      it('dispatches error when addonManager.install throws', async () => {
        ...
        await userEvent.click(button);
        expect(_addonManager.install).toHaveBeenCalled();
        await waitFor(() => expect(dispatch).toHaveBeenCalledWith(setInstallError({
    ...
    describe('uninstall', () => {
      ...
      it('dispatches error when addonManager.uninstall throws', async () => {
        const addonManagerOverrides = {
          uninstall: jest.fn().mockRejectedValue(new Error('Add-on Manager uninstall error')),
        };
        const dispatch = jest.spyOn(store, 'dispatch');
        renderWithCurrentVersion({
          addonManagerOverrides,
      ...
      it('tracks an addon uninstall', async () => {
        ...
        expect(fakeTracking.sendEvent).toHaveBeenCalledWith({
          action: getAddonTypeForTracking(ADDON_TYPE_EXTENSION),
          category: getAddonEventCategory(ADDON_TYPE_EXTENSION, UNINSTALL_ACTION),
          label: addon.guid,
        });
      });
      it('tracks a static theme uninstall', async () => {
        ...
        expect(fakeTracking.sendEvent).toHaveBeenCalledWith({
          action: getAddonTypeForTracking(ADDON_TYPE_STATIC_THEME),
          category: getAddonEventCategory(ADDON_TYPE_STATIC_THEME, UNINSTALL_ACTION),
          label: addon.guid,
        });
      });
