...
import { ADDON_TYPE_STATIC_THEME, DEFAULT_UTM_SOURCE, LANDING_PAGE_EXTENSION_COUNT, LANDING_PAGE_THEME_COUNT } from 'amo/constants';
import { createInternalAddonWithLang, fakeAddon, render as defaultRender, screen } from 'tests/unit/helpers';

describe(__filename, () => {
  function render(customProps = {}) {
    const addons = Array(LANDING_PAGE_EXTENSION_COUNT).fill(createInternalAddonWithLang(fakeAddon));
    const props = {
      addons,
      footerLink: {
        pathname: '/some-path/',
  ...
  it('renders AddonsCard in a non-loading state when not loading', () => {
    ...
    expect(screen.getByRole('link', {
      name: 'Footer text',
    })).toHaveAttribute('href', '/en-US/android/some-path/');
  });
  it('renders AddonsCard in a loading state when loading', () => {
    ...
    expect(screen.getAllByRole('alert')).toHaveLength(LANDING_PAGE_EXTENSION_COUNT * 4);
  ...
  it('passes addonInstallSource to AddonsCard', () => {
    ...
    const expectedLink = [`/en-US/android/addon/${addons[0].slug}/?utm_source=${DEFAULT_UTM_SOURCE}`, 'utm_medium=referral', `utm_content=${addonInstallSource}`].join('&');
    expect(screen.getByRole('link', {
      name: addons[0].name,
  ...
  it('overrides the placeholder prop value when isTheme is passed in', () => {
    ...
    expect(screen.getAllByRole('alert')).toHaveLength(LANDING_PAGE_THEME_COUNT * 4);
  });
  ...
  it('hides the footer link when there are less add-ons than LANDING_PAGE_THEME_COUNT', () => {
    render({
      addons: Array(LANDING_PAGE_THEME_COUNT - 1).fill(createInternalAddonWithLang({ ...fakeAddon,
  ...
  it('shows the footer link when there are more or as many add-ons as LANDING_PAGE_THEME_COUNT', () => {
    ...
    render({
      addons: Array(LANDING_PAGE_THEME_COUNT).fill(createInternalAddonWithLang({ ...fakeAddon,
        type: ADDON_TYPE_STATIC_THEME,
      })),
      isTheme: true,
      footerLink,
    ...
    expect(screen.getByRole('link', {
      name: footerText,
    })).toHaveAttribute('href', `/en-US/android${footerLink}`);
  });
  it('shows the footer link when there are more or as many add-ons as placeholderCount', () => {
    ...
    const placeholderCount = 2;
    render({
      addons: Array(placeholderCount).fill(createInternalAddonWithLang({ ...fakeAddon,
        type: ADDON_TYPE_STATIC_THEME,
      })),
      placeholderCount,
      footerLink,
  ...
  it('sets useThemePlaceholder to true if isTheme is true', () => {
    ...
    expect(screen.getAllByRole('listitem')[0]).toHaveClass('SearchResult--theme');
  });
  it('sets useThemePlaceholder to false if isTheme is false', () => {
    ...
    expect(screen.getAllByRole('listitem')[0]).not.toHaveClass('SearchResult--theme');
  });
});