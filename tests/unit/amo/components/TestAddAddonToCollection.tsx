...
import AddAddonToCollection, { extractId } from 'amo/components/AddAddonToCollection';
import { setClientApp } from 'amo/reducers/api';
import { CLIENT_APP_FIREFOX } from 'amo/constants';
import { ErrorHandler } from 'amo/errorHandler';
import { FETCH_USER_COLLECTIONS, addAddonToCollection, collectionName, fetchUserCollections, loadUserCollections } from 'amo/reducers/collections';
import collectionsSaga from 'amo/sagas/collections';
import { DEFAULT_LANG_IN_TESTS, createFakeCollectionDetail, createHistory, createInternalAddonWithLang, dispatchClientMetadata, dispatchClientMetadataWithSagas, dispatchSignInActions, fakeAddon, fakeI18n, render as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const createStoreWithSaga = () => dispatchClientMetadataWithSagas({
  ...
  describe('fetching user collections', () => {
    ...
    it('fetches user collections on update', async () => {
      ...
      await waitFor(() => expect(dispatch).toHaveBeenCalledWith(fetchUserCollections({
        errorHandlerId: createErrorHandlerId({
          addon,
          userId,
        }),
  ...
  describe('selecting a user collection', () => {
    ...
    it('displays expected name for collections without names', () => {
      ...
      const options = screen.getAllByRole('option');
      expect(options[2]).toHaveTextContent(collectionName({
        name: null,
        i18n: fakeI18n(),
      }));
    });
    it('shows a notice that you added to a collection', async () => {
      ...
      expect(screen.getByClassName('Notice-success')).toHaveTextContent(`Added to ${firstName}`);
    });
    ...
    it('does nothing when you select the prompt', async () => {
      ...
      render();
      dispatch.mockClear();
      await userEvent.selectOptions(screen.getByRole('combobox'), 'Select a collection…');
    ...
    it('lets you create a new collection by navigating to the collection page', async () => {
      ...
      await userEvent.selectOptions(screen.getByRole('combobox'), 'Create new collection');
      expect(pushSpy).toHaveBeenCalledWith(`/${lang}/${clientApp}/collections/add/?include_addon_id=${id}`);
    });
  });
  ...
  describe('extractId', () => {
    ...
    it('renders an ID with an add-on ID and user', () => {
      ...
      expect(extractId({
        addon,
        currentUserId,
      })).toEqual(`${addon.id}-${currentUserId}`);
    });
  });
});