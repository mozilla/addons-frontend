...
import { dispatchClientMetadata, render as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  function render({
    ...
    return defaultRender(<OverlayCard id={id} {...props}>
  ...
  it('passes a footer link', () => {
    ...
    render({
      footerLink: <a href={linkHref}>{linkText}</a>,
    });
  ...
  it('passes children', () => {
    const childText = 'hi';
    render({
      overlayChildren: <div className="kids">hi</div>,
    });
  ...
  describe('Tests for Overlay', () => {
    it('is hidden by default', () => {
      render();
      expect(screen.getByClassName('Overlay')).not.toHaveClass('Overlay--visible');
    });
    ...
    it('becomes visible when the UIState changes to visible: true', async () => {
      ...
      await waitFor(() => expect(screen.getByClassName('Overlay')).toHaveClass('Overlay--visible'));
    });
