...
import { createHistory, createInternalAddonWithLang, dispatchClientMetadata, fakeAddon, render as defaultRender, screen, userAgentsByPlatform } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('can add a custom className', () => {
    ...
    expect(screen.getByClassName('WrongPlatformWarning')).toHaveClass(className);
  });
  it('calls _isFirefoxForAndroid to check for Android user agent', () => {
    ...
    expect(_isFirefoxForAndroid).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('calls _isAndroidInstallable to check for Android compatibility', () => {
    ...
    expect(_isAndroidInstallable).toHaveBeenCalledWith({
      addon,
    });
  });
  ...
  it('calls _correctedLocationForPlatform with isHomePage defaulted to false', () => {
    ...
    expect(_correctedLocationForPlatform).toHaveBeenCalledWith(expect.objectContaining({
      clientApp,
      isHomePage: false,
    }));
  });
  it('returns nothing when user agent is Firefox for Android and add-on is compatible', () => {
    ...
    expect(screen.queryByClassName('WrongPlatformWarning')).not.toBeInTheDocument();
  });
  it('generates the expected message when user agent is Firefox for iOS', () => {
    _isFirefoxForIOS.mockReturnValue(true);

    render();
    expect(screen.getByText('Add-ons are not compatible with Firefox for iOS. Try installing them on Firefox for desktop.')).toBeInTheDocument();
  });
  it('generates the expected message when being directed to the mobile home page', () => {
    ...
    expect(screen.getByRole('link', {
      name: 'click here',
    })).toHaveAttribute('href', mobileLink);
    expect(screen.getByText(/To find add-ons compatible with Firefox for Android,/)).toBeInTheDocument();
  });
  ...
  it('generates the expected message when being directed to other than the mobile home page, from other pages', () => {
    ...
    expect(screen.getByText(/To find add-ons compatible with Firefox on desktop,/)).toBeInTheDocument();
  });
