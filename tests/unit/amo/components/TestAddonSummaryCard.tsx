...
import { createHistory, createInternalAddonWithLang, createLocalizedString, fakeAddon, render as defaultRender, screen, within } from 'tests/unit/helpers';
...
describe(__filename, () => {
  const render = ({
    ...
    return defaultRender(<AddonSummaryCard addon={addon ? createInternalAddonWithLang(addon) : addon} headerText={headerText} {...props} />, renderOptions);
  };
  ...
  describe('Card header', () => {
    it('renders a fallback icon without an add-on', () => {
      ...
      expect(screen.getByAltText('Add-on icon')).toHaveAttribute('src', 'default.svg');
    });
    ...
    it('adds UTM query parameters to the link on the icon when there are some', () => {
      ...
      expect(screen.getByRole('link', {
        name: 'Add-on icon',
      })).toHaveAttribute('href', `/en-US/android${getAddonURL(addon.slug)}?utm_medium=${utmMedium}`);
    });
    ...
    it('passes queryParamsForAttribution with the value of `utm_content` when available', () => {
      ...
      expect(screen.getByRole('link', {
        name: addonName,
      })).toHaveAttribute('href', `/en-US/android${getAddonURL(addon.slug)}?utm_content=${utmContent}`);
    });
  });
  describe('overallRatingStars', () => {
    ...
    it('renders RatingsByStar without an add-on', () => {
      ...
      expect(within(screen.getByClassName('RatingsByStar')).getAllByRole('alert')).toHaveLength(10);
    });
    it('renders RatingsByStar with an add-on', () => {
      ...
      expect(screen.getByTitle('There are no five-star reviews')).toHaveAttribute('href', `/en-US/android/addon/${addon.slug}/reviews/?score=5`);
    });
    it('renders loading text without an add-on', () => {
      render({
        addon: null,
      });
      expect(within(screen.getByClassName('AddonSummaryCard-addonAverage')).getByRole('alert')).toBeInTheDocument();
    });
    it('renders empty text without add-on ratings', () => {
      ...
      expect(screen.getByClassName('AddonSummaryCard-addonAverage')).toHaveTextContent('');
    });
