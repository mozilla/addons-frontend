...
describe(__filename, () => {
  ...
  function render(props) {
    return defaultRender(<Link {...props} />, {
      store,
    });
  }
  ...
  it('passes `to` without leading slash without base', () => {
    ...
    }).toThrow('"to" prop cannot contain a relative path; it must start with a "/".');
  });
  ...
  it('prefixes the `to.pathname` prop', () => {
    ...
    expect(getLink()).toHaveAttribute('href', `/${lang}/${clientApp}${pathname}`);
  });
  ...
  it('renders children when `to` is used', () => {
    ...
    expect(screen.getByRole('link', {
      name: text,
    })).toHaveAttribute('href', `/${lang}/${clientApp}${to}`);
  });
  ...
  it('renders children when `href` is used', () => {
    ...
    expect(screen.getByRole('link', {
      name: text,
    })).toHaveAttribute('href', `/${lang}/${clientApp}${href}`);
  });
