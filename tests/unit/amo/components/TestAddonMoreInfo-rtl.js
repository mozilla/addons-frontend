...
describe(__filename, () => {
  ...
  it('renders a custom license link', () => {
    ...
    expect(screen.getByText('License')).toBeInTheDocument();
    const link = screen.getByText(licenseName);
    // TODO: We are just hardcoding the expected lang and app here.
    // We don't usually do that with frontend tests, but it is
    // common with server tests, so maybe it's ok to start doing
    // that here as well?
    expect(link).toHaveAttribute(
      'href',
  ...
  // TODO: This requires us to know how to set up the test to display
  // admin links, and to understand the output of the AddonAdminLinks
  // component.
  //
  // Maybe we should consider testing all of AddonAdminLinks in here as a
  // result.
  // It's not actually used anywhere else, so that kind of makes sense.
