...
import { createInternalAddonWithLang, createLocalizedString, dispatchClientMetadata, fakeAddon, render as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const render = (props = {}) => {
    return defaultRender(<AddonTitle {...props} />, {
      store,
    });
  };
  ...
  it('renders a single author', () => {
    const author = { ...fakeAddon.authors[0],
      id: 123,
    };
    render({
      addon: createInternalAddonWithLang({ ...fakeAddon,
  ...
  it('renders multiple authors', () => {
    const author1 = { ...fakeAddon.authors[0],
      name: 'Author 1',
      id: 101,
    };
    const author2 = { ...fakeAddon.authors[0],
      name: 'Author 2',
      id: 102,
    };
    render({
    ...
    expect(screen.getByTextAcrossTags(` by ${author1.name}, ${author2.name}`)).toBeInTheDocument();
  });
  it('renders without authors', () => {
    ...
    expect(screen.queryByClassName('AddonTitle-author')).not.toBeInTheDocument();
  });
  ...
  it('handles RTL mode', () => {
    ...
    expect(screen.getByClassName('AddonTitle-author')).toHaveTextContent(`${author1.name} ,${author2.name} by`);
  });
  ...
  it('accepts some query params for attribution to append to the add-on URL', () => {
    const queryParamsForAttribution = {
      some: 'value',
    };
    const addon = createInternalAddonWithLang(fakeAddon);
    render({
    ...
    expect(screen.getByRole('link', {
      name: addon.name,
    })).toHaveAttribute('href', `/en-US/android${getAddonURL(addon.slug)}?some=value`);
  });
});