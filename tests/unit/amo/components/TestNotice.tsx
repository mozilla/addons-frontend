...
describe(__filename, () => {
  ...
  const render = ({
    ...
    return defaultRender(<Notice id="some-id" type="success" {...props}>
        {children}
      </Notice>, {
      store,
    });
  ...
  it('requires actionText when specifying a button action', () => {
    ...
    })).toThrow(/actionText is required/);
  });
  it('renders a button with a `to` property', () => {
    ...
    expect(screen.getByRole('link', {
      name: actionText,
    })).toHaveAttribute('href', `/${lang}/${clientApp}${actionTo}`);
  });
  it('renders a button with an `href` property', () => {
    ...
    expect(screen.getByRole('link', {
      name: actionText,
    })).toHaveAttribute('href', actionHref);
  });
  ...
  it('changes UI state when dismissing a notice', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(setUIStateAction({
      id: getUIStateId(id),
      change: {
        wasDismissed: true,
      },
  ...
  it('hides a dismissed notice', async () => {
    ...
    store.dispatch(setUIStateAction({
      id: getUIStateId(id),
      change: {
        wasDismissed: true,
      },
  ...
  it('requires an ID prop for dismissible notices', () => {
    expect(() => render({
      dismissible: true,
      id: undefined,
    })).toThrow(/id property must be defined/);
  });
  it('passes an actionTarget to Button', () => {
    ...
    expect(screen.getByRole('link', {
      name: actionText,
    })).toHaveAttribute('target', actionTarget);
  });
