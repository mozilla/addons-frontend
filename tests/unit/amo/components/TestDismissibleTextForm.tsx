...
import { createFakeDebounce, createFakeLocalState, dispatchClientMetadata, render as defaultRender, screen, within } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const render = (customProps = {}) => {
    ...
    return defaultRender(<DismissibleTextForm {...props} />, {
      store,
    });
  };
  ...
  it('lets you configure the submit button class', () => {
    render({
      submitButtonClassName: 'my-class',
    });
  ...
  it('hides a formFooter by default', () => {
    render();
    expect(screen.queryByClassName('DismissibleTextForm-formFooter')).not.toBeInTheDocument();
  });
  it('renders a formFooter', () => {
    const footerText = 'Some footer text';
    render({
      formFooter: <div>{footerText}</div>,
    });
    expect(within(screen.getByClassName('DismissibleTextForm-formFooter')).getByText(footerText)).toBeInTheDocument();
  });
  ...
  describe('LocalState', () => {
    ...
    it('does not populate the form with null data', async () => {
      ...
      render({
        _createLocalState: () => // Set up LocalState to load null data.
