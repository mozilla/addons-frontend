...
import VPNPromoBanner, { IMPRESSION_COUNT_KEY, VPN_PROMO_CAMPAIGN, VPN_PROMO_CATEGORY, VPN_PROMO_CLICK_ACTION, VPN_PROMO_DISMISS_ACTION, VPN_PROMO_IMPRESSION_ACTION, VPN_URL } from 'amo/components/VPNPromoBanner';
import { CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX, DEFAULT_UTM_SOURCE, DEFAULT_UTM_MEDIUM } from 'amo/constants';
import { EXPERIMENT_CONFIG, VARIANT_HIDE, VARIANT_SHOW } from 'amo/experiments/20210714_amo_vpn_promo';
import { loadAddon } from 'amo/reducers/addons';
import { EXPERIMENT_COOKIE_NAME, NOT_IN_EXPERIMENT, defaultCookieConfig } from 'amo/withExperiment';
import { DEFAULT_LANG_IN_TESTS, changeLocation, createExperimentData, createFakeLocalStorage, createFakeTracking, dispatchClientMetadata, fakeAddon, fakeCookies, render as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const createCookies = () => {
    return fakeCookies({
      get: jest.fn().mockReturnValue(createExperimentData({
  ...
  const render = ({
    ...
    const renderResults = defaultRender(<Route path="/:lang/:application(firefox|android)/addon/:slug/">
        <VPNPromoBanner _tracking={createFakeTracking()} _localStorage={createFakeLocalStorage()} cookies={createCookies()} variant={variant} {...props} />
      </Route>, renderOptions);
  ...
  it.each([VARIANT_HIDE, NOT_IN_EXPERIMENT, null])('does not display the promo when the variant is %s', (variant) => {
    render({
      variant,
    });
  ...
  it('renders a link with the expected href without an add-on loaded', () => {
    const queryString = [`utm_campaign=${VPN_PROMO_CAMPAIGN}`, `utm_medium=${DEFAULT_UTM_MEDIUM}`, `utm_source=${DEFAULT_UTM_SOURCE}`].join('&');
  ...
  it('renders a link with the expected href with an add-on loaded', () => {
    const queryString = [`utm_campaign=${VPN_PROMO_CAMPAIGN}`, `utm_content=${addonId}`, `utm_medium=${DEFAULT_UTM_MEDIUM}`, `utm_source=${DEFAULT_UTM_SOURCE}`].join('&');
    const href = `${VPN_URL}?${queryString}`;
  ...
  const clickCta = async () => userEvent.click(screen.getByRole('link', {
    name: 'Get Mozilla VPN',
  }));
  ...
  it('reads and updates the experiment cookie when the cta is clicked', async () => {
    ...
    expect(cookies.set).toHaveBeenCalledWith(EXPERIMENT_COOKIE_NAME, createExperimentData({
      id: EXPERIMENT_CONFIG.id,
  ...
  describe('tracking', () => {
    ...
    it('sends a tracking event and increases the count for the impression on mount', () => {
      ...
      expect(_localStorage.setItem).toHaveBeenCalledWith(IMPRESSION_COUNT_KEY, nextImpressionCount);
    });
    it.each([VARIANT_HIDE, NOT_IN_EXPERIMENT, null])('does not send a tracking event for the impression on mount when the variant is %s', (variant) => {
      const _tracking = createFakeTracking();
    ...
    it.each([VARIANT_HIDE, NOT_IN_EXPERIMENT, null])('does not send a tracking event for the impression on update when the variant is %s', async (variant) => {
      const _tracking = createFakeTracking();
