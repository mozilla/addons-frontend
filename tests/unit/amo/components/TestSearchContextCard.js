...
describe(__filename, () => {
  ...
  it('should render results with categoryName and query for addonType ADDON_TYPE_OPENSEARCH when search is loaded', () => {
    const query = 'test';
    const categoryName = 'Bookmarks';
    ...
    expect(root.find('.SearchContextCard-header')).toIncludeText(
      `2 Search Tools found for "${query}" in ${categoryName}`,
    );
  });

  it('should render results with categoryName and no query for addonType ADDON_TYPE_OPENSEARCH when there is no query and when search is loaded', () => {
    const categoryName = 'Bookmarks';
    ...
    expect(root.find('.SearchContextCard-header')).toIncludeText(
      `2 Search Tools found in ${categoryName}`,
    );
  });
});
