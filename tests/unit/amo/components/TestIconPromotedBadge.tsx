...
describe(__filename, () => {
  ...
  it.each([['IconPromotedBadge-large', 'large'], ['IconPromotedBadge-small', 'small']])('adds the class "%s" for size="%s"', (className, size) => {
    const {
      root,
    } = render({
      size,
  ...
  it.each(['recommended', 'verified'])('adds the expected classes for category="%s"', (category) => {
    render({
      category,
    });
    expect(screen.getByTagName('circle')).toHaveClass(`IconPromotedBadge-circle-bgColor--${category}`);
    expect(screen.getByTagName('path')).toHaveClass(`IconPromotedBadge-iconPath--${category}`);
  });
  it.each(['recommended', 'verified'])('uses the expected path for category="%s"', (category) => {
    render({
      category,
    });
  ...
  it.each([['line', 'By Firefox'], ['recommended', 'Recommended'], ['verified', 'Verified']])('adds an alt property for category="%s" when showAlt is true', (category, alt) => {
    render({
      category,
      showAlt: true,
    });
  ...
  it.each(['recommended', 'verified'])('sets the icon with category="%s" to inline content', (category) => {
    const {
      root,
    } = render({
      category,
