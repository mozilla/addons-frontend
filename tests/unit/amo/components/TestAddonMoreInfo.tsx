...
import { ADDON_TYPE_DICT, ADDON_TYPE_EXTENSION, ADDON_TYPE_LANG, ADDON_TYPE_STATIC_THEME, ADDONS_EDIT, CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX, STATS_VIEW } from 'amo/constants';
import { formatFilesize } from 'amo/i18n/utils';
import { createInternalAddonWithLang, createLocalizedString, createStubErrorHandler, dispatchClientMetadata, dispatchSignInActions, fakeAddon, fakeAuthor, fakeCategory, fakeI18n, fakeFile, fakeVersion, render as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('renders LoadingText if no add-on is present', () => {
    ...
    expect(screen.queryByText('End-User License Agreement')).not.toBeInTheDocument();
  });
  ...
  it('renders the homepage of an add-on', () => {
    ...
    expect(link).toHaveAttribute('href', 'https://outgoing.mozilla.org/hamster');
  ...
  it('renders file size of an add-on', () => {
    ...
    expect(screen.getByText(formatFilesize({
      size,
      i18n: fakeI18n(),
    }))).toBeInTheDocument();
  });
  ...
  it('renders a custom license link', () => {
    ...
    expect(screen.getByText('License')).toBeInTheDocument();
    const link = screen.getByText(licenseName);
    expect(link).toHaveAttribute('href', `/en-US/android/addon/${addon.slug}/license/`);
  });
  ...
  it('renders the license info without a link if the url is null', () => {
    _loadVersions({
      license: {
        name: createLocalizedString('justText'),
        url: null,
      },
    });
  ...
  it('renders the privacy policy and link', () => {
    ...
    expect(link).toHaveAttribute('href', '/en-US/android/addon/chill-out/privacy/');
  });
  ...
  it('renders the EULA and link', () => {
    ...
    expect(link).toHaveAttribute('href', '/en-US/android/addon/chill-out/eula/');
  });
  ...
  it('links to stats if add-on author is viewing the page', () => {
    ...
    expect(statsLink).toHaveAttribute('href', '/en-US/android/addon/coolio/statistics/');
  });
  ...
  it.each([ADDON_TYPE_EXTENSION, ADDON_TYPE_DICT, ADDON_TYPE_LANG, ADDON_TYPE_STATIC_THEME])('links to version history if add-on is a %s', (type) => {
    const addon = createInternalAddonWithLang({ ...fakeAddon,
      type,
    });
    render({
    ...
    expect(link).toHaveAttribute('href', `/en-US/android/addon/${addon.slug}/versions/`);
  });
  ...
  describe('related categories', () => {
    ...
    it('renders related categories', () => {
      ...
      expect(screen.getByText(name1)).toHaveAttribute('href', `/en-US/firefox/extensions/category/${slug1}/`);
      expect(screen.getByText(name2)).toHaveAttribute('href', `/en-US/firefox/extensions/category/${slug2}/`);
    });
    ...
    it.each([ADDON_TYPE_DICT, ADDON_TYPE_LANG])('does not render related categories for addonType=%s', (type) => {
      const addon = createInternalAddonWithLang({ ...fakeAddon,
        categories: {
          [CLIENT_APP_ANDROID]: [categories[0].slug],
        },
        type,
      });
      store.dispatch(loadCategories({
  ...
  describe('UTM parameters', () => {
    ...
    it('renders links with UTM query params when there are some', () => {
      ...
      expect(screen.getByText('Visit stats dashboard')).toHaveAttribute('href', `/en-US/android/addon/${addon.slug}/statistics/?${expectedQueryString}`);
      expect(screen.getByText('tofulicense')).toHaveAttribute('href', `/en-US/android/addon/${addon.slug}/license/?${expectedQueryString}`);
      expect(screen.getByText('Read the privacy policy for this add-on')).toHaveAttribute('href', `/en-US/android/addon/${addon.slug}/privacy/?${expectedQueryString}`);
      expect(screen.getByText('Read the license agreement for this add-on')).toHaveAttribute('href', `/en-US/android/addon/${addon.slug}/eula/?${expectedQueryString}`);
      expect(screen.getByText('See all versions')).toHaveAttribute('href', `/en-US/android/addon/${addon.slug}/versions/?${expectedQueryString}`);
    });
  });
});