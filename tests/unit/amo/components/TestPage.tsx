...
import { GET_FIREFOX_BANNER_CLICK_ACTION, GET_FIREFOX_BANNER_DISMISS_ACTION, GET_FIREFOX_BANNER_DISMISS_CATEGORY, GET_FIREFOX_BANNER_UTM_CONTENT } from 'amo/components/GetFirefoxBanner';
import { GET_FIREFOX_BUTTON_CLICK_CATEGORY } from 'amo/components/GetFirefoxButton';
import { ADDONS_REVIEW, ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME, API_ERROR_AUTHENTICATION_EXPIRED, API_ERRORS_SESSION_EXPIRY, CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX, DOWNLOAD_FIREFOX_BASE_URL, DOWNLOAD_FIREFOX_UTM_CAMPAIGN, VIEW_CONTEXT_LANGUAGE_TOOLS } from 'amo/constants';
...
import { createCapturedErrorHandler, createFakeAutocompleteResult, dispatchAutocompleteResults, dispatchClientMetadata, dispatchSignInActions, getElement, getFakeLoggerWithJest as getFakeLogger, getMockConfig, mockMatchMedia, render as defaultRender, screen, userAgents } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const render = ({
    ...
    const renderResults = defaultRender(<Page {...props}>{children || <div>Some content</div>}</Page>, renderOptions);
  ...
  it('does not assign an extra className to the home page', () => {
    ...
    expect(screen.queryByClassName('Page-not-homepage')).not.toBeInTheDocument();
  });
  it('does not assign an extra className when there is a hero promo', () => {
    ...
    render();
    expect(screen.queryByClassName('Page-no-hero-promo')).not.toBeInTheDocument();
  });
  ...
  it.each(API_ERRORS_SESSION_EXPIRY)('renders AuthFailed for 401 error with a session expiry code: %s', (code) => {
    const errorHandler = createCapturedErrorHandler({
      status: 401,
      detail: 'something',
      code,
  ...
  it.each([401, 403, 404, 451])('does not render children when there is a %s error', (status) => {
    const childText = 'Some text in a child';
    const children = <div>{childText}</div>;
    const errorHandler = createCapturedErrorHandler({
      status,
  ...
  it.each([401, 403, 404, 451])('logs a debug message when there is a %s error', (status) => {
  ...
  describe('Tests for GetFirefoxBanner', () => {
    ...
    describe('Not firefox', () => {
      ...
      it('renders a dismissable warning Notice', () => {
        render(props);
        expect(screen.getByClassName('GetFirefoxBanner')).toHaveClass('Notice-warning');
        expect(screen.getByClassName('GetFirefoxBanner')).toHaveClass('Notice-dismissible');
      });
      it('has the expected text', () => {
        render(props);
        expect(screen.getByTextAcrossTags(`To use these add-ons, you'll need to download Firefox`)).toBeInTheDocument();
      });
      it('sets the href on the button with the expected utm params', () => {
        render(props);
        const expectedHref = [`${DOWNLOAD_FIREFOX_BASE_URL}?s=direct`, `utm_campaign=${DOWNLOAD_FIREFOX_UTM_CAMPAIGN}`, `utm_content=${GET_FIREFOX_BANNER_UTM_CONTENT}`, `utm_medium=referral&utm_source=addons.mozilla.org`].join('&');
        expect(screen.getByRole('link', {
          name: 'download Firefox',
  ...
  describe('Tests for Header', () => {
    it('renders "Firefox Browser Add-ons" in a heading when isHomePage is true', () => {
      ...
      expect(screen.getByRole('banner')).not.toHaveClass('Header--loaded-page-is-anonymous');
    });
    ...
    it('displays link to my collections when user is signed in', () => {
      ...
      render();
      expect(screen.getByText('View My Collections')).toHaveAttribute('href', '/en-US/android/collections/');
    });
    it('displays a view profile link when user is signed in', () => {
      ...
      render();
      expect(screen.getByText('View My Profile')).toHaveAttribute('href', `/en-US/android/user/${id}/`);
    });
    it('displays an edit profile link when user is signed in', () => {
      _dispatchSignInActions();

      render();
      expect(screen.getByText('Edit My Profile')).toHaveAttribute('href', '/en-US/android/users/edit');
    });
    ...
    it('displays the reviewer tools link when user has a reviewer permission', () => {
      ...
      render();
      expect(screen.getByText('Reviewer Tools')).toHaveAttribute('href', '/en-US/reviewers/');
    });
    ...
    it('displays a "manage my submissions" link when user is logged in', () => {
      ...
      render();
      expect(screen.getByText('Manage My Submissions')).toHaveAttribute('href', '/en-US/developers/addons/');
    });
    it('displays blog, devhub and extension workshop links in the header on Firefox', () => {
      ...
      expect(screen.getByTitle('Submit and manage extensions and themes')).toHaveAttribute('href', '/en-US/developers/');
      expect(screen.getByTitle('Learn how to create extensions and themes')).toHaveAttribute('href', `${extensionWorkshopUrl}/${makeQueryStringWithUTM({
    ...
    it('does not display links for devhub and extension workshop on non-Firefox', () => {
      ...
      expect(screen.queryByTitle('Learn how to create extensions and themes')).not.toBeInTheDocument();
    });
    it('disables the logout button when the site is in readonly mode', () => {
      ...
      expect(logOutButton).toHaveAttribute('title', 'This action is currently unavailable. Please reload the page in a moment.');
    });
    ...
    it('does not render a menu when the loaded page is anonymous and user is logged in', () => {
      ...
      render();
      expect(screen.getByRole('banner')).toHaveClass('Header--loaded-page-is-anonymous');
    ...
    it('adds a className to the search form for the desktop site', () => {
      ...
      render();
      expect(screen.getByRole('search')).toHaveClass('Header-search-form--desktop');
    });
    it('does not add a className to the search form for the mobile site', () => {
      ...
      render();
      expect(screen.getByRole('search')).not.toHaveClass('Header-search-form--desktop');
    });
    describe('Tests for SectionLinks', () => {
      it('renders four sections on Firefox', () => {
        ...
        // a drop-down menu
        expect(screen.getByText('Dictionaries & Language Packs')).toHaveAttribute('href', '/en-US/firefox/language-tools/');
      ...
      it('renders Language Tools active when viewContext is languageTools', () => {
        ...
        render();
        expect(screen.getByText('Dictionaries & Language Packs')).toHaveClass('SectionLinks-dropdownlink--active');
      });
      it('shows Firefox name and hides link in header on Desktop', () => {
        ...
        expect(screen.queryByClassName(`SectionLinks-clientApp-${CLIENT_APP_ANDROID}`)).toHaveTextContent('Add-ons for Android');
        expect(screen.queryByClassName(`SectionLinks-clientApp-${CLIENT_APP_FIREFOX}`)).not.toBeInTheDocument();
      });
  ...
  describe('Tests for Footer', () => {
    it('renders a footer', () => {
      ...
      expect(screen.getByRole('link', {
        name: 'Privacy',
      })).toHaveAttribute('href', 'https://www.mozilla.org/privacy/websites/');
      expect(screen.getByRole('link', {
        name: 'Cookies',
      })).toHaveAttribute('href', 'https://www.mozilla.org/privacy/websites/');
      expect(screen.getByRole('link', {
        name: 'Legal',
      })).toHaveAttribute('href', 'https://www.mozilla.org/about/legal/terms/mozilla/');
      ...
      expect(screen.getByRole('link', {
        name: 'Report a bug',
      })).toHaveAttribute('href', 'https://developer.mozilla.org/docs/Mozilla/Add-ons/Contact_us');
      expect(screen.getByRole('link', {
        name: 'VPN',
      })).toHaveAttribute('href', `https://www.mozilla.org/products/vpn/${makeQueryStringWithUTM({
        utm_content: 'footer-link',
        utm_campaign: null,
      })}#pricing`);
      expect(screen.getByRole('link', {
        name: 'Relay',
      })).toHaveAttribute('href', `https://relay.firefox.com/${makeQueryStringWithUTM({
        utm_content: 'footer-link',
        utm_campaign: null,
      })}`);
      expect(screen.getByRole('link', {
        name: 'Monitor',
      })).toHaveAttribute('href', `https://monitor.firefox.com/${makeQueryStringWithUTM({
        utm_content: 'footer-link',
        utm_campaign: null,
      })}`);
      expect(screen.getByRole('link', {
        name: 'Browsers',
      })).toHaveAttribute('href', `https://www.mozilla.org/firefox/browsers/${makeQueryStringWithUTM({
        utm_content: 'footer-link',
        utm_campaign: null,
      })}`);
      expect(screen.getByRole('link', {
        name: 'Pocket',
      })).toHaveAttribute('href', `https://getpocket.com${makeQueryStringWithUTM({
        utm_content: 'footer-link',
        utm_campaign: null,
      })}`);
      expect(screen.getByRole('link', {
        name: 'Desktop',
      })).toHaveAttribute('href', `https://www.mozilla.org/firefox/new/${makeQueryStringWithUTM({
        utm_content: 'footer-link',
        utm_campaign: null,
      })}`);
      expect(screen.getByRole('link', {
        name: 'Mobile',
      })).toHaveAttribute('href', `https://www.mozilla.org/firefox/mobile/${makeQueryStringWithUTM({
        utm_content: 'footer-link',
        utm_campaign: null,
      })}`);
      expect(screen.getByRole('link', {
        name: 'Enterprise',
      })).toHaveAttribute('href', `https://www.mozilla.org/firefox/enterprise/${makeQueryStringWithUTM({
        utm_content: 'footer-link',
        utm_campaign: null,
      })}`);
      expect(screen.getByRole('link', {
        name: 'Developer Policies',
      })).toHaveAttribute('href', `${extensionWorkshopUrl}/documentation/publish/add-on-policies/${makeQueryStringWithUTM({
        utm_medium: 'photon-footer',
        utm_campaign: null,
      })}`);
      expect(screen.getByRole('link', {
        name: 'Community Blog',
      })).toHaveAttribute('href', `https://blog.mozilla.com/addons${makeQueryStringWithUTM({
        utm_campaign: null,
        utm_content: 'footer-link',
        utm_medium: 'referral',
      })}`);
  ...
  describe('Tests for LanguagePicker', () => {
    ...
    it('only changes the locale section of the URL', async () => {
      ...
      expect(window.location.pathname).toEqual('/en-US/firefox/en-US-to-en-GB-guide/');
      await userEvent.selectOptions(screen.getByRole('combobox', {
        name: 'Change language',
      }), screen.getByRole('option', {
        name: 'عربي',
      }));
      expect(window.location).toEqual('/ar/firefox/en-US-to-en-GB-guide/?foo=en-US');
    });
  });
  describe('Tests for SearchForm', () => {
    it('sets the form action URL', () => {
      ...
      expect(screen.getByRole('search')).toHaveAttribute('action', `/en-GB/${CLIENT_APP_FIREFOX}/search/`);
    });
    ...
    it.each([['/url/to/extension/detail/page', '/url/to/extension/detail/page'], // It should parse the pathname from the second one.
    ['https://example.org/url/to/extension/detail/page', '/url/to/extension/detail/page']])('pushes a new route to %s when a suggestion is selected', async (url, expected) => {
    ...
    it.each([[CLIENT_APP_ANDROID, 'Firefox Add-ons for Android'], [CLIENT_APP_FIREFOX, 'Firefox Add-ons']])('renders an opensearch link for %s', async (clientApp, title) => {
      const lang = 'en-CA';
      ...
      await waitFor(() => expect(getElement('link[rel="search"]')).toBeInTheDocument());
      const link = getElement('link[rel="search"]');
      expect(link).toHaveAttribute('type', 'application/opensearchdescription+xml');
      expect(link).toHaveAttribute('href', `/${lang}/${clientApp}/opensearch.xml`);
      expect(link).toHaveAttribute('title', title);
    });
  });
  describe('Tests for AuthExpired', () => {
    const expiredAuthErrorHandler = () => createCapturedErrorHandler({
    ...
    it('logs out the user', () => {
      const dispatch = jest.spyOn(store, 'dispatch');
      render({
        errorHandler: expiredAuthErrorHandler(),
      });
