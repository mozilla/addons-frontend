...
describe(__filename, () => {
  ...
  it.each([1, 2, 3, 4, 5])('lets you select a %s star rating', async (rating) => {
  ...
  it('renders correct full stars for a rating', () => {
    const verifyRating = (currentRating) => {
      ...
      [1, 2, 3].forEach((rating) => {
        expect(getStarButton({
          rating,
          currentRating,
        })).toHaveClass('Rating-selected-star');
      });
      [4, 5].forEach((rating) => {
        expect(getStarButton({
          rating,
  ...
  it('renders correct half stars for a rating', () => {
    const verifyRating = (currentRating) => {
      ...
      const fourthStar = getStarButton({
        currentRating,
        rating: 4,
      });
  ...
  it('renders an accessible description for null stars', () => {
    renderWithEmptyRating();
    [1, 2, 3, 4, 5].forEach((rating) => {
      expect(screen.getByTitle(`Rate this add-on ${rating} out of 5`)).toBeInTheDocument();
    });
  });
  ...
  it('renders an appropriate title when updating a rating', () => {
    ...
    [1, 2, 4, 5].forEach((rating) => {
      expect(screen.getByTitle(`Update your rating to ${rating} out of 5`)).toBeInTheDocument();
    });
    expect(screen.getByTitle(`Rated ${userRating} out of 5`)).toBeInTheDocument();
  });
  ...
  describe('readOnly=true', () => {
    ...
    it('does not classify as editable when read-only', () => {
      ...
      expect(screen.getByClassName('Rating')).not.toHaveClass('Rating--editable');
    });
