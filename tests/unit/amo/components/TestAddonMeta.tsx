...
import { createHistory, createInternalAddonWithLang, fakeAddon, fakeI18n, render as defaultRender, screen, within } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  describe('average daily users', () => {
    ...
    it('renders no users', () => {
      ...
      expect(within(overallRating).getAllByClassName('MetadataCard-content')[0]).toHaveTextContent('');
    });
  ...
  describe('ratings', () => {
    ...
    it('renders a count of multiple reviews', () => {
      ...
      const listURL = `/en-US/android${reviewListURL({
        addonSlug: slug,
      })}`;
    ...
    it('handles an addon without ratings', () => {
      ...
      expect(screen.queryByClassName('Rating--loading')).not.toBeInTheDocument();
    });
    it('renders RatingsByStar with an add-on', () => {
      ...
      expect(link).toHaveAttribute('href', `/en-US/android/addon/${addon.slug}/reviews/?score=5`);
    });
    ...
    it('renders the average rating', () => {
      ...
      expect(screen.getByText(`${roundToOneDigit(average)} Stars`)).toBeInTheDocument();
      expect(screen.getByText(`Rated ${roundToOneDigit(average)} out of 5`)).toBeInTheDocument();
    });
