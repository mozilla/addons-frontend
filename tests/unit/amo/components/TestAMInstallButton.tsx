...
import { ADDON_TYPE_STATIC_THEME, DISABLED, DISABLING, DOWNLOADING, ENABLED, ENABLING, INACTIVE, INSTALLED, INSTALLING, OS_ALL, UNINSTALLED, UNINSTALLING, UNKNOWN } from 'amo/constants';
import { createInternalAddonWithLang, createInternalVersionWithLang, fakeAddon, fakeTheme, fakeVersion, render as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const createInternalVersionWithInstallURL = ({
    ...
    return createInternalVersionWithLang({ ...fakeVersion,
      file: { ...fakeVersion.file,
        platform: OS_ALL,
        url: installURL,
      },
    });
  ...
  const getButton = (buttonText = 'Add to Firefox') => screen.getByRole('button', {
    name: buttonText,
  });

  const getLink = (buttonText = 'Add to Firefox') => screen.getByRole('link', {
    name: buttonText,
  });
  ...
  it.each([ENABLED, INSTALLED])('renders a "remove" button when add-on is %s', (status) => {
    render({
      status,
    });
  ...
  it.each([ENABLED, INSTALLED])('allows to uninstall an add-on when canUninstall is undefined and add-on is %s', (status) => {
    render({
      status,
  ...
  it.each([ENABLED, INSTALLED])('allows to uninstall an add-on when canUninstall is true and add-on is %s', (status) => {
    render({
      status,
      canUninstall: true,
    });
  ...
  it.each([ENABLED, INSTALLED])('does not allow to uninstall an add-on when canUninstall is false and add-on is %s', (status) => {
    render({
      status,
      canUninstall: false,
    });
  ...
  it.each([DOWNLOADING, DISABLING, ENABLING, INSTALLING, UNINSTALLING])('renders a loading Icon when add-on is %s', (status) => {
    render({
      status,
    });
    expect(screen.queryByClassName('AMInstallButton-button')).not.toBeInTheDocument();
    expect(screen.getByClassName('AMInstallButton-loading-button')).toBeInTheDocument();
    expect(screen.getByClassName('AMInstallButton-loader-container')).toBeInTheDocument();
  });
  it.each([[DOWNLOADING, 'Downloading'], [ENABLING, 'Enabling'], [INSTALLING, 'Installing'], [UNINSTALLING, 'Uninstalling']])('sets an `alt` prop to the Icon when status is %s', (status, title) => {
    render({
      status,
    });
