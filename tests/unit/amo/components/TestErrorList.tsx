...
describe(__filename, () => {
  ...
  it.each(API_ERRORS_SESSION_EXPIRY)('renders a reload button for session expiry error: %s', async (code) => {
    const _window = {
      location: {
        reload: jest.fn(),
      },
    ...
    await userEvent.click(screen.getByRole('button', {
      name: 'Reload To Continue',
    }));
    // The button should reload the location.
    expect(_window.location.reload).toHaveBeenCalled();
  });
  it.each(API_ERRORS_SESSION_EXPIRY)('handles multiple session expiry error: %s', (code) => {
