...
import AutoSearchInput, { SEARCH_TERM_MIN_LENGTH } from 'amo/components/AutoSearchInput';
import { ADDON_TYPE_EXTENSION, CLIENT_APP_ANDROID, RECOMMENDED, SEARCH_SORT_POPULAR, SEARCH_SORT_RANDOM } from 'amo/constants';
import { AUTOCOMPLETE_STARTED, autocompleteCancel, autocompleteStart } from 'amo/reducers/autocomplete';
import { changeLocation, createFailedErrorHandler, createFakeAutocompleteResult, createFakeDebounce, createInternalSuggestionWithLang, dispatchAutocompleteResults, createHistory, dispatchClientMetadata, render as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const render = ({
    ...
    return defaultRender(<AutoSearchInput debounce={createFakeDebounce()} inputName={inputName} onSuggestionSelected={onSuggestionSelected} {...props} />, renderOptions);
  };
  ...
  describe('search input', () => {
    ...
    it('sets the search value to an empty string when `location.query` is not a string', () => {
      ...
      render({
        location: `/?${defaultInputName}=a&${defaultInputName}=b`,
      });
      expect(screen.getByRole('searchbox')).toHaveValue('');
    });
  ...
  describe('fetching search suggestions', () => {
    it('fetches search suggestions', async () => {
      ...
      render();
      await typeInSearch(query);
      expect(dispatch).toHaveBeenCalledWith(autocompleteStart({
        errorHandlerId,
        filters: {
          query,
        },
    ...
    it('does not pass a `random` sort filter', async () => {
      ...
      const query = 'ad blocker';
      render({
        location: `/?sort=${SEARCH_SORT_RANDOM}`,
      });
      await typeInSearch(query);
      expect(dispatch).toHaveBeenCalledWith(autocompleteStart({
        errorHandlerId,
    ...
    it('keeps the search results menu open while searching', async () => {
      ...
      expect(screen.getByClassName('AutoSearchInput')).toHaveClass('AutoSearchInput--autocompleteIsOpen');
    });
  ...
  describe('clearing search suggestions', () => {
    it('closes suggestion menu on escape', async () => {
      ...
      expect(screen.getByClassName('AutoSearchInput')).not.toHaveClass('AutoSearchInput--autocompleteIsOpen');
    });
  ...
  describe('selecting search suggestions', () => {
    it('executes a callback when selecting a suggestion', async () => {
      ...
      expect(onSuggestionSelected).toHaveBeenCalledWith(createInternalSuggestionWithLang(fakeResult));
    });
  ...
  it('renders a suggestion', async () => {
    ...
    // arrow icon.
    expect(screen.getByText(selectSuggestionText)).toHaveClass('visually-hidden');
    // Verifies that SearchSuggestion displays a class name with its type.
    expect(screen.getByClassName('SearchSuggestion')).toHaveClass(`SearchSuggestion--${ADDON_TYPE_EXTENSION}`);
  });
  describe('getting suggestion data', () => {
    it('returns suggestion results', async () => {
      const firstName = 'Addon One';
      const secondName = 'Addon Two';
      const firstResult = createFakeAutocompleteResult({
        name: firstName,
      });
      const secondResult = createFakeAutocompleteResult({
        name: secondName,
      });
      render();
      await typeInSearch('test');
