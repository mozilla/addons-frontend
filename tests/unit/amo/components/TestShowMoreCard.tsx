...
import ShowMoreCard, { DEFAULT_MAX_HEIGHT, extractId } from 'amo/components/ShowMoreCard';
import { setUIState } from 'amo/reducers/uiState';
import { createHistory, dispatchClientMetadata, render as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const render = ({
    ...
    return defaultRender(<ShowMoreCard contentId={contentId} id={id} {...props}>
        {children}
      </ShowMoreCard>, renderOptions);
  };
  ...
  it('reveals more text when clicking "read more" link', async () => {
    ...
    expect(screen.getByClassName('ShowMoreCard')).toHaveClass('ShowMoreCard--expanded');
  });
  ...
  it('executes truncateToMaxHeight when it receives props changes', async () => {
    ...
    expect(_truncateToMaxHeight).toHaveBeenCalledWith(expect.objectContaining({
