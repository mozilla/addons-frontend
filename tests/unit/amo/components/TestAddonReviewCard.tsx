...
import defaultUserEvent, { PointerEventsCheckLevel } from '@testing-library/user-event';

import { SAVED_RATING, STARTED_SAVE_RATING, STARTED_SAVE_REVIEW, beginDeleteAddonReview, cancelDeleteAddonReview, createInternalReview, deleteAddonReview, flagReview, flashReviewMessage, hideEditReviewForm, hideFlashedReviewMessage, hideReplyToReviewForm, sendReplyToReview, setReview, setReviewWasFlagged, showEditReviewForm, showReplyToReviewForm, updateAddonReview } from 'amo/actions/reviews';
import AddonReviewCard from 'amo/components/AddonReviewCard';
import { extractId as addonReviewManagerExtractId } from 'amo/components/AddonReviewManager';
import { ALL_SUPER_POWERS, REVIEW_FLAG_REASON_BUG_SUPPORT, REVIEW_FLAG_REASON_LANGUAGE, REVIEW_FLAG_REASON_SPAM } from 'amo/constants';
import { reviewListURL } from 'amo/reducers/reviews';
import { logOutUser } from 'amo/reducers/users';
import { createFailedErrorHandler, createInternalAddonWithLang, dispatchClientMetadata, dispatchSignInActionsWithStore, fakeAddon, fakeI18n, fakeReview, render as defaultRender, screen, within } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const fakeRatingOnly = Object.freeze({ ...fakeReview,
    body: undefined,
    rating: 4,
  });
  beforeEach(() => {
  ...
  const render = ({
    ...
    return defaultRender(<AddonReviewCard addon={addon} siteUserCanReply={siteUserCanReply} {...props} />, {
      store,
    });
  };
  ...
  const _setReviewReply = ({
    ...
    const reply = _setReview({ ...fakeReview,
      addon: {
        id: addon.id,
        slug: addon.slug,
      },
  ...
  const signInAndDispatchSavedReview = ({
    ...
    return _setReview({ ...externalReview,
      user: { ...externalReview.user,
        id: reviewUserId,
        ...reviewUserProps,
      },
    });
  ...
  function createAddonAndReview({
    ...
    const review = _setReview({ ...externalReview,
      addon: {
        id: addon.id,
        slug: addon.slug,
      },
  ...
  const signInAsAddonDeveloper = ({
    developerUserId = 51123,
  } = {}) => {
    dispatchSignInActionsWithStore({
      store,
  ...
  const openFlagMenu = async ({
    isReply = false,
  } = {}) => userEvent.click(screen.getByRole('button', {
    name: isReply ? 'Flag this developer response' : 'Flag this review',
  }));

  const clickDeleteRating = async () => userEvent.click(screen.getByRole('button', {
    name: 'Delete rating',
  }));

  const clickDeleteReview = async () => userEvent.click(screen.getByRole('button', {
    name: 'Delete review',
  }));

  const clickEditReply = async () => userEvent.click(screen.getByRole('link', {
    name: 'Edit reply',
  }));

  const clickEditReview = async () => userEvent.click(screen.getByRole('link', {
    name: 'Edit review',
  }));

  const clickReplyToReview = async () => userEvent.click(screen.getByRole('link', {
  ...
  it('renders a privileged notice for deleted review', () => {
    ...
    expect(screen.getByText('This rating or review has been deleted. You are only seeing it because of elevated permissions.')).toBeInTheDocument();
  });
  it('lets you begin editing your review', () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(showEditReviewForm({
      reviewId: review.id,
    }));
  });
  ...
  it('begins deleting when clicking a delete link for a review', () => {
    ...
    fireEvent(button, clickEvent);
    expect(preventDefaultWatcher).toHaveBeenCalled();
    expect(dispatch).toHaveBeenCalledWith(beginDeleteAddonReview({
      reviewId: review.id,
    }));
  });
  ...
  it('does not render a confirmation message for slim=true', async () => {
    ...
    await clickDeleteReview();
    expect(screen.queryByText('Do you really want to delete this review?')).not.toBeInTheDocument();
  });
  it('renders a confirmation prompt for deleting a review', async () => {
    ...
    await clickDeleteReview();
    expect(screen.getByText('Do you really want to delete this review?')).toBeInTheDocument();
  });
  it('renders a confirmation prompt for deleting a rating', async () => {
    ...
    await clickDeleteRating();
    expect(screen.getByText('Do you really want to delete this rating?')).toBeInTheDocument();
  });
  it('lets you cancel deleting a review', async () => {
    ...
    fireEvent(button, clickEvent);
    expect(preventDefaultWatcher).toHaveBeenCalled();
    expect(dispatch).toHaveBeenCalledWith(cancelDeleteAddonReview({
      reviewId: review.id,
    }));
  });
  it('dispatches deleteReview and displayes a deleting message when a user deletes a review', async () => {
    ...
    fireEvent(button, clickEvent);
    expect(preventDefaultWatcher).toHaveBeenCalled();
    expect(dispatch).toHaveBeenCalledWith(deleteAddonReview({
  ...
  it('adds AddonReviewCard-ratingOnly to rating-only reviews', () => {
    ...
    expect(screen.getByClassName('AddonReviewCard')).toHaveClass('AddonReviewCard-ratingOnly');
  });
  it('does not add AddonReviewCard-ratingOnly to reviews with a body', () => {
    ...
    expect(screen.getByClassName('AddonReviewCard')).not.toHaveClass('AddonReviewCard-ratingOnly');
  });
  ...
  it('lets you flag a developer reply', () => {
    renderNestedReply();
    expect(screen.getByRole('button', {
      name: 'Flag this developer response',
    })).toBeInTheDocument();
  });
  ...
  it('allows review replies when siteUserCanReply is true', () => {
    ...
    fireEvent(button, clickEvent);
    expect(preventDefaultWatcher).toHaveBeenCalled();
    expect(dispatch).toHaveBeenCalledWith(showReplyToReviewForm({
      reviewId: review.id,
    }));
  });
  ...
  it('configures a reply-to-review text form when editing', async () => {
    const {
      reply,
    } = renderNestedReplyForSignedInDeveloper();
    await userEvent.click(screen.getByRole('link', {
      name: 'Edit reply',
    }));
    const textArea = screen.getByPlaceholderText('Write a reply to this review.');
  ...
  it('dispatches a finish action when dismissing a reply-to-review text form', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(hideReplyToReviewForm({
      reviewId: review.id,
    }));
  });
  it('allows a user to reply to a review', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(sendReplyToReview({
      errorHandlerId: getDefaultErrorHandlerId(review.id),
      originalReviewId: review.id,
      body: replyBody,
    }));
    expect(screen.getByRole('button', {
      name: 'Publishing reply',
    })).toHaveClass('Button--disabled');
    expect(screen.queryByRole('button', {
      name: 'Publish reply',
    })).not.toBeInTheDocument();
    // The saga would dispatch this to hide the form.
    store.dispatch(hideReplyToReviewForm({
      reviewId: review.id,
    }));
    await waitFor(() => expect(screen.queryByPlaceholderText('Write a reply to this review.')).not.toBeInTheDocument());
  });
  it('resets the reply form state when there is an error', async () => {
    ...
    await clickReplyToReview();
    await userEvent.type(screen.getByPlaceholderText('Write a reply to this review.'), 'Body of the review');
    await userEvent.click(screen.getByRole('button', {
      name: 'Publish reply',
    }));
    createFailedErrorHandler({
  ...
  it('configures DismissibleTextForm with an ID', async () => {
    ...
    const thisReview = _setReview({ ...fakeReview,
      id: 1,
    });

    const anotherReview = _setReview({ ...fakeReview,
      id: 2,
    });
    ...
    store.dispatch(showReplyToReviewForm({
      reviewId: anotherReview.id,
    }));
    expect(screen.queryByPlaceholderText('Write a reply to this review.')).not.toBeInTheDocument();
    // Dispatch an action to show the form for a review in this card.
    store.dispatch(showReplyToReviewForm({
      reviewId: thisReview.id,
    }));
    expect(await screen.findByPlaceholderText('Write a reply to this review.')).toBeInTheDocument();
  });
  it('renders errors', () => {
  ...
  describe('AddonReviewManager integration', () => {
    ...
    it('hides the review form on cancel', async () => {
      ...
      expect(dispatch).toHaveBeenCalledWith(hideEditReviewForm({
        reviewId: review.id,
      }));
    });
  ...
  describe('byLine', () => {
    it('renders a byLine with a permalink to the review', () => {
      ...
      expect(screen.getByTitle(i18n.moment(review.created).format('lll'))).toHaveAttribute('href', `/en-US/android${reviewListURL({
        addonSlug: slug,
        id: review.id,
      })}`);
    });
    ...
    it('renders a byLine without a link when the reviewAddon has an empty slug and a falsey id', () => {
      ...
      expect(screen.queryByTitle(i18n.moment(review.created).format('lll'))).not.toBeInTheDocument();
    });
    it('renders a byLine with a relative date', () => {
      ...
      expect(screen.getByTitle(i18n.moment(review.created).format('lll'))).toHaveTextContent(i18n.moment(review.created).fromNow());
    });
    ...
    it('renders a short byLine for replies by default', () => {
      ...
      expect(screen.getByTextAcrossTags(`posted ${i18n.moment(reply.created).fromNow()}`)).toBeInTheDocument();
    });
    it('renders a short byLine explicitly', () => {
      ...
      expect(screen.getByTextAcrossTags(`posted ${i18n.moment(review.created).fromNow()}`)).toBeInTheDocument();
    });
  ...
  describe('Developer reply to a review', () => {
    ...
    it('allows a developer to delete a reply', async () => {
      ...
      expect(screen.getByText('Do you really want to delete this reply?')).toBeInTheDocument();
      const button = screen.getByRole('button', {
        name: 'Delete',
      });
      const clickEvent = createEvent.click(button);
  ...
  describe('Tests for UserRating', () => {
    ...
    it('passes yellowStars: false to Rating if you did not write the review', () => {
      ...
      expect(screen.queryByClassName('Rating--yellowStars')).not.toBeInTheDocument();
    });
  ...
  describe('Tests for FlagReview and FlagReviewMenu', () => {
    ...
    it.each([[REVIEW_FLAG_REASON_BUG_SUPPORT, 'This is a bug report or support request', 'Flagged as a bug report or support request'], [REVIEW_FLAG_REASON_LANGUAGE, 'This contains inappropriate language', 'Flagged for inappropriate language'], [REVIEW_FLAG_REASON_SPAM, 'This is spam', 'Flagged as spam']])('can flag a review for %s', async (reason, prompt, postText) => {
      const dispatch = jest.spyOn(store, 'dispatch');
      const review = createReviewAndSignInAsUnrelatedUser();
      render({
        review,
      ...
      fireEvent(button, clickEvent);
      expect(preventDefaultWatcher).toHaveBeenCalled();
      expect(dispatch).toHaveBeenCalledWith(flagReview({
    ...
    it('renders loading text while in progress', async () => {
      ...
      expect(within(screen.getByRole('tooltip')).getByRole('alert')).toBeInTheDocument();
    });
    ...
    describe('Tests for FlagReviewMenu', () => {
      ...
      it('requires you to be signed in', async () => {
        ...
        expect(screen.getByRole('link', {
          name: 'Log in to flag this review',
        })).toBeInTheDocument();
      });
      it('prompts you to flag a developer response after login', async () => {
        ...
        expect(screen.getByRole('link', {
          name: 'Log in to flag this response',
        })).toBeInTheDocument();
      });
  ...
  describe('Tests for UserReview', () => {
    ...
    it('renders newlines in review bodies', () => {
      ...
      expect(screen.getByTextAcrossTags(`It's awesome  isn't it?`)).toBeInTheDocument();
      expect(within(screen.getByClassName('UserReview-body')).getByTagName('br')).toBeInTheDocument();
    });
    ...
    */
    it('does not add UserReview-emptyBody when there is a body', () => {
      ...
      expect(screen.getByClassName('UserReview-body')).not.toHaveClass('UserReview-emptyBody');
    });
  ...
  describe('Tests for AddonReviewManagerRating', () => {
    it('sets readOnly correctly when onSelectRating is defined', async () => {
      render({
        review: signInAndDispatchSavedReview(),
      });
      // AddonReviewManagerRating is rendered in edit mode.
      await clickEditReview();
      expect(screen.getByTitle('Update your rating to 1 out of 5')).toBeInTheDocument();
    });
  });
  describe('Tests for AddonReviewManager', () => {
    const getErrorHandlerId = (reviewId) => `src/amo/components/AddonReviewManager/index.js-${reviewId}`;
    ...
    it('renders DismissibleTextForm text', async () => {
      ...
      await clickEditReview();
      expect(screen.getByPlaceholderText('Write about your experience with this add-on.')).toHaveValue(body);
    });
    it('renders a DismissibleTextForm formFooter', async () => {
      ...
      expect(screen.getByTextAcrossTags('Please follow our review guidelines.')).toBeInTheDocument();
    });
    it('updates the rating when you select a star', async () => {
      ...
      expect(dispatch).toHaveBeenCalledWith(updateAddonReview({
        score: newScore,
        errorHandlerId: getErrorHandlerId(review.id),
        reviewId: review.id,
      }));
    ...
    it('updates the review body when you submit the form', async () => {
      ...
      await clickEditReview();
      const input = screen.getByPlaceholderText('Write about your experience with this add-on.');
    ...
    it('flashes a saving rating message', async () => {
      ...
      await clickEditReview();
      const flashMessage = screen.getByClassName('AddonReviewManager-savedRating');
      expect(flashMessage).toHaveTextContent('Saving');
      expect(flashMessage).not.toHaveClass('RatingManagerNotice-savedRating-hidden');
    ...
    it('hides a flashed rating message', async () => {
      ...
      await clickEditReview();
      expect(screen.getByClassName('AddonReviewManager-savedRating')).toHaveClass('RatingManagerNotice-savedRating-hidden');
    });
    ...
    it('allows one to submit a review when no review text exists yet', async () => {
      ...
      await userEvent.type(screen.getByPlaceholderText('Write about your experience with this add-on.'), body);
      await userEvent.click(screen.getByRole('button', {
        name: 'Submit review',
      }));
      expect(dispatch).toHaveBeenCalledWith(showEditReviewForm({
      ...
      await waitFor(() => expect(screen.getByRole('button', {
        name: 'Submitting review',
      })).toHaveClass('Button--disabled'));
    });
    describe('extractId', () => {
      it('extracts an ID from the review', () => {
        const id = 551224;
        expect(addonReviewManagerExtractId({
