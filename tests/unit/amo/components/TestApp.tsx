...
import { SET_CLIENT_APP, SET_USER_AGENT, setClientApp as setClientAppAction, setUserAgent as setUserAgentAction } from 'amo/reducers/api';
import { CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX, INSTALL_STATE } from 'amo/constants';
import { changeLocation, dispatchClientMetadata, getElement, render as defaultRender } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('uses navigator.userAgent if userAgent prop is empty', () => {
    ...
    const _navigator = {
      userAgent: 'Firefox 10000000.0',
    };
    const dispatch = jest.spyOn(store, 'dispatch');
    render({
      _navigator,
    });
    expect(dispatch).toHaveBeenCalledWith(setUserAgentAction(_navigator.userAgent));
  });
  ...
  it('configures a default HTML title for Android', async () => {
    ...
    render();
    await waitFor(() => expect(getElement('title')).toHaveTextContent(`Add-ons for Firefox Android (${lang})`));
  });
