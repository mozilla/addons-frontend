...
import { createHistory, dispatchClientMetadata, dispatchSignInActionsWithStore, render as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('is disabled when the site is in readonly mode', () => {
    ...
    render();
    expect(screen.getByTitle('This action is currently unavailable. Please reload the page in a moment.')).toHaveClass('Button--disabled');
  });
  ...
  describe('createHandleLogOutFunction()', () => {
    it('logs out a signed-in user on the client side even if the API returns an error', () => {
      const error = api.createApiError({
        response: {
          status: 401,
        },
      });
