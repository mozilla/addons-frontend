...
describe(__filename, () => {
  ...
  describe('classifyPermission', () => {
    ...
    it('classifies host permissions', () => {
      ...
      const testPermissions = ['*://*.mozilla.org/*', '*://mozilla.org/', 'http://mozilla.org/', 'https://*/path', 'https://*/path/', 'https://mozilla.org/*', 'https://mozilla.org/a/b/c/', 'https://mozilla.org/*/b/*/'];
  ...
  describe('formatPermissions', () => {
    ...
    it('returns all permissions in the expected order', () => {
      ...
      const testPermissions = ['tabs', hostPermissionA, 'nativeMessaging', hostPermissionB, 'bookmarks'];
      const result = permissionUtils.formatPermissions(testPermissions);
      expect(result).toHaveLength(4);
      // Native messaging next.
      expectPermission(result[0], 'nativeMessaging', 'Exchange messages with programs other than Firefox');
      ...
      expect(result[3].props.permissions).toEqual([hostPermissionA, hostPermissionB]);
    });
  });
});