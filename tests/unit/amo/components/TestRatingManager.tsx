...
import { ADDON_TYPE_DICT, ADDON_TYPE_EXTENSION, ADDON_TYPE_LANG, ADDON_TYPE_STATIC_THEME } from 'amo/constants';
import { FETCH_LATEST_USER_REVIEW, SAVED_RATING, STARTED_SAVE_RATING, createAddonReview, fetchLatestUserReview, flashReviewMessage, hideFlashedReviewMessage, setLatestReview, setReview, updateAddonReview } from 'amo/actions/reviews';
import { hasAbuseReportPanelEnabled } from 'amo/addonManager';
import RatingManager from 'amo/components/RatingManager';
import { SEND_ADDON_ABUSE_REPORT, initiateAddonAbuseReportViaFirefox, loadAddonAbuseReport, sendAddonAbuseReport } from 'amo/reducers/abuse';
import { createFailedErrorHandler, createFakeAddonAbuseReport, createInternalAddonWithLang, createInternalVersionWithLang, createLocalizedString, dispatchClientMetadata, dispatchSignInActionsWithStore, fakeAddon, fakeReview, fakeVersion, render as defaultRender, screen } from 'tests/unit/helpers';
...
jest.mock('amo/constants', () => ({ ...jest.requireActual('amo/constants'),
...
describe(__filename, () => {
  ...
  const render = ({
    ...
    return defaultRender(<RatingManager addon={addon} version={version} {...props} />, {
      store,
    });
  };
  ...
  it('prompts you to rate the add-on by name', () => {
    ...
    expect(screen.getByTextAcrossTags(`How are you enjoying ${name}?`)).toBeInTheDocument();
  });
  it('dispatches fetchLatestUserReview on construction', () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(fetchLatestUserReview({
      addonId: addon.id,
  ...
  it('passes review=undefined before the saved review has loaded', () => {
    ...
    expect(screen.queryByTitle('There are no ratings yet')).not.toBeInTheDocument();
  });
  ...
  it('flashes a saving rating message', () => {
    ...
    renderWithReview();
    const flashMessage = screen.getByClassName('RatingManagerNotice-savedRating');
  ...
  it('hides a flashed rating message', () => {
    ...
    renderWithReview();
    expect(screen.getByClassName('RatingManagerNotice-savedRating')).toHaveClass('RatingManagerNotice-savedRating-hidden');
    // Shows controls.
  ...
  it('sets a custom className for RatingManagerNotice when a review exists', () => {
    renderWithReview();
    expect(screen.getByClassName('RatingManagerNotice-savedRating')).toHaveClass('RatingManager-savedRating-withReview');
  });
  it('does not set a custom className for RatingManagerNotice when no review exists', () => {
    render();
    expect(screen.getByClassName('RatingManagerNotice-savedRating')).not.toHaveClass('RatingManager-savedRating-withReview');
  });
  describe('when user is signed out', () => {
    ...
    it.each([[ADDON_TYPE_DICT, 'dictionary'], [ADDON_TYPE_EXTENSION, 'extension'], [ADDON_TYPE_LANG, 'language pack'], [ADDON_TYPE_STATIC_THEME, 'theme'], // See: https://github.com/mozilla/addons-frontend/issues/3601.
    ['xul', 'add-on'], // This simulates a future code change where a valid type is added
    // but we haven't given it a custom prompt yet.
    ['banana', 'add-on']])('renders a login prompt for a %s', (type, prompt) => {
      ...
      expect(screen.getByRole('link', {
        name: `Log in to rate this ${prompt}`,
      })).toBeInTheDocument();
    });
  });
  describe('inline features', () => {
    it('allows a user to delete a review', async () => {
      ...
      expect(screen.getByTextAcrossTags(`Are you sure you want to delete your review of ${defaultAddonName}?`)).toBeInTheDocument();
    ...
    it('prompts to delete a rating when beginningToDeleteReview', async () => {
      ...
      expect(screen.getByTextAcrossTags(`Are you sure you want to delete your rating of ${defaultAddonName}?`)).toBeInTheDocument();
    });
    ...
    it('shows and hides UserRating and prompt based on editing status', async () => {
      ...
      expect(screen.getByClassName('RatingManager-UserRating')).toBeInTheDocument();
      await userEvent.click(screen.getByRole('link', {
        name: 'Edit review',
      }));
      expect(screen.queryByClassName('RatingManager-legend')).not.toBeInTheDocument();
      expect(screen.queryByClassName('RatingManager-UserRating')).not.toBeInTheDocument();
      await userEvent.click(screen.getByRole('button', {
        name: 'Cancel',
      }));
      expect(screen.getByClassName('RatingManager-legend')).toBeInTheDocument();
  ...
  describe('Tests for UserRating', () => {
    ...
    it('passes a null review to Rating', () => {
      ...
      expect(screen.getByTextAcrossTags('Rate this add-on 1 out of 5')).toBeInTheDocument();
  ...
  describe('Tests for ReportAbuseButton', () => {
    it('allows a user to report an add-on for abuse', async () => {
      ...
      expect(screen.getByClassName('ReportAbuseButton')).not.toHaveClass('ReportAbuseButton--is-expanded');
      ...
      await userEvent.type(screen.getByPlaceholderText('Explain how this add-on is violating our policies.'), message);
      await userEvent.click(screen.getByRole('button', {
      ...
      expect(dispatch).toHaveBeenCalledWith(sendAddonAbuseReport({
      ...
      expect(screen.getByClassName('ReportAbuseButton')).toHaveClass('ReportAbuseButton--is-expanded');
    });
    ...
    it.each([ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME])('initiates an abuse report via Firefox when the "report" button is clicked if supported and add-on type is %s', (addonType) => {
      ...
      expect(preventDefaultWatcher).toHaveBeenCalled();
      expect(screen.getByClassName('ReportAbuseButton')).not.toHaveClass('ReportAbuseButton--is-expanded');
      expect(dispatch).toHaveBeenCalledWith(initiateAddonAbuseReportViaFirefox({
        addon,
      }));
    });
    it.each([ADDON_TYPE_DICT, ADDON_TYPE_LANG])('does not initiate an abuse report via Firefox when add-on type is %s', async (addonType) => {
      hasAbuseReportPanelEnabled.mockImplementation(() => {
        return true;
      });
      const addon = createInternalAddonWithLang({ ...fakeAddon,
      ...
      expect(screen.getByClassName('ReportAbuseButton')).toHaveClass('ReportAbuseButton--is-expanded');
      expect(dispatch).not.toHaveBeenCalledWith(initiateAddonAbuseReportViaFirefox({
        addon,
      }));
    });
    ...
    it('shows a success message and hides the button if report via Firefox was dispatched', () => {
      const addon = fakeAddon;
      store.dispatch(loadAddonAbuseReport({
        addon: {
          guid: addon.guid,
          id: addon.id,
          slug: addon.slug,
    ...
    it('does not disable the "Report this add-on for abuse" button if a report is in progress', () => {
      ...
      store.dispatch(initiateAddonAbuseReportViaFirefox({
        addon: fakeAddon,
      }));
      render();
      expect(screen.queryByRole('button', {
    ...
    it('does not allow dispatch if textarea is whitespace', async () => {
      ...
      await userEvent.type(screen.getByPlaceholderText('Explain how this add-on is violating our policies.'), '     ');
      await userEvent.click(screen.getByRole('button', {
        name: 'Send abuse report',
      }));
      expect(screen.queryByRole('button', {
  ...
  describe('Tests for AddonReviewManagerRating', () => {
    it('lets you specify className', async () => {
      ...
      expect(screen.getByClassName('AddonReviewManagerRating')).toHaveClass('RatingManager-AddonReviewManagerRating');
    });
