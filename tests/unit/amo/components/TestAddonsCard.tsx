...
import { ADDON_TYPE_STATIC_THEME, CLIENT_APP_ANDROID, DEFAULT_UTM_SOURCE, RECOMMENDED } from 'amo/constants';
import { createHistory, createInternalAddonWithLang, createLocalizedString, dispatchClientMetadata, fakeAddon, fakeAuthor, fakeI18n, fakePreview, render as defaultRender, screen, within } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('does not render a horizontal class by default', () => {
    render();
    expect(screen.queryByClassName('AddonsCard--horizontal')).not.toBeInTheDocument();
  });
  ...
  it('renders placeholders when loading addons', () => {
    ...
    expect(screen.getAllByRole('alert')).toHaveLength(DEFAULT_API_PAGE_SIZE * 4);
  ...
  describe('Tests for SearchResult', () => {
    ...
    const createAddon = (props) => createInternalAddonWithLang({ ...fakeAddon,
      authors: [{
    ...
    it('renders a heading with a link to the detail page', () => {
      ...
      expect(screen.getByRole('link', {
        name,
      })).toHaveAttribute('href', `/en-US/android/addon/${slug}/`);
    });
    ...
    it('links the heading to the detail page with UTM params', () => {
      ...
      const expectedLink = [`/en-US/android/addon/${slug}/?utm_source=${DEFAULT_UTM_SOURCE}`, 'utm_medium=referral', `utm_content=${addonInstallSource}`].join('&');
      expect(screen.getByRole('link', {
        name,
    ...
    it('ignores an empty author list', () => {
      ...
      expect(screen.queryByClassName('SearchResult-author')).not.toBeInTheDocument();
    });
    ...
    it('calls the custom onImpression handler, passing the addon', () => {
      ...
      renderWithResult({
        props: {
          onAddonImpression: onImpression,
        },
      });
    ...
    it('renders a loading class name while there is no addon', () => {
      ...
      expect(screen.getByClassName('SearchResult-icon--loading')).toBeInTheDocument();
    });
    it('displays the thumbnail image as the default src for static theme', () => {
      const headerImageThumb = 'https://addons.mozilla.org/user-media/thumb/12345.png';
      renderWithResult({
        addonProps: {
          type: ADDON_TYPE_STATIC_THEME,
      ...
      expect(screen.getByAltText(name)).toHaveAttribute('src', headerImageThumb);
    });
    it('displays the full preview for static theme when showFullSizePreview: true', () => {
      const headerImageFull = 'https://addons.mozilla.org/user-media/full/12345.png';
      renderWithResult({
        addonProps: {
          type: ADDON_TYPE_STATIC_THEME,
          previews: [{ ...fakePreview,
    ...
    it('displays a message if the static theme preview image is unavailable', () => {
      ...
      expect(screen.getByText('No theme preview available')).toBeInTheDocument();
    });
    it("does not display a 'no theme preview available' message if the static theme preview image is available", () => {
      ...
      expect(screen.queryByText('No theme preview available')).not.toBeInTheDocument();
    });
    ...
    it('renders newlines in notes', () => {
      ...
      expect(within(screen.getByClassName('SearchResult-note')).getByTagName('br')).toBeInTheDocument();
    });
    ...
    it('displays a promoted badge when an add-on is promoted', () => {
      ...
      expect(screen.getByClassName('PromotedBadge')).toHaveClass('PromotedBadge-small');
      expect(screen.getByClassName('IconPromotedBadge')).toHaveClass('IconPromotedBadge-small');
      expect(screen.getByRole('link', {
    ...
    it('sets an extra css class to the icon wrapper when there is no theme image', () => {
      ...
      expect(screen.getByClassName('SearchResult-icon-wrapper--no-theme-image')).toBeInTheDocument();
    });
    ...
    it('does not set an extra css class to the icon wrapper when there is no add-on and we do not want to use a theme placeholder', () => {
      ...
      expect(screen.getByClassName('SearchResult-icon-wrapper')).toBeInTheDocument();
      expect(screen.queryByClassName('SearchResult-icon-wrapper--no-theme-image')).not.toBeInTheDocument();
    });
