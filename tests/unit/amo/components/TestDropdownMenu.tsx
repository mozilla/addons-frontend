...
describe(__filename, () => {
  ...
  const getMenuButton = (name = defaultMenuText) => screen.getByRole('button', {
    name,
  });

  const getItem = () => screen.getByRole('listitem');

  const clickMenu = async (name = defaultMenuText) => userEvent.click(getMenuButton(name));
  ...
  const render = ({
    ...
    return defaultRender(<DropdownMenu _window={_window} text={text} {...props}>
        {children}
      </DropdownMenu>);
  };
  ...
  it('renders items passed as children', async () => {
    render({
      children: <DropdownMenuItem>A section</DropdownMenuItem>,
    });
    await clickMenu();
  ...
  it('sets active on mouseEnter/clears on mouseLeave', async () => {
    _window.matchMedia = jest.fn().mockReturnValue({
      matches: true,
    });
    render();
  ...
  describe('Tests for DropdownMenuItem', () => {
    ...
    it('renders a `Link` passed in children', async () => {
      ...
      await renderWithItems(<DropdownMenuItem>
      ...
      expect(screen.getByRole('link', {
        name: linkText,
      })).toHaveAttribute('href', `/en-US/android${linkHref}`);
    });
    it('can visually detach a link item', async () => {
      await renderWithItems(<DropdownMenuItem detached>
    ...
    it('renders a `button` when `onClick` prop is supplied', async () => {
      ...
      await renderWithItems(<DropdownMenuItem onClick={onClick}>{buttonText}</DropdownMenuItem>);
      const item = getItem();
      expect(item).toHaveClass('DropdownMenuItem');
      expect(item).toHaveClass('DropdownMenuItem-link');
    ...
    it('can visually detach a button item', async () => {
      const onClick = jest.fn();
      await renderWithItems(<DropdownMenuItem onClick={onClick} detached>
    ...
    it('optionally takes a class name', async () => {
      const className = 'MyClass';
      await renderWithItems(<DropdownMenuItem className={className}>A section</DropdownMenuItem>);
      const item = getItem();
      expect(item).toHaveClass('DropdownMenuItem');
      expect(item).toHaveClass('DropdownMenuItem-section');
    ...
    it('can render a disabled button', async () => {
      const buttonText = 'A button';
      const onClick = jest.fn();
      await renderWithItems(<DropdownMenuItem onClick={onClick} disabled>
          {buttonText}
        </DropdownMenuItem>);
    ...
    it('renders a button with a title', async () => {
      ...
      await renderWithItems(<DropdownMenuItem onClick={onClick} title={title}>
          A button with title
        </DropdownMenuItem>);
      expect(screen.getByRole('button', {
        name: title,
