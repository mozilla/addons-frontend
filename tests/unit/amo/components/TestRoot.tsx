...
import { createHistory, dispatchClientMetadata, fakeCookies, fakeI18n, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const mountApp = ({
    ...
    if (!store || !connectedHistory) {
      thingsFromCreateStore = createStore({
        history,
      });
    }

    return render(<Root i18n={i18n} history={connectedHistory || thingsFromCreateStore.connectedHistory} store={store || thingsFromCreateStore.store} {...props}>
        <Child />
      </Root>);
  };
  ...
  it('provides cookies capability', () => {
    ...
    const AppWithCookies = withCookies(({
      cookies,
    }) => <App>
        <p>{cookies.get()}</p>
      </App>);
    mountApp({
      Child: AppWithCookies,
      cookies: _cookies,
    });
