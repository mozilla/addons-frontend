...
import FakeApp, { fakeAssets, fakeSRIData } from 'tests/unit/amo/server/fakeApp';
import { getFakeConfig, getElement, getElements, render as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('renders meta attrs inside helmet', async () => {
    render();
    expect(getElement('meta[name="description"]')).toHaveAttribute('content', 'test meta');
  });
  it('renders script[type="application/ld+json"] inside helmet', () => {
    render();
    // This is defined in the `FakeApp` component.
    expect(getElement('script[type="application/ld+json"]')).toBeInTheDocument();
  });
  it('renders GA script when trackingEnabled is true', () => {
    render({
      trackingEnabled: true,
    });
    const ga = getElement('script[src="https://www.google-analytics.com/analytics.js"]');
  ...
  it("doesn't render GA script when trackingEnabled is false", () => {
    ...
    expect(getElements('script[src="https://www.google-analytics.com/analytics.js"]')).toHaveLength(0);
  });
  ...
  it('does not render i18n js in the assets list', () => {
    render();
    expect(getElements('script[integrity="sha512-amo-i18n-js"]')).toHaveLength(0);
  });
  ...
  it('renders state as JSON', () => {
    const appState = {
      usersExample: ['first-user', 'second-user'],
    };
    render({
      appState,
    });
  ...
  it('renders favicon', () => {
    ...
    expect(getElement('link[rel="shortcut icon"]')).toHaveAttribute('href', `/favicon.ico?v=${_config.get('faviconVersion')}`);
  });
  it('renders font link preload with SRI', () => {
    render();
    const preloaded = getElements('link[rel="preload"]')[0];
    expect(preloaded).toHaveAttribute('href', 'Inter-roman-subset-en_de_fr_ru_es_pt_pl_it.var.woff2');
