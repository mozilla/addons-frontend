...
import { createHistory, createInternalAddonWithLang, dispatchClientMetadata, fakeAddon, fakeI18n, render as defaultRender, screen, within } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const render = ({
    ...
    return defaultRender(<RatingsByStar addon={addon} {...props} />, renderOptions);
  };

  const addonForGrouping = (grouping, addonParams = {}) => {
    return createInternalAddonWithLang({ ...fakeAddon,
      ratings: { ...fakeAddon.ratings,
        // This is the sum of all grouped rating counts.
        count: Object.values(grouping).reduce((total, currentValue) => total + currentValue),
  ...
  it('renders star labels, counts and links', () => {
    ...
    function validateLink(score, count, expectedTitle) {
      ...
      expect(link).toHaveAttribute('href', `/en-US/android${reviewListURL({
  ...
  it('adds UTM query parameters to the review links when there are some', () => {
    ...
    function validateLink(score, expectedTitle) {
      const link = screen.getByTitle(expectedTitle);
      const expectedQueryString = [`score=${score}`, `utm_medium=${utm_medium}`].join('&');
      expect(link).toHaveAttribute('href', `/en-US/android/addon/${addon.slug}/reviews/?${expectedQueryString}`);
    }
