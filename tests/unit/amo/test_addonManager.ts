...
import { GLOBAL_EVENTS, GLOBAL_EVENT_STATUS_MAP, INSTALL_EVENT_LIST, ON_INSTALLING_EVENT, ON_OPERATION_CANCELLED_EVENT, ON_UNINSTALLED_EVENT, SET_ENABLE_NOT_AVAILABLE } from 'amo/constants';
...
describe(__filename, () => {
  ...
  describe('hasAddonManager', () => {
    it('is true if mozAddonManager is in navigator', () => {
      expect(addonManager.hasAddonManager({
        navigator: {
          mozAddonManager: {},
        },
      })).toBeTruthy();
  ...
  describe('hasAbuseReportPanelEnabled', () => {
    ...
    it('returns false if abuseReportPanelEnabled is false', () => {
      expect(addonManager.hasAbuseReportPanelEnabled({
  ...
  describe('getAddon()', () => {
    ...
    it('should reject if mozAddonManager.getAddonByID() resolves with falsey addon', () => {
      ...
      return addonManager.getAddon('test-id', {
        _mozAddonManager: fakeMozAddonManager,
      }).then(unexpectedSuccess, (err) => expect(err.message).toEqual('Addon not found'));
    });
    it('rejects if thre is no addon manager', () => {
      sinon.stub(addonManager, 'hasAddonManager').returns(false);
      return addonManager.getAddon('foo').then(unexpectedSuccess, (err) => expect(err.message).toEqual('Cannot check add-on status'));
    });
  });
  describe('reportAbuse()', () => {
    ...
    it('returns the result of mozAddonManager.reportAbuse()', async () => {
      ...
      fakeMozAddonManager.reportAbuse.returns(Promise.resolve(reportAbuseResult));
    ...
    it('rejects if there is no addon manager', () => {
      sinon.stub(addonManager, 'hasAddonManager').returns(false);
      return addonManager.reportAbuse('test-id').then(unexpectedSuccess, (err) => expect(err.message).toEqual('Cannot report abuse via Firefox'));
    });
    it('rejects if abuseReportPanelEnabled is false', () => {
      fakeMozAddonManager.reportAbuse.returns(Promise.resolve(true));
      return addonManager.reportAbuse('test-id', {
        _mozAddonManager: { ...fakeMozAddonManager,
          abuseReportPanelEnabled: false,
        },
      }).then(unexpectedSuccess, (err) => expect(err.message).toEqual('Cannot report abuse via Firefox'));
    });
  });
  describe('install()', () => {
    ...
    it('should call installObj.addEventListener to setup events', async () => {
      ...
      sinon.assert.callCount(fakeInstallObj.addEventListener, INSTALL_EVENT_LIST.length + 2);
    });
    ...
    it('rejects if the install fails', () => {
      ...
      return addonManager.install(fakeInstallUrl, fakeCallback, {
        _mozAddonManager: fakeMozAddonManager,
  ...
  describe('addChangeListener', () => {
    ...
    describe('global events', () => {
      ...
      beforeEach(() => {
        ...
        handleChangeEvent = addonManager.addChangeListeners(fakeEventCallback, fakeMozAddonManager);
      });
      GLOBAL_EVENTS.forEach((event) => {
        ...
        const canUninstall = [ON_INSTALLING_EVENT, ON_UNINSTALLED_EVENT].includes(event) ? true : fakeAddon.canUninstall;
    ...
    it('throws on unknown event', () => {
      const handleChangeEvent = addonManager.addChangeListeners(fakeEventCallback, fakeMozAddonManager);
      expect(() => {
        handleChangeEvent({
          type: 'whatevs',
        });
    ...
    it('listens to onOperationCancelled', () => {
      ...
      sinon.assert.calledWith(fakeMozAddonManager.addEventListener, ON_OPERATION_CANCELLED_EVENT);
    });
    it('calls the callback when onOperationCancelled is received', async () => {
      ...
      sinon.assert.calledWith(fakeEventCallback, {
        guid,
        needsRestart,
        status: addonManager.getAddonStatus({
          addon: fakeAddon,
        }),
        canUninstall,
      });
  ...
  describe('enable()', () => {
    ...
    it('should throw if addon.setEnabled does not exist', () => {
      ...
      fakeMozAddonManager.getAddonByID.returns(Promise.resolve(fakeAddon));
      return addonManager.enable('whatevs', {
        _mozAddonManager: fakeMozAddonManager,
