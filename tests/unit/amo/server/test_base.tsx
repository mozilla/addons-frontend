...
import { createUserAccountResponse, getFakeConfig, getFakeLogger, userAuthSessionId } from 'tests/unit/helpers_node';
...
function createStoreAndSagas({
  ...
  return {
    connectedHistory: createReduxHistory(store),
    sagaMiddleware,
    store,
  };
}
...
const StubApp = () => <div>
  ...
  </div>; // This is an example of implementing a NotFound component
  ...
  describe('app', () => {
    it('enables gzip compression if client sends accept-encoding', async () => {
      ...
      const response = await testClient({
        ...
        sagaMiddleware,
        store,
      }).get('/en-US/firefox/').set('Accept-Encoding', 'gzip');
    ...
    it('sets a Cache-Control header', async () => {
      ...
      expect(response.headers['cache-control']).toEqual('max-age=0, s-maxage=180');
    });
    ...
    it('dispatches setAuthToken() if cookie is present', async () => {
      ...
      const response = await testClient({
        ...
        sagaMiddleware,
        store,
      }).get('/en-US/firefox/').set('cookie', `${defaultConfig.get('cookieName')}="${token}"`);
      const {
        api,
      } = store.getState();
      expect(response.statusCode).toEqual(200);
    ...
    it('dispatches setRequestId()', async () => {
      ...
      }).get('/en-US/firefox/') // The middleware will honor a request ID header rather than
      // generate a new one.
    ...
    it('dispatches setRegionCode() when the region code header is set on the request', async () => {
      ...
      await testClient({
        ...
        store,
        sagaMiddleware,
      }).get('/en-US/firefox/').set(REGION_CODE_HEADER, regionCode);
    ...
    it('does not dispatch setRegionCode() when the region code header is not set on the request', async () => {
      ...
      sinon.assert.neverCalledWith(dispatchSpy, setRegionCode(sinon.match.string));
    });
    it('fetches the user profile when given a token', async () => {
      ...
      mockUsersApi.expects('currentUserAccount').once().returns(Promise.resolve(user));
    ...
    it('fetches the site status when there is no user token', async () => {
      ...
      mockSiteApi.expects('getSiteStatus').once().returns(Promise.resolve(siteStatus));
    ...
    it('gracefully handles site status API errors', async () => {
      mockSiteApi.expects('getSiteStatus').once().rejects(new Error('Oh no, an API error'));
    ...
    it('returns a 500 error page when retrieving the user profile fails', async () => {
      mockUsersApi.expects('currentUserAccount').once().rejects(new Error('example of an API error'));
      const token = userAuthSessionId();
      const {
        connectedHistory,
        sagaMiddleware,
    ...
    it('fetches the user profile even when SSR is disabled', async () => {
      ...
      const response = await client.get('/en-US/firefox/').set('cookie', `${defaultConfig.get('cookieName')}="${token}"`);
      const {
        api,
        users,
      } = store.getState();
    ...
    it('catches all errors and returns a 500', async () => {
      ...
      sinon.assert.calledWith(_log.error, sinon.match(/Caught an unexpected error while handling the request/));
      // Error logged in the error handler.
      sinon.assert.calledWith(_log.error, sinon.match(/Showing 500 page for error: Error: oops/));
      // Error caught while trying to render a 500 page.
      sinon.assert.calledWith(_log.error, sinon.match(/Additionally, the error handler caught an error:/));
    });
    it('catches a URIError from _createHistory and returns a 404', async () => {
      ...
      expect(response.statusCode).toEqual(404);
      sinon.assert.calledWith(_log.error, sinon.match(/Caught an error during createHistory: URIError: oops/));
    });
    it('handles requests slightly differently when loaded page is anonymous', async () => {
      const url = '/en-US/firefox/';
      const config = getFakeConfig({
        anonymousPagePatterns: [url],
      });
      const token = userAuthSessionId();
      const {
        connectedHistory,
      ...
      const response = await testClient({
        ...
        store,
        sagaMiddleware,
      }).get(url).set('cookie', `${config.get('cookieName')}="${token}"`);
      const {
        api,
        site,
      } = store.getState();
    ...
    it('does not dispatch loadedPageIsAnonymous() when loaded page is not anoynmous', async () => {
      ...
      await testClient({
        ...
        store,
        sagaMiddleware,
      }).get('/en-US/firefox/').set('cookie', `${config.get('cookieName')}="${token}"`);
      const {
        site,
      } = store.getState();
    ...
    it('removes the cookie when user has been logged out', async () => {
      ...
      expect(auth_cookie).toContain(`${defaultConfig.get('cookieName')}=; Max-Age=0;`);
    ...
    it('sets correct Cache-Control header if request contained authentication cookie', async () => {
      ...
      const response = await testClient({
        ...
        sagaMiddleware,
        store,
      }).get('/en-US/firefox/').set('cookie', `${defaultConfig.get('cookieName')}="foo"`);
      expect(response.headers).not.toContain(X_ACCEL_EXPIRES_HEADER);
      expect(response.headers['cache-control']).toEqual('max-age=0, s-maxage=0');
    });
