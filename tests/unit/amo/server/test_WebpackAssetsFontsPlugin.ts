...
describe(__filename, () => {
  ...
  it('adds .woff2 file info to assets json file', () => {
    ...
    expect(assetsAfterPlugin.assets['./font1.woff2']).toEqual('https://example.com/static/font1-somehash.woff2');
    expect(assetsAfterPlugin.assets['./font2.woff2']).toEqual('https://example.com/static/font2-somehash.woff2');
  });
  it('does not touch non fonts assets, javascript or styles', () => {
    ...
    expect(assetsAfterPlugin.assets['./app.css']).toEqual(fakeAssets.assets['./app.css']);
  });
  ...
  it('requires a webpackAssetsFileName parameter', () => {
    expect(() => new WebpackAssetsFontsPlugin()).toThrowError(/options misses the property 'webpackAssetsFileName'/);
  });
});