...
describe(__filename, () => {
  ...
  function compile({
    ...
    plugins.push(new SriDataPlugin({
      saveAs: sriFile,
    }));
    ...
    return new Promise((resolve, reject) => compiler.run((error, stats) => {
      ...
      if (stats.compilation.errors && stats.compilation.errors.length) {
        return reject(new Error(`Webpack errors: ${stats.compilation.errors.join('; ')}`));
      }
  ...
  it('requires the SriPlugin', () => {
    ...
    }).then(() => expect(false).toBe(true), (error) => {
