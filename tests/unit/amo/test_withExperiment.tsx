...
import { DEFAULT_COOKIE_MAX_AGE, EXPERIMENT_COOKIE_NAME, EXPERIMENT_ENROLLMENT_CATEGORY, EXPERIMENT_ID_GA_DIMENSION, EXPERIMENT_ID_REGEXP, EXPERIMENT_VARIATION_GA_DIMENSION, NOT_IN_EXPERIMENT, defaultCookieConfig, getVariant, withExperiment } from 'amo/withExperiment';
// eslint-disable-next-line import/namespace
import * as defaultConfig from 'config/default';
import { createExperimentData, createFakeTracking, dispatchClientMetadata, fakeCookies, getFakeConfig, makeExperimentId, render as defaultRender, screen } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const render = ({
    ...
    const id = experimentProps && experimentProps.id || makeExperimentId('some-id');
  ...
  it.each([true, false])('injects a variant prop from a cookie whether a user is excluded or not', (isExcluded) => {
  ...
  it.each([true, false])('injects a variant prop from the redux store whether a user is excluded or not', (isExcluded) => {
    const shouldExcludeUser = jest.fn().mockReturnValue(isExcluded);
    const {
      store,
    } = dispatchClientMetadata();
    ...
    store.dispatch(storeExperimentVariant({
      id,
      variant: variantId,
    }));
    render({
  ...
  it('uses an updated cookie value on re-render', () => {
    ...
    const cookies = fakeCookies({
      get: jest.fn().mockReturnValue(createExperimentData({
        id,
        variantId: originalCookieVariant,
      })),
    });
    render({
      cookies,
    ...
    expect(screen.getByText(`variant: ${originalCookieVariant}`)).toBeInTheDocument();
    render({
      cookies: fakeCookies({
        get: jest.fn().mockReturnValue(createExperimentData({
          id,
          variantId: updatedCookieVariant,
        })),
      }),
      experimentProps: {
        id,
    ...
    expect(screen.getByText(`variant: ${updatedCookieVariant}`)).toBeInTheDocument();
  });
  ...
  it('makes the variant NOT_IN_EXPERIMENT if a user should be excluded, and no variant exists (cookie or store)', () => {
    ...
    expect(screen.getByText(`variant: ${NOT_IN_EXPERIMENT}`)).toBeInTheDocument();
  });
  ...
  it('calls getVariant to set a value for a cookie upon construction if needed', () => {
    ...
    expect(_getVariant).toHaveBeenCalledWith({
      variants,
    });
    expect(cookies.set).toHaveBeenCalledTimes(1);
    expect(cookies.set).toHaveBeenCalledWith(EXPERIMENT_COOKIE_NAME, createExperimentData({
  ...
  it('does not use a stored variant if it is not for this experiment', () => {
    ...
    expect(screen.getByText(`variant: ${thisExperimentVariant}`)).toBeInTheDocument();
  });
  ...
  it('dispatches storeExperimentVariant to store the variant', () => {
    ...
    expect(fakeDispatch).toHaveBeenCalledWith(storeExperimentVariant({
      id,
      variant: variantId,
    }));
  });
  ...
  it('sends an enrollment event if the experiment is not already present in the cookie', () => {
    ...
    expect(_tracking.sendEvent).toHaveBeenCalledWith(expect.objectContaining({
      action: variantId,
  ...
  describe('getVariant', () => {
    const variants = [{
      ...
      id: NOT_IN_EXPERIMENT,
      percentage: 0.7,
    }];
    it.each([[variants[0], 0.01], [variants[0], 0.1], [variants[1], 0.11], [variants[1], 0.3], [variants[2], 0.31], [variants[2], 0.99]])('assigns variant %s based on a percent of %s', (expectedVariant, randomNumber) => {
      const randomizer = jest.fn().mockReturnValue(randomNumber);
      expect(getVariant({
        randomizer,
        variants,
      })).toEqual(expectedVariant.id);
