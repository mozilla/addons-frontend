...
import { ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME, CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX, RECOMMENDED } from 'amo/constants';
import { addVersionCompatibilityToFilters, convertFiltersToQueryParams, convertQueryParamsToFilters, fixFiltersForClientApp, fixFiltersFromLocation, generateThresholdParams, paramsToFilter } from 'amo/searchUtils';
import { dispatchClientMetadata, getFakeConfig, userAgentsByPlatform } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  describe('fixFiltersForClientApp', () => {
    it('adds a clientApp filter if one does not exist', () => {
      ...
      const filters = {};
      const newFilters = fixFiltersForClientApp({
        api: state.api,
        filters,
      });
  ...
  describe('fixFiltersFromLocation', () => {
    it('removes clientApp and lang from filters', () => {
      ...
      expect(fixFiltersFromLocation(filters)).toEqual({
        page,
      });
    });
  });
  ...
  describe('paramsToFilter', () => {
    it('generates all the required threshold parameters for ratings and users', () => {
      expect(paramsToFilter).toMatchObject({
        ratings__gt: 'ratings__gt',
      });
      expect(paramsToFilter).toMatchObject({
        users__lte: 'users__lte',
      });
      expect(paramsToFilter).toMatchObject({
        users: 'users',
      });
