import { CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX, DOWNLOAD_FAILED, ERROR_CORRUPT_FILE, FATAL_ERROR, FATAL_INSTALL_ERROR, FATAL_UNINSTALL_ERROR, INSTALL_FAILED, OS_ALL, RECOMMENDED, SPONSORED, SPOTLIGHT, STRATEGIC, VERIFIED } from 'amo/constants';
import { createFakeAutocompleteResult, createInternalAddonWithLang, createInternalSuggestionWithLang, createInternalVersionWithLang, fakeAddon, fakeI18n, fakeVersion } from 'tests/unit/helpers';
import { getAddonJsonLinkedData, getErrorMessage, getFileHash, getPromotedCategory } from 'amo/utils/addons';

describe(__filename, () => {
  describe('getErrorMessage', () => {
    it.each([[INSTALL_FAILED, 'Installation failed'], [DOWNLOAD_FAILED, 'Download failed'], [ERROR_CORRUPT_FILE, 'Installation aborted because the add-on appears to be corrupt.'], [FATAL_INSTALL_ERROR, 'error occurred during installation'], [FATAL_UNINSTALL_ERROR, 'error occurred during uninstallation'], [FATAL_ERROR, 'unexpected error occurred.'], ['SOME OTHER ERROR', 'unexpected error occurred.']])('returns a readable message for %s', (error, message) => {
      expect(getErrorMessage({
        i18n: fakeI18n(),
        error,
      })).toMatch(new RegExp(message));
    });
  });
  ...
  describe('getPromotedCategory', () => {
    it('returns null if the addon is not promoted', () => {
      ...
      const suggestion = createInternalSuggestionWithLang(createFakeAutocompleteResult({
        promoted: null,
      }));
      expect(getPromotedCategory({
        addon,
    ...
    describe('forBadging === true', () => {
      it.each([SPOTLIGHT, STRATEGIC])('returns null if the category is not one for badges, category: %s', (category) => {
        const addon = createInternalAddonWithLang({ ...fakeAddon,
          promoted: {
            category,
            apps: [CLIENT_APP_ANDROID],
