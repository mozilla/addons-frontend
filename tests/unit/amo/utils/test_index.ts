...
import { ADDON_TYPE_EXTENSION, ADDON_TYPE_STATIC_THEME, CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX, DEFAULT_UTM_MEDIUM, DEFAULT_UTM_SOURCE, DOWNLOAD_FIREFOX_UTM_CAMPAIGN, PROMOTED_ADDONS_SUMO_URL } from 'amo/constants';
import { addQueryParamsToHistory, apiAddonType, apiAddonTypeIsValid, checkInternalURL, convertBoolean, getAddonURL, getCanonicalURL, getClientApp, getClientConfig, getClientAppAndLangFromPath, getPromotedBadgesLinkUrl, isAddonAuthor, isValidClientApp, makeQueryStringWithUTM, nl2br, normalizeFileNameId, removeProtocolFromURL, safePromise, sanitizeHTML, sanitizeUserHTML, stripLangFromAmoUrl, visibleAddonType } from 'amo/utils';
import { createFakeHistory, createFakeLocation, createInternalAddonWithLang, fakeAddon, getFakeConfig, unexpectedSuccess, userAgents } from 'tests/unit/helpers';
...
describe(__filename, () => {
  describe('getCanonicalURL', () => {
    it(`returns an absolute canonical URL`, () => {
      ...
      expect(getCanonicalURL({
        _config,
        locationPathname,
      })).toEqual(`${baseURL}${locationPathname}`);
    });
  });
  ...
  describe('checkInternalURL', () => {
    ...
    it('strips the host name for a full URL containing the current host', () => {
      const baseURL = 'https://example.org';
      const urlString = url.format({ ...url.parse(baseURL),
        pathname,
      });
      expect(checkInternalURL({
        _config: getFakeConfig({
          baseURL,
        }),
        urlString,
      }).relativeURL).toEqual(pathname);
    });
    ...
    it('does not strip the host name for a different host', () => {
      ...
      const urlString = url.format({ ...url.parse(otherBaseURL),
        pathname,
      });
      expect(checkInternalURL({
        _config: getFakeConfig({
    ...
    describe('isInternal prop', () => {
      ...
      it('returns false for an subdomain of the current host', () => {
        ...
        const urlString = url.format({ ...url.parse(subdomainBaseURL),
          pathname,
        });
        expect(checkInternalURL({
          _config: getFakeConfig({
      ...
      it('returns false if the current host is a subdomain of the proposed URLs host', () => {
        ...
        const urlString = url.format({ ...url.parse(proposedBaseURL),
          pathname,
        });
        expect(checkInternalURL({
          _config: getFakeConfig({
  ...
  describe('getPromotedBadgesLinkUrl', () => {
    it('returns a URL with utm_content specified', () => {
      ...
      expect(getPromotedBadgesLinkUrl({
        utm_content,
      })).toEqual(`${PROMOTED_ADDONS_SUMO_URL}${makeQueryStringWithUTM({
        utm_campaign: null,
        utm_content,
      })}`);
    });
  ...
  describe('stripLangFromAmoUrl', () => {
    ...
    it('should strip the lang if the link is internal', () => {
      ...
      expect(stripLangFromAmoUrl({
        _checkInternalURL,
        urlString,
      })).toEqual('https://somehost/somepath/');
    });
    ...
    it('maintains the querystring', () => {
      ...
      expect(stripLangFromAmoUrl({
        _checkInternalURL,
        urlString,
      })).toEqual('https://somehost/somepath/?a=b');
    });
    ...
    it('only strips the first instance of a lang', () => {
      ...
      expect(stripLangFromAmoUrl({
        _checkInternalURL,
        urlString,
      })).toEqual(`https://somehost/${validLang}/somepath/`);
    });
  });
  ...
  describe('isValidClientApp', () => {
    ...
    _config.set('validClientApplications', [CLIENT_APP_FIREFOX, CLIENT_APP_ANDROID]);

    it('should be valid if passed "firefox"', () => {
      expect(isValidClientApp(CLIENT_APP_FIREFOX, {
        _config,
      })).toEqual(true);
    });
    it('should be valid if passed "android"', () => {
      expect(isValidClientApp(CLIENT_APP_ANDROID, {
        _config,
      })).toEqual(true);
    });
    it('should be invalid if passed "whatever"', () => {
      expect(isValidClientApp('whatever', {
        _config,
      })).toEqual(false);
    });
  });
  describe('nl2br', () => {
    ...
    it('preserves line breaks between ul/li tags', () => {
      const htmlValue = '<ul>\n<li><strong></strong>\n</li>\n</ul>';
      expect(nl2br(htmlValue)).toEqual('<ul>\n<li><strong></strong>\n</li>\n</ul>');
    });
    it('preserves line breaks between ol/li tags', () => {
      const htmlValue = '<ol>\n<li><strong></strong>\n</li>\n</ol>';
      expect(nl2br(htmlValue)).toEqual('<ol>\n<li><strong></strong>\n</li>\n</ol>');
    });
    ...
    it('returns valid HTML', () => {
      const htmlValue = 'ul\nli<strong>foo\nbar</strong>';
      expect(nl2br(htmlValue)).toEqual('ul<br />li<strong>foo<br />bar</strong>');
    });
    it('converts line breaks between tags', () => {
      const htmlValue = '<strong>A title:</strong>\n<a href="something">A link</a>\nSome text';
      expect(nl2br(htmlValue)).toEqual('<strong>A title:</strong><br /><a href="something">A link</a><br />Some text');
    });
  });
  ...
  describe('sanitizeUserHTML', () => {
    ...
    it('allows some tags', () => {
      const customHtml = '<b>check</b> <i>out</i> <a href="http://mysite">my site</a>';
      expect(sanitize(customHtml)).toEqual(customHtml);
    });
    it('does not allow certain tags', () => {
      expect(sanitize('<b>my add-on</b> <script>alert("does XSS")</script>')).toEqual('<b>my add-on</b> ');
    });
  ...
  describe('normalizeFileNameId', () => {
    it('returns a path relative to the project root directory', () => {
      expect(normalizeFileNameId('/path/to/src/foo/index.js')).toEqual('src/foo/index.js');
    });
  ...
  describe('addQueryParamsToHistory', () => {
    it('adds a query object to history.location', () => {
      ...
      const historyWithQueryParams = addQueryParamsToHistory({
        history,
      });
  ...
  describe('getClientAppAndLangFromPath', () => {
    ...
    const lang = 'en-US';
    it.each([`/${lang}/${clientApp}/`, `/${lang}/${clientApp}/extensions/`, `/${lang}/${clientApp}/search/?q=test`])('extracts a lang and clientApp from a URL: %s', (urlString) => {
  ...
  describe('makeQueryStringWithUTM', () => {
    it('adds additional query params if provided', () => {
      ...
      expect(makeQueryStringWithUTM({
        anotherParam,
      })).toEqual(`?anotherParam=some-other-param&utm_campaign=${DOWNLOAD_FIREFOX_UTM_CAMPAIGN}&utm_medium=${DEFAULT_UTM_MEDIUM}&utm_source=${DEFAULT_UTM_SOURCE}`);
    });
  });
});