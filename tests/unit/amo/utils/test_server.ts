...
describe(__filename, () => {
  describe('viewFrontendVersionHandler', () => {
    ...
    it('exposes the version.json file', (done) => {
      ...
      const handler = viewFrontendVersionHandler({
        _config,
      });
  ...
  describe('viewHeartbeatHandler', () => {
    it('calls the site API and returns 200 if successful', async () => {
      ...
      const handler = viewHeartbeatHandler({
        _config,
        _fetch,
      });
      const res = new MockExpressResponse();
      await handler(null, res);
      expect(_fetch).toHaveBeenCalledWith(`${apiHost}${apiPath}${apiVersion}/site/?disable_caching`);
      expect(res.statusCode).toEqual(200);
    });
