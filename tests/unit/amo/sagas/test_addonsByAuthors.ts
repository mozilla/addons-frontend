...
import addonsByAuthorsReducer, { EXTENSIONS_BY_AUTHORS_PAGE_SIZE, THEMES_BY_AUTHORS_PAGE_SIZE, fetchAddonsByAuthors, loadAddonsByAuthors } from 'amo/reducers/addonsByAuthors';
import addonsByAuthorsSaga from 'amo/sagas/addonsByAuthors';
import { ADDON_TYPE_STATIC_THEME, SEARCH_SORT_POPULAR, SEARCH_SORT_TRENDING } from 'amo/constants';
...
describe(__filename, () => {
  ...
  it('calls the API with addonType if set', async () => {
    ...
    mockApi.expects('search').withArgs({
      api: state.api,
      filters: {
        addonType: ADDON_TYPE_STATIC_THEME,
        author: authorIds.sort().join(','),
        exclude_addons: undefined,
        // `callApi` will internally unset this
        page: '1',
        page_size: pageSize,
        sort: SEARCH_SORT_TRENDING,
      },
    }).once().returns(Promise.resolve(createAddonsApiResult(addons)));
