...
import blocksReducer, { abortFetchBlock, fetchBlock, loadBlock } from 'amo/reducers/blocks';
import blocksSaga from 'amo/sagas/blocks';
import { createApiError } from 'amo/api';
import apiReducer from 'amo/reducers/api';
import { createFakeBlockResult, createStubErrorHandler, dispatchClientMetadata } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  describe('fetchBlock', () => {
    function _fetchBlock(params) {
      sagaTester.dispatch(fetchBlock({
    ...
    it('calls the API to fetch a block by GUID', async () => {
      ...
      mockBlocksApi.expects('getBlock').withArgs({
        apiState: state.api,
        guid,
      }).resolves(block);
    ...
    it('aborts fetching for a failed block fetch', async () => {
      ...
      const abortAction = abortFetchBlock({
        guid,
      });
