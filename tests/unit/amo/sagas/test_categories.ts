...
import categoriesReducer, { LOAD_CATEGORIES, fetchCategories, loadCategories } from 'amo/reducers/categories';
...
describe(__filename, () => {
  ...
  it('should get Api from state then make API request to categories', async () => {
    ...
    const results = [];
    mockApi.expects('getCategories').once().withArgs({
    ...
    expect(calledActions[1]).toEqual(loadCategories({
      results,
    }));
    mockApi.verify();
  });
  ...
  it('should respond to all FETCH_CATEGORIES actions', async () => {
    const mockApi = sinon.mock(api);
    mockApi.expects('getCategories').twice().withArgs({
      api: { ...initialState.api,
      },
