...
import languageToolsReducer, { fetchLanguageTools, loadLanguageTools } from 'amo/reducers/languageTools';
import apiReducer from 'amo/reducers/api';
import languageToolsSaga from 'amo/sagas/languageTools';
import { createFakeLanguageTool, createStubErrorHandler, dispatchClientMetadata } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('calls the API for language tools', async () => {
    ...
    mockApi.expects('languageTools').once().withArgs({
      api: sagaTester.getState().api,
