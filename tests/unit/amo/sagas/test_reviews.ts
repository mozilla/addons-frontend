...
import { ABORTED, SAVED_RATING, SAVED_REVIEW, SET_ADDON_REVIEWS, STARTED_SAVE_RATING, STARTED_SAVE_REVIEW, unloadAddonReviews, createAddonReview, createInternalReview, deleteAddonReview, fetchLatestUserReview, fetchReview, fetchReviewPermissions, fetchReviews, fetchUserReviews, flagReview, hideEditReviewForm, hideFlashedReviewMessage, hideReplyToReviewForm, sendReplyToReview, setAddonReviews, flashReviewMessage, setReview, setReviewPermissions, setReviewReply, setLatestReview, setReviewWasFlagged, setUserReviews, updateAddonReview, updateRatingCounts } from 'amo/actions/reviews';
import { REVIEW_FLAG_REASON_OTHER, REVIEW_FLAG_REASON_SPAM } from 'amo/constants';
...
import { apiResponsePage, createExternalReview, createStubErrorHandler, dispatchSignInActions, fakeAddon, fakeReview, matchingSagaAction } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  describe('fetchReviews', () => {
    ...
    it('fetches reviews from the API', async () => {
      ...
      mockApi.expects('getReviews').once().withArgs({
        addon: fakeAddon.slug,
        apiState,
        page: '1',
        score: undefined, // note: no page_size parameter - we want/expect the default

      }).resolves(apiResponsePage({
      ...
      mockApi.verify();
      expect(action).toEqual(setAddonReviews({
        addonSlug: fakeAddon.slug,
        page: '1',
    ...
    it('can filter by rating score', async () => {
      const score = 5;
      mockApi.expects('getReviews').withArgs(sinon.match({
        score,
      })).resolves(apiResponsePage({
        results: [fakeReview],
      }));
  ...
  describe('fetchReviewPermissions', () => {
    function _fetchReviewPermissions(params = {}) {
      sagaTester.dispatch(fetchReviewPermissions({
        errorHandlerId: errorHandler.id,
        addonId: 5432,
        userId: 98432,
        ...params,
  ...
  describe('handleReplyToReview', () => {
    ...
    it('sends a reply to the review with the API', async () => {
      ...
      mockApi.expects('replyToReview').once().withArgs({
        apiState,
        body,
        originalReviewId,
        title,
    ...
    it('loads a review from the reply response', async () => {
      ...
      mockApi.expects('replyToReview').returns(Promise.resolve(reviewFromResponse));
    ...
    it('clears the error handler', async () => {
      ...
      mockApi.expects('replyToReview').returns(Promise.resolve(createReplyToReviewResponse()));
  ...
  describe('handleFlagReview', () => {
    ...
    it('posts a review flag to the API', async () => {
      ...
      mockApi.expects('flagReview').once().withArgs({
        apiState,
        note,
        reason,
        reviewId,
  ...
  describe('fetchUserReviews', () => {
    function _fetchUserReviews(params = {}) {
      sagaTester.dispatch(fetchUserReviews({
        errorHandlerId: errorHandler.id,
        userId: 123,
        ...params,
      }));
  ...
  describe('manageAddonReview', () => {
    function _createAddonReview(params = {}) {
      sagaTester.dispatch(createAddonReview({
        addonId: 54321,
    ...
    function _updateAddonReview({
      oldReview = createExternalReview({
        id: 88664,
      }),
      ...params
    } = {}) {
      invariant(!params.reviewId, 'Cannot set reviewId directly; set oldReview instead');
      sagaTester.dispatch(setReview(oldReview));
      sagaTester.dispatch(updateAddonReview({
    ...
    function matchMessage(expectedAction) {
      return (maybeAction) => {
        return maybeAction.type === expectedAction.type && maybeAction.payload.message === expectedAction.payload.message;
      };
    }
    ...
    it('creates an add-on review', async () => {
      ...
      mockApi.expects('submitReview').once().withArgs({
        addonId,
        apiState,
        body,
        score,
    ...
    it('dispatches updateRatingCounts after creating a review', async () => {
      const addonId = 98767;
      const submittedReview = createExternalReview({
        addonId,
        id: 987,
      });
      ...
      expect(sagaTester.getCalledActions().filter((a) => a.type === action.type)).toHaveLength(1);
    });
    ...
    it('dispatches updateRatingCounts after updating', async () => {
      ...
      const newReview = createExternalReview({ ...oldReviewParams,
        body: newBody,
      });
    ...
    it('hides the review form after successful update', async () => {
      ...
      mockApi.expects('submitReview').resolves(createExternalReview({
        body: newBody,
        id: oldReview.id,
      }));
      ...
      const expectedAction = hideEditReviewForm({
        reviewId: oldReview.id,
      });
      const action = await sagaTester.waitFor(expectedAction.type);
      expect(action).toEqual(expectedAction);
    ...
    it('hides the review form after successful creation', async () => {
      ...
      _createAddonReview({
        body: 'This is an essential add-on',
      });
    ...
    it('does not hide the review form when only saving a rating', async () => {
      ...
      const exampleHideAction = hideEditReviewForm({
        reviewId: oldReview.id,
      });
  ...
  describe('deleteAddonReview', () => {
    function _deleteAddonReview(params = {}) {
      sagaTester.dispatch(deleteAddonReview({
    ...
    function matchReviewId(expectedAction) {
      return (maybeAction) => {
        return maybeAction.type === expectedAction.type && maybeAction.payload.reviewId === expectedAction.payload.reviewId;
      };
    }
    ...
    it('deletes an add-on review', async () => {
      ...
      mockApi.expects('deleteReview').once().withArgs({
        apiState,
        reviewId,
      }).returns(Promise.resolve());
    ...
    it('clears reviews for an add-on review reply', async () => {
      ...
      const action = await matchingSagaAction(sagaTester, matchReviewId(expectedAction));
      expect(action).toEqual(expectedAction);
    });
  ...
  describe('fetchReview', () => {
    function _fetchReview(params = {}) {
      sagaTester.dispatch(fetchReview({
        errorHandlerId: errorHandler.id,
        reviewId: 1,
        ...params,
      }));
    ...
    it('fetches an add-on review', async () => {
      const reviewId = 12345;
      const review = fakeReview;
      mockApi.expects('getReview').once().withArgs({
        apiState,
        reviewId,
      }).resolves(review);
  ...
  describe('fetchLatestUserReview', () => {
    function _fetchLatestUserReview(params = {}) {
      sagaTester.dispatch(fetchLatestUserReview({
        addonId: fakeAddon.id,
    ...
    it('fetches and sets the latest user review', async () => {
      ...
      mockApi.expects('getLatestUserReview').withArgs({
        apiState,
        user: userId,
        addon: addonId,
      }).resolves(review);
      ...
      const setLatestAction = await matchingSagaAction(sagaTester, (a) => a.type === expectedSetLatestAction.type);
