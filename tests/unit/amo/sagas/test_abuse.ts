...
import abuseReducer, { finishAddonAbuseReportViaFirefox, initiateAddonAbuseReportViaFirefox, loadAddonAbuseReport, sendAddonAbuseReport } from 'amo/reducers/abuse';
...
import { createFakeAddonAbuseReport, createStubErrorHandler, dispatchSignInActions, fakeAddon } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  describe('sendAddonAbuseReport', () => {
    ...
    it('calls the API for abuse', async () => {
      ...
      const response = createFakeAddonAbuseReport({
        addon,
        message,
      });
  ...
  describe('initiateAddonAbuseReportViaFirefox', () => {
    function _initiateAddonAbuseReportViaFirefox(params) {
      sagaTester.dispatch(initiateAddonAbuseReportViaFirefox({
        addon: fakeAddon,
        ...params,
      }));
    }
    ...
    it('calls reportAbuse Firefox API', async () => {
      ...
      mockAddonManager.expects('reportAbuse').withArgs(addon.guid).resolves(false);
