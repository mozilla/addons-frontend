...
import addonsReducer, { fetchAddon, fetchAddonInfo, loadAddonInfo, loadAddon } from 'amo/reducers/addons';
import apiReducer from 'amo/reducers/api';
import addonsSaga from 'amo/sagas/addons';
import { createStubErrorHandler, dispatchSignInActions, fakeAddon, createFakeAddonInfo } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  describe('fetchAddon', () => {
    ...
    it('fetches an addon from the API', async () => {
      mockApi.expects('fetchAddon').once().withArgs({
  ...
  describe('fetchAddonInfo', () => {
    ...
    function _fetchAddonInfo(slug = 'some-slug') {
      sagaTester.dispatch(fetchAddonInfo({
    ...
    it('calls the API to fetch info', async () => {
      ...
      mockApi.expects('getAddonInfo').withArgs({
        api: state.api,
        slug,
      }).once().resolves(createFakeAddonInfo());
