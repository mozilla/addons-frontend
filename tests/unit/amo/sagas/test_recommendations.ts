...
import recommendationsReducer, { abortFetchRecommendations, fetchRecommendations, loadRecommendations } from 'amo/reducers/recommendations';
import recommendationsSaga from 'amo/sagas/recommendations';
import apiReducer from 'amo/reducers/api';
import { createStubErrorHandler, fakeAddon, dispatchClientMetadata } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('calls the API to fetch recommendations', async () => {
    ...
    mockApi.expects('getRecommendations').withArgs({
      api: state.api,
      guid,
      recommended: true,
    }).once().returns(Promise.resolve(recommendations));
  ...
  it('dispatches an error and aborts the fetching', async () => {
    ...
    expect(sagaTester.getCalledActions()[3]).toEqual(abortFetchRecommendations({
      guid,
    }));
  });
});