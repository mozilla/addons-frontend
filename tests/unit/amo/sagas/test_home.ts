...
import { MOBILE_HOME_PAGE_EXTENSION_COUNT, ADDON_TYPE_EXTENSION, RECOMMENDED, SEARCH_SORT_RANDOM } from 'amo/constants';
import homeReducer, { abortFetchHomeData, fetchHomeData, loadHomeData } from 'amo/reducers/home';
...
import { createAddonsApiResult, createHomeShelves, createStubErrorHandler, dispatchClientMetadata, fakeAddon } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  describe('fetchHomeData', () => {
    function _fetchHomeData(params) {
      sagaTester.dispatch(fetchHomeData({
    ...
    it('calls the API to fetch the data to display on home for the desktop site', async () => {
      ...
      mockShelvesApi.expects('getHomeShelves').withArgs(baseApiArgs).resolves(homeShelves);
    ...
    it('calls the APIs to fetch the data to display on home for the mobile site', async () => {
      ...
      mockSearchApi.expects('search').withArgs({ ...baseApiArgs,
        filters: {
