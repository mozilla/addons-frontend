...
import usersReducer, { abortUnsubscribeNotification, deleteUserAccount, deleteUserPicture, fetchUserAccount, fetchUserNotifications, finishUnsubscribeNotification, finishUpdateUserAccount, loadCurrentUserAccount, loadUserAccount, loadUserNotifications, logOutUser, unloadUserAccount, unsubscribeNotification, updateUserAccount } from 'amo/reducers/users';
...
import { createApiResponse, createStubErrorHandler, createUserAccountResponse, createUserNotificationsResponse, dispatchClientMetadata, userAuthSessionId } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  describe('loadCurrentUserAccount', () => {
    it('calls the API to fetch user profile after setAuthToken()', async () => {
      ...
      mockApi.expects('currentUserAccount').once().returns(Promise.resolve(user));
      sagaTester.dispatch(setAuthToken(userAuthSessionId()));
      const expectedLoadUserAction = loadCurrentUserAccount({
        user,
      });
      const firstCalledAction = await sagaTester.waitFor(expectedLoadUserAction.type);
      expect(firstCalledAction).toEqual(expectedLoadUserAction);
      const expectedLoadSiteStatusAction = loadSiteStatus({
        readOnly,
        notice,
      });
      const secondCalledAction = await sagaTester.waitFor(expectedLoadSiteStatusAction.type);
    ...
    it.skip('allows exceptions to be thrown', () => {
      ...
      mockApi.expects('currentUserAccount').returns(Promise.reject(expectedError));
      sagaTester.dispatch(setAuthToken(userAuthSessionId()));
    ...
    it.skip('throws api errors when status is not 401', async () => {
      const expectedError = createApiError({
        response: {
          status: 400,
        },
      });
  ...
  describe('updateUserAccount', () => {
    it('calls the API to update a user after updateUserAccount()', async () => {
      ...
      mockApi.expects('updateUserAccount').once().returns(Promise.resolve({ ...user,
        ...userFields,
      }));
      ...
      const errorClearingAction = await sagaTester.waitFor(expectedErrorClearingAction.type);
    ...
    it('can receive a picture file and picture data in payload', async () => {
      ...
      mockApi.expects('updateUserAccount').withArgs({
        api: state.api,
        userId: user.id,
        picture,
        ...userFields,
      }).once().returns(Promise.resolve({ ...user,
        ...userFields,
      }));
      sagaTester.dispatch(updateUserAccount({
        errorHandlerId: errorHandler.id,
    ...
    it('can receive a non-empty notifications object (dict) in payload', async () => {
      ...
      mockApi.expects('updateUserNotifications').withArgs({
        api: state.api,
        notifications,
        userId: user.id,
      }).once().returns(Promise.resolve(allNotifications));
      sagaTester.dispatch(updateUserAccount({
        errorHandlerId: errorHandler.id,
        notifications,
        picture: null,
    ...
    it('overrides the "announcements" notification if updated', async () => {
      ...
      const currentNotifications = createUserNotificationsResponse().map((notification) => {
      ...
      mockApi.expects('updateUserNotifications').withArgs({
        ...
        userId: user.id,
      }).once() // The API returns the currentNotifications because the Basket
  ...
  describe('deleteUserPicture', () => {
    it('calls the API to delete a profile picture', async () => {
      ...
      mockApi.expects('deleteUserPicture').once().withArgs({
        api: state.api,
        userId: user.id,
  ...
  describe('fetchUserNotifications', () => {
    it('calls the API to fetch the notifications of a user', async () => {
      ...
      sagaTester.dispatch(loadCurrentUserAccount({
        user,
      }));
      const notifications = createUserNotificationsResponse();
      mockApi.expects('userNotifications').once().returns(Promise.resolve(notifications));
  ...
  describe('deleteUserAccount', () => {
    it('calls the API to delete a profile', async () => {
      const state = sagaTester.getState();
      const userId = 123;
      mockApi.expects('deleteUserAccount').once().withArgs({
        api: state.api,
        userId,
      ...
      const expectedCalledAction = unloadUserAccount({
        userId,
      });
      const calledAction = await sagaTester.waitFor(expectedCalledAction.type);
      expect(calledAction).toEqual(expectedCalledAction);
  ...
  describe('unsubscribeNotification', () => {
    ...
    it('calls the API to unsubscribe a user from a notification', async () => {
      const state = sagaTester.getState();
      mockApi.expects('unsubscribeNotification').once().withArgs({
        api: state.api,
        hash,
        notification,
        token,
      }).returns(createApiResponse());
      sagaTester.dispatch(unsubscribeNotification({
        errorHandlerId: errorHandler.id,
        hash,
        notification,
        token,
      ...
      const firstCalledAction = await sagaTester.waitFor(expectedFirstAction.type);
      ...
      const secondCalledAction = await sagaTester.waitFor(expectedSecondAction.type);
