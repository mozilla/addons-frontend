...
import searchReducer, { SEARCH_LOADED, abortSearch, searchStart } from 'amo/reducers/search';
import searchSaga from 'amo/sagas/search';
import { createStubErrorHandler, dispatchSignInActions } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('searches the API for add-ons', async () => {
    ...
    mockApi.expects('search').once().returns(Promise.resolve({
      results: [],
      count: 0,
    }));
