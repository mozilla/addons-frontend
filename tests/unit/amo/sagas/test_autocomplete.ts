...
import autocompleteReducer, { autocompleteCancel, autocompleteStart, autocompleteLoad } from 'amo/reducers/autocomplete';
...
import { createFakeAutocompleteResult, createStubErrorHandler, dispatchSignInActions } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('calls the API for suggestions', async () => {
    ...
    mockApi.expects('autocomplete').once().returns(Promise.resolve({
      results,
    }));
    ...
    const expectedLoadAction = autocompleteLoad({
      results,
    });
  ...
  it('clears the error handler', async () => {
    ...
    expect(sagaTester.getCalledActions()[1]).toEqual(errorHandler.createClearingAction());
  });
  ...
  it('cancels the fetch saga when receiving AUTOCOMPLETE_CANCELLED', async () => {
    mockApi.expects('autocomplete').once() // Add a delay to the API call so that it slows down the fetch saga,
  ...
  it('can call the API for suggestions even after a cancellation', async () => {
    ...
    autocompleteApi.onCall(0) // Add a delay to the API call so that it slows down the fetch saga,
    // allowing the `autocompleteCancel()` to be handled. The delay does not
    // really matter since cancellation is expected as soon as
    // AUTOCOMPLETE_CANCELLED is fired.
    .returns(new Promise((resolve) => setTimeout(resolve, 500)));
    // This configures the API for the second autocomplete start.
    autocompleteApi.onCall(1).returns(Promise.resolve({
      results,
    }));
