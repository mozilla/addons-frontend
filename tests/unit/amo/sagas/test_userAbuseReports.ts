...
import userAbuseReportsReducer, { abortUserAbuseReport, loadUserAbuseReport, sendUserAbuseReport } from 'amo/reducers/userAbuseReports';
import apiReducer from 'amo/reducers/api';
import userAbuseReportsSaga from 'amo/sagas/userAbuseReports';
import { createFakeUserAbuseReport, createStubErrorHandler, createUserAccountResponse, dispatchSignInActions } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('resets the state when an error occurs', async () => {
    ...
    const abortAction = abortUserAbuseReport({
      userId,
    });
