...
describe(__filename, () => {
  ...
  it.each(acceptedRegions)(
    'includes users with a region code of: %s on desktop',
    (regionCode) => {
  ...
  it.each(acceptedRegions)(
    'excludes users with a locale of: %s on android',
    (regionCode) => {
      const { state } = dispatchClientMetadata({
  ...
  it.each(['CN', 'IN', 'BR'])(
    'excludes users with a locale of: %s on desktop',
    (regionCode) => {
      const { state } = dispatchClientMetadata({
        clientApp: CLIENT_APP_FIREFOX,
        regionCode,
