...
describe(__filename, () => {
  ...
  describe('formatFilesize', () => {
    ...
    it.each([['B', 123, '123'], ['KB', 1234, '1.21'], ['MB', 1234567, '1.18'], ['GB', 1234567890, '1.15'], ['TB', 1234567890123, '1.12']])('calls i18n.sprintf with the expected substitution for size %s', (sizeName, size, localizedSize) => {
      const i18n = fakeI18n();
      ...
      expect(i18n.sprintf).toHaveBeenCalledWith(`%(localizedSize)s ${sizeName}`, {
        localizedSize,
      });
    });
  });
  describe('replaceStringsWithJSX', () => {
    it('lets you replace format strings with JSX', () => {
      expect(utils.replaceStringsWithJSX({
        text: 'Click on %(redLinkStart)sred%(redLinkEnd)s or %(blueLinkStart)sblue%(blueLinkEnd)s, your choice',
        replacements: [['redLinkStart', 'redLinkEnd', (text) => <Link key="red" to="/red">
                  {text}
                </Link>], ['blueLinkStart', 'blueLinkEnd', (text) => <Link key="blue" to="/blue">
                  {text}
                </Link>]],
      })).toEqual(['Click on ', <Link key="red" to="/red">
          red
        </Link>, ' or ', <Link key="blue" to="/blue">
          blue
        </Link>, ', your choice']);
    });
    it('lets you replace format strings with JSX in any order', () => {
      expect(utils.replaceStringsWithJSX({
        ...
        replacements: [['redLinkStart', 'redLinkEnd', (text) => {
          return <Link key="red" to="/red">
                    {text}
                  </Link>;
        }], ['blueLinkStart', 'blueLinkEnd', (text) => {
          return <Link key="blue" to="/blue">
                    {text}
                  </Link>;
        }]],
      })).toEqual(['Click on ', <Link key="blue" to="/blue">
          blue
        </Link>, ' or ', <Link key="red" to="/red">
          red
        </Link>, ', your choice']);
    });
    ...
    it('throws an error when not all replacements have been used', () => {
      expect(() => {
        utils.replaceStringsWithJSX({
          ...
          replacements: [['startA', 'endA', (text) => text], ['startB', 'endB', (text) => text]],
        });
      }).toThrow(/Not all replacements have been used; unused keys: startA,endA; startB,endB/);
    });
    it('throws an error when `replacements` has duplicated keys', () => {
      expect(() => {
        utils.replaceStringsWithJSX({
          ...
          replacements: [['startA', 'endA', (text) => text], ['startA', 'endA', (text) => text]],
        });
