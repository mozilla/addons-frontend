...
describe(__filename, () => {
  ...
  it('stores the auth token', () => {
    ...
    expect(api({
      foo: 'bar',
    }, actions.setAuthToken(token))).toEqual({
      foo: 'bar',
      token,
    });
  });
  ...
  it('stores the clientApp', () => {
    ...
    expect(api(existingState, actions.setClientApp(clientApp))).toEqual({ ...existingState,
      clientApp,
    });
  });
  it('stores the userAgent', () => {
    ...
    expect(api(existingState, actions.setUserAgent(userAgent))).toEqual({ ...existingState,
      userAgent,
      userAgentInfo: {
        browser,
        device,
