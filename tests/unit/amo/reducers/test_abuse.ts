import abuseReducer, { SEND_ADDON_ABUSE_REPORT, finishAddonAbuseReportViaFirefox, hideAddonAbuseReportUI, initiateAddonAbuseReportViaFirefox, initialState, loadAddonAbuseReport, sendAddonAbuseReport, showAddonAbuseReportUI } from 'amo/reducers/abuse';
import { dispatchClientMetadata, createFakeAddonAbuseReport, fakeAddon } from 'tests/unit/helpers';

describe(__filename, () => {
  describe('reducer', () => {
    it('initializes properly', () => {
      const state = abuseReducer(initialState, {
    ...
    describe('hideAddonAbuseReportUI', () => {
      it('sets the uiVisible state to false', () => {
        const state = abuseReducer(initialState, hideAddonAbuseReportUI({
          addon: fakeAddon,
        }));
    ...
    describe('showAddonAbuseReportUI', () => {
      it('sets the uiVisible state to true', () => {
        const state = abuseReducer(initialState, showAddonAbuseReportUI({
          addon: fakeAddon,
        }));
        expect(state).toEqual({
          bySlug: {
    ...
    describe('loadAddonAbuseReport', () => {
      ...
      it('saves the abuse report response to the reducer', () => {
        const state = abuseReducer(initialState, loadAddonAbuseReport(response));
  ...
  describe('initiateAddonAbuseReportViaFirefox', () => {
    it('does not set the loading state to true', () => {
      ...
      const state = abuseReducer(initialState, initiateAddonAbuseReportViaFirefox({
        addon: fakeAddon,
      }));
      expect(state).toEqual(initialState);
    });
  ...
  describe('finishAddonAbuseReportViaFirefox', () => {
    it('sets the loading state to false', () => {
      ...
      let state = abuseReducer(initialState, initiateAddonAbuseReportViaFirefox({
        addon: fakeAddon,
      }));
