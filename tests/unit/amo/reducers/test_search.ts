...
import search, { abortSearch, initialState, searchLoad, searchStart } from 'amo/reducers/search';
...
describe(__filename, () => {
  ...
  describe('SEARCH_LOADED', () => {
    ...
    it('sets loading', () => {
      ...
      let state = search(stateWithLang, searchStart({
        errorHandlerId: 'foo',
        filters: {
          query: 'foo',
        },
    ...
    it('sets the results', () => {
      ...
      expect(state.results).toEqual(response.results.map((addon) => createInternalAddonWithLang(addon)));
    });
  });
});