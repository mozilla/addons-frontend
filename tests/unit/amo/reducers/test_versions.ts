...
import { loadCollectionAddons, loadCurrentCollectionPage, loadCurrentCollection } from 'amo/reducers/collections';
import { loadHomeData } from 'amo/reducers/home';
import { loadLanding } from 'amo/reducers/landing';
import { OUTCOME_RECOMMENDED, loadRecommendations } from 'amo/reducers/recommendations';
import { formatFilesize } from 'amo/i18n/utils';
import { DEFAULT_API_PAGE_SIZE } from 'amo/api';
import { ADDON_TYPE_EXTENSION, CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX } from 'amo/constants';
...
import versionsReducer, { createInternalVersion, fetchVersion, fetchVersions, getLoadingBySlug, getVersionById, getVersionInfo, getVersionsBySlug, initialState, loadVersions } from 'amo/reducers/versions';
import { createAddonsApiResult, createFakeCollectionAddon, createFakeCollectionAddonsListResponse, createFakeCollectionDetail, createInternalVersionWithLang, createLocalizedString, fakeExternalShelf, fakeAddon, fakeI18n, fakeFile, fakeVersion } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('defaults to its initial state', () => {
    expect(versionsReducer(undefined, {
      type: 'SOME_OTHER_ACTION',
    })).toEqual(initialState);
  });
  it('sets a loading flag when fetching versions', () => {
    const slug = 'some-slug';
    const state = versionsReducer(undefined, fetchVersions({
      errorHandlerId: 1,
      slug,
    }));
    expect(getLoadingBySlug({
      state,
      slug,
    })).toBe(true);
  });
  ...
  it('sets a loading flag when fetching a version', () => {
    const slug = 'some-slug';
    const state = versionsReducer(undefined, fetchVersion({
      errorHandlerId: 1,
      slug,
      versionId: 1,
    }));
  ...
  it('clears the loading flag when loading versions', () => {
    let state;
    const slug = 'some-slug';
    state = versionsReducer(stateWithLang, fetchVersions({
      errorHandlerId: 1,
      slug,
    }));
    state = versionsReducer(state, loadVersions({
      slug,
  ...
  it('loads versions', () => {
    ...
    const state = versionsReducer(stateWithLang, loadVersions({
      slug,
      versions,
    }));
    expect(getVersionsBySlug({
      slug,
      state,
    })).toEqual([createInternalVersionWithLang(versions[0]), createInternalVersionWithLang(versions[1])]);
  });
  it('loads a version with license text', () => {
    ...
    const firstStoredVersion = getVersionsBySlug({
      slug,
      state,
    })[0];
  ...
  describe('createInternalVersion', () => {
    it('returns an object with the expected AddonVersionType', () => {
      ...
      expect(createInternalVersion(version, lang)).toEqual({
        ...
        id: fakeVersion.id,
        isStrictCompatibilityEnabled: Boolean(fakeVersion.is_strict_compatibility_enabled),
        license: {
    ...
    it('overwrites version 91.1.0 with 91.0.0 for firefox', () => {
      ...
      const versionWithouFirefox = { ...fakeVersion,
        compatibility: {
          [CLIENT_APP_ANDROID]: {
            min: '91.1.0',
            max: '*',
      ...
      const internalVersionWithouFirefox = createInternalVersion(versionWithouFirefox, lang);
  ...
  describe('getLoadingBySlug', () => {
    it('returns false if versions have never been loaded', () => {
      const state = versionsReducer(undefined, {
        type: 'SOME_OTHER_ACTION',
      });
      expect(getLoadingBySlug({
        slug: 'some-slug',
  ...
  describe('getVersionInfo', () => {
    ...
    it('returns the expected compatibility string for a max=*', () => {
      ...
      }).compatibilityString).toEqual(`Works with ${app} ${min} and later`);
    });
    it('returns the expected compatibility string for max and min', () => {
      ...
      expect(_getVersionInfo({
        state,
        versionId: version.id,
      }).compatibilityString).toEqual(`Works with ${app} ${min} to ${max}`);
    });
    it('returns a compatibility string for multiple apps', () => {
      ...
      expect(_getVersionInfo({
        state,
        versionId: version.id,
      }).compatibilityString).toEqual(`Works with ${app1} ${min} and later, ${app2} ${min} and later`);
    });
  });
  ...
  describe('load versions for add-ons', () => {
    ...
    describe('LOAD_CURRENT_COLLECTION', () => {
      it('loads versions', () => {
        ...
        const state = versionsReducer(stateWithLang, loadCurrentCollection({
    ...
    describe('LOAD_CURRENT_COLLECTION_PAGE', () => {
      it('loads versions', () => {
        ...
        const state = versionsReducer(stateWithLang, loadCurrentCollectionPage({
          addonsResponse: createFakeCollectionAddonsListResponse({
            addons: [fakeCollectionAddon],
          }),
        }));
    ...
    describe('LOAD_COLLECTION_ADDONS', () => {
      it('loads versions', () => {
        ...
        const state = versionsReducer(stateWithLang, loadCollectionAddons({
          addons: [fakeCollectionAddon],
          slug: 'sone-slug',
        }));
        expect(getVersionById({
    ...
    describe('LOAD_HOME_DATA', () => {
      it('maintains license and release notes from pre-existing versions when loading homeShelves', () => {
        let state = versionsReducer(stateWithLang, loadVersions({
          slug: fakeAddon.slug,
          versions: [version],
        }));
        // Create a search result with missing license and release_notes.
        const searchResult = createAddonsApiResult([{ ...fakeAddon,
          current_version: { ...version,
            license: undefined,
            release_notes: undefined,
          },
        }]);
        state = versionsReducer(state, loadHomeData({
          homeShelves: {
      ...
      it('maintains license and release notes from pre-existing versions when loading shelves', () => {
        ...
        // Create a search result with missing license and release_notes.
        const result = createAddonsApiResult([{ ...fakeAddon,
          current_version: { ...version,
            license: undefined,
            release_notes: undefined,
          },
      ...
      it('handles invalid homeShelves', () => {
        const state = versionsReducer(stateWithLang, loadHomeData({
          homeShelves: null,
          shelves: {},
        }));
    ...
    describe('LOAD_LANDING', () => {
      it('loads versions for recommended add-ons', () => {
        const state = versionsReducer(stateWithLang, loadLanding({
    ...
    describe('LOAD_RECOMMENDATIONS', () => {
      const versionForRecommendations = { ...version,
        license: undefined,
        release_notes: undefined,
      };
      it('loads versions', () => {
        const state = versionsReducer(stateWithLang, loadRecommendations({
          addons: [{ ...fakeAddon,
      ...
      it('does not overwrite license and releaseNotes', () => {
        ...
        state = versionsReducer(state, loadRecommendations({
          addons: [{ ...fakeAddon,
            current_version: versionForRecommendations,
          }],
          guid: fakeAddon.guid,
    ...
    describe('LOAD_ADDON', () => {
      it('loads versions', () => {
        const state = versionsReducer(stateWithLang, loadAddon({
          addon: { ...fakeAddon,
            current_version: version,
          },
          slug: fakeAddon.slug,
    ...
    describe('SEARCH_LOADED', () => {
      it('loads versions', () => {
        const state = versionsReducer(stateWithLang, searchLoad({
          count: 1,
          pageSize: DEFAULT_API_PAGE_SIZE,
          results: [{ ...fakeAddon,
            current_version: version,
