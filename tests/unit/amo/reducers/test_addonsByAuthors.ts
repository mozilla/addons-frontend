...
import reducer, { EXTENSIONS_BY_AUTHORS_PAGE_SIZE, THEMES_BY_AUTHORS_PAGE_SIZE, fetchAddonsByAuthors, getAddonsForSlug, getAddonsForAuthorIds, getCountForAuthorIds, getLoadingForAuthorIds, initialState, joinAuthorIdsAndAddonType, loadAddonsByAuthors } from 'amo/reducers/addonsByAuthors';
import { setLang } from 'amo/reducers/api';
import { createInternalAddonWithLang, fakeAddon, fakeAuthor, fakeTheme } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  const fakeAuthorOne = { ...fakeAuthor,
    username: 'test',
    id: 51,
  };
  const fakeAuthorTwo = { ...fakeAuthor,
    username: 'test2',
    id: 61,
  };
  const fakeAuthorThree = { ...fakeAuthor,
    username: 'test3',
    id: 71,
  };
  ...
  describe('reducer', () => {
    ...
    it('ignores unrelated actions', () => {
      ...
      // change it.
      const state = reducer(stateWithLang, loadAddonsByAuthors({
        addons: [fakeAddon],
    ...
    it("always ensures extensions' page size is consistent", () => {
      ...
      expect(state.byAddonSlug[forAddonSlug]).toHaveLength(EXTENSIONS_BY_AUTHORS_PAGE_SIZE);
    });
    it("always ensures themes' page size is consistent", () => {
      ...
      expect(state.byAddonSlug[forAddonSlug]).toHaveLength(THEMES_BY_AUTHORS_PAGE_SIZE);
    });
    it('returns state if no excluded slug is specified', () => {
      const forAddonSlug = 'addon-slug';
      const previousState = reducer(stateWithLang, loadAddonsByAuthors({
        addons: [fakeAddon],
        authorIds: [fakeAddon.authors[0].id],
        count: 1,
        forAddonSlug,
    ...
    it('resets the loaded add-ons for the slug', () => {
      const forAddonSlug = 'addon-slug';
      const firstState = reducer(stateWithLang, loadAddonsByAuthors({
        addons: [fakeAddon],
        authorIds: [fakeAddon.authors[0].id],
        count: 1,
        forAddonSlug,
      ...
      expect(firstState.byAddonSlug).toEqual({
        'addon-slug': [fakeAddon.id],
      });
      ...
      expect(state.byAddonSlug).toMatchObject({
    ...
    it('does not remove the previously loaded add-ons for authorIds when type is different', () => {
      ...
      const prevState = reducer(stateWithLang, loadAddonsByAuthors({
        addonType: ADDON_TYPE_STATIC_THEME,
        addons: [fakeTheme],
        authorIds: [userId],
        count: 1,
      ...
      const state = reducer(prevState, fetchAddonsByAuthors({
    ...
    it('sets the loading state for authorIds + addonType on fetch', () => {
      ...
      expect(state.loadingFor).toMatchObject({
        [joinAuthorIdsAndAddonType([randomAuthorId1], ADDON_TYPE_STATIC_THEME)]: true,
      });
    });
  });
  describe('loadAddonsByAuthors()', () => {
    ...
    it('adds each add-on to each author array', () => {
      const firstAuthor = { ...fakeAuthor,
        id: 50,
        username: 'first',
      };
      const secondAuthor = { ...fakeAuthor,
        id: 60,
        username: 'second',
      };
      const authorIds = [firstAuthor.username, secondAuthor.username];
      const multiAuthorAddon = { ...fakeAddon,
    ...
    it('adds each different add-on to the byAddonId dictionary', () => {
      ...
      expect(newState.byAddonId).toEqual({
        ...
        [addons.secondAddon.id]: createInternalAddonWithLang(addons.secondAddon),
    ...
    it('sets the loading state for authorIds once loaded', () => {
      let state = reducer(stateWithLang, fetchAddonsByAuthors({
        authorIds: [randomAuthorId1],
        errorHandlerId: 'error-handler-id',
        pageSize: EXTENSIONS_BY_AUTHORS_PAGE_SIZE,
      }));
    ...
    it('sets the loading state for authorIds + addonType once loaded', () => {
      ...
      expect(state.loadingFor).toMatchObject({
        [joinAuthorIdsAndAddonType([randomAuthorId1], ADDON_TYPE_STATIC_THEME)]: false,
      });
    });
    ...
    it('sets the count for authorIds + addonType once loaded', () => {
      ...
      expect(state.countFor).toMatchObject({
        [joinAuthorIdsAndAddonType([randomAuthorId1], ADDON_TYPE_STATIC_THEME)]: count,
      });
    });
  });
  describe('getAddonsForSlug', () => {
    it('returns addons', () => {
      ...
      expect(getAddonsForSlug(state, 'test')).toEqual([createInternalAddonWithLang(addons.firstAddon), createInternalAddonWithLang(addons.secondAddon), createInternalAddonWithLang(addons.thirdAddon)]);
    });
  ...
  describe('getAddonsForAuthorIds selector', () => {
    it('returns addons for a single author', () => {
      ...
      expect(getAddonsForAuthorIds(state, [fakeAuthorTwo.id])).toEqual([createInternalAddonWithLang(addons.firstAddon), createInternalAddonWithLang(addons.secondAddon)]);
    });
    it('returns addons for multiple authors when only one has a loaded add-on', () => {
      ...
      const state = reducer(stateWithLang, _loadAddonsByAuthors({
        addons,
      }));
      expect(getAddonsForAuthorIds(state, [fakeAuthorTwo.id, randomAuthorId2])).toEqual([createInternalAddonWithLang(addonsMap.firstAddon), createInternalAddonWithLang(addonsMap.secondAddon)]);
    });
    it('returns addons for multiple authors of different add-ons', () => {
      ...
      expect(getAddonsForAuthorIds(state, [fakeAuthorOne.id, fakeAuthorThree.id])).toEqual([createInternalAddonWithLang(addonsMap.firstAddon), createInternalAddonWithLang(addonsMap.thirdAddon)]);
    });
    it('returns addons for multiple authors that share add-ons', () => {
      ...
      expect(getAddonsForAuthorIds(state, [fakeAuthorOne.id, fakeAuthorTwo.id])).toEqual([createInternalAddonWithLang(addonsMap.firstAddon), createInternalAddonWithLang(addonsMap.secondAddon)]);
    });
    it('omits addon with matching slug', () => {
      ...
      // list.
      const addons = fakeExternalAddons();
      const authorIds = [fakeAuthorOne.id, fakeAuthorTwo.id, fakeAuthorThree.id];
      const state = reducer(stateWithLang, loadAddonsByAuthors({
        addons: Object.values(addons),
        addonType: ADDON_TYPE_EXTENSION,
        authorIds,
      ...
      expect(getAddonsForAuthorIds(state, authorIds, ADDON_TYPE_EXTENSION, addons.firstAddon.slug)).toEqual([createInternalAddonWithLang(addons.secondAddon), createInternalAddonWithLang(addons.thirdAddon)]);
    });
    it("returns static themes when filtering for authors' themes", () => {
      ...
      expect(getAddonsForAuthorIds(state, authorIds, ADDON_TYPE_STATIC_THEME)).toEqual([createInternalAddonWithLang(addons.firstAddon), createInternalAddonWithLang(addons.secondAddon)]);
    });
    it("returns extensions when filtering for authors' extensions", () => {
      ...
      expect(getAddonsForAuthorIds(state, authorIds, ADDON_TYPE_EXTENSION)).toEqual([createInternalAddonWithLang(addons.firstAddon), createInternalAddonWithLang(addons.secondAddon), createInternalAddonWithLang(addons.thirdAddon)]);
    });
    it('returns nothing if no add-ons are found', () => {
      const addons = fakeExternalAddons();
      const state = reducer(stateWithLang, loadAddonsByAuthors({
  ...
  describe('getLoadingForAuthorIds', () => {
    ...
    it('returns loading for authorIds + addonType', () => {
      ...
      expect(getLoadingForAuthorIds(state, [randomAuthorId1], ADDON_TYPE_STATIC_THEME)).toEqual(true);
    });
  ...
  describe('getCountForAuthorIds', () => {
    ...
    it('returns count for authorIds + addonType', () => {
      ...
      expect(getCountForAuthorIds(state, [randomAuthorId1], ADDON_TYPE_STATIC_THEME)).toEqual(count);
    });
    it('returns null when there is no match', () => {
      const state = reducer(stateWithLang, loadAddonsByAuthors(params));
    ...
    it('resets count when fetching add-ons by authors', () => {
      const count = 123;
      const prevState = reducer(stateWithLang, loadAddonsByAuthors({ ...params,
        count,
      }));
  ...
  describe('joinAuthorIdsAndAddonType', () => {
    it('returns authorIds', () => {
      expect(joinAuthorIdsAndAddonType([randomAuthorId1, randomAuthorId2])).toEqual(`${randomAuthorId1}-${randomAuthorId2}`);
    });
    it('returns authorIds + addonType', () => {
      expect(joinAuthorIdsAndAddonType([randomAuthorId1, randomAuthorId2], ADDON_TYPE_STATIC_THEME)).toEqual(`${randomAuthorId1}-${randomAuthorId2}-${ADDON_TYPE_STATIC_THEME}`);
    });
    it('handles a single author ID', () => {
      expect(joinAuthorIdsAndAddonType([randomAuthorId1], ADDON_TYPE_STATIC_THEME)).toEqual(`${randomAuthorId1}-${ADDON_TYPE_STATIC_THEME}`);
    });
  });
});