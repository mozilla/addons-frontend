...
import { SAVED_RATING, beginDeleteAddonReview, cancelDeleteAddonReview, createInternalReview, deleteAddonReview, fetchReview, fetchReviewPermissions, fetchReviews, flagReview, flashReviewMessage, hideEditReviewForm, hideFlashedReviewMessage, hideReplyToReviewForm, sendReplyToReview, setAddonReviews, setInternalReview, setLatestReview, setReviewPermissions, setReview, setReviewReply, setReviewWasFlagged, setUserReviews, showEditReviewForm, showReplyToReviewForm, unloadAddonReviews } from 'amo/actions/reviews';
import { REVIEW_FLAG_REASON_SPAM } from 'amo/constants';
import reviewsReducer, { addReviewToState, changeViewState, expandReviewObjects, getReviewsByUserId, initialState, makeLatestUserReviewKey, reviewListURL, reviewsAreLoading, selectReviewPermissions, selectReviews, storeReviewObjects } from 'amo/reducers/reviews';
...
describe(__filename, () => {
  ...
  describe('FETCH_REVIEW', () => {
    it('sets a loading flag when fetching a review', () => {
      const reviewId = 1;
      const state = reviewsReducer(undefined, fetchReview({
        errorHandlerId: 1,
        reviewId,
      }));
  ...
  describe('setAddonReviews', () => {
    ...
    it('preserves existing add-on reviews', () => {
      ...
      const review1 = fakeReview;
      const addon2 = { ...fakeAddon,
        slug: 'something-else',
      };
      const review2 = { ...fakeReview,
        id: 3,
      };
      const review3 = { ...fakeReview,
        id: 4,
      };
      let state;
      state = reviewsReducer(state, _setAddonReviews({
      ...
      expect(_selectReviews({
        reviewsState,
        addonSlug: addon1.slug,
      }).reviews[0]).toEqual(review1.id);
      expect(_selectReviews({
        reviewsState,
        addonSlug: addon2.slug,
      }).reviews[0]).toEqual(review2.id);
      expect(_selectReviews({
        reviewsState,
        addonSlug: addon2.slug,
      }).reviews[1]).toEqual(review3.id);
    });
    ...
    it('stores review counts', () => {
      const state = reviewsReducer(undefined, _setAddonReviews({
        addonSlug: 'slug1',
        reviews: [fakeReview],
      }));
      const newState = reviewsReducer(state, _setAddonReviews({
        addonSlug: 'slug2',
      ...
      expect(_selectReviews({
        reviewsState,
        addonSlug: 'slug1',
      }).reviewCount).toEqual(1);
      expect(_selectReviews({
        reviewsState,
        addonSlug: 'slug2',
      }).reviewCount).toEqual(2);
    });
  ...
  describe('unloadAddonReviews', () => {
    const loadReviewDataIntoState = ({
      ...
      state = reviewsReducer(state, _setUserReviews({
        reviews: [review],
        userId,
      }));
      ...
      state = reviewsReducer(state, setReviewPermissions({
        addonId,
        userId,
        ...permissionsToSet,
      }));
    ...
    it('unloads cached review data', () => {
      ...
      state = reviewsReducer(state, unloadAddonReviews({
        addonId,
        reviewId,
      }));
    ...
    it('unloads cached view data even for deleted reviews', () => {
      ...
      const addonId = 1;
      const reviewId = 111;
      let state = reviewsReducer(undefined, deleteAddonReview({
        addonId,
        errorHandlerId: 1,
        reviewId,
      }));
    ...
    it('unloads review permissions', () => {
      ...
      expect(selectReviewPermissions({
        reviewsState: state,
        addonId,
        userId,
      })).toBeDefined();
  ...
  describe('storeReviewObjects', () => {
    it('stores review objects by ID', () => {
      const reviews = [createInternalReview({ ...fakeReview,
        id: 1,
      }), createInternalReview({ ...fakeReview,
        id: 2,
      })];
    ...
    it('preserves existing reviews', () => {
      const review1 = createInternalReview({ ...fakeReview,
        id: 1,
      });
      const review2 = createInternalReview({ ...fakeReview,
        id: 2,
      });
      const state = initialState;
      const byId = storeReviewObjects({
        state,
        reviews: [review1],
      });
      expect(storeReviewObjects({
  ...
  describe('showEditReviewForm', () => {
    ...
    it('exits beginDeleteAddonReview state', () => {
      ...
      let state;
      state = reviewsReducer(state, beginDeleteAddonReview({
        reviewId: review.id,
      }));
      state = reviewsReducer(state, showEditReviewForm({
        reviewId: review.id,
      }));
  ...
  describe('showReplyToReviewForm', () => {
    ...
    it('exits beginDeleteAddonReview state', () => {
      ...
      state = reviewsReducer(state, showReplyToReviewForm({
        reviewId: review.id,
      }));
      expect(state.view[review.id].beginningToDeleteReview).toEqual(false);
    });
  ...
  describe('beginDeleteAddonReview', () => {
    it('stores view state about beginning to delete a review', () => {
      ...
      const state = reviewsReducer(undefined, beginDeleteAddonReview({
        reviewId: review.id,
      }));
  ...
  describe('cancelDeleteAddonReview', () => {
    it('stores view state about cancelling deleting a review', () => {
      ...
      state = reviewsReducer(state, cancelDeleteAddonReview({
        reviewId: review.id,
      }));
      expect(state.view[review.id].beginningToDeleteReview).toEqual(false);
    });
  ...
  describe('setUserReviews', () => {
    ...
    it('stores review objects', () => {
      ...
      const state = reviewsReducer(undefined, _setUserReviews({
        reviews: [review1, review2],
        userId,
      }));
      expect(state.byId[review1.id]).toEqual(createInternalReview(review1));
  ...
  describe('makeLatestUserReviewKey', () => {
    it('makes a key', () => {
      expect(makeLatestUserReviewKey({
        userId: 1,
        addonId: 2,
  ...
  describe('setLatestReview()', () => {
    ...
    it('requires setReview()', () => {
      expect(() => {
        reviewsReducer(undefined, _setLatestReview({
          review: { ...fakeReview,
            id: 2,
          },
        }));
    ...
    it('sets the latest review', () => {
      ...
      expect(state.latestUserReview[makeLatestUserReviewKey({
        addonId,
        userId,
      })]).toEqual(review.id);
    });
    it('can set the latest review to null', () => {
      const addonId = 1;
      const userId = 3;
      const state = reviewsReducer(undefined, _setLatestReview({
        addonId,
        userId,
        review: null,
      }));
  ...
  describe('addReviewToState', () => {
    ...
    it('resets byUserId data when adding a new review', () => {
      ...
      const reviewId2 = 2;
      const userId = 123;
      const prevState = reviewsReducer(undefined, _setUserReviews({
        reviews: [{ ...fakeReview,
          id: reviewId1,
        }],
        userId,
    ...
    it('does not reset byUserId data when updating a rating', () => {
      const userId = 123;
      const externalReviewUser = { ...fakeReview.user,
        id: userId,
      };
      const ratingOnlyReview = { ...fakeReview,
        body: undefined,
        score: 4,
  ...
  describe('deleteAddonReview', () => {
    it('stores view state about deleting a review', () => {
      ...
      const state = reviewsReducer(undefined, deleteAddonReview({
    ...
    it('exits beginDeleteAddonReview state', () => {
      ...
      state = reviewsReducer(state, deleteAddonReview({
        addonId: fakeAddon.id,
        errorHandlerId: 'some-id',
        reviewId: review.id,
      }));
  ...
  describe('reviewsAreLoading', () => {
    it('returns false for an add-on for which reviews have never been fetched or loaded', () => {
      ...
      const state = reviewsReducer(undefined, fetchReviews({
        addonSlug: fetchedSlug,
        errorHandlerId: 1,
      }));
      expect(reviewsAreLoading({
        reviews: state,
      }, nonfetchedSlug)).toBe(false);
    });
    ...
    it('returns false for an add-on for which reviews have finished loading', () => {
      const slug = 'some-slug';
      let state = reviewsReducer(undefined, fetchReviews({
        addonSlug: slug,
        errorHandlerId: 1,
      }));
      state = reviewsReducer(state, _setAddonReviews({
  ...
  describe('LOCATION_CHANGE', () => {
    it('resets the view state', () => {
      let state;
      state = reviewsReducer(undefined, showEditReviewForm({
        reviewId: 1,
      }));
  ...
  describe('fetchReviewPermissions', () => {
    it('prepares to fetch permissions', () => {
      const addonId = 123;
      const userId = 321;
      const state = reviewsReducer(undefined, fetchReviewPermissions({
        addonId,
  ...
  describe('setReviewPermissions', () => {
    it('sets permissions', () => {
      ...
      const userId = 321;
      let state;
      state = reviewsReducer(state, fetchReviewPermissions({
        addonId,
        errorHandlerId: 'some-error-handler',
        userId,
      }));
  ...
  describe('reviewListURL', () => {
    it('returns a URL using the addon slug', () => {
      ...
      expect(reviewListURL({
        addonSlug,
      })).toEqual(`/addon/${addonSlug}/reviews/`);
    });
    it('returns a URL using the ID', () => {
      ...
      expect(reviewListURL({
        addonSlug,
        id,
      })).toEqual(`/addon/${addonSlug}/reviews/${id}/`);
    });
    it('returns a URL using the score', () => {
      ...
      expect(reviewListURL({
        addonSlug,
        score,
      })).toEqual(`/addon/${addonSlug}/reviews/?score=${score}`);
    });
    it('returns a URL using the ID and score', () => {
      ...
      expect(reviewListURL({
        ...
        id,
        score,
      })).toEqual(`/addon/${addonSlug}/reviews/${id}/?score=${score}`);
    });
  });
});