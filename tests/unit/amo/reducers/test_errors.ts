...
import { API_ERROR_SIGNATURE_EXPIRED, ERROR_ADDON_DISABLED_BY_ADMIN, ERROR_ADDON_DISABLED_BY_DEV, ERROR_UNKNOWN } from 'amo/constants';
import errors, { clearError, initialState, setError, setErrorMessage } from 'amo/reducers/errors';
...
describe(__filename, () => {
  ...
  it('preserves existing errors', () => {
    const action1 = setError({
      id: 'action1',
      error: createFakeApiError({
        nonFieldErrors: ['action1'],
      }),
    });
    const action2 = setError({
      id: 'action2',
      error: createFakeApiError({
        nonFieldErrors: ['action2'],
      }),
    });
    let state;
  ...
  it('gets non_field_errors from API error response', () => {
    const nonFieldErrors = ['both user_id and password cannot be blank', 'some other message'];
    const reducedError = getReducedError(createFakeApiError({
      nonFieldErrors,
    }));
  ...
  it('adds nested error messages', () => {
    const nested = {
      inner: 'This is a nested message.',
    };
    const error = createApiError({
      response: {
        status: 401,
