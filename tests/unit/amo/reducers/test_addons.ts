import { createInternalReview, unloadAddonReviews, updateRatingCounts } from 'amo/actions/reviews';
import { ADDON_TYPE_EXTENSION } from 'amo/constants';
import addons, { createGroupedRatings, createInternalAddon, createInternalAddonInfo, createInternalPreviews, fetchAddon, fetchAddonInfo, getAddonByID, getAddonByIdInURL, getAddonInfoBySlug, getAllAddons, initialState, isAddonInfoLoading, isAddonLoading, loadAddon, loadAddonInfo, selectLocalizedUrlWithOutgoing } from 'amo/reducers/addons';
import { setLang } from 'amo/reducers/api';
import { createFakeAddon, createInternalAddonWithLang, createLocalizedString, createStubErrorHandler, dispatchClientMetadata, fakeAddon, fakePreview, createFakeAddonInfo, fakeReview } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('defaults to its initial state', () => {
    expect(addons(undefined, {
      type: 'SOME_OTHER_ACTION',
    })).toEqual(initialState);
  });
  it('ignores unrelated actions', () => {
    const firstState = addons(stateWithLang, loadAddon({
      addon: fakeAddon,
      slug: fakeAddon.slug,
    }));
    expect(addons(firstState, {
  ...
  it('stores addons from entities', () => {
    ...
    const anotherFakeAddon = { ...fakeAddon,
      slug: 'testing1234',
      id: 6401,
    };
    const newState = addons(firstState, loadAddon({
      addon: anotherFakeAddon,
      slug: anotherFakeAddon.slug,
    }));
    const internalAddon = createInternalAddonWithLang(anotherFakeAddon);
    expect(newState.byID).toEqual({ ...firstState.byID,
      [anotherFakeAddon.id]: internalAddon,
    });
    expect(newState.bySlug).toEqual({ ...firstState.bySlug,
      [anotherFakeAddon.slug]: anotherFakeAddon.id,
    });
    expect(newState.byGUID).toEqual({ ...firstState.byGUID,
  ...
  it('stores all add-ons, indexed by id', () => {
    ...
    let state = addons(stateWithLang, loadAddon({
      addon: addon1,
      slug: addon1.slug,
    }));
    state = addons(state, loadAddon({
      addon: addon2,
      slug: addon2.slug,
    }));
  ...
  it('stores an internal representation of an extension', () => {
    ...
    const state = addons(stateWithLang, loadAddon({
      addon,
      slug: addon.slug,
    }));
    expect(getAddonByID(state, addon.id)).toEqual(createInternalAddonWithLang(addon));
  });
  ...
  describe('fetchAddon', () => {
    ...
    it('requires an error handler', () => {
      ...
      expect(() => fetchAddon(params)).toThrowError(/errorHandler cannot be empty/);
    });
    it('requires a slug', () => {
      const params = { ...defaultParams,
      };
    ...
    it('stores a loading state for an add-on', () => {
      ...
      const state = addons(undefined, fetchAddon({
        slug,
        errorHandler: createStubErrorHandler(),
      }));
  ...
  describe('getAddonByID', () => {
    ...
    it('returns an add-on by id', () => {
      ...
      expect(getAddonByID(store.getState().addons, fakeAddon.id)).toEqual(createInternalAddonWithLang(fakeAddon));
    });
  });
  describe('getAllAddons', () => {
    ...
    it('returns an array of add-ons', () => {
      ...
      expect(getAllAddons(store.getState())).toEqual([createInternalAddonWithLang(fakeAddon)]);
    });
  });
  describe('isAddonLoading', () => {
    ...
    it('returns false for an add-on that has finished loading', () => {
      ...
      let state = addons(stateWithLang, fetchAddon({
        slug,
        errorHandler: createStubErrorHandler(),
      }));
      state = addons(state, loadAddon({
  ...
  describe('unloadAddonReviews', () => {
    it('unloads all data for an add-on', () => {
      ...
      state = addons(state, unloadAddonReviews({
        addonId: id1,
        reviewId: 1,
      }));
    ...
    it('does nothing if the add-on was not loaded', () => {
      ...
      const newState = addons(state, unloadAddonReviews({
        addonId: id2,
        reviewId: 1,
      }));
  ...
  describe('updateRatingCounts', () => {
    ...
    function initStateWithAddon(addon = { ...fakeAddon,
      ...
      return addons(stateWithLang, loadAddon({
        addon,
        slug: addon.slug,
      }));
    }
    ...
    function reviewWithScore(score) {
      return createInternalReview({ ...fakeReview,
        score,
      });
    }
    ...
    it('increment group counts for a new rating', () => {
      ...
      state = addons(state, _updateRatingCounts({
        addonId: addon.id,
        oldReview: null,
    ...
    it('automatically initializes grouped ratings', () => {
      ...
      const storedAddon = getAddonByID(state, addon.id);
      expect(storedAddon.ratings.grouped_counts).toEqual(createGroupedRatings({
        3: 1,
      }));
    });
    ...
    it('increments the review count for newly added bodies', () => {
      ...
      expect(getAddonByID(state, addon.id).ratings.text_count).toEqual(reviewCount + 1);
    });
    ...
    it('recalculates average rating when the score changes', () => {
      ...
      expect(getAddonByID(state, addon.id).ratings.average.toFixed(3)).toEqual(average(ratings).toFixed(3));
    });
    ...
    it('sets bayesian_average to average', () => {
      ...
      expect(getAddonByID(state, addon.id).ratings.bayesian_average.toFixed(3)).toEqual(average(ratings).toFixed(3));
    });
  });
  describe('addonInfo', () => {
    it('sets a loading flag when fetching info', () => {
      const slug = 'some-slug';
      const state = addons(undefined, fetchAddonInfo({
        errorHandlerId: 1,
        slug,
      }));
      expect(isAddonInfoLoading({
        state,
        slug,
      })).toBe(true);
    });
    ...
    it('clears the loading flag when loading info', () => {
      let state;
      const slug = 'some-slug';
      state = addons(stateWithLang, fetchAddonInfo({
        errorHandlerId: 1,
        slug,
      }));
      state = addons(state, loadAddonInfo({
    ...
    it('loads info', () => {
      ...
      const state = addons(stateWithLang, loadAddonInfo({
        slug,
        info,
      }));
      expect(getAddonInfoBySlug({
        slug,
        state,
      })).toEqual(createInternalAddonInfo(info, lang));
    });
  ...
  describe('getAddonByIdInURL', () => {
    it('returns an add-on for a given known id', () => {
      ...
      expect(getAddonByIdInURL(state, slug)).toEqual(createInternalAddonWithLang(addon));
    });
  ...
  describe('createInternalAddonInfo', () => {
    it('coverts localized strings into simple strings', () => {
      ...
      const addonInfo = createInternalAddonInfo(createFakeAddonInfo({
        eula,
        privacyPolicy,
      }), lang);
  ...
  describe('createInternalPreviews', () => {
    it('coverts external previews into internal previews', () => {
      ...
      const preview1 = { ...fakePreview,
        caption: createLocalizedString(caption1, lang),
      };
      const preview2 = { ...fakePreview,
  ...
  describe('selectLocalizedUrlWithOutgoing', () => {
    it('selects the url and outgoing url in the correct locale', () => {
      ...
      expect(selectLocalizedUrlWithOutgoing({
        url: {
          fr: fr_urls.url,
          'en-US': en_urls.url,
        },
