...
import homeReducer, { abortFetchHomeData, createInternalLinkWithText, createInternalHomeShelves, createInternalPrimaryHeroShelfExternalAddon, createInternalSecondaryHeroModule, createInternalShelf, fetchHomeData, initialState, loadHomeData } from 'amo/reducers/home';
import { setClientApp, setLang } from 'amo/reducers/api';
import { selectLocalizedContent } from 'amo/reducers/utils';
import { createAddonsApiResult, createHomeShelves, createInternalAddonWithLang, createLocalizedString, createPrimaryHeroShelf, createSecondaryHeroShelf, dispatchClientMetadata, fakeAddon, fakeExternalShelf, fakePrimaryHeroShelfExternalAddon, getFakeConfig } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  describe('reducer', () => {
    ...
    const _createHomeShelves = (resultsProps = [fakeExternalShelf], primaryProps = {
      addon: fakeAddon,
    }) => {
    ...
    it('ignores unrelated actions', () => {
      const state = homeReducer(initialState, {
    ...
    it('loads just homeShelves', () => {
      ...
      expect(homeState.homeShelves).toEqual(createInternalHomeShelves(homeShelves, lang));
    ...
    it('loads just shelves', () => {
      ...
      const homeState = store.getState().home;
      expect(homeState.shelves[shelfName1]).toEqual([createInternalAddonWithLang(addon1)]);
      expect(homeState.shelves[shelfName2]).toEqual([createInternalAddonWithLang(addon2)]);
    ...
    it('loads both homeShelves and shelves', () => {
      ...
      const homeState = store.getState().home;
      expect(homeState.homeShelves).toEqual(createInternalHomeShelves(homeShelves, lang));
      expect(homeState.shelves[shelfName]).toEqual([createInternalAddonWithLang(addon)]);
    });
    it('sets `resultsLoaded` to `false` and `isLoading` to `true` when fetching home add-ons', () => {
      const loadedState = { ...initialState,
        resultsLoaded: true,
      };
      const state = homeReducer(loadedState, fetchHomeData({
    ...
    it('aborts fetching of data', () => {
      let state = homeReducer(undefined, fetchHomeData({
        errorHandlerId: 'some-error-handler-id',
      }));
    ...
    it('does not reset the state to the initial state after only one location change on the client', () => {
      ...
      const newState = homeReducer(firstState, {
        type: LOCATION_CHANGE,
      }, _config);
  ...
  describe('createInternalHomeShelves', () => {
    it('creates an internal representation of home shelves', () => {
      ...
      expect(createInternalHomeShelves(homeShelves, lang)).toEqual({
        results: homeShelves.results.map((result) => createInternalShelf(result, lang)),
        primary: {
        ...
        secondary: {
          ...
          modules: homeShelves.secondary.modules.map((module) => createInternalSecondaryHeroModule(module, lang)),
        },
      });
    });
    it('works when an addon is not defined', () => {
      ...
      expect(createInternalHomeShelves(homeShelves, lang).primary).toMatchObject({
        addon: undefined,
    ...
    it('works when secondary cta is null', () => {
      ...
      expect(createInternalHomeShelves(homeShelves, lang).secondary).toMatchObject({
        cta: null,
        description: homeShelves.secondary.description[lang],
      });
    });
    it(`works when a secondary module's cta is null`, () => {
      const primaryShelf = createPrimaryHeroShelf({
        addon: fakeAddon,
      });
      ...
      expect(createInternalHomeShelves(homeShelves, lang).secondary).toMatchObject({
        modules: [createInternalSecondaryHeroModule(secondaryShelf.modules[0], lang), createInternalSecondaryHeroModule(secondaryShelf.modules[1], lang), createInternalSecondaryHeroModule(secondaryShelf.modules[2], lang)],
      });
    });
    it('works when primary is null', () => {
      ...
      expect(createInternalHomeShelves(homeShelves, lang).primary).toEqual(null);
    });
    it('works when secondary is null', () => {
      ...
      expect(createInternalHomeShelves(homeShelves, lang).secondary).toEqual(null);
    });
    ...
    it('throws an exception if neither an addon nor an external entry is provided', () => {
      ...
      expect(() => createInternalHomeShelves(homeShelves, lang)).toThrow(/Either primary.addon or primary.external is required/);
    });
  });
  ...
  describe('createInternalPrimaryHeroShelfExternalAddon', () => {
    it('creates an internal representation of an external add-on', () => {
      const external = fakePrimaryHeroShelfExternalAddon;
      expect(createInternalPrimaryHeroShelfExternalAddon(external, lang)).toEqual({
        guid: external.guid,
