import reducer, { abortAddAddonToCollection, abortFetchCurrentCollection, abortFetchUserCollections, addAddonToCollection, addonAddedToCollection, addonRemovedFromCollection, beginCollectionModification, beginEditingCollectionDetails, collectionEditUrl, collectionName, collectionUrl, convertFiltersToQueryParams, createCollection, createInternalAddons, createInternalCollection, deleteCollection, expandCollections, fetchCurrentCollection, fetchCurrentCollectionPage, fetchUserCollections, finishCollectionModification, finishEditingCollectionDetails, getCollectionById, getCurrentCollection, initialState, loadCollectionAddons, loadCollectionIntoState, loadCurrentCollection, loadCurrentCollectionPage, loadUserCollections, removeAddonFromCollection, unloadCollectionBySlug, updateCollection } from 'amo/reducers/collections';
import { COLLECTION_SORT_NAME } from 'amo/constants';
import { setLang } from 'amo/reducers/api';
import { createFakeCollectionAddon, createFakeCollectionAddons, createFakeCollectionAddonsListResponse, createFakeCollectionDetail, createInternalCollectionWithLang, createStubErrorHandler, fakeAddon, fakeI18n, onLocationChanged } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  describe('reducer', () => {
    ...
    it('loads a collection', () => {
      ...
      const collectionDetail = createFakeCollectionDetail();
      const state = reducer(stateWithLang, _loadCurrentCollection({
        addonsResponse: collectionAddons,
        detail: collectionDetail,
      }));
      const loadedCollection = getCurrentCollection(state);
      expect(loadedCollection).not.toEqual(null);
      expect(loadedCollection).toEqual(createInternalCollectionWithLang({
        detail: collectionDetail,
        addonsResponse: collectionAddons,
      }));
    ...
    it('resets the add-ons when fetching a new collection page', () => {
      ...
      expect(state.current.loading).toEqual(true);
      expect(getCurrentCollection(state)).toEqual({ ...createInternalCollectionWithLang({
          detail: collectionDetail,
        }),
        addons: [],
        numberOfAddons: null,
    ...
    it('cannot load collection page without a current collection', () => {
      ...
      expect(() => reducer(stateWithLang, loadCurrentCollectionPage({
        addonsResponse,
    ...
    it('loads a collection page', () => {
      ...
      const loadedCollection = getCurrentCollection(state);
      expect(loadedCollection).not.toEqual(null);
      expect(loadedCollection.addons).toEqual(createInternalAddons(newAddons.results, lang));
    ...
    it('preserves collection data when fetching is aborted', () => {
      ...
      state = reducer(state, _loadCurrentCollection({
    ...
    it('aborts fetching a user collection', () => {
      ...
      state = reducer(state, abortFetchUserCollections({
        userId,
      }));
      const userState = state.userCollections[userId];
    ...
    it('loads user collections by ID', () => {
      ...
      const state = reducer(stateWithLang, loadUserCollections({
        userId,
      ...
      expect(state.byId[userState.collections[0]]).toEqual(createInternalCollectionWithLang({
        detail: firstCollection,
        pageSize: null,
      }));
      expect(state.byId[userState.collections[1]]).toEqual(createInternalCollectionWithLang({
        detail: secondCollection,
        pageSize: null,
      }));
    });
    ...
    it('unloads user collections after creating', () => {
      ...
      let state = reducer(stateWithLang, loadUserCollections({
        userId,
        collections: [collection],
      }));
    ...
    it('unloads user collections after updating', () => {
      ...
      expect(state.userCollections[userId].collections).toEqual([1]);
      state = reducer(state, updateCollection({
    ...
    it('unloads user collections after deleting', () => {
      ...
      expect(state.userCollections[userId].collections).toEqual([1]);
      state = reducer(state, deleteCollection({
        errorHandlerId: createStubErrorHandler().id,
        slug: 'some-slug',
        userId,
      }));
    ...
    it('unsets a hasAddonBeenRemoved flag when beginning to remove an add-on from a collection', () => {
      ...
      state = reducer(state, removeAddonFromCollection({
        addonId: 1,
        collectionId: 3,
        errorHandlerId: 'error-handler',
        filters: {},
    ...
    it('aborts adding an add-on to a collection', () => {
      ...
      state = reducer(state, abortAddAddonToCollection({
        addonId,
        userId,
      }));
      const savedState = state.addonInCollections[userId][addonId];
    ...
    it('appends a new add-on to the list of its collections', () => {
      ...
      const savedState = state.addonInCollections[userId][addonId];
      expect(savedState.collections).toEqual([firstCollection.id, secondCollection.id]);
    });
    ...
    it('resets the addonInCollections map when location changes', () => {
      ...
      state = reducer(state, onLocationChanged({
        pathname: '/',
      }));
      expect(state.addonInCollections).toEqual({});
    });
  ...
  describe('loadCollectionIntoState', () => {
    it('preserves existing collection addons', () => {
      ...
      state = loadCollectionIntoState({
        state,
        collection,
      });
      const collectionInState = state.byId[collection.id];
      expect(collectionInState.addons).toEqual(createInternalAddons(addonsResponse.results, lang));
    });
  ...
  describe('getCollectionById', () => {
    ...
    it('returns a collection', () => {
      ...
      expect(getCollectionById(getParams({
        id,
        state,
  ...
  describe('loadCollectionAddons', () => {
    ...
    it('loads collection add-ons', () => {
      ...
      expect(state.byId[collectionDetail.id].addons).toEqual(createInternalAddons(newAddons, lang));
    ...
    it('requires a loaded collection first', () => {
      ...
      expect(() => {
        reducer(stateWithLang, loadCollectionAddons({
          addons,
    ...
    it('requires an addons parameter', () => {
      ...
      expect(() => loadCollectionAddons(params)).toThrow(/addons parameter is required/);
    });
    it('requires a collectionId parameter', () => {
      const params = getParams();
      delete params.slug;
      expect(() => loadCollectionAddons(params)).toThrow(/slug parameter is required/);
    });
  });
  ...
  describe('unloadCollectionBySlug', () => {
    ...
    it('preserves other collections', () => {
      ...
      expect(state.byId[collection1Detail.id]).toEqual(createInternalCollectionWithLang({
        detail: collection1Detail,
        addonsResponse: collection1Addons,
      }));
    });
  ...
  describe('expandCollections', () => {
    ...
    it('returns collection objects when ids are passed in via meta.collections', () => {
      ...
      expect(collections[0]).toEqual(createInternalCollectionWithLang({
        detail: firstCollection,
        pageSize: null,
      }));
      expect(collections[1]).toEqual(createInternalCollectionWithLang({
        detail: secondCollection,
        pageSize: null,
      }));
    });
  ...
  describe('convertFiltersToQueryParams', () => {
    it('returns expected query params from filters', () => {
      ...
      expect(convertFiltersToQueryParams({
        collectionSort: sort,
        page,
      })).toEqual({
  ...
  describe('collectionEditUrl', () => {
    it('calls collectionUrl and appends `edit/` to it', () => {
      ...
      expect(collectionEditUrl({ ...params,
        _collectionUrl,
      })).toEqual('/base/url/edit/');
  ...
  describe('createInternalCollection', () => {
    it('prevents the name to be `null`', () => {
      ...
      expect(createInternalCollection({
        detail,
        lang,
      })).toHaveProperty('name', '');
    });
