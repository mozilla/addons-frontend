import userAbuseReportsReducer, { SEND_USER_ABUSE_REPORT, abortUserAbuseReport, hideUserAbuseReportUI, initialState, loadUserAbuseReport, sendUserAbuseReport, showUserAbuseReportUI } from 'amo/reducers/userAbuseReports';
import { createFakeUserAbuseReport, createUserAccountResponse } from 'tests/unit/helpers';
  ...
  describe('reducer', () => {
    ...
    describe('abortUserAbuseReport', () => {
      it('resets the state of this abuse report', () => {
        ...
        let state = userAbuseReportsReducer(initialState, showUserAbuseReportUI({
          userId,
        }));
        state = userAbuseReportsReducer(state, showUserAbuseReportUI({
          userId,
        }));
        ...
        state = userAbuseReportsReducer(state, abortUserAbuseReport({
          userId,
        }));
    ...
    describe('hideUserAbuseReportUI', () => {
      it('sets the uiVisible state to false', () => {
        const userId = createUserAccountResponse();
        const state = userAbuseReportsReducer(initialState, hideUserAbuseReportUI({
          userId,
        }));
        expect(state).toMatchObject({
          byUserId: {
    ...
    describe('showUserAbuseReportUI', () => {
      it('sets the uiVisible state to true', () => {
        ...
        const state = userAbuseReportsReducer(initialState, showUserAbuseReportUI({
          userId,
        }));
        expect(state).toMatchObject({
          byUserId: {
    ...
    describe('loadUserAbuseReport', () => {
      ...
      it('saves the abuse report response to the reducer', () => {
        ...
        const state = userAbuseReportsReducer(initialState, loadUserAbuseReport({
          message: abuseReport.message,
          reporter: abuseReport.reporter,
          userId: abuseReport.user.id,
        }));
        expect(state.byUserId[abuseReport.user.id].message).toEqual('I am Groot!');
      });
