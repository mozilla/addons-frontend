import { ADDON_TYPE_DICT, ADDON_TYPE_EXTENSION, ADDON_TYPE_LANG, ADDON_TYPE_STATIC_THEME, CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX } from 'amo/constants';
import categories, { FETCH_CATEGORIES, fetchCategories, loadCategories, initialState } from 'amo/reducers/categories';
...
describe(__filename, () => {
  ...
  describe('LOAD_CATEGORIES', () => {
    ...
    beforeEach(() => {
      ...
      state = categories(initialState, loadCategories({
        results,
      }));
    });
      ...
      expect(state.categories).toEqual({
        firefox: {
          ...
          [ADDON_TYPE_EXTENSION]: {
            'alert-update': { ...fakeCategory,
              application: CLIENT_APP_FIREFOX,
              name: 'Alerts & Update',
              slug: 'alert-update',
              type: ADDON_TYPE_EXTENSION,
            },
            security: { ...fakeCategory,
              application: CLIENT_APP_FIREFOX,
              name: 'Security',
              slug: 'security',
              type: ADDON_TYPE_EXTENSION,
  ...
  describe('FETCH_CATEGORIES', () => {
    ...
    it('puts the error handler ID in the payload', () => {
      ...
      expect(_fetchCategories({
        errorHandlerId,
      }).payload.errorHandlerId).toEqual(errorHandlerId);
    });
    it('sets loading', () => {
      const state = categories(initialState, fetchCategories({
        errorHandlerId: 'some-handler',
      }));
