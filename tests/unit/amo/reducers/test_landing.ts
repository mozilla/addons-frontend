import landing, { getLanding, initialState, loadLanding } from 'amo/reducers/landing';
...
describe(__filename, () => {
  describe('getLanding', () => {
    ...
    it('optionally takes a category', () => {
      ...
      const actionWithCategorySet = getLanding({ ...getActionParams(),
        category,
      });
  ...
  describe('reducer', () => {
    ...
    describe('LOAD_LANDING', () => {
      ...
      it('sets the results', () => {
        const state = landing(stateWithLang, loadLanding({
          addonType: ADDON_TYPE_STATIC_THEME,
          recommended: {
            count: 2,
            results: [{ ...fakeAddon,
        ...
        expect(state.recommended.results).toEqual([createInternalAddonWithLang({ ...fakeAddon,
          slug: 'foo',
        }), createInternalAddonWithLang({ ...fakeAddon,
          slug: 'food',
        })]);
        expect(state.highlyRated).toEqual({
          count: 0,
          results: [],
        });
      ...
      it('does not set null keys', () => {
        const previousState = { ...stateWithLang,
          highlyRated: 'hello',
        };
