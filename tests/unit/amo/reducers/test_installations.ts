import { DISABLED, DOWNLOADING, DOWNLOAD_PROGRESS, ENABLED, ERROR, INSTALL_CANCELLED, INSTALL_COMPLETE, INSTALL_ERROR, INSTALLED, INSTALLING, START_DOWNLOAD, UNINSTALL_COMPLETE, UNINSTALLED, UNINSTALLING } from 'amo/constants';
...
describe(__filename, () => {
  ...
  it('adds an add-on to state', () => {
    ...
    expect(installations(undefined, setInstallState({ ...fakeInstalledAddon,
      guid,
  ...
  it('marks an add-on as installing on START_DOWNLOAD', () => {
    const guid = 'my-addon@me.com';
    const state = installations(undefined, setInstallState({ ...fakeInstalledAddon,
      guid,
      status: UNINSTALLED,
    }));
    expect(installations(state, {
