...
import { apiResponsePage, createStubErrorHandler, dispatchSignInActions, fakeReview, unexpectedSuccess } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  describe('submitReview', () => {
    ...
    it('does not patch the version for existing reviews', async () => {
      ...
      mockApi.expects('callApi').withArgs({
        endpoint: `ratings/rating/${params.reviewId}`,
        body: { // Make sure that version is not passed in.
          ...defaultParams,
          body: params.body,
          version: undefined,
        },
  ...
  describe('getReviews', () => {
    it('allows you to fetch reviews with filters', async () => {
      ...
      const fakeResponse = getReviewsResponse({
        reviews: [fakeReview],
      });
      mockApi.expects('callApi').withArgs({
        auth: true,
  ...
  describe('getLatestUserReview', () => {
    ...
    it('throws an error if multple reviews are received', async () => {
      mockApi.expects('callApi').resolves(getReviewsResponse({
  ...
  describe('replyToReview', () => {
    ...
    it('calls the API', async () => {
      const originalReview = { ...fakeReview,
        id: 321,
      };
