...
import { apiResponsePage, createApiResponse, createFakeAutocompleteResult, createFakeLocation, createStubErrorHandler, dispatchClientMetadata, dispatchSignInActions, generateHeaders, getFakeConfig, getFakeLogger, unexpectedSuccess, urlWithTheseParams } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  describe('core.callApi', () => {
    ...
    it('transforms method to upper case', async () => {
      mockWindow.expects('fetch').withArgs(sinon.match.any, sinon.match({
        method: 'GET',
    ...
    it('adds some query string params to all requests', async () => {
      ...
      mockWindow.expects('fetch').withArgs(urlWithTheseParams({
        lang,
      })).returns(createApiResponse());
      await api.callApi({
        endpoint: 'resource',
    ...
    it('does not encode URL paths', async () => {
      ...
      mockWindow.expects('fetch').withArgs(sinon.match(`/api/${apiVersion}/${endpoint}/`)).returns(createApiResponse());
      await api.callApi({
        endpoint,
      });
      mockWindow.verify();
    ...
    it('does not double-encode URL querystrings', async () => {
      const endpoint = 'project-ă-ă-â-â-日本語';
      const params = {
        sort: 'users,recommended',
        category: 'bookmarks',
      };
      const expectedQueryString = api.makeQueryString(params);
      mockWindow.expects('fetch').withArgs(sinon.match(`/api/${apiVersion}/${endpoint}/${expectedQueryString}`)).returns(createApiResponse());
      await api.callApi({
        endpoint,
        params,
      });
    ...
    it('handles non-JSON responses', async () => {
      ...
      const responseData = await api.callApi({
        endpoint: 'resource',
      });
      mockWindow.verify();
    ...
    it('handles any fetch error', async () => {
      mockWindow.expects('fetch').returns(Promise.reject(new Error('this could be any error')));
      const errorHandler = createStubErrorHandler();
      sinon.stub(errorHandler, 'handle');
      await api.callApi({
        endpoint: 'resource',
    ...
    it('only adds a preceding slash when necessary', async () => {
      const endpoint = '/some-endpoint/';
      mockWindow.expects('fetch').withArgs(sinon.match(`/api/${apiVersion}/some-endpoint/`)).returns(createApiResponse());
      await api.callApi({
        endpoint,
      });
      mockWindow.verify();
    ...
    it('converts true boolean to a string literal', async () => {
      mockWindow.expects('fetch').withArgs(sinon.match('?anyBoolean=true')).returns(createApiResponse());
      await api.callApi({
    ...
    it('converts false boolean to a string literal', async () => {
      mockWindow.expects('fetch').withArgs(sinon.match('?anyBoolean=false')).returns(createApiResponse());
      await api.callApi({
        endpoint: 'some-endpoint/',
        params: {
          anyBoolean: false,
    ...
    it('sets a custom API version', async () => {
      const endpoint = '/some-endpoint/';
      const version = 'v123';
      mockWindow.expects('fetch').withArgs(sinon.match(`/api/${version}/some-endpoint/`)).returns(createApiResponse());
      await api.callApi({
        endpoint,
        version,
      });
    ...
    it('logs a warning message when content type is not JSON', async () => {
      ...
      mockWindow.verify();
      sinon.assert.calledWith(_log.warn, `Response from API was not JSON (was Content-Type: ${contentType}) %o`, {
  ...
  describe('makeQueryString', () => {
    ...
    it('encodes the queryString', () => {
      ...
      expect(query).toEqual('?sort=recommended%2Cpopular&category=a%20category');
    });
  });
  describe('fetchAddon', () => {
    const {
    ...
    it('sets the slug', async () => {
      mockWindow.expects('fetch').withArgs(sinon.match(`/api/${apiVersion}/addons/addon/foo/`)).returns(mockResponse());
      await _fetchAddon({
        slug: 'foo',
      });
      mockWindow.verify();
    ...
    it('passes app and appversion', async () => {
      const userAgent = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.12; rv:61.0) Gecko/20100101 Firefox/61.0';
    ...
    it('does not pass appversion when it is not known', async () => {
      ...
      mockWindow.expects('fetch').withArgs(sinon.match((url) => !url.includes('appversion='))).returns(mockResponse());
      await _fetchAddon({
        apiState: state.api,
      });
      mockWindow.verify();
    ...
    it('fails when the add-on is not found', async () => {
      ...
      await _fetchAddon({
        ...
        expect(error.message).toMatch(new RegExp(`Error calling: /api/${apiVersion}/addons/addon/foo/`));
      });
    });
    it('includes the authorization token if available', async () => {
      const {
        api: apiState,
      } = dispatchSignInActions().state;
      mockWindow.expects('fetch').callsFake((urlString, request) => {
        expect(request.headers.authorization).toEqual(`Session ${apiState.token}`);
        return mockResponse();
      });
      const addon = await _fetchAddon({
        api: apiState,
        slug: 'bar',
      });
  ...
  describe('createApiError', () => {
    ...
    it('includes an abbreviated URL', () => {
      ...
      expect(error.message).toMatch(new RegExp(`Error calling: /api/${apiVersion}/addons/addon/123/`));
    });
    ...
    it('includes response status in the error message', () => {
      ...
      expect(error.message).toEqual('Error calling: /api/resource/ (status: 422)');
    });
  });
  describe('startLoginUrl', () => {
    const getStartLoginQs = ({
      ...
      return querystring.parse(api.startLoginUrl({
        _config,
        _window,
        location,
      }).split('?')[1]);
    ...
    it('uses the API version from config', () => {
      expect(api.startLoginUrl({
        location: createFakeLocation(),
      })).toContain(`/api/${config.get('apiVersion')}/accounts/login/start/`);
    });
  });
  describe('logOutFromServer', () => {
    it('makes a delete request to the session endpoint', async () => {
      ...
      mockWindow.expects('fetch').withArgs(sinon.match(`/api/${apiVersion}/accounts/session/`), sinon.match({
        credentials: 'include',
        method: 'DELETE',
      })).returns(mockResponse);
  ...
  describe('allPages', () => {
    it('gets results for all pages', async () => {
      ...
      const getNextResponse = sinon.mock().twice().callsFake(() => {
        page += 1;
        let next;
        let results = [];
      ...
      const {
        results,
      } = await api.allPages(getNextResponse);
    ...
    it('passes through response data', async () => {
      ...
      const response = await api.allPages(() => Promise.resolve(proxiedResponse));
    ...
    it('passes through errors', async () => {
      ...
      await api.allPages(() => Promise.reject(error)).then(unexpectedSuccess, (errorResult) => {
  ...
  describe('validateLocalizedString', () => {
    it('throws an error for invalid locale keys', () => {
      const description = {
        notAValidKey: 'some description',
      };
      expect(() => api.validateLocalizedString(description)).toThrow(/Unknown locale: "notAValidKey"/);
    });
    ...
    it('throws an error for non-object values', () => {
      expect(() => api.validateLocalizedString(9)).toThrow('Expected an object type, got "number"');
    });
  });
});