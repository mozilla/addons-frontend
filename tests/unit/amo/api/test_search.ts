...
import { ADDON_TYPE_EXTENSION, CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX, RECOMMENDED, SEARCH_SORT_RANDOM, SEARCH_SORT_RELEVANCE } from 'amo/constants';
import { createApiResponse, dispatchSignInActions, fakeAddon, unexpectedSuccess, urlWithTheseParams, userAgents } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  it('removes sort=random when the promoted filter is missing', () => {
    mockWindow.expects('fetch').withArgs(sinon.match((url) => !url.includes('sort'))).returns(mockResponse());
    return _search({
      filters: {
  ...
  it('removes sort=random when the promoted and query filters are set', () => {
    ...
    const q = 'some_query';
    mockWindow.expects('fetch').withArgs(sinon.match((url) => !url.includes('sort'))).withArgs(urlWithTheseParams({
      promoted,
      q,
    })).returns(mockResponse());
    return _search({
