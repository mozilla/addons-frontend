...
import { currentUserAccount, deleteUserAccount, deleteUserPicture, unsubscribeNotification, updateUserAccount, updateUserNotifications, userAccount, userNotifications } from 'amo/api/users';
import { getCurrentUser } from 'amo/reducers/users';
import { createApiResponse, createUserAccountResponse, createUserNotificationsResponse, dispatchSignInActions, dispatchClientMetadata } from 'tests/unit/helpers';
...
describe(__filename, () => {
  ...
  describe('updateUserAccount', () => {
    ...
    it('sends a FormData body when a picture file is supplied', async () => {
      ...
      mockApi.expects('callApi').withArgs(sinon.match(({
        body,
      }) => {
        return deepEqual(Array.from(body.entries()), Array.from(expectedBody.entries()));
      })).returns(mockResponse());
      await updateUserAccount(params);
      mockApi.verify();
    });
