...
describe(__filename, () => {
  ...
  it('lets you save and load data', () => {
    ...
    return localState.save(state).then(() => localState.load()).then((savedState) => {
  ...
  it('lets you remove data', () => {
    return localState.save({
      name: 'Aristotle',
    }).then(() => localState.clear()).then(() => localState.load()).then((data) => {
      expect(data).toBe(null);
    });
  });
  ...
  it('lets you work with multiple instances', () => {
    ...
    return localState1.save({
      number: 1,
    }).then(() => localState2.save({
      number: 2,
    })).then(() => localState1.load()).then((data) => {
      expect(data).toEqual({
        number: 1,
      });
    }).then(() => localState2.load()).then((data) => {
      expect(data).toEqual({
        number: 2,
      });
    });
