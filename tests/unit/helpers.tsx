...
import { getDefaultNormalizer, render as libraryRender, screen as libraryScreen, within as libraryWithin } from '@testing-library/react';

import { DOWNLOAD_FIREFOX_BASE_URL, ADDON_TYPE_EXTENSION, ADDON_TYPE_LANG, ADDON_TYPE_STATIC_THEME, CLIENT_APP_ANDROID, CLIENT_APP_FIREFOX, ENABLED, OS_ALL } from 'amo/constants';
import I18nProvider from 'amo/i18n/Provider';
import { EXTENSIONS_BY_AUTHORS_PAGE_SIZE, THEMES_BY_AUTHORS_PAGE_SIZE, loadAddonsByAuthors as defaultLoadAddonsByAuthors } from 'amo/reducers/addonsByAuthors';
...
import { setClientApp, setLang, setRegionCode, setAuthToken, setUserAgent } from 'amo/reducers/api';
...
import { autocompleteLoad, autocompleteStart, createInternalSuggestion } from 'amo/reducers/autocomplete';
...
import { createUserAccountResponse as createUserAccountResponseNode, getFakeConfig as getFakeConfigNode, getFakeLogger as getFakeLoggerNode, userAuthSessionId as userAuthSessionIdNode } from './helpers_node';
...
export const fakeAddon = Object.freeze({
  ...
  description: createLocalizedString('This is a longer description of the chill out add-on'),
  ...
  support_url: {
    ...
    outgoing: createLocalizedString('https://outgoing.mozilla.org/supporthamster'),
  },
  tags: ['chilling'],
...
export const fakeTheme = Object.freeze({ ...fakeAddon,
  authors: [{
    name: 'MaDonna',
...
export const createPrimaryHeroShelf = ({
  ...
  return {
    ...
    external: addon === undefined && external === undefined ? 'external-link' : external,
...
export function dispatchSignInActions({
  ...
  const {
    store,
  } = dispatchClientMetadata(otherArgs);
...
export const dispatchSearchResults = async ({
  ...
  await act(async () => store.dispatch(searchStart({
    errorHandlerId: createStubErrorHandler().id,
    filters,
  })));
  await act(async () => store.dispatch(searchLoad({
...
export const dispatchAutocompleteResults = async ({
  ...
  await act(async () => store.dispatch(autocompleteStart({
    errorHandlerId: createStubErrorHandler().id,
    filters,
  })));
  await act(async () => store.dispatch(autocompleteLoad({
    results,
  })));
  return {
    store,
...
export function createFakeCollectionAddon({
  ...
  return {
    addon,
    notes: createLocalizedString(notes),
  };
}
...
export const createFakeCollectionAddonsListResponse = ({
  ...
  return {
    ...
    page_size: pageSize,
    results: createFakeCollectionAddons({
      addons,
    }),
  };
};
...
export const userAgents = {
  androidWebkit: [oneLine`Mozilla/5.0 (Linux; U; Android 4.0.3; ko-kr; LG-L160L Build/IML74K)
      AppleWebkit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30`, oneLine`Mozilla/5.0 (Linux; U; Android 2.3.4; fr-fr; HTC Desire Build/GRJ22)
      AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1`],
  chromeAndroid: [oneLine`Mozilla/5.0 (Linux; Android 4.1.1; Galaxy Nexus Build/JRO03C)
      AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.166 Mobile
      Safari/535.19`, oneLine`Mozilla/5.0 (Linux; Android 4.0.4; Galaxy Nexus Build/IMM76K)
      AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.166 Mobile
      Safari/535.19`, oneLine`Mozilla/5.0 (Linux; Android 6.0.1; Nexus 6P Build/MMB29P)
      AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.83 Mobile
      Safari/537.36`],
  chrome: [oneLine`Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko)
      Chrome/41.0.2228.0 Safari/537.36`, userAgentsByPlatform.mac.chrome41],
  firefox: [userAgentsByPlatform.linux.firefox10, userAgentsByPlatform.windows.firefox40, userAgentsByPlatform.mac.firefox33, 'Mozilla/5.0 (X11; Linux i586; rv:31.0) Gecko/20100101 Firefox/31.0', // Firefox ESR 52
  oneLine`Mozilla/5.0 (Macintosh; Intel Mac OS X 10.12; rv:52.2.1)
      Gecko/20100101 Firefox/52.2.1`, userAgentsByPlatform.mac.firefox57],
  firefoxOS: [userAgentsByPlatform.firefoxOS.firefox26, 'Mozilla/5.0 (Tablet; rv:26.0) Gecko/26.0 Firefox/26.0', 'Mozilla/5.0 (TV; rv:44.0) Gecko/44.0 Firefox/44.0', 'Mozilla/5.0 (Mobile; nnnn; rv:26.0) Gecko/26.0 Firefox/26.0'],
  firefoxAndroid: [userAgentsByPlatform.android.firefox40Mobile, userAgentsByPlatform.android.firefox40Tablet, 'Mozilla/5.0 (Android 4.4; Mobile; rv:41.0) Gecko/41.0 Firefox/41.0', 'Mozilla/5.0 (Android 4.4; Tablet; rv:41.0) Gecko/41.0 Firefox/41.0', 'Mozilla/5.0 (Android 4.4; Tablet; rv:57.0) Gecko/57.0 Firefox/57.0', userAgentsByPlatform.android.firefox69, userAgentsByPlatform.android.firefox70],
  firefoxIOS: [userAgentsByPlatform.ios.firefox1iPodTouch, userAgentsByPlatform.ios.firefox1iPhone, userAgentsByPlatform.ios.firefox1iPad],
};
...
export function generateHeaders(headerData = {
...
export const urlWithTheseParams = (params) => {
  return sinon.match((urlString) => {
    const {
      query,
    } = urllib.parse(urlString, true);

    for (const param in params) {
      if (query[param] === undefined || query[param] !== params[param].toString()) {
        return false;
      }
    }
 ...
 */
export async function matchingSagaAction(sagaTester, isMatch, {
  maxTries = 50,
} = {}) {
  let calledActions = [];
  let foundAction;
  ...
  if (!foundAction) {
    throw new Error(`
      ...
      The saga dispatched these action types: ${calledActions.map((action) => action.type).join(', ') || '(none at all)'}`);
  }
...
export function createFakeDebounce() {
  return jest.fn((callback) => (...args) => callback(...args));
}
...
export const createInternalAddonWithLang = (addon, lang = DEFAULT_LANG_IN_TESTS) => {
...
export const createInternalVersionWithLang = (version, lang = DEFAULT_LANG_IN_TESTS) => {
  return createInternalVersion(version, lang);
};
export const createInternalHomeShelvesWithLang = (homeShelves, lang = DEFAULT_LANG_IN_TESTS) => {
...
export const createInternalSuggestionWithLang = (suggestion, lang = DEFAULT_LANG_IN_TESTS) => {
...
export const screen = { ...libraryScreen,
...
export const render = (ui, options = {}) => {
  ...
  return { ...result,
    history,
    root: result.container.firstChild,
  };
};
...
export const getSearchErrorHandlerId = (page) => `src/amo/components/Search/index.js-${page}`;
export const fakeAuthors = [{ ...fakeAuthor,
  name: 'Bob',
  username: 'bsilverberg',
  id: 1,
}, { ...fakeAuthor,
  name: 'Matt',
  username: 'tofumatt',
...
export const loadAddonsByAuthors = ({
  ...
  const pageSize = addonType === ADDON_TYPE_STATIC_THEME ? THEMES_BY_AUTHORS_PAGE_SIZE : EXTENSIONS_BY_AUTHORS_PAGE_SIZE;
  const addons = [];
  ...
  store.dispatch(defaultLoadAddonsByAuthors({
    addons,
    addonType,
    authorIds: multipleAuthors ? [fakeAuthors[0].id, fakeAuthors[1].id] : [fakeAuthors[0].id],
    count: addons.length,
    forAddonSlug,
    pageSize,
  }));
...
export const createVPNExperimentCookie = (variant) => {
  const cookieContent = JSON.stringify(createExperimentData({
