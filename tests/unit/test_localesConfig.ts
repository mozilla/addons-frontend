...
describe(__filename, () => {
  ...
  for (const localeDir of glob.sync('locale/*')) {
    ...
    it(`should have a "${lang}" entry for locale dir in config.langs`, () => expect(langs).toContain(lang));
  }
      ...
      it(`${localeJSFile} should not have incorrect data (Hint: Fix in po file and rebuild locale JS)`, () => {
        ...
        Object.keys(localeFile.locale_data.messages).forEach((key) => {
          ...
          if (placeholderCount > 0) {
            const numberTranslationMatches = localeFile.locale_data.messages[key].length;
