...
# Build
#
FROM node:26.9.0-slim AS builder
...
# Bring in the server-side locale catalogs generated by build-locales during the
# image build. src/amo/server/base.js require()s src/locale/<locale>/amo.js at
# SSR time to translate pages
COPY --from=builder --chown=${app_uid}:${app_uid} /srv/node/src/locale ${app_dir}/src/locale
