...
Locale files are stored in an external repository that you need to clone into the `locale/` directory inside your addons-frontend clone in order to build the files needed for those locales to work.

```bash
git clone https://github.com/mozilla-l10n/addons-frontend-l10n.git locale/
```
...
Locales are defined in two parts.

1. Entirely new locales are added via [`src/amo/languages.js`](https://github.com/mozilla/addons-frontend/blob/master/src/amo/languages.js).

2. Enabled locales are defined separately. See [`config/default.js`](https://github.com/mozilla/addons-frontend/blob/master/config/default.js) and look for the `langs` list.

These lists should always be kept in sync with `addons-server`. See the documentation [here](https://github.com/mozilla/addons-server/blob/master/docs/topics/development/localization_and_internationalization.md) for details.
...
Locales are updated automatically as a part of our CI. On every push to master `npm run extract-locales` is run which extracts locale strings from our codebase, merges any changes to the source language files, commits the changes to a branch on https://github.com/mozilla-l10n/addons-frontend-l10n and creates a pull request from that branch.
