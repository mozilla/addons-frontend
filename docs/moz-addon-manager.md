...
## Adding pref `xpinstall.signatures.dev-root`

This pref is not exist by default, so to add this pref, right click in `about:config` in Firefox , select `new` and then add `xpinstall.signatures.dev-root` as `Boolean`.
